let w5000 = [
    ["speak", "[ˈspiːk]", "խոսալ, տիրապետել"],
    ["can", "[kæn]", "կարենալ"],
    ["understand", "[ˌʌndərˈstænd]", "հասկանալ"],
    ["you", "[ju]", "դու, դուք"],
    ["it", "[ˈɪt]", "էտ, դա"],
    ["very", "[ˈveri]", "չափազանց"],
    ["well", "[ˈwel]", "լավ"],
    ["work", "[ˈwɝːk]", "աշխատանք, աշխատել"],
    ["study", "[ˈstʌdi]", "ուսուցանել"],
    ["and", "[ænd]", "և"],
    ["learn", "[ˈlɝːn]", "սովորել"],
    ["like", "[ˈlaɪk]", "հավանել"],
    ["this", "[ðɪs]", "այս"],
    ["place", "[ˈpleɪs]", "տեղ, տեղավորել"],
    ["city", "[ˈsɪti]", "քաղաք, մեծ քաղաք, քաղաքային"],
    ["live", "[ˈlaɪv]", "ապրել"],
    ["in", "[ɪn]", "մեջ"],
    ["country", "[ˈkʌntri]", "երկիր"],
    ["we", "[wi]", "մենք"],
    ["they", "[ˈðeɪ]", "նրանք"],
    ["help", "[ˈhelp]", "օգնություն, օգնել"],
    ["me", "[ˈmiː]", "ինձ"],
    ["really", "[ˈrɪli]", "իսկապես"],
    ["want", "[ˈwɑːnt]", "ուզել"],
    ["lesson", "[ˈlesən]", "դաս"],
    ["video", "[ˈvɪdioʊ]", "վիդեո"],
    ["program", "[ˈproʊɡræm]", "ծրագիր"],
    ["see", "[ˈsiː]", "տեսնել"],
    ["my", "[ˈmaɪ]", "իմ"],
    ["result", "[rəˈzəlt]", "արդյունք"],
    ["know", "[ˈnoʊ]", "իմանալ"],
    ["go", "[ˈɡoʊ]", "գնալ"],
    ["to", "[tuː]", "դեպի"],
    ["school", "[ˈskuːl]", "դպրոց"],
    ["often", "[ˈɔːfən]", "հաճախ"],
    ["here", "[hɪər]", "այստեղ"],
    ["there", "[ðer]", "այնտեղ"],
    ["language", "[ˈlæŋɡwɪdʒ]", "լեզու (խոսակցական)"],
    ["every", "[ˈevri]", "յուրաքանչյուր"],
    ["day", "[ˈdeɪ]", "օր"],
    ["do", "[duː]", "անել"],
    ["travel", "[ˈtrævəl]", "շրջագայել, շրջագայություն"],
    ["year", "[ˈjɪr̩]", "տարի"],
    ["travelling", "[ˈtrævəlɪŋ]", "շրջագայող"],
    ["game", "[ˈɡeɪm]", "խաղ"],
    ["play", "[ˈpleɪ]", "խաղալ"],
    ["so", "[ˈsoʊ]", "այդքան, այսպիսով, որովհետև"],
    ["also", "[ˈɒlsoʊ]", "նույնպես"],
    ["think", "[ˈθɪŋk]", "մտածել"],
    ["about", "[əˈbaʊt]", "մասին, մոտավոր"],
    ["sometimes", "[səmˈtaɪmz]", "երբեմն"],
    ["love", "[ˈlʌv]", "սեր, սիրել"],
    ["song", "[ˈsɒŋ]", "երգ, բանաստեղծություն"],
    ["music", "[ˈmjuːzɪk]", "երաժշտություն"],
    ["read", "[riːd]", "կարդալ, կարդալը"],
    ["book", "[ˈbʊk]", "գիրք, գրքային, ամսագիր"],
    ["more", "[ˈmɔːr]", "ավելի"],
    ["practice", "[ˈprækˌtɪs]", "պրակտիկա, փորձ, մարզում"],
    ["these", "[ðiːz]", "սրանք"],
    ["usually", "[ˈjuːʒəwəli]", "սովորաբար"],
    ["buy", "[ˈbaɪ]", "գնել"],
    ["thing", "[ˈθɪŋ]", "իր"],
    ["absolutely", "[ˌæbsəˈluːtli]", "բացարձակ"],
    ["agree", "[əˈɡriː]", "համաձայնվել"],
    ["better", "[ˈbetər]", "առավել լավ, լավագույն"],
    ["cost", "[ˈkɑːst]", "արժեք, արժենալ"],
    ["far", "[ˈfɑːr]", "հեռավոր, հեռու"],
    ["feel", "[ˈfiːl]", "զգալ"],
    ["he", "[hi]", "նա, ինքը (արական)"],
    ["less", "[ˈles]", "առավել նվազ, նվազագույն"],
    ["make", "[ˈmeɪk]", "սարքել"],
    ["mistake", "[ˌmɪˈsteɪk]", "սխալ, սխալվել"],
    ["much", "[ˈmʌtʃ]", "շատ"],
    ["near", "[ˈnɪr]", "մոտիկ, մոտավոր"],
    ["pound", "[ˈpaʊnd]", "ֆունտ"],
    ["problem", "[ˈprɑːbləm]", "խնդիր"],
    ["she", "[ʃi]", "նա, ինքը (իգական)"],
    ["that", "[ðæt]", "որ, այն"],
    ["ticket", "[ˈtɪkət]", "տոմս"],
    ["together", "[təˈɡeðər]", "միասին"],
    ["with", "[wɪθ]", "հետ (ինչոր բանի հետ միասին)"],
    ["call", "[ˈkɒl]", "կանչել, զանքել, կանչ, զանգ"],
    ["happen", "[ˈhæpən]", "տեղի ունենալ"],
    ["hear", "[ˈhɪr]", "լսել"],
    ["how", "[ˈhaʊ]", "ինչպես, ինչքան"],
    ["idea", "[aɪˈdiːə]", "մտահաղացում"],
    ["little", "[ˈlɪtəl]", "փոքր"],
    ["a lot", "[ə lɒt]", "շատ"],
    ["lots of", "[lɒts əv]", "շատ"],
    ["on", "[ɑːn]", "վրա, դրանով"],
    ["people", "[ˈpiːpəl]", "մարդիք"],
    ["person", "[ˈpɝːsən]", "անհատ, մարդ"],
    ["say", "[ˈseɪ]", "ասել, խոսալ (ուղակի ասել)"],
    ["tell", "[ˈtel]", "ասել, խոսալ, պատմել (ասել ինչոր մեկին)"],
    ["too", "[ˈtuː]", "նույնպես, չափազանց, շատ"],
    ["us", "[əz]", "մեզ, մեզնով"],
    ["watch", "[ˈwɑːtʃ]", "ժամացույց, դիտել"],
    ["why", "[ˈwaɪ]", "ինչու"],
    ["apple", "[ˈæpəl]", "խնձոր"],
    ["boy", "[ˌbɔɪ]", "տղա"],
    ["brother", "[ˈbrʌðr̩]", "եղբայր"],
    ["bus", "[ˈbəs]", "ավտոբուս"],
    ["car", "[ˈkɑːr]", "ավտոմեքենա"],
    ["free", "[ˈfriː]", "ազատ, անվճար"],
    ["girl", "[ˈɡɝːl]", "աղջիկ"],
    ["have", "[hæv]", "ունենալ"],
    ["hotel", "[ˌhoʊˈtel]", "հյուրատուն"],
    ["house", "[ˈhaʊs]", "տուն, շինություն"],
    ["job", "[ˈdʒɑːb]", "աշխատանք"],
    ["man", "[ˈmæn]", "տղամարդ"],
    ["money", "[ˈmʌni]", "փող"],
    ["orange", "[ˈɔːrəndʒ]", "նարինջ, նարնջագույն"],
    ["question", "[ˈkwestʃən]", "հարց"],
    ["sister", "[ˈsɪstər]", "քույր"],
    ["some", "[sʌm]", "որոշ, ինչոր"],
    ["teacher", "[ˈtiːtʃər]", "ուսուցիչ"],
    ["time", "[ˈtaɪm]", "ժամանակ"],
    ["university", "[ˌjuːnɪˈvɝːsəti]", "համալսարան"],
    ["woman", "[ˈwʊmən]", "կինարմատ"],
    ["your", "[jər]", "քո, ձեր"],
    ["bank", "[ˈbæŋk]", "բանկ, բանկային"],
    ["coffee", "[ˈkɑːfi]", "սուրճ, սրճային"],
    ["computer", "[kəmˈpjuːtər]", "համակարգիչ, համակարգչային"],
    ["different", "[ˈdɪfərənt]", "տարբեր"],
    ["drink", "[ˈdrɪŋk]", "խմել, խմիչք"],
    ["eat", "[ˈiːt]", "ուտել"],
    ["exercise", "[ˈeksərˌsaɪz]", "վարժություն"],
    ["family", "[ˈfæməli]", "ընտանիք, ընտանյա"],
    ["fish", "[ˈfɪʃ]", "ձուկ"],
    ["juice", "[ˈdʒuːs]", "հյութ"],
    ["milk", "[ˈmɪlk]", "կաթ, կթել, կաթնային"],
    ["motivation", "[ˌmoʊtɪˈveɪʃən]", "մոտիվացյա"],
    ["progress", "[ˈprɑːˌɡres]", "առաջընթաց"],
    ["sport", "[ˈspɔːrt]", "սպորտ"],
    ["student", "[ˈstuːdənt]", "ուսանող"],
    ["taxi", "[ˈtæksi]", "տաքսի"],
    ["tea", "[ˈtiː]", "թեյ"],
    ["water", "[ˈwɒtər]", "ջուր, ջրային, ջրել"],
    ["airport", "[ˈerˌpɔːrt]", "օդանավակայան"],
    ["apartment", "[əˈpɑːrtmənt]", "բնակարան, սենյակ"],
    ["cake", "[ˈkeɪk]", "տորթ, թխվածք"],
    ["child", "[ˈtʃaɪld]", "երեխա, սերունդ"],
    ["children", "[ˈtʃɪldrən]", "երեխաներ"],
    ["chocolate", "[ˈtʃɒklət]", "շոկոլադ, շոկոլադե"],
    ["daughter", "[ˈdɒtər]", "դուստր"],
    ["meat", "[ˈmiːt]", "միս"],
    ["men", "[ˈmen]", "մարդիք"],
    ["seafood", "[ˈsiːˌfuːd]", "ծովամթերք"],
    ["son", "[ˈsən]", "որդի"],
    ["three", "[ˈθriː]", "երեք"],
    ["women", "[ˈwɪmən]", "կանայք, իգական սեռ"],
    ["boyfriend", "[ˈbɔɪfrend]", "սիրած ընկեր"],
    ["ice cream", "[aɪs kriːm]", "պաղպաղակ"],
    ["foreign", "[ˈfɔːrən]", "օտար, օտարերկրյա"],
    ["girlfriend", "[ˈɡɝːlˌfrend]", "սիրած ընկերուհի"],
    ["hospital", "[ˈhɑːˌspɪtəl]", "հիվանդանոց"],
    ["pizza", "[ˈpiːtsə]", "պիցցա"],
    ["swimming", "[ˈswɪmɪŋ]", "լող"],
    ["be", "[bi]", "լինել, գտնվել, պետք է"],
    ["become", "[bɪˈkʌm]", "դառնալ"],
    ["businessman", "[ˈbɪznəˌsmæn]", "գործարար"],
    ["correctly", "[kəˈrektli]", "ճիշտ, կոռեկտ"],
    ["designer", "[dəˈzaɪnər]", "դիզայներ, նախագծող, գծագրիչ"],
    ["doctor", "[ˈdɑːktər]", "բժիշկ, բուժել"],
    ["fluently", "[ˈfluːəntli]", "սահուն կերպով"],
    ["happy", "[ˈhæpi]", "երջանիկ, ուրախ, հաջողակ"],
    ["her", "[hər]", "իրա (իգական)"],
    ["him", "[hɪm]", "իրան (արական)"],
    ["manager", "[ˈmænədʒər]", "մենեջեր, միջնորդ ղեկավար"],
    ["new", "[nuː]", "նոր"],
    ["now", "[ˈnaʊ]", "հիմա"],
    ["programmer", "[ˈproʊɡræmə]", "ծրագրավորող"],
    ["them", "[ðəm]", "իրենց"],
    ["visit", "[ˈvɪzət]", "այցելություն, այցելել"],
    ["without", "[wɪðˈaʊt]", "առանց"],
    ["would", "[ˈwʊd]", "կ (ցանկություն)"],
    ["abroad", "[əˈbrɔːd]", "արտասահման"],
    ["father", "[ˈfɑːðr̩]", "հայր"],
    ["his", "[hɪz]", "իրա (արական)"],
    ["month", "[ˈmənθ]", "ամիս"],
    ["mother", "[ˈmʌðr̩]", "մայր"],
    ["next", "[ˈnekst]", "հաջորդ, մյուս"],
    ["their", "[ˈðer]", "իրանց"],
    ["week", "[ˈwiːk]", "շաբաթ"],
    ["big", "[ˈbɪɡ]", "մեծ"],
    ["business", "[ˈbɪznəs]", "գործարարություն, բիզնես"],
    ["control", "[kənˈtroʊl]", "ղեկավարել, վերահսկողություն"],
    ["course", "[ˈkɔːrs]", "դասընթաց, երթուղի, ընթացք"],
    ["education", "[ˌedʒəˈkeɪʃən]", "կրթություն, ուսուցում"],
    ["food", "[ˈfuːd]", "սնունդ"],
    ["for", "[fɔːr]", "համար (ինչոր բանի համար)"],
    ["future", "[ˈfjuːtʃər]", "ապագա"],
    ["not", "[ˈnɑːt]", "ոչ, չե, չի"],
    ["our", "[ˈaʊər]", "մեր"],
    ["pay", "[ˈpeɪ]", "վճար, վճարել, վճարովի"],
    ["politics", "[ˈpɑːləˌtɪks]", "քաղաքականություն"],
    ["situation", "[ˌsɪtʃuːˈeɪʃən]", "իրավիճակ"],
    ["spend", "[ˈspend]", "ծախսել, անցկացնել"],
    ["talk", "[ˈtɔːk]", "խոսել, զրուցել"],
    ["those", "[ðoʊz]", "նրանք"],
    ["any", "[ˈeni]", "ցանկացած, որեվէ"],
    ["cat", "[kæt]", "կատու, մտրակ, մտրակել, խարսխամբարձ"],
    ["dog", "[ˈdɔːɡ]", "շուն"],
    ["exam", "[ɪgˈzæm]", "քննություն"],
    ["friend", "[ˈfrend]", "ընկեր, ընկերուհի"],
    ["interest", "[ˈɪntrəst]", "հետաքրքրություն, հետաքրքրել"],
    ["interesting", "[ˈɪntrəstɪŋ]", "հետաքրքիր"],
    ["Internet", "[ˈɪntərˌnet]", "համացանց, Ինտերնետ"],
    ["maths", "[ˈmæθs]", "մաթեմատիկա"],
    ["motivate", "[ˈmoʊtɪveɪt]", "մոտիվացնել"],
    ["only", "[ˈoʊnli]", "միայն, գեթ"],
    ["plan", "[ˈplæn]", "պլան, պլանավորել"],
    ["sell", "[ˈsel]", "վաճառել"],
    ["service", "[ˈsɝːvəs]", "ծառայություն, սպասարկում"],
    ["subject", "[sʌbˈdʒekt]", "ենթակա, առարկա, ենթարկել"],
    ["teach", "[ˈtiːtʃ]", "սովորեցնել, ուսուցանել, դասավանդել"],
    ["test", "[ˈtest]", "փորձարկում, ստուգել"],
    ["use", "[ˈjuːs]", "օգտագործել, կիրառել, կիրառում"],
    ["worry", "[ˈwɜːri]", "անհանգստություն, անհանգստանալ"],
    ["address", "[ˈæˌdres]", "հասցե, դիմում"],
    ["again", "[əˈɡen]", "կրկին"],
    ["another", "[əˈnʌðr̩]", "մեկ այլ, ուրիշ"],
    ["bad", "[ˈbæd]", "վատ"],
    ["because", "[bɪˈkɒz]", "որովհետև, քանզի"],
    ["communicate", "[kəˈmjuːnəˌket]", "շփվել, հաղորդակցվել"],
    ["fast", "[ˈfæst]", "հապճեպ, արագ, պաս"],
    ["get", "[ˈɡet]", "ստանալ, հասնել"],
    ["good", "[ˈɡʊd]", "լավ"],
    ["name", "[ˈneɪm]", "անուն, անվանում"],
    ["need", "[ˈniːd]", "կարիք, կարիք ունենալ"],
    ["other", "[ˈʌðr̩]", "այլ, ուրիշ, մնացած"],
    ["prefer", "[prəˈfɝː]", "նախընտրել, գերադասել"],
    ["professionally", "[prəˈfeʃənəli]", "մասնագիտորեն"],
    ["remember", "[rəˈmembər]", "հիշել"],
    ["slowly", "[ˈsloʊli]", "դանդաղորեն"],
    ["website", "[ˈwebˌsaɪt]", "վեբ կայք"],
    ["who", "[ˈhuː]", "ով"],
    ["boring", "[ˈbɔːrɪŋ]", "ձանձրալի,  հորատում"],
    ["cinema", "[ˈsɪnəmə]", "կինոթատրոն"],
    ["dancing", "[ˈdænsɪŋ]", "պարը, պարային"],
    ["find", "[ˈfaɪnd]", "գտնել, որոշել"],
    ["intensively", "[ˌɪnˈtensɪvli]", "ինտենսիվորեն"],
    ["many", "[ˈmeni]", "շատերը, շատ, բազմաթիվ"],
    ["movie", "[ˈmuːvi]", "ֆիլմ"],
    ["or", "[ɔːr]", "կամ"],
    ["park", "[ˈpɑːrk]", "այգի, կայան, կայանել"],
    ["reading", "[ˈriːdɪŋ]", "ընթերցանություն"],
    ["real", "[riːəl]", "իրական, իսկապես"],
    ["show", "[ˈʃoʊ]", "ցուցադրել, ցուցահանդես"],
    ["singing", "[ˈsɪŋɪŋ]", "երգեցողություն"],
    ["support", "[səˈpɔːrt]", "աջակցություն, աջակցել"],
    ["train", "[ˈtreɪn]", "գնացք, մարզել, մարզվել"],
    ["weather", "[ˈweðər]", "եղանակ"],
    ["write", "[ˈraɪt]", "գրել"],
    ["actor", "[ˈæktər]", "դերասան"],
    ["actress", "[ˈæktrəs]", "դերասանուհի"],
    ["change", "[ˈtʃeɪndʒ]", "փոփոխություն, փոփոխել"],
    ["cheap", "[ˈtʃiːp]", "էժան"],
    ["discuss", "[ˌdɪˈskəs]", "քննարկել"],
    ["expensive", "[ɪkˈspensɪv]", "թանկ"],
    ["five", "[ˈfaɪv]", "հինգ"],
    ["from", "[frʌm]", "-ից"],
    ["important", "[ˌɪmˈpɔːrtənt]", "կարևոր"],
    ["listen", "[ˈlɪsən]", "լսել"],
    ["look", "[ˈlʊk]", "նայել, հայացք"],
    ["method", "[ˈmeθəd]", "մեթոդ"],
    ["phone", "[ˈfoʊn]", "հեռախոս"],
    ["seem", "[ˈsiːm]", "թվալ"],
    ["strange", "[ˈstreɪndʒ]", "տարօրինակ, օտար"],
    ["that", "[ðæt]", "որ, այդ, որպեսզի"],
    ["application", "[ˌæpləˈkeɪʃən]", "կիրառություն, դիմում, հավելված"],
    ["clothes", "[kloʊðz]", "հագուստ, զգեստ"],
    ["come", "[ˈkəm]", "գալ"],
    ["cook", "[ˈkʊk]", "խոհարար, պատրաստել, եփել"],
    ["cooking", "[ˈkʊkɪŋ]", "խոհարարություն, եփվածք"],
    ["dance", "[ˈdæns]", "պար, պարել"],
    ["early", "[ˈɝːli]", "վաղ, վաղաժամ"],
    ["enough", "[əˈnəf]", "բավական է, բավական"],
    ["get up", "[ˈɡet ʌp]", "վեր կենալ, բարձրանալ"],
    ["group", "[ˈɡruːp]", "խումբ, խմբավորում, խմբավորել"],
    ["home", "[hoʊm]", "տուն, տնային"],
    ["late", "[ˈleɪt]", "ուշ, վերջին"],
    ["order", "[ˈɔːrdər]", "պատվեր, պատվիրել, կարգ, հրաման"],
    ["pet", "[ˈpet]", "ընտանի, ընտանի կենդանի"],
    ["profession", "[prəˈfeʃən]", "մասնագիտություն, արհեստ"],
    ["recommend", "[ˌrekəˈmend]", "խորհուրդ տալ, առաջադրել"],
    ["shop", "[ˈʃɑːp]", "խանութ, գնել"],
    ["toy", "[ˌtɔɪ]", "խաղալիք, խաղալ"],
    ["wake up", "[weɪk ʌp]", "արթնանալ"],
    ["all", "[ɔːl]", "բոլորը, ամբողջ, լրիվ"],
    ["document", "[ˈdɑːkjəment]", "փաստաթուղթ"],
    ["download", "[ˌdaʊnˈloʊd]", "ներբեռնել, բեռնել, բեռնում"],
    ["file", "[ˈfaɪl]", "ֆայլ"],
    ["island", "[ˈaɪlənd]", "կղզի"],
    ["leave", "[ˈliːv]", "լքել"],
    ["level", "[ˈlevəl]", "մակարդակ"],
    ["regularly", "[ˈreɡjələrli]", "պարբերաբար"],
    ["sad", "[ˈsæd]", "տխուր, ցավալի"],
    ["unhappy", "[ʌnˈhæpi]", "տխուր, դժբախտ, անհաջող"],
    ["what", "[ˈhwʌt]", "ինչ, ինչպիսի"],
    ["when", "[hwen]", "երբ, երբ որ"],
    ["where", "[ˈhwer̩]", "ուր, որտեղ"],
    ["which", "[hwɪtʃ]", "որը, որմեկը"],
    ["angry", "[ˈæŋɡri]", "զայրացած"],
    ["at", "[æt]", "մոտ, ի, առ, դեպի, -ով"],
    ["best", "[ˈbest]", "լավագույն, մեծագույն"],
    ["busy", "[ˈbɪzi]", "զբաղված"],
    ["fact", "[ˈfækt]", "փաստ, իրողություն"],
    ["husband", "[ˈhʌzbənd]", "ամուսին"],
    ["old", "[oʊld]", "հին, ծեր, նախկին"],
    ["right", "[ˈraɪt]", "աջ, իրավունք, ճիշտ, ճշտել"],
    ["room", "[ˈruːm]", "սենյակ"],
    ["secretary", "[ˈsekrəteri]", "քարտուղար"],
    ["singer", "[ˈsɪŋər]", "երգիչ"],
    ["sure", "[ˈʃʊr]", "վստահ, համոզված, իհարկե"],
    ["wife", "[ˈwaɪf]", "կին, կին ամուսին"],
    ["wonderful", "[ˈwʌndərfəl]", "հրաշալի, զարմանալի"],
    ["young", "[ˈjəŋ]", "երիտասարդ, ջահել"],
    ["alright", "[ɔːlˈraɪt]", "լավ, դե լավ"],
    ["beautiful", "[ˈbjuːtəfəl]", "գեղեցիկ, չքնաղ"],
    ["dangerous", "[ˈdeɪndʒərəs]", "վտանգավոր"],
    ["difficult", "[ˈdɪfəkəlt]", "դժվար"],
    ["easy", "[ˈiːzi]", "հեշտ"],
    ["effective", "[ɪˈfektɪv]", "արդյունավետ"],
    ["fine", "[ˈfaɪn]", "տուգանք, հիանալի"],
    ["hard-working", "[ˈwɜːrkɪŋ]", "աշխատասեր"],
    ["lazy", "[ˈleɪzi]", "ծույլ"],
    ["married", "[ˈmerid]", "ամուսնացած"],
    ["ok", "[ˌoʊˈkeɪ]", "լավ, հավանություն տալ"],
    ["parents", "[ˈpeərənts]", "ծնողներ"],
    ["poor", "[ˈpʊr]", "աղքատ"],
    ["possible", "[ˈpɑːsəbəl]", "հնարավոր"],
    ["ready", "[ˈredi]", "պատրաստ"],
    ["rich", "[ˈrɪtʃ]", "հարուստ"],
    ["single", "[ˈsɪŋɡəl]", "միայնակ, միակ, մի"],
    ["smart", "[ˈsmɑːrt]", "խելացի"],
    ["afraid", "[əˈfreɪd]", "վախեցած"],
    ["answer", "[ˈænsər]", "պատասխան, պատասխանել"],
    ["correct", "[kəˈrekt]", "ճիշտ, կոռեկտ, ստույգ, ուղղել"],
    ["decision", "[dəˈsɪʒən]", "որոշում, եզրակացություն"],
    ["of", "[ʌv]", "-ի, -ից, համար"],
    ["popular", "[ˈpɑːpjələr]", "հանրաճանաչ, հանրամատչելի"],
    ["assistant", "[əˈsɪstənt]", "օգնական"],
    ["cheaper", "[ˈtʃiːpər]", "ավելի էժան"],
    ["choice", "[tʃɔɪs]", "ընտրություն"],
    ["colleague", "[ˈkɑːliɡ]", "գործընկեր, կոլեգա"],
    ["director", "[dəˈrektər]", "տնօրեն, ղեկավար"],
    ["driver", "[ˈdraɪvər]", "վարորդ, մեքենավար, ղեկավարող ծրագիր"],
    ["kitchen", "[ˈkɪtʃən]", "խոհանոց"],
    ["native speaker", "[ˌneɪtɪv ˈspiːkə]", "մայրենի լեզվի խոսնակ"],
    ["price", "[ˈpraɪs]", "գին, գնահատել"],
    ["professional", "[prəˈfeʃənəl]", "մասնագիտորեն"],
    ["trainer", "[ˈtreɪnər]", "մարզիչ"],
    ["worker", "[ˈwɝːkər]", "աշխատող, աշխատավոր, բանվոր"],
    ["bag", "[ˈbæɡ]", "պայուսակ, պարկ"],
    ["box", "[ˈbɑːks]", "տուփ, արկղ, բռունցքով հարվածել"],
    ["dress", "[ˈdres]", "զգեստ"],
    ["jeans", "[ˈdʒiːnz]", "ջինս, ջինսեր"],
    ["pair", "[ˈper]", "զույգ"],
    ["trousers", "[ˈtraʊzərz]", "տաբատ, շալվար"],
    ["upset", "[əpˈset]", "հուսահատվել, խանգարում, խանգարված, խառնաշփոթ"],
    ["worse", "[ˈwɝːs]", "շատ վատ"],
    ["wrong", "[ˈrɒŋ]", "սխալ, ոչ ճիշտ լինելը"],
    ["each", "[ˈiːtʃ]", "յուրաքանչյուր"],
    ["lie", "[ˈlaɪ]", "սուտ, ստել, պառկել"],
    ["moment", "[ˈmoʊmənt]", "պահ, ակնթարթ"],
    ["necessary", "[ˈnesəˌseri]", "անհրաժեշտ"],
    ["photo", "[ˈfoˌtoʊ]", "լուսանկար"],
    ["relax", "[rəˈlæks]", "թուլանալ, հանգստանալ"],
    ["simple", "[ˈsɪmpəl]", "հասարակ"],
    ["smile", "[ˈsmaɪl]", "ժպիտ, ժպտալ"],
    ["sofa", "[ˈsoʊfə]", "բազմոց"],
    ["text", "[ˈtekst]", "տեքստ"],
    ["tradition", "[trəˈdɪʃən]", "ավանդույթ, սովորույթ"],
    ["under", "[ˈʌndr̩]", "տակ, ներքևում"],
    ["writer", "[ˈraɪtər]", "գրող, հեղինակ"],
    ["dancer", "[ˈdænsər]", "պարող"],
    ["earn", "[ˈɝːn]", "վաստակել"],
    ["everything", "[ˈevriθɪŋ]", "ամեն ինչ"],
    ["fat", "[ˈfæt]", "ճարպ, ճարպոտ, չաղ"],
    ["life", "[ˈlaɪf]", "կյանք, ապրելակերպ"],
    ["reality", "[riˈæləti]", "իրականություն"],
    ["same", "[ˈseɪm]", "նույնը, միանման"],
    ["tasty", "[ˈteɪsti]", "համեղ"],
    ["team", "[ˈtiːm]", "թիմ"],
    ["against", "[əˈɡenst]", "հակառակ, դեմ, ընդդեմ"],
    ["casual", "[ˈkæʒəwəl]", "պատահական"],
    ["choose", "[ˈtʃuːz]", "ընտրել"],
    ["formal", "[ˈfɔːrməl]", "ձևաչափորեն"],
    ["informal", "[ˌɪnˈfɔːrməl]", "ոչ ձևաչափորեն"],
    ["journalist", "[ˈdʒɝːnələst]", "լրագրող"],
    ["nowadays", "[ˈnaʊəˌdez]", "այսօրվա դրությամբ, մեր օրերում, այժմ"],
    ["useful", "[ˈjuːsfəl]", "օգտակար"],
    ["walk", "[ˈwɑːk]", "քայլել, զբոսանք"],
    ["wear", "[ˈwer]", "կրել, կրելը, հագնելը"],
    ["worried", "[ˈwɜːrid]", "անհանգստացած"],
    ["after", "[ˈæftər]", "հետո"],
    ["close", "[kloʊz]", "փակել, մոտիկ, ավարտ"],
    ["could", "[kʊd]", "կարող էր"],
    ["door", "[ˈdɔːr]", "դուռ"],
    ["example", "[ɪgˈzæmpəl]", "օրինակ"],
    ["floor", "[ˈflɔːr]", "հատակ, հարկ (օրինակ՝ 3-րդ հարկ)"],
    ["give", "[ˈɡɪv]", "տալ, հանձնել"],
    ["homework", "[ˈhoʊmwɜːk]", "տնային աշխատանք"],
    ["later", "[ˈleɪtər]", "ուշ, ավելի ուշ"],
    ["letter", "[ˈletər]", "նամակ, տառերով նշագրել, գրություն, տառ"],
    ["open", "[ˈoʊpən]", "բաց, բացել"],
    ["put", "[ˈpʊt]", "դնել"],
    ["smoke", "[smoʊk]", "ծուխ, ծխել"],
    ["stop", "[ˈstɑːp]", "կանգառ, կանգ առնել, դադարել"],
    ["story", "[ˈstɔːri]", "պատմություն, պատմվածք"],
    ["swim", "[ˈswɪm]", "լողալ"],
    ["table", "[ˈteɪbəl]", "սեղան, աղյուսակ, ցուցակ"],
    ["window", "[ˈwɪndoʊ]", "պատուհան, լուսամուտ"],
    ["afternoon", "[ˌæftərˈnuːn]", "կեսօրից հետո"],
    ["ask", "[ˈæsk]", "հարցնել, խնդրել"],
    ["bike", "[ˈbaɪk]", "հեծանիվ"],
    ["careful", "[ˈkerfəl]", "զգույշ"],
    ["check", "[ˈtʃek]", "ստուգում, ստուգել, չեկ"],
    ["continue", "[kənˈtɪnjuː]", "շարունակել"],
    ["email", "[ˈiːmeɪl]", "էլ. փոստ"],
    ["evening", "[ˈiːvənɪŋ]", "երեկո, երեկույթ"],
    ["horse", "[ˈhɔːrs]", "ձի"],
    ["information", "[ˌɪnfərˈmeɪʃən]", "տեղեկատվություն, տեղեկություն"],
    ["lose", "[ˈluːz]", "պարտվել, կորցնել, վնաս կրել"],
    ["meet", "[ˈmiːt]", "տեսնվել, տեսակցություն"],
    ["must", "[mʌst]", "պետք է, պարտավոր լինել"],
    ["o'clock", "[əˈklɒk]", "ժամը"],
    ["please", "[ˈpliːz]", "խնդրել"],
    ["ride", "[ˈraɪd]", "քշել, ուղևորություն, զբոսանք"],
    ["should", "[ʃʊd]", "պետք է"],
    ["something", "[ˈsʌmθɪŋ]", "ինչ-որ բան, որևէ բան, մի բան"],
    ["stay", "[ˈsteɪ]", "մնալ, կանգնել"],
    ["today", "[təˈdeɪ]", "այսօր"],
    ["try", "[ˈtraɪ]", "փորձել, փորձարկում"],
    ["urgently", "[ˈɜːdʒəntli]", "շտապ"],
    ["win", "[ˈwɪn]", "հաղթանակ, հաղթել"],
    ["right now", "[raɪt naʊ]", "հենց հիմա"],
    ["believe", "[bəˈliːv]", "հավատալ"],
    ["drive", "[ˈdraɪv]", "վարել, քշել, երթ"],
    ["eye", "[ˈaɪ]", "աչք"],
    ["finish", "[ˈfɪnɪʃ]", "ավարտ, ավարտել"],
    ["God", "[ˈɡɑːd]", "Աստված"],
    ["last", "[ˈlæst]", "վերջին, անցյալ, տևել"],
    ["let's", "[lets]", "եկեք, թույլ տվեք"],
    ["message", "[ˈmesədʒ]", "հաղորդագրություն"],
    ["send", "[ˈsend]", "ուղարկել"],
    ["serious", "[ˈsɪriəs]", "լուրջ"],
    ["sleep", "[sˈliːp]", "քուն, քնել"],
    ["start", "[ˈstɑːrt]", "սկիզբ, սկսել"],
    ["take", "[ˈteɪk]", "վերցնել"],
    ["almost", "[ˈɔːlmoʊst]", "գրեթե, համարյա"],
    ["begin", "[bɪˈɡɪn]", "սկսել, սկսվել"],
    ["build", "[ˈbɪld]", "կառուցել"],
    ["cheaply", "[ˈtʃiːpli]", "էժան"],
    ["cup", "[kʌp]", "բաժակ, գավաթ"],
    ["easily", "[ˈiːzəli]", "հեշտությամբ"],
    ["expensively", "[ɪkˈspensɪvli]", "թանկ"],
    ["faster", "[ˈfæstər]", "ավելի արագ"],
    ["glass", "[ˈɡlæs]", "ապակի, բաժակ"],
    ["goods", "[ˈɡʊdz]", "ապրանքներ, ապրանք"],
    ["grammar", "[ˈɡræmər]", "քերականություն"],
    ["hate", "[ˈheɪt]", "ատելություն, զզվանք, ատել"],
    ["improve", "[ˌɪmˈpruːv]", "բարելավել, կատարելագործել"],
    ["kind", "[ˈkaɪnd]", "տեսակ, բարի"],
    ["listening", "[ˈlɪsənɪŋ]", "լսելը"],
    ["morning", "[ˈmɔːrnɪŋ]", "առավոտ"],
    ["offer", "[ˈɒfər]", "առաջարկ, առաջարկել"],
    ["product", "[ˈprɑːdəkt]", "ապրանք, արտադրանք, մթերք"],
    ["rarely", "[ˈrerli]", "հազվադեպ, բացառիկ կերպով"],
    ["rule", "[ˈruːl]", "կանոն, կանոնադրություն, ղեկավարել"],
    ["bring", "[ˈbrɪŋ]", "բերել"],
    ["dinner", "[ˈdɪnər]", "ընթրիք"],
    ["first", "[ˈfɝːst]", "առաջին"],
    ["follow", "[ˈfɑːloʊ]", "հետևել"],
    ["great", "[ˈɡreɪt]", "մեծ"],
    ["lunch", "[ˈlʌntʃ]", "ճաշ, նախաճաշել"],
    ["museum", "[mjuːˈziːəm]", "թանգարան"],
    ["pleasure", "[ˈpleʒər]", "հաճույք"],
    ["podcast", "[ˈpɒdkɑːst]", "աուդիո շոու հեռարձակում, փոդքասթ"],
    ["present", "[ˈprezənt]", "ներկա, ներկայացնել"],
    ["project", "[prəˈdʒekt]", "նախագիծ, ծրագիր"],
    ["recommendation", "[ˌrekəmənˈdeɪʃən]", "առաջարկություն"],
    ["sandwich", "[ˈsænwɪdʒ]", "սենդվիչ"],
    ["second", "[ˈsekənd]", "երկրորդ, վայրկյան"],
    ["theatre", "[ˈθiːətər]", "թատրոն"],
    ["third", "[ˈθɝːd]", "երրորդ"],
    ["tomorrow", "[təˈmɑːˌroʊ]", "վաղը, էգուց"],
    ["will", "[wɪl]", "կամք, ցանկություն"],
    ["immediately", "[ˌɪˈmiːdiətli]", "անմիջապես, անհապաղ"],
    ["office", "[ˈɑːfəs]", "գրասենյակ, վարչություն, հիմնարկ"],
    ["party", "[ˈpɑːrti]", "երեկույթ, կուսակցություն, կուսակցական"],
    ["pass", "[ˈpæs]", "հանձնել, անցնել, պաս"],
    ["refuse", "[rəˈfjuːz]", "մերժել, հրաժարվել, թափոն"],
    ["restaurant", "[ˈrestəˌrɑːnt]", "ռեստորան"],
    ["sign", "[ˈsaɪn]", "նշան, ստորագրել"],
    ["successful", "[səkˈsesfəl]", "հաջողակ, բարեհաջող"],
    ["tonight", "[təˈnaɪt]", "այսօրվա երեկոն, այս գիշեր"],
    ["arrive", "[əˈraɪv]", "ժամանել"],
    ["convenient", "[kənˈviːnjənt]", "հարմար, պատշաճ"],
    ["disagree", "[ˌdɪsəˈɡriː]", "չհամաձայնվել"],
    ["funny", "[ˈfʌni]", "զվարճալի"],
    ["hard", "[ˈhɑːrd]", "դժվար, ուժեղ, կոպիտ, ամուր"],
    ["harder", "[ˈhɑːdə]", "առավել դժվար, առավել ուժեղ"],
    ["higher", "[ˈhaɪər]", "բարձրագույն, վերին"],
    ["holiday", "[ˈhɑːlədeɪ]", "տոն, արձակուրդ"],
    ["loin", "[ˌlɔɪn]", "մեջք, գոտկատեղ"],
    ["laugh", "[ˈlæf]", "ծիծաղ, ծիծաղել"],
    ["match", "[ˈmætʃ]", "համընկնում, մրցություն, լուցկի"],
    ["ocean", "[ˈoʊʃən]", "օվկիանոս"],
    ["rent", "[ˈrent]", "վարձավճար, վարձում, վարձել"],
    ["sea", "[ˈsiː]", "ծով, ծովային"],
    ["shopping", "[ˈʃɑːpɪŋ]", "գնումներ"],
    ["solve", "[ˈsɑːlv]", "լուծել"],
    ["summer", "[ˈsʌmər]", "ամառ"],
    ["absent", "[ˈæbsənt]", "բացակա, բացակայող, բացակայել"],
    ["beach", "[ˈbiːtʃ]", "լողափ, ծովափ"],
    ["else", "[ˈels]", "այլապես, էլի"],
    ["gym", "[ˈdʒɪm]", "մարզասրահ, մարմնամարզություն"],
    ["graduate", "[ˈɡrædʒəwət]", "շրջանավարտ, գիտական աստիճան ունեցող"],
    ["long", "[ˈlɔːŋ]", "երկար, հեռավոր"],
    ["news", "[ˈnuːz]", "նորություններ"],
    ["surprise", "[sərˈpraɪz]", "անակնկալ,զարմանք"],
    ["whose", "[ˈhuːz]", "ում, որի"],
    ["present", "[ˈprezənt]", "ներկա, ներկայացնել"],
    ["last", "[ˈlæst]", "վերջին, տևել, լաստ"],
    ["invite", "[ˌɪnˈvaɪt]", "հրավիրել"],
    ["lend", "[ˈlend]", "պարտքով տալ"],
    ["opportunity", "[ˌɑːpərˈtuːnəti]", "հնարավորություն"],
    ["picnic", "[ˈpɪkˌnɪk]", "պիկնիկ, հաճելի ժամանակ, զբոսախնջույք"],
    ["prepare", "[priˈper]", "նախապատրաստել, նախապատրաստվել, պատրաստել, պատրաստվել"],
    ["return", "[rəˈtɝːn]", "վերադարձ, վերադարձնել"],
    ["weekend", "[ˈwiːˌkend]", "շաբաթավերջ"],
    ["zoo", "[ˈzuː]", "կենդանաբանական այգի, գազանանոց"],
    ["ago", "[əˈɡoʊ]", "առաջ (ժամանակի առումով)"],
    ["app", "[ˈæp]", "հավելված"],
    ["compare", "[kəmˈper]", "համեմատել, համեմատություն"],
    ["fail", "[ˈfeɪl]", "անհաջողություն, ձախողվել"],
    ["high", "[ˈhaɪ]", "բարձր"],
    ["jump", "[ˈdʒəmp]", "ցատկել, ցատկ"],
    ["link", "[ˈlɪŋk]", "հղում, միանալ"],
    ["move", "[ˈmuːv]", "շարժվել, տեղափոխվել"],
    ["quickly", "[ˈkwɪkli]", "արագ, շուտով"],
    ["save", "[ˈseɪv]", "խնայել, պահպանել"],
    ["share", "[ˈʃer]", "կիսվել, բաժին, բաշխել"],
    ["training", "[ˈtreɪnɪŋ]", "մարզում, վարժեցում"],
    ["trip", "[ˈtrɪp]", "ճանապարհորդություն, սայթակել"],
    ["yesterday", "[ˈjestr̩di]", "երեկ"],
    ["anything", "[ˈeniˌθɪŋ]", "ինչ-որ բան, որեվէ բան"],
    ["badminton", "[ˈbædˌmɪntən]", "բադմինտոն"],
    ["explain", "[ɪkˈspleɪn]", "բացատրել, մեկնաբանել"],
    ["forget", "[fərˈɡet]", "մոռանալ"],
    ["Friday", "[ˈfraɪdeɪ]", "Ուրբաթ"],
    ["lock", "[ˈlɑːk]", "կողպեք, փականք, կողպել"],
    ["Monday", "[ˈmʌndeɪ]", "Երկուշաբթի"],
    ["night", "[ˈnaɪt]", "գիշեր, գիշերային, մութ"],
    ["past", "[ˈpæst]", "անցած, անցյալ"],
    ["physical", "[ˈfɪzɪkəl]", "ֆիզիկական"],
    ["Saturday", "[ˈsætr̩deɪ]", "շաբաթ"],
    ["sound", "[ˈsaʊnd]", "ձայն, հնչել"],
    ["Sunday", "[ˈsʌndeɪ]", "կիրակի"],
    ["Thursday", "[ˈθɝːzdeɪ]", "հինգշաբթի"],
    ["Tuesday", "[ˈtuːzdi]", "երեքշաբթի"],
    ["Wednesday", "[ˈwenzdeɪ]", "չորեքշաբթի"],
    ["word", "[ˈwɝːd]", "բառ"],
    ["country house", "[ˈkʌntri haʊs]", "ամառանոց, գյուղական տուն"],
    ["before", "[bɪˈfɔːr]", "առաջ"],
    ["beginning", "[bɪˈɡɪnɪŋ]", "սկիզբ, սկսնակ"],
    ["birthday", "[ˈbɝːθˌde]", "ծննդյան օր"],
    ["end", "[ˈend]", "վերջ"],
    ["large", "[ˈlɑːrdʒ]", "ընդարձակ, լայն, մեծ չափի, խոշոռ"],
    ["marry", "[ˈmeri]", "ամուսնանալ"],
    ["modern", "[ˈmɑːdərn]", "ժամանակակից, արդի"],
    ["produce", "[prəˈduːs]", "արտադրել, արտադրանք"],
    ["secret", "[ˈsiːkrət]", "գաղտնիք, գաղտնի"],
    ["social", "[ˈsoʊʃəl]", "հասարակական, սոցիալական"],
    ["society", "[səˈsaɪəti]", "հասարակություն"],
    ["surname", "[ˈsɝːˌnem]", "ազգանուն"],
    ["technology", "[tekˈnɑːlɪdʒi]", "տեխնոլոգիա, տեխնիկական գիտություններ"],
    ["decide", "[dəˈsaɪd]", "որոշել"],
    ["earlier", "[ˈɝːliər]", "ավելի վաղ"],
    ["sweater", "[ˈswetər]", "սվիտեր, շահագործող"],
    ["winter", "[ˈwɪntər]", "ձմեռ, ձմեռային"],
    ["final", "[ˈfaɪnəl]", "վերջնական"],
    ["happily", "[ˈhæpɪli]", "ուրախությամբ, բարեհաջող"],
    ["include", "[ˌɪnˈkluːd]", "ներառել"],
    ["interview", "[ˈɪntərˌvjuː]", "հարցազրույց"],
    ["left", "[ˈleft]", "ձախ"],
    ["turn", "[ˈtɝːn]", "շրջադարձ, պտույտ, շրջել"],
    ["offer", "[ˈɒfər]", "առաջարկ, առաջարկել"],
    ["hope", "[hoʊp]", "հույս, հուսալ"],
    ["table tennis", "[ˈteɪbəl ˌtenɪs]", "սեղանի թենիս"],
    ["animal", "[ˈænəməl]", "կենդանի, անասուն"],
    ["body", "[ˈbɑːdi]", "մարմին"],
    ["cry", "[ˈkraɪ]", "լաց լինել, լաց, ճիչ"],
    ["depend", "[dəˈpend]", "կախված լինել"],
    ["face", "[ˈfeɪs]", "դեմք, երես, տեսք"],
    ["forest", "[ˈfɔːrəst]", "անտառ"],
    ["hair", "[ˈher]", "մազեր, մազ, մազային"],
    ["kill", "[ˈkɪl]", "սպանել, սպանություն"],
    ["kiss", "[ˈkɪs]", "համբույր, համբուրել"],
    ["minute", "[ˈmɪnət]", "րոպե"],
    ["myself", "[ˌmaɪˈself]", "ես ինքս, ինձ, ինքս ինձ"],
    ["suddenly", "[sʌdənli]", "հանկարծակի"],
    ["wash", "[ˈwɑːʃ]", "լվանալ, լվանալը"],
    ["achieve", "[əˈtʃiːv]", "հասնել, նվաճել"],
    ["aim", "[ˈeɪm]", "նպատակ"],
    ["always", "[ˈɔːlweɪz]", "միշտ"],
    ["celebrate", "[ˈseləˌbret]", "տոնել"],
    ["die", "[ˈdaɪ]", "մեռնել"],
    ["everyone", "[ˈevriˌwʌn]", "բոլորը, յուրաքանչյուրը"],
    ["longer", "[ˈlɔːŋɡər]", "ավելի երկար, երկարատև"],
    ["discover", "[ˌdɪˈskəvər]", "հայտնաբերել"],
    ["armchair", "[ˈɑːrmˌtʃer]", "բազկաթոռ"],
    ["article", "[ˈɑːrtəkəl]", "հոդված"],
    ["beautifully", "[ˈbjuːtəfli]", "գեղեցկորեն"],
    ["break", "[ˈbreɪk]", "ընդմիջում, խզել"],
    ["camel", "[ˈkæməl]", "ուղտ, դեղնաշականակագույն"],
    ["cut", "[ˈkət]", "կտրել, կտրվածք"],
    ["favorite", "[ˈfeɪvərət]", "սիրելի, սիրված"],
    ["finger", "[ˈfɪŋɡər]", "մատ"],
    ["flower", "[ˈflaʊər]", "ծաղիկ"],
    ["fly", "[ˈflaɪ]", "թռնել, ճանճ"],
    ["himself", "[ˌhɪmˈself]", "ինքն իրեն, իրեն"],
    ["keep", "[ˈkiːp]", "պահել, գլխավոր աշտարակ"],
    ["loudly", "[ˈlaʊdli]", "ուժեղ հնչող, բարձրաձայն"],
    ["pen", "[ˈpen]", "գրիչ"],
    ["perfectly", "[ˈpɝːfəktli]", "կատարելապես, լիովին"],
    ["promise", "[ˈprɑːməs]", "խոստանալ, խոստում"],
    ["sing", "[ˈsɪŋ]", "երգել"],
    ["sit down", "[ˈsɪt ˈdaʊn]", "նստել"],
    ["break down", "[ˈbreɪk ˈdaʊn]", "քանդել, քանդվել"],
    ["studies", "[ˈstʌdiz]", "ուսում, կրթություն"],
    ["bridge", "[ˈbrɪdʒ]", "կամուրջ"],
    ["championship", "[ˈtʃæmpiənˌʃɪp]", "առաջնություն, չեմպիոնատ"],
    ["competition", "[ˌkɑːmpəˈtɪʃən]", "մրցակցություն, կոնկուրս, մրցախաղ"],
    ["key", "[ˈkiː]", "բանալի, ստեղն"],
    ["lawyer", "[ˈlɔɪər]", "փաստաբան, իրավաբան"],
    ["mathematics", "[ˌmæθəˈmætɪks]", "մաթեմատիկա"],
    ["mean", "[ˈmiːn]", "նշանակում է, միջին արժեք"],
    ["phrase", "[ˈfreɪz]", "արտահայտություն, դարձված"],
    ["quietly", "[ˈkwaɪətli]", "լուռ"],
    ["sentence", "[ˈsentəns]", "նախադասություն, դատապարտել, դատավճիռ"],
    ["sorry", "[ˈsɑːri]", "ներողություն, ափսոսել"],
    ["wallet", "[ˈwɒlət]", "դրամապանակ"],
    ["banana", "[bəˈnænə]", "բանան"],
    ["beer", "[ˈbɪr]", "գարեջուր"],
    ["copy", "[ˈkɑːpi]", "պատճեն, պատճենել, նմուշի օրինակ"],
    ["desk", "[ˈdesk]", "սեղան, գրասեղան, աշխատանքային սեղան"],
    ["fall", "[ˈfɑːl]", "անկում, ընկնել, վայր գցել"],
    ["kilo", "[ˈkɪˌloʊ]", "կիլոգրամ"],
    ["paper", "[ˈpeɪpər]", "թուղթ, թերթ, թղթյա"],
    ["potatoes", "[pəˈteɪtoʊz]", "կարտոֆիլ"],
    ["then", "[ˈðen]", "ապա, հետո"],
    ["tree", "[ˈtriː]", "ծառ"],
    ["wine", "[ˈwaɪn]", "գինի, գինու"],
    ["too", "[ˈtuː]", "նույնպես, չափազանց"],
    ["as", "[æz]", "ինչպես, որպես"],
    ["be away", "[bi əˈweɪ]", "հեռու լինել, չլինել տեղում"],
    ["famous", "[ˈfeɪməs]", "հայտնի"],
    ["gift", "[ˈɡɪft]", "նվեր"],
    ["guest", "[ˈɡest]", "հյուր"],
    ["hungry", "[ˈhʌŋɡri]", "քաղցած, սոված"],
    ["ill", "[ˈɪl]", "հիվանդ"],
    ["normal", "[ˈnɔːrməl]", "նորմալ, սովորական"],
    ["be out", "[bi ˈaʊt]", "լինել դռանը"],
    ["special", "[ˈspeʃəl]", "հատուկ, առանձնահատուկ"],
    ["terrible", "[ˈterəbəl]", "սարսափելի"],
    ["tired", "[ˈtaɪərd]", "հոգնած"],
    ["village", "[ˈvɪlɪdʒ]", "գյուղ, գյուղական"],
    ["worst", "[ˈwɝːst]", "ամենավատ, վատագույն"],
    ["alone", "[əˈloʊn]", "միայնակ"],
    ["autumn", "[ˈɒtəm]", "աշուն, աշնանային"],
    ["garden", "[ˈɡɑːrdən]", "այգի, պարտեզ, բանջարանոց"],
    ["healthy", "[ˈhelθi]", "առողջ"],
    ["ineffective", "[ˌɪnəˈfektɪv]", "անարդյունավետ"],
    ["pocket", "[ˈpɑːkət]", "գրպան, գրպանային"],
    ["spring", "[ˈsprɪŋ]", "գարուն, գարնանային"],
    ["bathroom", "[ˈbæˌθruːm]", "լոգարան, լողասենյակ"],
    ["century", "[ˈsentʃəri]", "դար"],
    ["living room", "[ˈlɪvɪŋ ruːm]", "հյուրասենյակ"],
    ["optimistic", "[ˌɑːptəˈmɪstɪk]", "լավատես, լավատեսական, օպտիմիստական"],
    ["pessimistic", "[ˌpesəˈmɪstɪk]", "պեսիմիստ"],
    ["behind", "[bəˈhaɪnd]", "ետևում, ետեվից, ետեվը"],
    ["classroom", "[ˈklæsˌruːm]", "դասարան, դասասենյակ"],
    ["corner", "[ˈkɔːrnər]", "անկյուն"],
    ["incorrect", "[ˌɪnkəˈrekt]", "սխալ"],
    ["unusual", "[ʌˈnjuːʒəwəl]", "անսովոր, արտասովոր"],
    ["yard", "[ˈjɑːrd]", "բակ, յարդ"],
    ["dentist", "[ˈdentəst]", "ատամնաբույժ"],
    ["engineer", "[ndʒəˈnɪr]", "ինժեներ, նախագծել"],
    ["impossible", "[ˌɪmˈpɑːsəbəl]", "անհնար"],
    ["lonely", "[ˈloʊnli]", "միայնակ, մեկուսացված"],
    ["motivating", "[ˈmoʊtɪveɪtɪŋ]", "մոտիվացնել, խթանել"],
    ["at all", "[ət ɔːl]", "ընդհանրապես"],
    ["but", "[bʌt]", "բայց, սակայն"],
    ["contract", "[ˈkɑːnˌtrækt]", "պայմանագիր, պայմանագրային"],
    ["fence", "[ˈfens]", "ցանկապատ, ցանկապատել"],
    ["herself", "[hərˈself]", "ինքն իրեն"],
    ["homemade", "[ˈhomˈmeɪd]", "տնական"],
    ["hot", "[hɑːt]", "տաք, շոգ"],
    ["hour", "[ˈaʊər]", "ժամ"],
    ["meeting", "[ˈmiːtɪŋ]", "հանդիպում, տեսակցություն, հավաքույթ"],
    ["over", "[ˈoʊvr̩]", "վերին, չափից ավելի, ավարտվել է, դրանից վերև, դրանից անց"],
    ["river", "[ˈrɪvər]", "գետ, գետային"],
    ["rope", "[roʊp]", "պարան, ճոպան"],
    ["sooner", "[ˈsuːnər]", "ավելի շուտ, շուտափույթ"],
    ["street", "[ˈstriːt]", "փողոց, փողոցի"],
    ["yourself", "[jərˈself]", "ինքդ քեզ, դուք ինքներդ"],
    ["by", "[baɪ]", "կողմից, միջոցով, օգնությամբ"],
    ["factor", "[ˈfæktər]", "գործոն, ֆակտոր, գործակից"],
    ["most", "[moʊst]", "մեծամասնություն, ամենաշատը, մեծ մասը"],
    ["smoking", "[ˈsmoʊkɪŋ]", "ծխելը"],
    ["topic", "[ˈtɑːpɪk]", "թեմա, քննարկման առարկա"],
    ["artist", "[ˈɑːrtəst]", "նկարիչ, արվեստագետ"],
    ["a bit", "[ə bɪt]", "մի քիչ"],
    ["company", "[ˈkʌmpəni]", "ընկերություն"],
    ["ground", "[ˈɡraʊnd]", "հող, հողանցում, գետին"],
    ["happiness", "[ˈhæpinəs]", "երջանկություն, հաջողություն"],
    ["kindness", "[ˈkaɪndnəs]", "բարություն, շնորհ, սիրալիրություն"],
    ["parcel", "[ˈpɑːrsəl]", "ծանրոց, փաթեթ, փաթեթավորել"],
    ["pie", "[ˈpaɪ]", "կարկանդակ, կաչաղակ"],
    ["process", "[ˈproʊses]", "գործընթաց, պրոցես, ընթացք"],
    ["accident", "[ˈæksədənt]", "վթար, դժբախտ պատահար, դեպք"],
    ["alcohol", "[ˈælkəˌhɑːl]", "ալկոհոլ, սպիրտ"],
    ["bill", "[ˈbɪl]", "հաշիվ"],
    ["cafe", "[kəˈfeɪ]", "սրճարան"],
    ["chance", "[ˈtʃæns]", "հնարավորություն"],
    ["criminal", "[ˈkrɪmənəl]", "քրեական, հանցագործ"],
    ["health", "[ˈhelθ]", "առողջություն"],
    ["if", "[ˈɪf]", "եթե"],
    ["point", "[ˈpɔɪnt]", "կետ, վայր"],
    ["screen", "[ˈskriːn]", "էկրան"],
    ["view", "[ˈvjuː]", "տեսք, հայացք, տեսարան"],
    ["bicycle", "[ˈbaɪsɪkəl]", "велосипед, ездить на велосипеде"],
    ["butter", "[ˈbʌtr̩]", "масло, сливочное масло, грубая лесть, намазывать маслом, грубо льстить"],
    ["cheese", "[ˈtʃiːz]", "сыр, творог, шишка, болван, важная персона, срыгивать молоко, переставать, бросать"],
    ["desert", "[ˈdezr̩t]", "пустынный, необитаемый, пустыня, заслуга, награда, покидать, дезертировать"],
    ["donkey", "[ˈdɑːŋki]", "осел, дурак"],
    ["egg", "[ˈeɡ]", "яйцо, яйцеклетка, зародыш, яичный, науськивать, подстрекать"],
    ["hand", "[ˈhænd]", "рука, сторона, стрелка, ручной, подручный, передавать, помочь, вручать"],
    ["head", "[ˈhed]", "голова, руководитель, глава, головной, главный, старший, возглавлять, направлять"],
    ["knowledge", "[ˈnɑːlədʒ]", "знание, знания, познания, осведомленность, знакомство, эрудиция, наука, известие"],
    ["material", "[məˈtɪriəl]", "материал, вещество, ткань, материя, данные, материальный, существенный"],
    ["several", "[ˈsevrəl]", "несколько, некоторые, каждый, свой"],
    ["between", "[bɪˈtwiːn]", "между, между"],
    ["blogger", "[ˈblɒɡər]", "блогер"],
    ["cell phone", "[ˈsel fəʊn]", "мобильник"],
    ["circus", "[ˈsɝːkəs]", "цирк, амфитеатр, круглая площадь, горный амфитеатр, занимательное зрелище"],
    ["delicious", "[dəˈlɪʃəs]", "восхитительный, очень вкусный, приятный, прелестный"],
    ["dictionary", "[ˈdɪkʃəˌneri]", "словарь"],
    ["fashionable", "[ˈfæʃənəbəl]", "модный, фешенебельный, светский, светский человек"],
    ["just", "[dʒəst]", "просто, только что, как раз, именно, точно, точный, справедливый, верный, заслуженный"],
    ["mostly", "[ˈmoʊstli]", "главным образом, по большей части, обычно, обыкновенно"],
    ["speech", "[ˈspiːtʃ]", "речь, выступление, слова, язык, спич, речевой"],
    ["accent", "[əkˈsent]", "акцент, ударение, произношение, речь, язык, акцентировать, подчеркивать"],
    ["appreciate", "[əˈpriːʃieɪt]", "ценить, оценивать, понимать, принимать во внимание, ощущать, повышать в ценности"],
    ["area", "[ˈeriə]", "площадь, область, район, зона, сфера, участок, пространство, поверхность, ареал, край"],
    ["brilliant", "[ˈbrɪljənt]", "блестящий, блистательный, выдающийся, бриллиант, диамант"],
    ["classical", "[ˈkəlæsɪkəl]", "классический, гуманитарный"],
    ["cycling", "[ˈsaɪkəlɪŋ]", "езда на велосипеде"],
    ["department", "[dəˈpɑːrtmənt]", "отдел, департамент, отделение, факультет, ведомственный"],
    ["enjoy", "[ɪnˈdʒɔɪ]", "наслаждаться, пользоваться, обладать, получать удовольствие"],
    ["excellent", "[ˈeksələnt]", "отличный, превосходный, великолепный"],
    ["experience", "[ɪkˈspɪriəns]", "опыт, опыт работы, переживание, стаж, испытывать, чувствовать, изведать"],
    ["jogging", "[ˈdʒɑːɡɪŋ]", "бег разминочным темпом"],
    ["swimming pool", "[ˈswɪmɪŋ puːl]", "бассейн"],
    ["twice", "[ˈtwaɪs]", "дважды, вдвое"],
    ["vocabulary", "[voˈkæbjəˌleri]", "словарь, лексика, терминология, лексикон, словарный"],
    ["shop assistant", "[ʃɒp əˈsɪstənt]", "продавец"],
    ["advanced", "[ədˈvænst]", "передовой, продвинутый, выдвинутый, прогрессивный, повышенного типа"],
    ["baby", "[ˈbeɪbi]", "ребенок, малыш, младенец, детский, младенческий, баловать"],
    ["basic", "[ˈbeɪsɪk]", "основной, основный, базовый, базисный, бейсик"],
    ["black", "[ˈblæk]", "черный, чернокожий, темный, чернь, черный, черный цвет, чернить, ваксить"],
    ["blonde", "[ˈblɑːnd]", "блондинка, шелковая кружевная ткань, светлый, белокурый, белокожий"],
    ["blue", "[ˈbluː]", "синий, голубой, лазурный, синить, воронить, синий цвет, голубой цвет"],
    ["building", "[ˈbɪldɪŋ]", "здание, строительство, строение, строительный, строящий"],
    ["cold", "[koʊld]", "холодный, слабый, холодность, холод, простуда, точно, достоверно"],
    ["colour", "[ˈkʌlər]", "цвет, колорит, оттенок, цветовой, цветной, окрашивать, красить"],
    ["device", "[dɪˈvaɪs]", "устройство, прибор, аппарат, приспособление, средство, прием, механизм, способ"],
    ["effect", "[ɪˈfekt]", "эффект, влияние, осуществление, действие, осуществлять, производить, выполнять"],
    ["financial", "[fəˈnænʃəl]", "финансовый, обладающий материальным достатком"],
    ["fresh", "[ˈfreʃ]", "свежий, новый, недавно, прохлада, половодье, свежеть"],
    ["fruit", "[ˈfruːt]", "фрукты, фрукт, плод, плоды, фруктовый, плодоносить"],
    ["green", "[ˈɡriːn]", "зеленый, сырой, свежий, зелень, зеленый цвет, зеленеть"],
    ["intermediate", "[ˌɪntərˈmiːdiət]", "промежуточный, средний, промежуточное звено, посредничать, вмешиваться"],
    ["low", "[ˈloʊ]", "низкий, небольшой, низко, слабо, тихо, мычать, мычание, низина"],
    ["pencil", "[ˈpensəl]", "карандаш, кисть, манера, стиль, рисовать, рисовать карандашом"],
    ["pleasant", "[ˈplezənt]", "приятный, милый, славный, отрадный, веселый, шутливый"],
    ["positive", "[ˈpɑːzətɪv]", "положительный, позитивный, уверенный, позитив, положительная степень"],
    ["profitable", "[ˈprɑːfətəbəl]", "доходный, выгодный, прибыльный, высокодоходный, полезный, благоприятный"],
    ["short", "[ˈʃɔːrt]", "короткий, краткий, коротко, кратко, резко, короткое замыкание"],
    ["small", "[ˈsmɒl]", "небольшой, малый, маленький, мелкий, узкая тонкая часть, нижнее белье"],
    ["strong", "[ˈstrɒŋ]", "сильный, крепкий, прочный, сильно, решительно, сильные, здоровые"],
    ["town", "[ˈtaʊn]", "город, городок, местечко, городской, превращать в город"],
    ["vegetables", "[ˈvedʒtəbəlz]", "овощи"],
    ["voice", "[ˌvɔɪs]", "голос, глас, мнение, залог, голосовой, выражать"],
    ["warm", "[ˈwɔːrm]", "теплый, горячий, жаркий, нагреваться, нагревать, согревание"],
    ["achievement", "[əˈtʃiːvmənt]", "достижение, успех, выполнение, подвиг"],
    ["amazing", "[əˈmeɪzɪŋ]", "удивительный, изумительный, поразительный"],
    ["common", "[ˈkɑːmən]", "общий, распространенный, обычный, частый, общее, выгон, пустырь, здравый смысл"],
    ["detail", "[dəˈteɪl]", "подробный, детальный, подробность, деталь, наряд, детализировать"],
    ["event", "[ɪˈvent]", "событие, случай, происшествие, результат, исход, такт, ядерное превращение, номер"],
    ["expert", "[ˈekspərt]", "эксперт, специалист, знаток, экспертный, опытный, квалифицированный"],
    ["friendly", "[ˈfrendli]", "дружественный, дружественно, дружелюбно, товарищеская встреча"],
    ["generous", "[ˈdʒenərəs]", "щедрый, великодушный, обильный, большой, благородный, изрядный, густой, интенсивный"],
    ["guy", "[ˈɡaɪ]", "парень, ванта, малый, оттяжка, пугало, чучело, осмеивать, издеваться, расчаливать"],
    ["hospitable", "[ˈhɑːˈspɪtəbəl]", "гостеприимный, открытый, восприимчивый"],
    ["number", "[ˈnʌmbr̩]", "число, номер, количество, цифра, сумма, числиться, нумеровать, насчитывать"],
    ["partner", "[ˈpɑːrtnər]", "партнер, соучастник, напарник, супруг, быть партнером, ставить в пару"],
    ["realise", "[ˈrɪəlaɪz]", "осознавать"],
    ["unique", "[juːˈniːk]", "уникальный, единственный в своем роде, уникум"],
    ["custom", "[ˈkʌstəm]", "заказы, обычай, привычка, пошлина, изготовленный на заказ"],
    ["differently", "[ˈdɪfrəntli]", "иначе, различно, по-иному, по-разному"],
    ["extremely", "[ɪkˈstriːmli]", "чрезвычайно, очень, крайне, в высшей степени, бесконечно"],
    ["fantastic", "[fænˈtæstɪk]", "фантастический, превосходный, чудесный, причудливый, гротескный, экстравагантный"],
    ["huge", "[ˈhjuːdʒ]", "огромный, громадный, гигантский"],
    ["learner", "[ˈlɝːnər]", "учащийся, учащийся, ученик"],
    ["lovely", "[ˈlʌvli]", "прекрасный, милый, красивый, красотка"],
    ["official", "[əˈfɪʃəl]", "официальный, служебный, должностной, чиновник, должностное лицо, служащий"],
    ["roof", "[ˈruːf]", "крыша, кровля, потолок, кров, покрывать, настилать крышу, крыть крышу"],
    ["shock", "[ˈʃɑːk]", "ударный, шоковый, шокировать, потрясать, шок, удар, потрясение, толчок"],
    ["solution", "[səˈluːʃən]", "решение, раствор, разрешение, растворение, отгадка, объяснение, распускание"],
    ["statement", "[ˈsteɪtmənt]", "заявление, утверждение, оператор, высказывание, изложение, ведомость, сообщение"],
    ["success", "[səkˈses]", "успех, удача, человек, пользующийся успехом"],
    ["typical", "[ˈtɪpəkəl]", "типичный, символический"],
    ["waste", "[ˈweɪst]", "отходы, потери, лом, пустыня, терять, тратить впустую, негодный, отработанный"],
    ["boss", "[ˈbɒs]", "босс, хозяин, шеф, руководитель, выступ, распоряжаться, управлять, хозяйничать"],
    ["carefully", "[ˈkerfəli]", "осторожно, внимательно, аккуратно, бережно, заботливо, с осторожностью"],
    ["credit card", "[ˈkredɪt kɑːd]", "кредитка"],
    ["elder sister", "[ˈeldə ˈsɪstə]", "старшая сестра"],
    ["following", "[ˈfɒloʊɪŋ]", "следующий, следующее, после, вследствие, вслед"],
    ["freelancer", "[ˈfriːlɑːnsə]", "человек, работающий не по найму, внештатный сотрудник"],
    ["greatly", "[ˈɡreɪtli]", "значительно, очень, весьма, возвышенно, благородно"],
    ["hang", "[ˈhæŋ]", "висеть, вешать, развесить, подвешивать, вид, наклон, смысл, манера, склон, скат"],
    ["inspire", "[ˌɪnˈspaɪr]", "вдохновлять, вдохновить, внушать, воодушевить, воодушевлять, вселить, вселять"],
    ["interrupt", "[ˌɪntəˈrəpt]", "прерывание, прерывать, перебивать, мешать"],
    ["lots of", "[lɒts əv]", "много / уйма"],
    ["opinion", "[əˈpɪnjən]", "мнение, взгляд, убеждение, воззрение, заключение специалиста, судебное решение"],
    ["others", "[ˈʌðəz]", "другие"],
    ["patient", "[ˈpeɪʃənt]", "пациент, больной, терпеливый, упорный, настойчивый"],
    ["picture", "[ˈpɪktʃər]", "картина, изображение, картинка, рисунок, изобразить, представлять себе"],
    ["poster", "[ˈpoʊstə]", "плакат, афиша, объявление, плакатный, рекламировать"],
    ["sale", "[ˈseɪl]", "продажа, распродажа, сбыт, продажа с торгов, продажа с аукциона"],
    ["shoes", "[ʃuːz]", "обувь"],
    ["store", "[ˈstɔːr]", "магазин, склад, универмаг, хранить, накапливать, запасной, запасный, готовый"],
    ["online store", "[ɒnˈlaɪn stɔː]", "интернет-магазин"],
    ["such", "[ˈsətʃ]", "такой, таковой, такой-то, определенный, такой, такие, таковой, те, тот"],
    ["teenager", "[ˈtiːˌnedʒər]", "подросток"],
    ["vet", "[ˈvet]", "ветеринар, ветеран, проверять, исследовать, заслуженный, маститый"],
    ["wall", "[ˈwɒl]", "стенной, настенный, стена, стенка, вал, барьер, бок, обносить стеной"],
    ["way", "[ˈweɪ]", "путь, способ, сторона, дорога, метод, далеко, значительно, чересчур"],
    ["weak", "[ˈwiːk]", "слабый, слабовольный, бессильный, слабоумный"],
    ["younger brother", "[ˈjʌŋɡə ˈbrʌðə]", "младший брат"],
    ["accountant", "[əˈkaʊntənt]", "бухгалтер, счетовод, ответчик"],
    ["bedroom", "[ˈbeˌdruːm]", "спальня"],
    ["bright", "[ˈbraɪt]", "яркий, светлый, ясный, ярко, блестяще, яркость"],
    ["chess", "[ˈtʃes]", "шахматы, оконная рама, шахматный"],
    ["college", "[ˈkɑːlɪdʒ]", "колледж, коллегия, университет, высшее учебное заведение, корпорация, тюрьма"],
    ["comment", "[ˈkɑːment]", "комментарий, замечание, отзыв, примечание, комментировать, делать замечания"],
    ["disappointed", "[ˌdɪsəˌpɔɪntəd]", "разочарованный, разочаровавшийся, огорченный, несбывшийся"],
    ["diving", "[ˈdaɪvɪŋ]", "дайвинг, ныряние, прыжки в воду, водолазный, пикирующий"],
    ["drawing", "[ˈdrɒɪŋ]", "чертеж, рисунок, рисование, графика, черчение, розыгрыш, вытягивание, волочение"],
    ["episode", "[ˈepɪsoʊd]", "эпизод"],
    ["expression", "[ɪkˈspreʃən]", "выражение, экспрессия, выразительность, оборот речи, выжимание"],
    ["gardening", "[ˈɡɑːrdənɪŋ]", "садоводство"],
    ["genius", "[ˈdʒiːnjəs]", "гений, гениальность, одаренность, гениальный человек, гениальная личность"],
    ["heavy", "[ˈhevi]", "тяжелый, сильный, густой, тяжело, сильно, тягостно, тяжеловес, тяжелое орудие"],
    ["hobby", "[ˈhɑːbi]", "хобби, конек, чеглок, палочка с лошадиной головой, лошадка, пони"],
    ["humour", "[ˈhjuːmər]", "юмор, настроение, чувство юмора, нрав, потакать, ублажать, уважить"],
    ["jacket", "[ˈdʒækət]", "куртка, жакет, рубашка, суперобложка, френч, надевать куртку, надевать жакет"],
    ["lamp", "[ˈlæmp]", "лампа, светильник, фонарь, ламповый, светить, освещать"],
    ["learning", "[ˈlɝːnɪŋ]", "изучение, познания, учение, ученость, учеба, изучающий"],
    ["lecture", "[ˈlektʃər]", "лекция, доклад, наставление, нотация, читать лекцию, отчитывать"],
    ["main", "[ˈmeɪn]", "основной, главный, магистральный, сильный, магистраль, главная часть"],
    ["nurse", "[ˈnɝːs]", "медсестра, няня, медицинская сестра, кормить, нянчить, ухаживать"],
    ["prison", "[ˈprɪzən]", "тюрьма, острог, тюремный, заключать в тюрьму"],
    ["resource", "[ˈriːsɔːrs]", "ресурс, средство, запас, возможность, находчивость, способ, изобретательность"],
    ["sense", "[ˈsens]", "смысл, чувство, ощущение, значение, чувствовать, ощущать, понимать"],
    ["shocked", "[ˈʃɑːkt]", "потрясенный"],
    ["skiing", "[ˈskiːɪŋ]", "катание на лыжах, лыжный спорт, лыжный"],
    ["suit", "[ˈsuːt]", "костюм, иск, масть, набор, комплект, устраивать, соответствовать, подходить"],
    ["surprised", "[sərˈpraɪzd]", "удивленный, изумленный, в изумлении"],
    ["taste", "[ˈteɪst]", "вкус, пристрастие, склонность, стиль, отведать, пробовать, дегустировать"],
    ["thief", "[ˈθiːf]", "вор, воровка, нагар"],
    ["towel", "[ˈtaʊəl]", "полотенце, вытирать полотенцем"],
    ["ambitious", "[æmˈbɪʃəs]", "честолюбивый, претенциозный, самолюбивый, стремящийся, жаждущий"],
    ["bed", "[ˈbed]", "спать, ложиться в постель, кровать, постель, слой, ложе, постельный"],
    ["bottle", "[ˈbɑːtəl]", "бутылка, флакон, бутыль, бутылочный, разливать по бутылкам"],
    ["church", "[ˈtʃɝːtʃ]", "церковь, храм, богослужение, церковный, духовный"],
    ["earth", "[ˈɝːθ]", "земля, заземление, грунт, земной шар, почва, заземлять, окучивать, закапывать"],
    ["empty", "[ˈempti]", "пустой, голодный, порожний, опорожнять, опорожняться, порожняк, порожняя тара"],
    ["energy", "[ˈenərdʒi]", "энергия, сила, силы, мощность, энергичность"],
    ["enthusiasm", "[enˈθuːziˌæzəm]", "энтузиазм, восторг, увлечение, восторженность, религиозное исступление"],
    ["experienced", "[ɪkˈspɪriənst]", "опытный, бывалый, знающий, сведущий"],
    ["feeling", "[ˈfiːlɪŋ]", "чувство, ощущение, настроение, чувствительный, прочувствованный"],
    ["full", "[ˈfʊl]", "полный, целый, наполненный, вполне, очень, как раз, валять, кроить широко"],
    ["joke", "[dʒoʊk]", "шутка, анекдот, посмешище, смешной случай, шутить, подшучивать, балагурить, дразнить"],
    ["narrow", "[ˈneroʊ]", "узкий, небольшой, теснить, суживать, узкая часть, узкий проход"],
    ["obviously", "[ˈɑːbviəsli]", "очевидно, явно, видно, ясно, по-видимому"],
    ["police", "[pəˈliːs]", "полиция, полицейский, полицейские силы, охранять, контролировать"],
    ["rainy", "[ˈreɪni]", "дождливый, дождевой"],
    ["road", "[roʊd]", "дорога, путь, шоссе, улица, рейд, дорожный"],
    ["round", "[ˈraʊnd]", "круглый, вокруг, кругом, вокруг, кругом, по, раунд, тур, круг, округлять"],
    ["slim", "[sˈlɪm]", "тонкий, стройный, изящный, слабый, похудеть, худеть, терять в весе"],
    ["sunny", "[ˈsʌni]", "солнечный, веселый, радостный, солнечно"],
    ["thin", "[ˈθɪn]", "тонкий, худой, тоненький, тонко, редеть, худеть, разжижать"],
    ["understanding", "[ˌʌndərˈstændɪŋ]", "понимание, взаимопонимание, разум, понимающий, разумный, отзывчивый, чуткий"],
    ["uninformative", "[ˌʌnɪnˈfɔːmətɪv]", "малоинформативный"],
    ["wide", "[ˈwaɪd]", "широкий, большой, обширный, далекий, широко, далеко, повсюду, мимо цели"],
    ["anymore", "[ˌeniˈmɔːr]", "больше не, уже не"],
    ["belong", "[bɪˈlɔːŋ]", "принадлежать, относиться, находиться, происходить, быть связанным, быть родом из"],
    ["class", "[ˈklæs]", "класс, тип, урок, занятие, классовый, классный, классифицировать"],
    ["clean", "[ˈkliːn]", "чистый, ровный, чистить, очищать, чистка, уборка, начисто, полностью"],
    ["difference", "[ˈdɪfərəns]", "разница, различие, разность, разногласие, отличать, вычислять разность"],
    ["guitar", "[ɡəˈtɑːr]", "гитара"],
    ["jazz", "[ˈdʒæz]", "джаз, пестрота, джазовый, кричащий, грубый"],
    ["lead", "[ˈled]", "приводить, вести, свинец, руководство, шаг, свинцовый"],
    ["lifestyle", "[ˈlaɪfˌstaɪl]", "стиль жизни"],
    ["magazine", "[ˈmæɡəˌzin]", "журнал, магазин, кассета, магазинный, журнальный"],
    ["newspaper", "[ˈnuːzˌpepər]", "газета, газетный"],
    ["piano", "[piˈænoʊ]", "пианино, фортепьяно, рояль, фортепьянный, пиано"],
    ["pop music", "[ˈpɒp mjuːzɪk]", "поп-музыка"],
    ["public", "[ˈpʌblɪk]", "общественный, государственный, общественность, публика, народ, трактир"],
    ["relative", "[ˈrelətɪv]", "относительный, сравнительный, взаимный, родственник, родственница"],
    ["rock", "[ˈrɑːk]", "рок, скала, камень, утес, качаться, качать, укачивать, каменный, горный"],
    ["snowboarding", "[ˈsnəʊbɔːdɪŋ]", "сноубординг"],
    ["transport", "[trænˈspɔːrt]", "транспорт, перевозка, транспортный, транспортировать"],
    ["violin", "[vaɪəˈlɪn]", "скрипка, скрипач"],
    ["close", "[kloʊz]", "близкий, близко, почти, закрытие, конец, закрывать, близ"],
    ["among", "[əˈmʌŋ]", "среди, между, у, из, в числе, из числа, посреди"],
    ["crazy", "[ˈkreɪzi]", "сумасшедший, безумный, псих, фанатик, чрезвычайно"],
    ["deep", "[ˈdiːp]", "глубокий, глубинный, низкий, глубоко, вглубь, глубина, глубь, бездна, море"],
    ["dream", "[ˈdriːm]", "мечтать, грезить, фантазировать, сниться, мечта, сон, сновидение, греза, видение"],
    ["economics", "[ˌekəˈnɑːmɪks]", "экономика, экономическая наука, политическая экономия, народное хозяйство"],
    ["ingredient", "[ɪŋˈɡriːdiənt]", "ингредиент, составная часть"],
    ["irregular", "[ɪˈreɡjələr]", "нерегулярный, неправильный, неравномерный, неровный, незаконный, нестандартный"],
    ["interested", "[ˈɪntrəstəd]", "заинтересованный, корыстный, пристрастный, предубежденный"],
    ["lake", "[ˈleɪk]", "озеро, красочный лак"],
    ["mad", "[ˈmæd]", "безумный, сумасшедший, бешеный, сходить с ума, сводить с ума"],
    ["natural", "[ˈnætʃərəl]", "природный, естественный, натуральный, бекар, кретин, идиот от рождения, дурачок"],
    ["regular", "[ˈreɡjələr]", "регулярный, обычный, очередной, регулярные войска, монах"],
    ["risk", "[ˈrɪsk]", "риск, рисковать, отваживаться"],
    ["unfortunately", "[ʌnˈfɔːrtʃənətli]", "к сожалению, к несчастью"],
    ["unofficial", "[ˌʌnəˈfɪʃəl]", "неофициальный"],
    ["verb", "[ˈvɝːb]", "глагол"],
    ["apologise", "[əˈpɒlədʒaɪz]", "извиняться, просить прощения, приносить официальные извинения"],
    ["clinic", "[ˈklɪnɪk]", "клиника, лечебница, медпункт, амбулатория, клинический"],
    ["confidence", "[ˈkɑːnfədəns]", "уверенность, доверие, вера, самоуверенность, смелость, самонадеянность"],
    ["dependent", "[dəˈpendənt]", "зависимый, зависящий, подчиненный, иждивенец, подчиненный, служащий"],
    ["financially", "[fəˈnænʃəli]", "в финансовом отношении"],
    ["illness", "[ˈɪlnəs]", "болезнь, заболевание, нездоровье, хворь, страдание, немочь"],
    ["independent", "[ˌɪndəˈpendənt]", "независимый, самостоятельный, конгрегационалист"],
    ["necklace", "[ˈnekləs]", "ожерелье, колье, монисто"],
    ["nervous", "[ˈnɝːvəs]", "нервный, нервничающий, нервозный, слабонервный, взволнованный, робкий, возбудимый"],
    ["salary", "[ˈsæləri]", "зарплата, заработная плата, оклад, жалованье"],
    ["sick", "[ˈsɪk]", "больной, болезненный, больные, болезнь, натравливать"],
    ["surgeon", "[ˈsɝːdʒən]", "хирург, военврач, военный врач, офицер медицинской службы, военно-морской врач"],
    ["task", "[ˈtæsk]", "задача, задание, урок, урочная работа, задавать работу, перегружать"],
    ["translator", "[trænsˈleɪtər]", "переводчик, конвертор, переводческий"],
    ["unimportant", "[əˌnɪmˈpɔːrtənt]", "неважный, малозначительный"],
    ["well-paid", "[ˈwelˈpeɪd]", "хорошо оплачиваемый"],
    ["agency", "[ˈeɪdʒənsi]", "агентство, представительство, деятельность, посредничество, сила, фактор, действие"],
    ["behaviour", "[bəˈheɪvjər]", "поведение, режим, поступки, манеры"],
    ["classmate", "[ˈklæˌsmet]", "одноклассник, соученик, школьный товарищ"],
    ["client", "[ˈklaɪənt]", "клиент, заказчик, пользователь, подзащитный, покупатель, постоянный покупатель"],
    ["comfortable", "[ˈkʌmfərtəbəl]", "удобный, комфортабельный, комфортный, стеганое ватное одеяло"],
    ["complain", "[kəmˈpleɪn]", "жаловаться, пожаловаться, сетовать, выражать недовольство, поплакаться, ныть"],
    ["condition", "[kənˈdɪʃən]", "состояние, условие, положение, обусловливать, кондиционировать"],
    ["consider", "[kənˈsɪdər]", "считать, считаться, рассматривать, учитывать, принимать во внимание, полагать"],
    ["design", "[dəˈzaɪn]", "дизайн, разработка, конструкция, проектировать, разрабатывать"],
    ["employee", "[emˌplɔɪi]", "работник, служащий, работающий по найму"],
    ["exist", "[ɪgˈzɪst]", "существовать, быть, находиться, жить, иметься, просуществовать"],
    ["groupmate", "[ˈɡruːpmeɪt]", "одногруппник"],
    ["invest", "[ˌɪnˈvest]", "инвестировать, вкладывать, вкладывать деньги, облекать, облачать, окружать"],
    ["investor", "[ˌɪnˈvestər]", "вкладчик"],
    ["matter", "[ˈmætər]", "материя, вопрос, дело, вещество, материал, иметь значение, гноиться"],
    ["overpay", "[ˌoʊvəˈpeɪ]", "переплачивать"],
    ["perfect", "[pərˈfekt]", "совершенствовать, идеальный, совершенный, перфект"],
    ["profit", "[ˈprɑːfət]", "прибыль, доход, выгода, польза, барыш, нажива, воспользоваться, получать прибыль"],
    ["quality", "[ˈkwɑːləti]", "качество, высокое качество, сорт, высококачественный"],
    ["reliable", "[rəˈlaɪəbəl]", "надежный, достоверный, прочный, заслуживающий доверия, благонадежный"],
    ["robot", "[ˈroʊbɑːt]", "робот, автомат, автоматический"],
    ["sales", "[ˈseɪlz]", "торговый, распродажа по сниженным ценам"],
    ["satisfaction", "[ˌsætəsˈfækʃən]", "удовлетворение, компенсация, сатисфакция, искупление, расплата"],
    ["science", "[ˈsaɪəns]", "наука, естественные науки, знание, умение, научный"],
    ["seriously", "[ˈsɪriəsli]", "серьезно, всерьез"],
    ["shirt", "[ˈʃɝːt]", "рубашка, рубаха, сорочка, блузка, блуза, надевать рубашку"],
    ["size", "[ˈsaɪz]", "размер, объем, формат, величина, сечение, калибровать, проклеивать, шлихтовать"],
    ["skirt", "[ˈskɝːt]", "юбка, подол, край, окраина, женщина, пола, обходить, огибать, идти вдоль края"],
    ["surprising", "[sərˈpraɪzɪŋ]", "удивительный, неожиданный, поразительный"],
    ["tourist", "[ˈtʊrəst]", "туристический, туристский, турист, путешественник"],
    ["weigh", "[ˈweɪ]", "весить, взвешивать, взвешиваться, оценивать, влиять, сравнивать, иметь вес"],
    ["chemistry", "[ˈkeməstri]", "химия, химическая технология"],
    ["community", "[kəˈmjuːnəti]", "сообщество, община, общество, общность, общественный"],
    ["composition", "[ˌkɑːmpəˈzɪʃən]", "состав, композиция, структура, сочинение, композиционный"],
    ["economy", "[ɪˈkɑːnəmi]", "экономика, хозяйство, экономия, экономичность, сбережения, бережливость, структура"],
    ["essay", "[eˈseɪ]", "эссе, очерк, опыт, этюд, попытка, набросок, подвергать испытанию, пытаться"],
    ["history", "[hɪstr̩i]", "история, прошлое, историческая наука, историческая пьеса"],
    ["image", "[ˈɪmədʒ]", "изображение, образ, имидж, облик, отражение, отображать, изображать"],
    ["model", "[ˈmɑːdəl]", "модель, образец, макет, моделировать, лепить, образцовый, примерный"],
    ["organisation", "[ˌɔːɡənaɪˈzeɪʃən]", "организация"],
    ["outside", "[ˈaʊtˈsaɪd]", "вне, снаружи, наружу, вне, за пределами, внешний, наружный, наружность"],
    ["powerful", "[ˈpaʊərfəl]", "мощный, сильный, могущественный, могучий, влиятельный, яркий, значительный"],
    ["reputation", "[ˌrepjəˈteɪʃən]", "репутация, реноме, слава, доброе имя"],
    ["tall", "[ˈtɒl]", "высокий, невероятный, хвастливый, гордо, хвастливо, важно"],
    ["tax", "[ˈtæks]", "налог, сбор, пошлина, бремя, оброк, облагать налогом, таксировать, обложить"],
    ["windy", "[ˈwindi]", "ветреный, обдуваемый ветром, пустой, болтун"],
    ["present", "[ˈprezənt]", "настоящий, представлять, настоящее время, налицо"],
    ["around", "[əˈraʊnd]", "вокруг, около, вблизи, кругом, поблизости, вокруг, около, по, приблизительно, за"],
    ["develop", "[dɪˈveləp]", "развиваться, развивать, разрабатывать, совершенствовать, совершенствоваться"],
    ["fitness", "[ˈfɪtnəs]", "фитнес, пригодность, приспособленность, годность, соответствие"],
    ["imagination", "[ˌɪˌmædʒəˈneɪʃən]", "воображение, фантазия, образ, творческая фантазия"],
    ["income", "[ˈɪnˌkəm]", "доход, прибыль, поступления, заработок"],
    ["marketing", "[ˈmɑːrkətɪŋ]", "маркетинг, сбыт, торговля, предметы торговли, изучение условий рынка"],
    ["musician", "[mjuːˈzɪʃən]", "музыкант, композитор, вокалист, оркестрант, музыкант-исполнитель"],
    ["on your own", "[ɒn jɔːr əʊn]", "самостоятельно"],
    ["personal", "[ˈpɝːsənəl]", "личный, персональный, субъективный, светская хроника в газете"],
    ["president", "[ˈprezəˌdent]", "президент, председатель, ректор, губернатор, директор компании, директор банка"],
    ["scientist", "[ˈsaɪəntəst]", "ученый, естествоиспытатель"],
    ["separately", "[ˈsepərətli]", "отдельно, особо, врозь, вразброс"],
    ["series", "[ˈsɪriz]", "серия, ряд, цикл, цепь, группа, выпуск, комплект, круг, система, свита, отдел"],
    ["skill", "[ˈskɪl]", "навык, умение, мастерство, квалификация, искусство, ловкость, сноровка, выучка"],
    ["specialist", "[ˈspeʃələst]", "специалист"],
    ["stable", "[ˈsteɪbəl]", "стабильный, устойчивый, конюшня, хлев, стойло, манеж, ставить в конюшню"],
    ["world", "[ˈwɝːld]", "мир, свет, вселенная, общество, царство, мировой, всемирный"],
    ["by yourself", "[baɪ jɔːˈself]", "сам (один)"],
    ["by myself", "[baɪ maɪˈself]", "сам / самостоятельно"],
    ["cousin", "[ˈkʌzən]", "кузен, двоюродный брат, кузина, двоюродная сестра, родственник"],
    ["dish", "[ˈdɪʃ]", "блюдо, тарелка, кушанье, миска, красотка, выгибать, класть на блюдо"],
    ["inexpensive", "[ˌɪnɪkˈspensɪv]", "недорогой, дешевый"],
    ["noisy", "[ˈnɔɪzi]", "шумный, шумящий, шумливый, кричащий, галдящий, яркий"],
    ["offline", "[ˌɒfˈlaɪn]", "автономный, оффлайновый, выключенный, автономно, в оффлайне"],
    ["proud", "[ˈpraʊd]", "гордый, надменный, горделивый, высокомерный, великолепный, ретивый, самодовольный"],
    ["quite", "[ˈkwaɪt]", "довольно, вполне, совсем, совершенно, очень, полностью, действительно, всецело"],
    ["routine", "[ruːˈtiːn]", "рутинный, обычный, текущий, шаблонный, рутина, распорядок, шаблон"],
    ["step", "[ˈstep]", "шаг, ступень, подножка, порог, степ, ход, ступать, шагать, делать шаги"],
    ["tongue", "[ˈtəŋ]", "язык, шпунт, язычок, шип, болтать, лизать, языковый"],
    ["traditional", "[trəˈdɪʃənəl]", "традиционный, основанный на обычае"],
    ["account", "[əˈkaʊnt]", "счет, расчет, отчет, приходиться, отчитываться, бухгалтерский"],
    ["behave", "[bəˈheɪv]", "вести себя, поступать, работать"],
    ["crisis", "[ˈkraɪsəs]", "кризис, перелом, кризисный"],
    ["economic", "[ˌekəˈnɑːmɪk]", "экономический, хозяйственный, экономичный, экономный, экономически выгодный"],
    ["premium", "[ˈpriːmiəm]", "премия, надбавка, страховая премия, высшего качества, первосортный"],
    ["purpose", "[ˈpɝːpəs]", "цель, назначение, намерение, иметь целью, намереваться"],
    ["respect", "[rəˈspekt]", "уважать, соблюдать, почитать, чтить, уважение, отношение, почтение, внимание"],
    ["spoil", "[ˌspɔɪl]", "портить, портиться, баловать, грабить, добыча, вынутый грунт, трофей"],
    ["strangely", "[ˈstreɪndʒli]", "странно, необыкновенно, сдержанно"],
    ["vegetarian", "[ˌvedʒəˈteriən]", "вегетарианский, вегетарианец"],
    ["version", "[ˈvɝːʒən]", "версия, вариант, текст, перевод, переводить"],
    ["acquaintance", "[əˈkweɪntəns]", "знакомство, ознакомление, знакомый, знакомый"],
    ["adapt", "[əˈdæpt]", "адаптировать, приспосабливать, приспосабливаться, применять, применяться"],
    ["architect", "[ˈɑːrkəˌtekt]", "архитектор, зодчий, создатель, творец"],
    ["depressed", "[dəˈprest]", "подавленный, угнетенный, сниженный, пониженный, унылый, вогнутый, ослабленный"],
    ["depression", "[dəˈpreʃən]", "депрессия, снижение, углубление, впадина, разрежение, уныние, тоска"],
    ["difficulty", "[ˈdɪfɪˌkəlti]", "трудность, затруднение, препятствие"],
    ["economist", "[ɪˈkɑːnəməst]", "экономист, бережливый человек"],
    ["explanation", "[ˌekspləˈneɪʃən]", "объяснение, разъяснение, толкование, выяснение, оправдание"],
    ["firm", "[ˈfɝːm]", "фирма, торговый дом, твердый, крепкий, твердо, крепко, укреплять"],
    ["grow", "[ˈɡroʊ]", "расти, выращивать, увеличиваться, возрасти, становиться, вырастать, отрастить"],
    ["immigrant", "[ˈɪməɡrənt]", "иммигрант, переселенец, иммигрантский, переселяющийся"],
    ["provide", "[prəˈvaɪd]", "обеспечивать, предоставлять, предусматривать, давать, снабжать, прокормить"],
    ["role", "[roʊl]", "роль"],
    ["suffer", "[ˈsʌfər]", "страдать, терпеть, испытывать, вытерпеть, претерпевать, выстрадать, сносить"],
    ["talented", "[ˈtæləntəd]", "талантливый, одаренный, даровитый"],
    ["district", "[ˈdɪˌstrɪkt]", "районный, окружной, район, округ, зона, участок, районировать"],
    ["easygoing", "[ˌiːzɪˈɡoʊɪŋ]", "спокойный, легкий, покладистый, добродушный, беззаботный, беспечный"],
    ["grade", "[ˈɡreɪd]", "класс, ранг, сорт, оценка, степень, качество, сортировать, нивелировать"],
    ["inside", "[ˌɪnˈsaɪd]", "внутрь, внутри, внутри, в, внутренний, скрытый, внутренности"],
    ["kid", "[ˈkɪd]", "обманывать, надувать, ребенок, малыш, парень, младший, молодой, лайковый"],
    ["mark", "[ˈmɑːrk]", "знак, марка, отметка, метка, след, оценка, отмечать, маркировать, помечать"],
    ["melodic", "[məˈlɑːdɪk]", "мелодичный, мелодический"],
    ["membership", "[ˈmembərˌʃɪp]", "членство, количество членов, членский"],
    ["overcrowded", "[ˌoʊvəˈkraʊdɪd]", "переполненный"],
    ["policeman", "[pəˈliːsmən]", "полицейский, полисмен"],
    ["schoolboy", "[ˈskuːlbɔɪ]", "школьник, ученик"],
    ["spacious", "[ˈspeɪʃəs]", "просторный, вместительный, обширный, широкий, поместительный, всеобъемлющий"],
    ["spoilt", "[ˈspɔɪlt]", "испорченный, избалованный, балованный"],
    ["unexpected", "[ˌʌnɪkˈspektəd]", "неожиданный, непредвиденный, нежданный, внезапный"],
    ["point", "[ˈpɔɪnt]", "точка, пункт, момент, очко, место, дело, смысл, указывать, острить, целиться, говорить"],
    ["age", "[ˈeɪdʒ]", "возраст, век, эпоха, период, старость, стареть, постареть, стариться, старить"],
    ["channel", "[ˈtʃænəl]", "канал, русло, желоб, проток, протока, проводить канал"],
    ["confident", "[ˈkɑːnfədənt]", "уверенный, самоуверенный, самонадеянный, наперсник, задушевный друг"],
    ["elementary", "[ˌelɪˈmentəri]", "элементарный, начальный, простой, первичный, неразложимый, первоначальный"],
    ["grandparents", "[ˈɡrændˌperənts]", "дедушка и бабушка"],
    ["granny", "[ˈɡræni]", "бабушка, бабуля, бабуся, старушка, тяжелое орудие"],
    ["mountain", "[ˈmaʊntən]", "гора, множество, куча, масса, горный, высокогорный, гористый, нагорный"],
    ["outstanding", "[ˌɑːwtˈstændɪŋ]", "выдающийся, незаурядный, знаменитый, невыполненный, неуплаченный, просроченный"],
    ["pregnant", "[ˈpreɡnənt]", "беременная, богатый, чреватый, содержательный, полный смысла, полный значения"],
    ["smell", "[ˈsmel]", "запах, обоняние, амбре, чуточка, пахнуть, чувствовать запах, понюхать"],
    ["stressful", "[ˈstresfəl]", "стрессовый, напряженный, создающий напряжение, ведущий к стрессу"],
    ["completely", "[kəmˈpliːtli]", "полностью, совершенно, вполне, всецело, вконец, безраздельно, сполна, дотла, дочиста"],
    ["risky", "[ˈrɪski]", "рискованный, опасный, авантюрный"],
    ["interest", "[ˈɪntrəst]", "заинтересовать, интересовать, интерес, процент, заинтересованность"],
    ["accurate", "[ˈækjərət]", "точный, тщательный, меткий, калиброванный, правильный"],
    ["attention", "[əˈtenʃən]", "внимание, внимательность, уход, забота, заботливость, ухаживание"],
    ["attract", "[əˈtrækt]", "привлекать, притягивать, влечь, манить, прельстить, пленять, прельщать, приваживать"],
    ["attractive", "[əˈtræktɪv]", "привлекательный, притягательный, заманчивый, привлекающий, располагающий"],
    ["description", "[dəˈskrɪpʃən]", "описание, изображение, вид, род, сорт, вычерчивание"],
    ["expect", "[ɪkˈspekt]", "ожидать, рассчитывать, ждать, надеяться, предполагать, полагать, думать"],
    ["frighten", "[ˈfraɪtən]", "пугать, стращать, страшить, перепугать, тревожить"],
    ["shocking", "[ˈʃɑːkɪŋ]", "отвратительный, очень, копнение"],
    ["stylish", "[ˈstaɪlɪʃ]", "стильный, элегантный, модный, шикарный"],
    ["air", "[ˈer]", "воздух, вид, атмосфера, ария, воздушный, пневматический, проветривать"],
    ["bowling", "[ˈboʊlɪŋ]", "боулинг, игра в кегли, игра в шары"],
    ["breathe", "[ˈbriːð]", "дышать, вдыхать, вздохнуть, подышать, выдыхать, перевести дух, жить, дуть, благоухать"],
    ["calories", "[ˈkæləriz]", "калории"],
    ["charity", "[ˈtʃerəti]", "благотворительность, милосердие, благотворительные дела"],
    ["draw", "[ˈdrɒ]", "ничья, жеребьевка, тираж, вытягивание, тяга, рисовать, привлекать, черпать, проводить"],
    ["fond of", "[ˈfɒnd əv]", "любящий что-то"],
    ["golf", "[ˈɡɑːlf]", "гольф, играть в гольф"],
    ["grandfather", "[ˈɡrændˌfɑːðər]", "дед, дедушка"],
    ["grandmother", "[ˈɡræn ˌmʌðr̩]", "бабушка, баловать"],
    ["judo", "[ˈdʒuːˌdoʊ]", "дзюдо"],
    ["karate", "[kəˈrɑːti]", "каратэ, карате"],
    ["keen on", "[ˈkiːn ɒn]", "увлекающийся чем-то"],
    ["mind", "[ˈmaɪnd]", "разум, ум, внимание, взгляд, возражать, помнить, мягкий, слабый"],
    ["nothing", "[ˈnʌθɪŋ]", "ничего, ничто, мелочь, ноль, ничего, ничто, совсем нет"],
    ["particularly", "[ˌpɑːrˈtɪkjələrli]", "особенно, в частности, в особенности, очень, чрезвычайно, особым образом, детально"],
    ["soccer", "[ˈsɑːkər]", "футбол, футбольный"],
    ["spare time", "[ˈspeə taɪm]", "свободное время"],
    ["circle", "[ˈsɝːkəl]", "круг, окружность, кружок, кольцо, цикл, кружиться, вращаться, окружать"],
    ["couple", "[ˈkʌpəl]", "пара, два, чета, пара сил, элемент, свора, соединять, соединяться, связывать"],
    ["criticism", "[ˈkrɪtəˌsɪzəm]", "критика, критицизм, критический разбор, критическая статья"],
    ["factory", "[ˈfæktəri]", "завод, фабрика, фактория, заводской, фабричный"],
    ["feed", "[ˈfiːd]", "корм, подача, питание, кормить, кормиться, питать, кормовой, загрузочный"],
    ["knife", "[ˈnaɪf]", "нож, скальпель, резец, ножевой, резать ножом"],
    ["never", "[ˈnevər]", "никогда, ни разу, вовек, конечно, нет, не может быть"],
    ["prestigious", "[preˈstɪdʒəs]", "престижный, авторитетный"],
    ["repeat", "[rəˈpiːt]", "повторять, повторяться, повторный, повторение, реприза"],
    ["resort", "[rəˈzɔːrt]", "курорт, прибежище, курортный, прибегать к, посещать"],
    ["scissors", "[ˈsɪzərz]", "ножницы"],
    ["throw", "[ˈθroʊ]", "бросок, ход, метание, бросание, сброс, бросать, кинуть, кидать, забросить, метать"],
    ["carry", "[ˈkæri]", "перенос, переноска, перевозка, волок, нести, проводить, носить, переносить"],
    ["count on", "[ˈkaʊnt ɑːn]", "рассчитывать на"],
    ["crossroads", "[ˈkrɒsroʊdz]", "перекресток, перепутье, распутье"],
    ["in front of", "[ɪn frʌnt əv]", "перед"],
    ["imagine", "[ˌɪˈmædʒən]", "вообразить, воображать, представлять себе, думать, предполагать, догадываться"],
    ["off", "[ˈɒf]", "от, с, у, выключено, вон, выключенный, прекращать"],
    ["post", "[poʊst]", "пост, должность, после, почтой, вывешивать, почтовый"],
    ["pretty", "[ˈprɪti]", "довольно, достаточно, значительный, хорошенький, безделушка"],
    ["ring", "[ˈrɪŋ]", "кольцо, ринг, звон, круг, ободок, рым, конфорка, звонить, звенеть, звучать, раздаваться"],
    ["thank", "[ˈθæŋk]", "благодарить, поблагодарить, благодарность"],
    ["villa", "[ˈvɪlə]", "вилла"],
    ["add", "[ˈæd]", "добавлять, сложить, прибавлять, придать, складывать, увеличивать, присоединять"],
    ["belt", "[ˈbelt]", "пояс, ремень, приводной ремень, зона, подпоясывать, гнать, опоясывать"],
    ["businessmen", "[ˈbɪznəsmən]", "бизнесмены"],
    ["clearly", "[ˈklɪrli]", "очевидно, несомненно, конечно, толком, по-видимому"],
    ["daily", "[ˈdeɪli]", "ежедневный, повседневный, ежедневно, ежедневная газета"],
    ["deliver", "[dəˈlɪvər]", "поставлять, доставлять, передавать, наносить, выпускать, избавлять, производить"],
    ["dishes", "[ˈdɪʃəz]", "посуда"],
    ["express", "[ɪkˈspres]", "экспресс, курьер, выражать, курьерский, с нарочным, спешно"],
    ["fill in", "[ˈfɪl ɪn]", "заполнять, пополнять, замещать, вписывать, занимать, бить, колотить"],
    ["form", "[ˈfɔːrm]", "форма, вид, бланк, класс, формат, образ, анкета, образовывать, формировать"],
    ["gap", "[ˈɡæp]", "разрыв, зазор, щель, пробел, промежуток, дефицит, отставание, брешь, интервал, отверстие"],
    ["half", "[ˈhæf]", "половинный, неполный, половина, тайм, часть, наполовину, почти"],
    ["heart", "[ˈhɑːrt]", "сердце, центр, суть, душа, сердцевина, ядро, любовь, чувства, сущность, мужество"],
    ["inform", "[ˌɪnˈfɔːrm]", "сообщать, сообщить, информировать, уведомлять, известить, извещать, доносить"],
    ["law", "[ˈlɑː]", "закон, право, юриспруденция, суд, заповеди, законный, правовой, юридический"],
    ["let", "[ˈlet]", "позволять, давать, препятствовать, препятствие, помеха, сдача внаем"],
    ["miss", "[ˈmɪs]", "мисс, промах, отсутствие, барышня, потеря, скучать, пропускать, упускать"],
    ["receive", "[rəˈsiːv]", "получать, принимать, воспринимать, вмещать, выслуживать, признавать правильным"],
    ["reduce", "[rəˈduːs]", "уменьшить, уменьшать, снижать, сокращать, сводить, понижать, редуцировать"],
    ["star", "[ˈstɑːr]", "звезда, звездочка, светило, звездный, выдающийся, играть главные роли"],
    ["urgent", "[ˈɝːdʒənt]", "срочный, настоятельный, насущный, безотлагательный, экстренный, настойчивый"],
    ["broaden", "[ˈbrɒdən]", "расширять, расширяться"],
    ["care", "[ˈker]", "уход, забота, заботы, внимание, заботиться, ухаживать, беспокоиться"],
    ["conversation", "[ˌkɑːnvərˈseɪʃən]", "разговор, беседа, переговоры, жанровая картина"],
    ["discussion", "[ˌdɪˈskəʃən]", "обсуждение, дискуссия, прения, переговоры, смакование"],
    ["fun", "[ˈfən]", "веселье, забава, потеха, шутка, шутить"],
    ["horizon", "[həˈraɪzən]", "горизонт, кругозор, ярус, отложение одного возраста"],
    ["innocent", "[ˈɪnəsənt]", "невинный, невиновный, наивный, безвредный, невинный младенец, простак"],
    ["luck", "[ˈlək]", "удача, везение, счастье, судьба, случай, быть везучим"],
    ["outdoors", "[ˈaʊtˈdɔːrz]", "на открытом воздухе, улица, двор, наружный"],
    ["partly", "[ˈpɑːrtli]", "частично, отчасти, частью, до некоторой степени"],
    ["poverty", "[ˈpɑːvərti]", "бедность, нищета, скудость, нужда, убожество, скудность, оскудение"],
    ["private", "[ˈpraɪvət]", "личный, частный, конфиденциальный, рядовой, половые органы"],
    ["pronunciation", "[proˌnənsiˈeɪʃən]", "произношение, выговор, произнесение, фонетическая транскрипция"],
    ["reach", "[ˈriːtʃ]", "достигать, связаться, доходить, охват, досягаемость, доступность"],
    ["record", "[rəˈkɔːrd]", "запись, рекорд, регистрация, записывать, рекордный"],
    ["seldom", "[ˈseldəm]", "редко"],
    ["sight", "[ˈsaɪt]", "вид, взгляд, зрение, зрелище, поле зрения, увидеть, заметить, наблюдать, разглядеть"],
    ["soft", "[ˈsɑːft]", "мягкий, нежный, тихий, легкий, слабый, гибкий, мягко, нежно, тихо"],
    ["suppose", "[səˈpoʊz]", "предполагать, полагать, думать, допускать, предположение, гипотеза"],
    ["thoroughly", "[ˈθɝːoli]", "тщательно, основательно, совершенно, вполне, до конца, вконец"],
    ["touch", "[ˈtətʃ]", "прикасаться, касаться, прикосновение, штрих, чуточку"],
    ["wish", "[ˈwɪʃ]", "желание, пожелание, просьба, желать, захотеть, хотеть"],
    ["accept", "[ækˈsept]", "принимать, признавать, соглашаться, брать, допускать, акцептовать, утверждать"],
    ["alarm clock", "[əˈlɑːm klɒk]", "будильник"],
    ["certainly", "[ˈsɝːtənli]", "конечно, безусловно, несомненно, непременно, наверное"],
    ["club", "[ˈkləb]", "клуб, дубина, дубинка, клюшка, булава, трефы, бить, собираться, собираться вместе"],
    ["divide", "[dɪˈvaɪd]", "раздел, разделение, дележ, водораздел, делить, делиться, разделять, разделяться"],
    ["entrepreneur", "[ˌɑːntrəprəˈnɝː]", "предприниматель, антрепренер, владелец предприятия"],
    ["harmful", "[ˈhɑːrmfəl]", "вредный, пагубный, губительный, тлетворный, разлагающий"],
    ["importance", "[ˌɪmˈpɔːrtəns]", "важность, значение, значительность, важничанье, заносчивость"],
    ["into", "[ˌɪnˈtuː]", "в, на, к"],
    ["latest", "[ˈleɪtəst]", "последний, поздно, недавно, новинка"],
    ["loser", "[ˈluːzər]", "проигравший, проигравший, неудачник, лузер, теряющий"],
    ["memory", "[ˈmeməri]", "память, воспоминание, запоминающее устройство, запись, регистрация"],
    ["pain", "[ˈpeɪn]", "боль, страдание, горе, огорчение, болеть, причинять боль, мучить, огорчать"],
    ["part", "[ˈpɑːrt]", "частью, частично, отчасти, часть, сторона, роль, деталь, разнимать, расставаться"],
    ["polite", "[pəˈlaɪt]", "вежливый, учтивый, любезный, обходительный, благовоспитанный, изящный, изысканный"],
    ["promising", "[ˈprɑːməsɪŋ]", "многообещающий, подающий надежды"],
    ["talent", "[ˈtælənt]", "талант, талантливость, дар, одаренность, способность, талантливый человек"],
    ["usual", "[ˈjuːʒəwəl]", "обычный, обыкновенный, всегдашний, то, что обычно принято"],
    ["able", "[ˈeɪbəl]", "в состоянии, способный, умеющий, могущий, умелый, талантливый, компетентный, знающий"],
    ["anniversary", "[ˌænəˈvɝːsəri]", "годовщина, юбилей, юбилейный, ежегодный, годовой"],
    ["cash", "[ˈkæʃ]", "наличные, наличные деньги, кассовый, платить деньги по чеку"],
    ["concert", "[ˈkɑːnsərt]", "концерт, согласие, концертный, договариваться"],
    ["discovery", "[ˌdɪˈskəvəri]", "открытие, обнаружение, раскрытие, развертывание"],
    ["field", "[ˈfiːld]", "поле, область, полевой, военно-полевой, выдвигать"],
    ["laptop", "[ˈlæpˌtɑːp]", "портативный компьютер, лаптоп, наколенный"],
    ["maybe", "[ˈmeɪbi]", "может быть, авось"],
    ["medicine", "[ˈmedəsən]", "медицина, лекарство, медикамент, магия, амулет, колдовство, талисман"],
    ["recording", "[rəˈkɔːrdɪŋ]", "запись, регистрация, звукозапись, записывающий, регистрирующий, пишущий"],
    ["revolution", "[ˌrevəˈluːʃən]", "революция, вращение, оборот, переворот, полный оборот, цикл, кругооборот"],
    ["get rid of", "[ɡet rɪd əv]", "избавиться от"],
    ["rise", "[ˈraɪz]", "рост, подъем, повышение, увеличение, восход, подниматься, повышаться, возрасти"],
    ["silly", "[ˈsɪli]", "глупый, простой, глуповатый, глупыш, дурак, несмышленыш"],
    ["stadium", "[ˈsteɪdiəm]", "стадион, стадий, стадия, ристалище"],
    ["sun", "[ˈsən]", "солнце, солнечные лучи, солнечный свет, загорать, греть на солнце"],
    ["tablet", "[ˈtæblət]", "таблетка, блокнот, дощечка, кусок, прикреплять табличку"],
    ["unforgettable", "[ˌʌnfərˈɡetəbəl]", "незабываемый, незабвенный"],
    ["go up", "[ˈɡoʊ ʌp]", "подниматься, расти, повышаться, восходить, взорваться, сгорать, разоряться"],
    ["go down", "[ˈɡoʊ ˈdaʊn]", "спускаться, снижаться, опускаться, падать, сходить, уменьшаться, садиться, затонуть"],
    ["up", "[ʌp]", "вверх, вверху, по, вверх по, поднимать, подъем, успех, повышающийся"],
    ["down", "[ˈdaʊn]", "вниз, внизу, вниз, вниз по, пух, спуск, нисходящий, опускать"],
    ["conference", "[ˈkɑːnfərəns]", "конференция, совещание, съезд, ассоциация"],
    ["conflict", "[ˈkɑːnflɪkt]", "конфликт, противоречие, столкновение, противоречить, бороться"],
    ["contest", "[ˈkɑːntest]", "конкурс, соревнование, состязание, оспаривать, спорить, бороться"],
    ["glad", "[ˈɡlæd]", "довольный, радостный, счастливый, веселый, гладиолус"],
    ["guarantee", "[ˌɡerənˈtiː]", "гарантия, гарант, залог, гарантировать, гарантийный"],
    ["guess", "[ˈɡes]", "предположение, догадка, предполагать, полагать, догадываться"],
    ["initiative", "[ˌɪˈnɪʃətɪv]", "инициатива, почин, инициативный, начальный, вводный"],
    ["participate", "[parˈtɪsəˌpet]", "участвовать, принимать участие, соучаствовать, разделять, иметь общее"],
    ["perhaps", "[pərˈhæps]", "возможно, может быть, пожалуй, авось"],
    ["react", "[riˈækt]", "реагировать, воздействовать, противодействовать, влиять, отзываться"],
    ["resolve", "[riˈzɑːlv]", "решимость, решение, решительность, решать, разрешать, решаться"],
    ["sum", "[ˈsəm]", "сумма, итог, количество, сущность, складывать, подводить итог"],
    ["no one", "[ˈnəʊ wʌn]", "никто"],
    ["contact", "[ˈkɑːnˌtækt]", "контакт, связь, связи, связывающий, контактный, связаться, соприкасаться"],
    ["attend", "[əˈtend]", "посещать, присутствовать, ходить, следить, заниматься, заботиться, обслуживать"],
    ["extra", "[ˈekstrə]", "дополнительный, лишний, дополнительно, особо, высший сорт, статист"],
    ["additional", "[əˈdɪʃənəl]", "дополнительный, добавочный"],
    ["bargain", "[ˈbɑːrɡən]", "сделка, выгодная покупка, торг, торговаться, заключать сделку"],
    ["complete", "[kəmˈpliːt]", "полный, совершенный, завершать, дополнить, полностью, совершенно"],
    ["plane", "[ˈpleɪn]", "самолет, плоскость, уровень, плоский, плоскостной, строгать, выравнивать"],
    ["foot", "[ˈfʊt]", "нога, подножие, фут, стопа, ступня, ножка, оплачивать, идти пешком, танцевать"],
    ["intensive", "[ˌɪnˈtensəv]", "интенсивный, напряженный, усилительный"],
    ["page", "[ˈpeɪdʒ]", "страница, полоса, паж, яркое событие, нумеровать страницы"],
    ["shape", "[ˈʃeɪp]", "форма, вид, фигура, облик, формировать, фасонный, профильный"],
    ["fit", "[ˈfɪt]", "подходить, соответствующий, пригонка, посадка, впору"],
    ["train", "[ˈtreɪn]", "поезд, состав, шлейф, цепь, череда, кортеж, тренироваться, тренировать, обучать"],
    ["case", "[ˈkeɪs]", "случай, дело, корпус, чехол, футляр, ящик, покрывать, упаковывать в ящик, обшивать"],
    ["confirm", "[kənˈfɝːm]", "подтверждать, утверждать, конфирмовать, подкреплять, поддерживать, закреплять"],
    ["date", "[ˈdeɪt]", "дата, срок, время, свидание, период, финик, датировать, устареть"],
    ["divorced", "[dɪˈvɔːrst]", "разведенный"],
    ["increase", "[ˌɪnˈkriːs]", "увеличение, рост, прирост, возрастание, увеличивать, увеличиваться, расти"],
    ["uncomfortable", "[ʌnˈkəmfərtəbəl]", "неудобный, неблагоустроенный, испытывающий неудобство, стесненный"],
    ["brown", "[ˈbraʊn]", "коричневый, карий, бурый, коричневый цвет, подрумянивать"],
    ["citizen", "[ˈsɪtəzən]", "гражданин, гражданка, горожанин, горожанка, штатский"],
    ["coin", "[ˌkɔɪn]", "монета, монетка, деньги, звонкая монета, чеканить, придумывать, штамповать"],
    ["collect", "[kəˈlekt]", "собирать, собираться, наложенным платежом, краткая молитва"],
    ["cottage", "[ˈkɑːtədʒ]", "коттедж, изба, хижина, деревенский дом, небольшой дом, одноэтажный дом"],
    ["foreigner", "[ˈfɔːrənər]", "иностранец, чужеземец, чужестранец, чужой, иноземец, иностранный корабль"],
    ["fully", "[ˈfʊli]", "полностью, вполне, совершенно"],
    ["impress", "[ˌɪmˈpres]", "впечатление, отпечаток, оттиск, печать, впечатлять, производить впечатление"],
    ["lucky", "[ˈlʌki]", "удачливый, счастливый, удачный, везучий, фартовый, случайный, приносящий счастье"],
    ["manage", "[ˈmænədʒ]", "управлять, руководить, справляться, школа верховой езды"],
    ["measure", "[ˈmeʒər]", "мера, мерило, размер, степень, такт, измерять, оценивать, мерить, мериться"],
    ["protest", "[prəˈtest]", "протест, опротестование, протестовать, возражать"],
    ["require", "[ˌriˈkwaɪər]", "требовать, нуждаться, затребовать, приказывать"],
    ["rest", "[ˈrest]", "остальное, остальные, отдых, остаток, покой, отдыхать, покоиться, опираться, спать"],
    ["satisfied", "[ˈsætəsˌfaɪd]", "довольный, удовлетворенный"],
    ["true", "[ˈtruː]", "истинный, верный, истина, точно, правдиво, выверять, пригонять"],
    ["unpleasant", "[ʌnˈplezənt]", "неприятный, отталкивающий"],
    ["wise", "[ˈwaɪz]", "мудрый, осведомленный, надоумить, способ, образ"],
    ["clear", "[ˈklɪr]", "ясно, совершенно, ясный, понятный, очищать, очищаться, просвет"],
    ["colourful", "[ˈkʌləfəl]", "красочный, яркий"],
    ["illustration", "[ˌɪləˈstreɪʃən]", "иллюстрация, рисунок, пример, иллюстрирование, пояснение"],
    ["light", "[ˈlaɪt]", "свет, светило, светить, зажигать, легкий, светлый, легко, налегке"],
    ["predictable", "[prəˈdɪktəbəl]", "предсказуемый"],
    ["presentation", "[ˌprezənˈteɪʃən]", "презентация, представление, вручение, предъявление, показ, подарок, поднесение"],
    ["rice", "[ˈraɪs]", "рис, рисовый, размельчать"],
    ["station", "[ˈsteɪʃən]", "станция, пункт, место, станционный, вокзальный, размещать, дислоцировать"],
    ["style", "[ˈstaɪl]", "стиль, тип, манера, фасон, слог, вкус, мода, шик, величать, модернизировать, титуловать"],
    ["tiring", "[ˈtaɪrɪŋ]", "утомительный, изнурительный"],
    ["translate", "[trænzˈleɪt]", "переводиться, переводить, транслировать, преобразовывать, переносить, перемещать"],
    ["translation", "[trænzˈleɪʃən]", "перевод, трансляция, сдвиг, перемещение, смещение, поступательное движение"],
    ["virus", "[ˈvaɪrəs]", "вирус, зараза, яд, вирусный"],
    ["in advance", "[ɪn ədˈvɑːns]", "заранее"],
    ["born", "[ˈbɔːrn]", "родившийся, рожденный, прирожденный, ограниченный, с узким кругозором"],
    ["cancel", "[ˈkænsəl]", "отмена, аннулирование, погашение, аннулировать, отменять, уничтожать"],
    ["flight", "[ˈflaɪt]", "рейс, полет, перелет, бегство, полетный, совершать перелет"],
    ["housework", "[ˈhaʊswərk]", "работа по дому"],
    ["ideal", "[aɪˈdiːl]", "идеальный, совершенный, воображаемый, идеал, верх совершенства"],
    ["install", "[ˌɪnˈstɒl]", "устанавливать, монтировать, вселить, помещать, устраивать, водворить, настраивать"],
    ["notice", "[ˈnoʊtɪs]", "уведомление, извещение, внимание, замечать, отмечать, обращать внимание"],
    ["reserve", "[rəˈzɝːv]", "резерв, заповедник, запас, резервный, запасной, резервировать"],
    ["seat", "[ˈsiːt]", "сиденье, место, местонахождение, билет, сидеть, усаживать, вмещать, сажать"],
    ["disappear", "[ˌdɪsəˈpɪr]", "исчезать, пропадать, скрываться, деваться, сгинуть, улетучиться, теряться"],
    ["enter", "[ˈentər]", "вводить, входить, вступать, поступать, выход на сцену"],
    ["operation", "[ˌɑːpəˈreɪʃən]", "операция, работа, эксплуатация, действие, управление, процесс, оператор, команда"],
    ["vacation", "[veˈkeɪʃən]", "отпуск, каникулы, отдыхать, брать отпуск, отпускной, каникулярный"],
    ["badly", "[ˈbædli]", "плохо, сильно, очень, дурно, нехорошо, больной"],
    ["luckily", "[ˈlʌkəli]", "к счастью, по счастливой случайности"],
    ["repair", "[rəˈper]", "ремонт, восстановление, исправление, ремонтировать, восстанавливать, чинить"],
    ["ridiculous", "[rəˈdɪkjələs]", "смешной, нелепый, смехотворный"],
    ["splendid", "[ˈsplendəd]", "великолепный, роскошный, замечательный, отличный, превосходный, блестящий"],
    ["save up", "[ˈseɪv ʌp]", "копить, делать сбережения, поднакапливать"],
    ["suit", "[ˈsuːt]", "костюм, иск, масть, набор, комплект, устраивать, соответствовать, подходить"],
    ["book", "[ˈbʊk]", "книга, книжка, журнал, том, заказывать, книжный"],
    ["ability", "[əˈbɪləti]", "способность, возможность, умение, ловкость, дарование, компетенция"],
    ["advise", "[ædˈvaɪz]", "консультировать, советовать, желательный"],
    ["album", "[ˈælbəm]", "альбом, долгоиграющая пластинка, книга автографов, альбом-пластинка"],
    ["although", "[ˌɒlˈðoʊ]", "хотя, если бы даже, несмотря на то, что, несмотря на, если бы даже"],
    ["analytical", "[ˌænəˈlɪtɪkəl]", "аналитический"],
    ["chapter", "[ˈtʃæptər]", "глава, сюжет, тема, серия, разбивать книгу на главы"],
    ["majority", "[məˈdʒɔːrəti]", "большинство, совершеннолетие, чин майора"],
    ["midnight", "[ˈmɪdˌnaɪt]", "полночь, непроглядная тьма, полуночный"],
    ["paint", "[ˈpeɪnt]", "краска, окраска, румяна, покрасить, красить, краситься"],
    ["recovery", "[rɪˈkʌvri]", "восстановление, извлечение, возмещение, выздоровление, утилизация, оздоровление"],
    ["release", "[riˈliːs]", "освобождение, высвобождение, версия, выпускать, освобождать, отпускать"],
    ["rude", "[ˈruːd]", "грубый, неприличный, невежливый, резкий, сырой, невоспитанный, внезапный"],
    ["rudely", "[ˈruːdli]", "грубо"],
    ["speedy", "[ˈspiːdi]", "быстрый, скорый, поспешный, безотлагательный, проворный"],
    ["subtitle", "[ˈsʌbˌtaɪtəl]", "субтитр, подзаголовок"],
    ["type", "[ˈtaɪp]", "тип, модель, разновидность, типаж, род, шрифт, печатать, напечатать, отпечатать"],
    ["war", "[ˈwɔːr]", "война, борьба, военный, воевать, соперничать"],
    ["attack", "[əˈtæk]", "атака, нападение, атаковать, нападать, нападающий, штурмовой"],
    ["authorities", "[əˈθɔːrətiz]", "власти"],
    ["cheek", "[ˈtʃiːk]", "щека, наглость, косяк, нахальство, дерзость, говорить дерзости, нахальничать"],
    ["during", "[ˈdʊrɪŋ]", "во время, в течение, в процессе, в продолжение"],
    ["enemy", "[ˈenəmi]", "враг, противник, неприятель, оппонент, вражеский, неприятельский, враждебный"],
    ["farm", "[ˈfɑːrm]", "ферма, хозяйство, хутор, сельскохозяйственный, обрабатывать землю"],
    ["a few", "[ə fjuː]", "несколько"],
    ["fortunately", "[ˈfɔːrtʃənətli]", "к счастью, по счастливой случайности"],
    ["lip", "[ˈlɪp]", "губа, край, выступ, губной, неискренний, целовать, касаться губами"],
    ["look up", "[ˈlʊk ʌp]", "искать, смотреть вверх, поднимать глаза, улучшаться, навещать, повышаться"],
    ["map", "[ˈmæp]", "карта, план, таблица, лицо, личность, картографировать, чертить карту"],
    ["mirror", "[ˈmɪrər]", "зеркало, зеркальце, отражение, отражать, отображать"],
    ["peacefully", "[ˈpiːsfəli]", "мирно, миролюбиво, тихо, спокойно"],
    ["someone", "[ˈsʌˌmwən]", "кто-то, кто-нибудь, кто-либо, некто, какой-нибудь один"],
    ["successfully", "[səkˈsesfəli]", "успешно"],
    ["tour", "[ˈtʊr]", "тур, поездка, экскурсия, гастролировать, гастрольный"],
    ["thought", "[ˈθɔːt]", "мысль, мышление, мнение, дума, внимание, соображения, размышление, забота, намерение"],
    ["analyse", "[ˈænəlaɪz]", "анализировать, разбирать, продумывать, разлагать"],
    ["automatically", "[ˌɒtəˈmætɪkəli]", "автоматически, машинально"],
    ["customer", "[ˈkʌstəmər]", "клиент, заказчик, покупатель, завсегдатай"],
    ["lie", "[ˈlaɪ]", "ложь, обман, положение, направление, логово, лежать, находиться, лгать, солгать"],
    ["agreement", "[əˈɡriːmənt]", "соглашение, договор, согласие, согласование, уговор"],
    ["appearance", "[əˈpɪrəns]", "появление, внешний вид, вид, внешность, видимость, выступление, явление, наружность"],
    ["concentrate", "[ˈkɑːnsənˌtret]", "концентрат, обогащенный продукт, сконцентрировать, концентрировать"],
    ["describe", "[dəˈskraɪb]", "описывать, охарактеризовать, характеризовать, характеризоваться, изображать"],
    ["destroy", "[dəˌstrɔɪ]", "уничтожать, разрушать, истреблять, губить, крушить, ломать, громить, подрывать"],
    ["hurry up", "[ˈhʌri ʌp]", "торопить, поторапливать, торопиться, спешить, поторапливаться"],
    ["invitation", "[ˌɪnvəˈteɪʃən]", "приглашение"],
    ["loss", "[ˈlɒs]", "потеря, убыток, утрата, ущерб, проигрыш, урон, пропажа, угар"],
    ["radio", "[ˈreɪdiˌoʊ]", "радио, радиоприемник, радиовещание, радировать, передавать по радио"],
    ["unprofessionally", "[ˌʌnprəˈfeʃənəli]", "непрофессионально"],
    ["ball", "[ˈbɒl]", "мяч, шар, бал, шарик, удар, шаровой, свивать, свиваться"],
    ["blow", "[ˈbloʊ]", "удар, дутье, дуновение, продувка, дуть, взрывать, дунуть, выдувать, раздувать"],
    ["catch", "[ˈkætʃ]", "поймать, ловить, уловить, догнать, улов, выгода, захват, задвижка, добыча"],
    ["cave", "[ˈkeɪv]", "пещера, полость, впадина, выдалбливать, обрушивать кровлю"],
    ["grow up", "[ˈɡroʊ ʌp]", "вырастать, подрасти, возмужать, становиться взрослым, мужать, созревать"],
    ["hide", "[ˈhaɪd]", "скрывать, скрываться, прятать, прятаться, шкура, укрытие, кожа, тайник"],
    ["hit", "[ˈhɪt]", "хит, удар, попадание, удача, успех, выпад, удариться, бить, биться, поражать, ударять"],
    ["at once", "[ət wʌns]", "немедленно / сразу"],
    ["otherwise", "[ˈʌðr̩waɪz]", "иначе, в противном случае, иным образом, иной, другой"],
    ["overcome", "[ˌoʊvəˈkʌm]", "преодолевать, побороть, перебороть, побеждать, превозмочь, одолевать, изживать"],
    ["shelf", "[ˈʃelf]", "полка, шельф, уступ, отмель, выступ, мель, риф, бедрок, привальный брус"],
    ["sportsman", "[ˈspɔːrtsmən]", "спортсмен, охотник, рыболов, честный человек, порядочный человек"],
    ["steal", "[ˈstiːl]", "кража, воровство, выгодная покупка, воровать, красть, красться, увести, угонять"],
    ["sudden", "[ˈsʌdən]", "внезапный, неожиданный, стремительный, скоропостижный, поспешный"],
    ["treasure", "[ˈtreʒər]", "сокровище, хранить, хранить как сокровище"],
    ["wind", "[wɪnd]", "ветер, дыхание, дух, виток, ветровой, ветряной, духовой, заводить, наматывать"],
    ["withdraw", "[wɪðˈdrɔː]", "изымать, отзывать, уходить, забирать, увести, удаляться, отводить, ретироваться"],
    ["accidentally", "[ˌæksəˈdentəli]", "случайно, нечаянно, непредумышленно"],
    ["across", "[əˈkrɒs]", "через, поперек, сквозь, в ширину, через, поперек, сквозь"],
    ["arm", "[ˈɑːrm]", "рука, рычаг, рукав, рукоятка, ветвь, вооружиться, вооружать, вооружаться"],
    ["beside", "[bəˈsaɪd]", "рядом, возле, рядом с, около, возле, кроме, помимо, мимо"],
    ["bravely", "[ˈbreɪvli]", "отважно, храбро"],
    ["courageously", "[kəˈreɪdʒəsli]", "смело, отважно"],
    ["deal", "[ˈdiːl]", "сделка, дело, обращение, иметь дело, бороться, еловый, сосновый"],
    ["fight", "[ˈfaɪt]", "борьба, бой, сражение, драка, спор, мордобой, бороться, сражаться, воевать, драться"],
    ["forgive", "[fərˈɡɪv]", "прощать, не взыскивать, не требовать"],
    ["freedom", "[ˈfriːdəm]", "свобода, право, независимость, воля, вольность, приволье, привилегия"],
    ["heroically", "[hɪˈrəʊɪkli]", "героически, геройски, эпически, рискованно, опасно"],
    ["hurt", "[ˈhɝːt]", "боль, вред, ущерб, повредить, болеть, обижать, пострадавший"],
    ["independence", "[ˌɪndəˈpendəns]", "независимость, самостоятельность, независимое состояние"],
    ["knee", "[ˈniː]", "колено, коленка, наколенник, кница, подкос, ударить коленом, касаться коленом"],
    ["leg", "[ˈleɡ]", "нога, ножка, этап, ветвь, колено, стойка, бежать, шагать, проталкивать ногой"],
    ["masterpiece", "[ˈmæstr̩ˌpis]", "шедевр"],
    ["piece", "[ˈpiːs]", "часть, кусок, штука, деталь, штучный, чинить, латать"],
    ["pound", "[ˈpaʊnd]", "фунт, фунт стерлингов, загон, тюрьма, растереть, бить, биться, истолочь, толочь"],
    ["soldier", "[ˈsoʊldʒə]", "солдат, воин, военнослужащий, военный, служить в армии, увиливать от работы"],
    ["supplier", "[səˈplaɪər]", "снабженец, подавальщик"],
    ["beat", "[ˈbiːt]", "бить, биться, отбивать, удар, ритм, биение, бой, такт, усталый, измотавшийся"],
    ["bell", "[ˈbel]", "колокол, звонок, колокольчик, колпак, мычать, кричать"],
    ["concentration", "[ˌkɑːnsənˈtreɪʃən]", "концентрация, сосредоточение, сосредоточенность, сгущение, уплотнение"],
    ["incredibly", "[ˌɪnˈkredəbli]", "неправдоподобно, невероятно, чрезвычайно, крайне, очень, маловероятно"],
    ["palace", "[ˈpæləs]", "дворец, чертог, особняк, роскошное здание, дворцовый"],
    ["patience", "[ˈpeɪʃəns]", "терпение, терпеливость, пасьянс, настойчивость"],
    ["purse", "[ˈpɝːs]", "кошелек, портмоне, сумочка, морщить, морщиться, тучный"],
    ["television", "[ˈteləˌvɪʒən]", "телевидение, дальновидение, телевизионный"],
    ["uniform", "[ˈjuːnəˌfɔːrm]", "равномерный, единый, форма, униформа, одевать в форму"],
    ["carton", "[ˈkɑːrtən]", "картон, картонная коробка, картонка, тонкий картон, белый круг в центре мишени"],
    ["dark", "[ˈdɑːrk]", "темный, черный, мрачный, темно, темнота, тьма, тень, потемки"],
    ["list", "[ˈlɪst]", "список, перечень, реестр, перечислять, крениться, сделанный из каймы"],
    ["login", "[lɒɡˈɪn]", "вход, логин, входить, логиниться"],
    ["may", "[ˈmeɪ]", "может, мочь, иметь возможность, дева"],
    ["network", "[ˈneˌtwərk]", "сеть, сетка, цепь, схема, сообщество, создавать сеть"],
    ["password", "[ˈpæˌswərd]", "пароль, пропуск"],
    ["performance", "[pərˈfɔːrməns]", "производительность, исполнение, эффективность, выполнение, деятельность"],
    ["somehow", "[ˈsʌmˌhɑːw]", "как-то, почему-то, как-нибудь, как-нибудь"],
    ["unexpectedly", "[ˌʌnɪkˈspektɪdli]", "неожиданно"],
    ["second", "[ˈsekənd]", "второй, другой, второй, секунда, вторым номером, подкреплять"],
    ["complaint", "[kəmˈpleɪnt]", "жалоба, недовольство, болезнь, недуг, недомогание, боль"],
    ["dislike", "[ˌdɪsˈlaɪk]", "неприязнь, нелюбовь, антипатия, не любить, недолюбливать"],
    ["format", "[ˈfɔːrˌmæt]", "формат, форма, размер, формат книги, форматировать, оформлять"],
    ["bestseller", "[ˌbestˈselə]", "бестселлер"],
    ["export", "[ɪkˈspɔːrt]", "экспорт, вывоз, экспортировать, вывозить, экспортный, вывозной"],
    ["fame", "[ˈfeɪm]", "слава, известность, молва, репутация, прославлять"],
    ["fear", "[ˈfɪr]", "страх, опасение, боязнь, вероятность, опасаться, бояться, страшиться, ожидать"],
    ["figure", "[ˈfɪɡjər]", "рисунок, фигура, цифра, диаграмма, образ, фигурировать, полагать, считать"],
    ["finally", "[ˈfaɪnəli]", "окончательно, в конце концов, в конечном счете, в конце, в заключение, под конец"],
    ["fortune", "[ˈfɔːrtʃən]", "состояние, фортуна, счастье, удача, судьба, случаться, наткнуться"],
    ["high-quality", "[ˌhaɪ ˈkwɒləti]", "высококачественный, высокосортный"],
    ["introduce", "[ˌɪntrəˈduːs]", "вводить, вносить, представлять, знакомить, применять, включать, привносить"],
    ["low-quality", "[ləʊ ˈkwɒləti]", "низкокачественный"],
    ["mention", "[ˈmenʃən]", "упоминание, ссылка, отзыв, упомянуть, упоминать, ссылаться"],
    ["might", "[ˈmaɪt]", "мощь, сила, могущество, энергия, может быть, можно"],
    ["recognition", "[ˌrekəgˈnɪʃən]", "признание, распознавание, опознавание, узнавание, опознание, одобрение, сознание"],
    ["army", "[ˈɑːrmi]", "армия, войско, масса, множество, армейский, войсковой"],
    ["government", "[ˈɡʌvərmənt]", "правительство, управление, правление, форма правления, провинция, штат"],
    ["nearest", "[ˈnɪrəst]", "ближайший"],
    ["territory", "[ˈterəˌtɔːri]", "территория, область, земля, сфера, половина поля"],
    ["coat", "[ˈkoʊt]", "пальто, слой, шерсть, пиджак, оболочка, шубка, покрывать, облицовывать, обмазывать"],
    ["curtains", "[ˈkɜːtənz]", "шторы"],
    ["hook", "[ˈhʊk]", "крюк, крючок, хук, ловушка, скоба, зацепка, подцепить, завербовать, поймать"],
    ["machine", "[mɪˈʃiːn]", "машинный, машина, аппарат, станок, печатать, шить"],
    ["painting", "[ˈpeɪntɪŋ]", "картина, живопись, покраска, окраска, рисующий"],
    ["pillow", "[ˈpɪloʊ]", "подушка, вкладыш, подкладка, подшипник, подложить подушку, служить подушкой"],
    ["truth", "[ˈtruːθ]", "истина, правда, правдивость, точность, правильность установки"],
    ["test", "[ˈtest]", "испытание, тест, проверка, испытательный, пробный, тестировать, проверять"],
    ["arrest", "[əˈrest]", "арест, задержание, наложение ареста, арестовать, арестовывать, задерживать"],
    ["atmosphere", "[ˈætməsˌfɪr]", "атмосфера, обстановка, воздух, окружающая среда, газообразная оболочка"],
    ["create", "[kriˈeɪt]", "создавать, творить, делать, созидать, вызывать, производить, возводить, суетиться"],
    ["criticize", "[ˈkrɪtəˌsaɪz]", "критиковать, осуждать, подвергать суровой критике"],
    ["elephant", "[ˈeləfənt]", "слон, формат бумаги"],
    ["impolitely", "[ˌɪmpəˈlaɪtli]", "невежливо"],
    ["reason", "[ˈriːzən]", "причина, основание, разум, повод, рассудок, рассуждать, размышлять, аргументировать"],
    ["remark", "[rəˈmɑːrk]", "замечание, примечание, наблюдение, отмечать, замечать, высказаться"],
    ["sort", "[ˈsɔːrt]", "сортировать, классифицировать, род, вид, сорт, разновидность, разряд, манера"],
    ["awful", "[ˈɑːfəl]", "ужасный, внушающий страх, очень"],
    ["celebrity", "[səˈlebrəti]", "знаменитость, известность, знаменитый человек"],
    ["copybook", "[ˈkɒpɪbʊk]", "образцовый, шаблонный, тетрадь с прописями"],
    ["definitely", "[ˈdefənətli]", "определенно, точно, несомненно, ясно"],
    ["diet", "[ˈdaɪət]", "диета, питание, режим питания, пища, держать на диете"],
    ["human", "[ˈhjuːmən]", "человек, человеческое существо, человеческий, людской, человечий"],
    ["justice", "[ˈdʒʌstɪs]", "юстиция, правосудие, справедливость, судья, управа"],
    ["line", "[ˈlaɪn]", "линь, линия, строка, ряд, линейный, проводить линию, набивать"],
    ["medical", "[ˈmedəkəl]", "медицинский, врачебный, терапевтический, студент-медик"],
    ["pool", "[ˈpuːl]", "бассейн, пул, плавательный бассейн, объединять в общий фонд"],
    ["reaction", "[riˈækʃən]", "реакция, реагирование, противодействие, реактивный"],
    ["alive", "[əˈlaɪv]", "живой, в живых, действующий, бодрый, заживо, под напряжением"],
    ["ancient", "[ˈeɪntʃənt]", "древний, старинный, старый, античный, старец, старейшина"],
    ["constant", "[ˈkɑːnstənt]", "постоянный, константный, неизменный, постоянная, константа"],
    ["instead", "[ˌɪnˈsted]", "вместо, взамен"],
    ["issue", "[ˈɪʃuː]", "выпуск, проблема, эмиссия, издание, выдавать, выпускать, издавать, исходить"],
    ["monastery", "[ˈmɑːnəˌsteri]", "монастырь"],
    ["stamp", "[ˈstæmp]", "марка, штамп, печать, клеймо, штемпель, штамповать, топать, клеймить"],
    ["still", "[ˈstɪl]", "еще, все еще, все-таки, неподвижный, тишина, кадр, успокаиваться"],
    ["point out", "[ˈpɔɪnt ˈaʊt]", "указывать, показывать, обращать внимание"],
    ["addicted", "[əˈdɪktəd]", "склонный"],
    ["darkness", "[ˈdɑːrknəs]", "темнота, мрак, темень, морока, хмурость"],
    ["drugs", "[drʌɡz]", "наркотики"],
    ["fed up", "[fed ʌp]", "сыт по горло"],
    ["gambling", "[ˈɡæmbəlɪŋ]", "азартная игра, игорный"],
    ["ghost", "[ɡoʊst]", "призрак, привидение, дух, тень, душа, преследовать, бродить как привидение"],
    ["heavily", "[ˈhevəli]", "сильно, тяжело, с трудом, медленно, тягостно"],
    ["nice", "[ˈnaɪs]", "хороший, приятный, милый, красивый, славный, добрый, любезный, вкусный, элегантный"],
    ["optimism", "[ˈɑːptəˌmɪzəm]", "оптимизм"],
    ["pity", "[ˈpɪti]", "жалость, сожаление, сострадание, жалеть, соболезновать"],
    ["ruin", "[ˈruːən]", "руина, руины, разорение, гибель, крушение, разрушать, портить, губить, разорять"],
    ["spider", "[ˈspaɪdər]", "паук, крестовина, звезда, таган, кровопийца"],
    ["vacancy", "[ˈveɪkənsi]", "вакансия, пустота, свободное место, пробел, безучастность, бездеятельность"],
    ["baseball", "[ˈbeɪsbɒl]", "бейсбол"],
    ["bitterly", "[ˈbɪtərli]", "горько"],
    ["castle", "[ˈkæsəl]", "замок, дворец, ладья, убежище, твердыня, рокировать, рокироваться"],
    ["death", "[ˈdeθ]", "смерть, гибель, конец, смертный, смертельный"],
    ["differ", "[ˈdɪfər]", "отличаться, отличать, различаться, различать, разниться, расходиться во мнениях"],
    ["excited", "[ɪkˈsaɪtəd]", "возбужденный, взволнованный, экзальтированный"],
    ["generation", "[ˌdʒenəˈreɪʃən]", "поколение, генерация, генерирование, образование, род, порождение, зарождение"],
    ["mess", "[ˈmes]", "беспорядок, бардак, столовая, путаница, пачкать, обоссать, грязнить, испражняться"],
    ["nightclub", "[ˈnaɪtˌkləb]", "ночной клуб, посещать ночные клубы"],
    ["nightlife", "[ˈnaɪˌtlaɪf]", "ночная жизнь, ночные развлечения"],
    ["numerous", "[ˈnuːmərəs]", "многочисленный, многие, множественный"],
    ["obviously", "[ˈɑːbviəsli]", "очевидно, явно, видно, ясно, по-видимому"],
    ["purchase", "[ˈpɝːtʃəs]", "покупка, приобретение, закупка, стоимость, покупать, приобретать, закупать"],
    ["sky", "[ˈskaɪ]", "небо, небеса, погода, климат, высоко забросить, вешать под потолок"],
    ["skyscraper", "[ˈskaɪˌskrepər]", "небоскреб"],
    ["accuse", "[əˈkjuːz]", "обвинять, винить, предъявлять обвинение"],
    ["crime", "[ˈkraɪm]", "преступление, преступность, криминал, карать за нарушение устава"],
    ["forward", "[ˈfɔːrwərd]", "вперед, дальше, нападающий, передний, передовой, пересылать"],
    ["impatience", "[ˌɪmˈpeɪʃəns]", "нетерпение, нетерпимость, раздражительность"],
    ["insist", "[ˌɪnˈsɪst]", "настаивать, настоять, утверждать, настойчиво утверждать"],
    ["murder", "[ˈmɝːdər]", "убийство, душегубство, убивать, коверкать, совершать убийство"],
    ["rely", "[rəˈlaɪ]", "полагаться, надеяться, зависеть"],
    ["reply", "[rəˈplaɪ]", "ответ, отчет, отвечать"],
    ["robbery", "[ˈrɑːbəri]", "ограбление, грабеж, разбой, кража, непомерно высокая цена"],
    ["shout", "[ˈʃaʊt]", "крик, окрик, возглас, громкий крик, кричать, покричать, накричать, звать"],
    ["subscribe", "[səbˈskraɪb]", "подписать, подписываться, абонировать, абонироваться, присоединяться"],
    ["unacceptable", "[ˌʌnækˈseptəbəl]", "неприемлемый, нежелательный"],
    ["look for", "[lʊk fɔː]", "искать"],
    ["approve", "[əˈpruːv]", "утверждать, одобрять, санкционировать, апробировать, показывать, проявлять"],
    ["backache", "[ˈbæˌkek]", "боль в спине, прострел, боль в пояснице, люмбаго"],
    ["candidate", "[ˈkændədet]", "кандидат"],
    ["congratulate", "[kənˈɡrætʃəˌlet]", "поздравлять"],
    ["consist", "[kənˈsɪst]", "состоять, заключаться в, совпадать, состав"],
    ["escape", "[əˈskeɪp]", "побег, выход, бегство, утечка, бежать, вырваться, спасательный"],
    ["headache", "[ˈheˌdek]", "головная боль, помеха, неприятность, источник беспокойства"],
    ["remind", "[riˈmaɪnd]", "напоминать"],
    ["report", "[riˈpɔːrt]", "отчет, доклад, сообщение, сообщить, сообщать, отчетный"],
    ["succeed", "[səkˈsiːd]", "преуспевать, иметь успех, следовать за, наследовать, сменять, достигать цели"],
    ["vote", "[voʊt]", "голосовать, баллотировать, голосование, голос, голоса, право голоса"],
    ["addiction", "[əˈdɪkʃən]", "наркомания, склонность, пагубная привычка"],
    ["advantage", "[ædˈvæntɪdʒ]", "преимущество, польза, плюс, перевес, способствовать, благоприятствовать"],
    ["associate", "[əˈsoʊsiət]", "ассоциировать, связывать, связанный, присоединенный, партнер, компаньон, коллега"],
    ["attitude", "[ˈætəˌtuːd]", "отношение, позиция, мироощущение, поза, осанка, положение самолета в воздухе"],
    ["blackboard", "[ˈblækˌbɔːrd]", "классная доска, рабочая область"],
    ["cancer", "[ˈkænsər]", "рак, карцинома, бедствие, бич"],
    ["cause", "[ˈkəz]", "причина, дело, повод, основание, процесс, вызывать, причинять, быть причиной"],
    ["cheating", "[ˈtʃiːtɪŋ]", "мошенничество"],
    ["cure", "[ˈkjʊr]", "кюре, лечение, лекарство, излечение, лечить, исцелять, вылечивать, исправлять"],
    ["disadvantage", "[ˌdɪsədˈvæntɪdʒ]", "невыгодное положение, неудобство, ущерб, причинять ущерб"],
    ["discipline", "[ˈdɪsəplən]", "дисциплина, дисциплинированность, дисциплинировать, наказывать"],
    ["disease", "[ˌdɪˈziːz]", "болезнь, заболевание, недуг, вызывать заболевание"],
    ["forever", "[fəˈrevər]", "навсегда, навечно, постоянно, вечность"],
    ["lack", "[ˈlæk]", "отсутствие, недостаток, нужда, не хватать, недоставать, нуждаться"],
    ["linguistics", "[lɪŋˈɡwɪstɪks]", "лингвистика, языкознание"],
    ["lung", "[ˈləŋ]", "легкое"],
    ["negative", "[ˈneɡətɪv]", "отрицательный, негативный, негатив, отрицание, отрицать, нейтрализовать"],
    ["refusal", "[rəˈfjuːzəl]", "отказ, право первого выбора"],
    ["search", "[ˈsɝːtʃ]", "поиски, поиск, обыск, розыск, искать, поискать, поисковый"],
    ["specialise", "[ˈspeʃəlaɪz]", "специализироваться"],
    ["temporary", "[ˈtempəˌreri]", "временный, переходящий, временный рабочий или служащий"],
    ["whiteboard", "[ˈwaɪtbɔːd]", "белая доска"],
    ["bar", "[ˈbɑːr]", "бар, полоса, стержень, запрещать, запирать, брусковый, исключая, не считая"],
    ["bottom", "[ˈbɑːtəm]", "нижний, последний, дно, нижняя часть, под, основывать"],
    ["cathedral", "[kəˈθiːdrəl]", "собор, кафедральный собор, кафедральный, соборный"],
    ["danger", "[ˈdeɪndʒər]", "опасность, угроза, гроза"],
    ["east", "[ˈiːst]", "восток, ост, к востоку, на восток, восточный"],
    ["general", "[ˈdʒenr̩əl]", "общий, генеральный, обычный, генерал, полководец"],
    ["north", "[ˈnɔːrθ]", "север, норд, к северу, на север, северный"],
    ["rate", "[ˈreɪt]", "скорость, ставка, темп, тариф, коэффициент, оценивать, тарифицировать"],
    ["satisfactory", "[ˌsætəsˈfæktəri]", "удовлетворительный, достаточный, приемлемый, хороший, приятный"],
    ["south", "[ˈsaʊθ]", "юг, зюйд, южный ветер, к югу, на юг, южный"],
    ["strict", "[ˈstrɪkt]", "строгий, точный, требовательный, определенный, неукоснительный, взыскательный"],
    ["welcome", "[ˈwelkəm]", "приветствовать, одобрять, приветствие, желанный, приятный"],
    ["west", "[ˈwest]", "запад, вест, западный ветер, к западу, на запад, западный"],
    ["whole", "[hoʊl]", "целый, весь, цельный, здоровый, родной, целое, все"],
    ["April", "[ˈeɪprəl]", "апрель, апрельский"],
    ["august", "[ˈɑːɡəst]", "августейший, августовский, величественный"],
    ["Christmas", "[ˈkrɪsməs]", "рождество, рождественский"],
    ["December", "[dəˈsembər]", "декабрь, декабрьский"],
    ["February", "[ˈfebjəˌweri]", "февраль"],
    ["January", "[ˈdʒænjuːˌeri]", "январь, январский"],
    ["July", "[ˌdʒuːˈlaɪ]", "июль, июльский"],
    ["June", "[ˈdʒuːn]", "июнь, июньский"],
    ["march", "[ˈmɑːrtʃ]", "марш, ход, маршировка, маршировать, шествовать, походный, маршевый"],
    ["middle", "[ˈmɪdəl]", "средний, середина, талия, помещать в середину"],
    ["November", "[noʊˈvembə]", "ноябрь, ноябрьский"],
    ["October", "[ɒkˈtoʊbə]", "октябрь, октябрьский"],
    ["September", "[sepˈtembər]", "сентябрь, сентябрьский"],
    ["till", "[ˈtɪl]", "до, пока, не раньше, пахать, пока, касса, тилль"],
    ["until", "[ʌnˈtɪl]", "до, не раньше, до тех пор пока"],
    ["may", "[ˈmeɪ]", "может, мочь, иметь возможность, дева"],
    ["market", "[ˈmɑːrkət]", "рынок, базар, торговля, спрос, рыночный, базарный, продавать, сбывать"],
    ["remote control", "[rɪˈməʊt kənˈtrəʊl]", "пульт"],
    ["somewhere", "[ˈsʌˌmwer]", "где-то, где-нибудь, куда-то, куда-нибудь, где-либо, куда-либо, кое-куда"],
    ["hat", "[ˈhæt]", "шляпа, шапка, слой породы над жилой, надевать шляпу"],
    ["hold", "[hoʊld]", "удерживать, держать, держаться, проводить, трюм, владение, удерживание, власть, захват"],
    ["log in", "[lɒɡ ɪn]", "войти в систему"],
    ["log out", "[ˌlɒɡ ˈaʊt]", "выходить, отмечать время ухода в специальном журнале, регистрировать события"],
    ["marriage", "[ˈmerɪdʒ]", "брак, замужество, женитьба, свадьба, марьяж, брачный"],
    ["T-shirt", "[ˈtiː ʃɜːt]", "футболка"],
    ["biology", "[baˈjɑːlədʒi]", "биология"],
    ["brightly", "[ˈbraɪtli]", "ярко, беспечально"],
    ["chat", "[ˈtʃæt]", "болтать, беседовать, беседа, болтовня, дружеский разговор"],
    ["cheer", "[ˈtʃɪr]", "радость, настроение, аплодисменты, ободрять, аплодировать, воодушевлять"],
    ["hamburger", "[ˈhæmbərɡər]", "гамбургер, рубленый бифштекс, булочка с котлетой"],
    ["polluted", "[pəˈluːtəd]", "загрязненный, развращенный"],
    ["rain", "[ˈreɪn]", "дождь, град, поток, потоки, дождевой, литься, лить, сыпаться"],
    ["shine", "[ˈʃaɪn]", "блеск, свет, сияние, лоск, глянец, светить, светиться, сиять, блистать"],
    ["shower", "[ˈʃaʊər]", "душ, ливень, поток, град, обильные осадки, поливать, принимать душ, осыпать"],
    ["show", "[ˈʃoʊ]", "шоу, выставка, показ, показывать, проявлять, выставочный"],
    ["bake", "[ˈbeɪk]", "испечь, выпекать, печь, печься, запекать, выпечка, выпекание"],
    ["bark", "[ˈbɑːrk]", "кора, лай, барк, корабль, кашель, хина, кожа, лаять, гавкать, рявкать, сдирать кору"],
    ["bread", "[ˈbred]", "хлеб, кусок хлеба, пища, деньги, панировать, обваливать в сухарях"],
    ["directly", "[dəˈrektli]", "непосредственно, прямо, точно, немедленно, как только"],
    ["sausage", "[ˈsɒsədʒ]", "колбаса, сосиска, сарделька, колбасный"],
    ["stool", "[ˈstuːl]", "стул, табурет, табуретка, скамеечка, стульчак, судно"],
    ["tie", "[ˈtaɪ]", "галстук, связь, ничья, завязка, узы, связать, связывать, завязаться, завязать"],
    ["airplane", "[ˈerˌplen]", "самолет, аэроплан"],
    ["algebra", "[ˈældʒəbrə]", "алгебра"],
    ["bath", "[ˈbæθ]", "ванна, баня, купание, купальня, купать, мыть"],
    ["cookie", "[ˈkʊki]", "печенье, домашнее печенье, булочка"],
    ["doll", "[ˈdɑːl]", "кукла, куколка, наряжать, наряжаться"],
    ["fix", "[ˈfɪks]", "фиксировать, исправлять, устанавливать, затруднительное положение"],
    ["geometry", "[dʒiˈɑːmətri]", "геометрия"],
    ["land", "[ˈlænd]", "земля, суша, страна, почва, земельный, наземный, приземляться, приземлять"],
    ["stranger", "[ˈstreɪndʒər]", "незнакомец, чужой, чужак, чужеземец, чужестранец, посторонний человек, иноземец"],
    ["take off", "[ˈteɪk ˈɒf]", "снимать, взлететь, сбрасывать, удалять, сбавлять, уводить, убавлять, отлеплять"],
    ["watch", "[ˈwɑːtʃ]", "часы, вахта, наблюдение, смотреть, наблюдать, вахтенный"],
    ["boil", "[ˌbɔɪl]", "кипение, фурункул, нарыв, точка кипения, кипятить, кипятиться, варить, вариться"],
    ["brush", "[ˈbrəʃ]", "кисть, щетка, хвост, столкновение, стычка, чистить щеткой, причесывать, вычистить"],
    ["comics", "[ˈkɑːmɪks]", "комиксы, бульварная литература"],
    ["dad", "[ˈdæd]", "папа, папочка"],
    ["degree", "[dɪˈɡriː]", "степень, градус, уровень, ступень, качество, ученая степень, звание, ранг, положение"],
    ["disturb", "[ˌdɪˈstɝːb]", "беспокоить, нарушать, мешать, побеспокоить, волновать, будоражить, смущать"],
    ["grandpa", "[ˈɡrændˌpɑː]", "дедушка"],
    ["kettle", "[ˈketəl]", "чайник, котел, котелок, металлический чайник, бадья, коробка компаса"],
    ["knock", "[ˈnɑːk]", "детонация, стук, удар, резкая критика, стучать, стучаться, постучать, сбивать"],
    ["mom", "[ˈmɑːm]", "мама"],
    ["pack", "[ˈpæk]", "пакет, упаковка, пак, пачка, упаковывать, упаковочный, вьючный"],
    ["pray", "[ˈpreɪ]", "молиться, помолиться, просить, умалять, молитва, просьба"],
    ["teeth", "[ˈtiːθ]", "зубы"],
    ["through", "[θruː]", "через, посредством, сквозной, прямой, благодаря, насквозь, пролезать"],
    ["twins", "[ˈtwɪnz]", "близнецы, двойня, двойняшки, многоплодие"],
    ["underground", "[ˈʌndərˌɡrɑːwnd]", "подземный, подпольный, метро, подполье, под землей, подпольно"],
    ["slow down", "[sləʊ daʊn]", "замедлиться"],
    ["cope", "[koʊp]", "справляться, совладать, бороться, сладить, риза, будка, кабина, кожух, колпак"],
    ["deserve", "[dəˈzɝːv]", "заслуживать, быть достойным, стоить"],
    ["exchange", "[ɪksˈtʃeɪndʒ]", "обмен, биржа, замена, размен, обменивать, поменять, обменный, меновой"],
    ["firmly", "[ˈfɝːmli]", "твердо, крепко, неизменно"],
    ["loud", "[ˈlaʊd]", "громкий, шумный, звучный, резкий, громко"],
    ["owe", "[ˈoʊ]", "быть должным, причитаться, быть в долгу, быть обязанным кому-л."],
    ["sharply", "[ˈʃɑːrpli]", "точно"],
    ["soup", "[ˈsuːp]", "суп, похлебка, нитроглицерин, повышать мощность"],
    ["split up", "[ˈsplɪt ʌp]", "дробиться, разделять, разделяться, раскалывать, раскалываться, распределять"],
    ["suspicious", "[səˈspɪʃəs]", "подозрительный, сомнительный"],
    ["thrilling", "[ˈθrɪlɪŋ]", "захватывающий, волнующий, вибрирующий, дрожащий"],
    ["ache", "[ˈeɪk]", "боль, болеть, ныть, жаждать, испытывать боль"],
    ["ankle", "[ˈæŋkəl]", "щиколотка"],
    ["argue", "[ˈɑːrɡjuː]", "спорить, доказывать, аргументировать, обсуждать, убеждать, разубеждать"],
    ["asleep", "[əˈsliːp]", "спящий, тупой, вялый, затекший, заснувший навеки"],
    ["bird", "[ˈbɝːd]", "птица, пташка, парень, человек, бабенка, тюрьма"],
    ["retell", "[riˈtel]", "пересказать"],
    ["rollerblading", "[ˈrəʊləbleɪdɪŋ]", "катание на роликах"],
    ["skateboarding", "[ˈskeɪtbɔːdɪŋ]", "катание на роликовой доске, катание на скейтборде, сухопутный серфинг"],
    ["summary", "[ˈsʌməri]", "резюме, сводка, краткое изложение, краткий, суммарный, скорый"],
    ["while", "[ˈwaɪl]", "в то время как, до тех пор пока, проводить время"],
    ["already", "[ɒlˈredi]", "уже, же"],
    ["childhood", "[ˈtʃaɪldˌhʊd]", "детство, детский возраст, младенчество"],
    ["diploma", "[dɪˈploʊmə]", "диплом, свидетельство, выдавать диплом"],
    ["English-speaking", "[ˈɪŋɡlɪʃ ˌspiːkɪŋ]", "англоговорящий, англоязычный, говорящий на английском языке"],
    ["ever", "[ˈevr̩]", "когда-либо, всегда, только"],
    ["injure", "[ˈɪndʒər]", "ранить, повредить, обидеть, оскорбить, портить, причинять боль, ушибить"],
    ["iron", "[ˈaɪərn]", "железо, утюг, железный, твердый, жестокий, гладить, утюжить"],
    ["local", "[ˈloʊkəl]", "местный, локальный, частный, низовой, местный житель, местная организация"],
    ["recover", "[rɪˈkʌvər]", "восстанавливать, оправляться, выздоравливать, извлекать, регенерировать"],
    ["since", "[ˈsɪns]", "с, после, со времени, поскольку, с тех пор, поскольку, так как"],
    ["superstar", "[ˌsuːpərˈstɑːr]", "суперзвезда"],
    ["yet", "[jet]", "еще, все же, тем не менее, все еще, все же, но, однако, несмотря на это"],
    ["film", "[ˈfɪlm]", "фильм, пленка, кино, кино-, экранизировать"],
    ["lottery", "[ˈlɑːtəri]", "лотерея"],
    ["receptionist", "[rəˈsepʃənəst]", "регистратор, портье, секретарь в приемной"],
    ["staff", "[ˈstæf]", "персонал, штат, штаб, кадры, штатный, штабной, укомплектовывать штаты"],
    ["sushi", "[ˈsuːʃi]", "суши"],
    ["engaged", "[enˈɡeɪdʒd]", "занято, занятый, вовлеченный, заинтересованный"],
    ["exhausted", "[ɪgˈzɒstəd]", "исчерпанный, измученный, истощенный, обессиленный, изнуренный, уставший"],
    ["lately", "[ˈleɪtli]", "недавно, за последнее время, на днях"],
    ["library", "[ˈlaɪbreri]", "библиотека, книгохранилище, библиотечный"],
    ["religion", "[rəˈlɪdʒən]", "религия, культ, религиозное учение, монашество, святыня"],
    ["date", "[ˈdeɪt]", "дата, срок, время, свидание, период, финик, датировать, устареть"],
    ["announce", "[əˈnaʊns]", "объявлять, сообщать, анонсировать, огласить, заявлять, возвещать, извещать"],
    ["boat", "[boʊt]", "лодка, судно, шлюпка, корабль, кататься на лодке, перевозить в лодке"],
    ["camera", "[ˈkæmərə]", "камера, фотокамера, фотоаппарат, кинокамера, киноаппарат"],
    ["discount", "[ˈdɪskaʊnt]", "скидка, дисконт, процент скидки, дисконтировать, обесценивать, снижать"],
    ["donate", "[doʊˈneɪt]", "жертвовать, дарить"],
    ["drown", "[ˈdraʊn]", "заглушать, тонуть, топиться, топить, заливать, затоплять"],
    ["passport", "[ˈpæˌspɔːrt]", "паспорт, паспортный"],
    ["payment", "[ˈpeɪmənt]", "оплата, платеж, уплата, взнос, плата, вознаграждение, возмездие"],
    ["preparation", "[ˌprepəˈreɪʃən]", "подготовка, препарат, приготовление, обогащение, лекарство, подготавливание"],
    ["recently", "[ˈriːsəntli]", "недавно, в последнее время, на днях, намедни"],
    ["red", "[ˈred]", "красный, рыжий, алый, багровый, багряный, красный цвет, золото, красный шар"],
    ["ship", "[ˈʃɪp]", "корабль, судно, парусное судно, грузить товары, садиться на корабль"],
    ["stupidly", "[ˈstjuːpɪdli]", "глупо"],
    ["sink", "[ˈsɪŋk]", "раковина, сток, слив, углубление, тонуть, потопить, утонуть, погружать"],
    ["upload", "[ˌʌpˈloʊd]", "загружать"],
    ["weight", "[ˈweɪt]", "вес, масса, груз, тяжесть, значение, нагрузка, весить, нагружать, увеличивать вес"],
    ["white", "[ˈwaɪt]", "белый, седой, бледный, серебристый, чистый, белый, белила, белизна, белый цвет, белок"],
    ["yellow", "[ˈjeloʊ]", "желтый, трусливый, ревнивый, желтить, желтеть, желтый цвет, желтизна"],
    ["apply", "[əˈplaɪ]", "применять, относиться, использовать, обращаться, прикладывать, касаться, прилагать"],
    ["briefly", "[ˈbriːfli]", "кратко, коротко, вкратце, сжато"],
    ["capital", "[ˈkæpətəl]", "капитал, столица, заглавная буква, капитальный, основной, главный, заглавный"],
    ["click", "[ˈklɪk]", "щелчок, защелка, щелканье, щелкающий звук, щелкать, ладить, иметь успех"],
    ["confirmation", "[ˌkɑːnfərˈmeɪʃən]", "подтверждение, утверждение, конфирмация, подкрепление, квитирование"],
    ["feedback", "[ˈfiːdˌbæk]", "обратная связь, ответная реакция"],
    ["forecast", "[ˈfɔːrˌkæst]", "прогноз, предсказание, прогнозировать, предсказывать"],
    ["petrol station", "[ˈpetrəl ˈsteɪʃən]", "заправка"],
    ["publish", "[ˈpʌblɪʃ]", "публиковать, издавать, опубликовывать, напечатать, выпускать, печататься, оглашать"],
    ["response", "[rəˈspɑːns]", "ответ, реакция, отклик, ответное чувство"],
    ["top", "[ˈtɑːp]", "верхний, высший, вершина, верх, первое место, превышать, покрывать"],
    ["volume", "[ˈvɑːljuːm]", "том, объем, громкость, емкость, книга, масса, объемный, крупномасштабный"],
    ["webinar", "[ˈwebɪnɑː]", "вебинар"],
    ["barbecue", "[ˈbɑːrbɪˌkjuː]", "барбекю, пикник, жарить, жарить мясо"],
    ["insurance", "[ˌɪnˈʃʊrəns]", "страхование, страховка, страховой"],
    ["nobody", "[ˈnoʊbədi]", "никто, никому не известный человек"],
    ["pleased", "[ˈpliːzd]", "довольный"],
    ["print", "[ˈprɪnt]", "печать, печатание, печатать, напечатать, печатный, ситцевый"],
    ["soaked", "[səʊkt]", "промокший насквозь"],
    ["switch off", "[ˈswɪtʃ ˈɒf]", "выключать, выключить, отключаться, выключать ток"],
    ["unless", "[ənˈles]", "если не, пока не, за исключением, кроме"],
    ["turn off", "[ˈtɝːn ˈɒf]", "выключать, сворачивать, поворачивать, закрывать, вызывать неприязнь"],
    ["damage", "[ˈdæmədʒ]", "урон, повреждение, ущерб, вред, порча, убыток, повредить, повреждать, наносить ущерб"],
    ["delay", "[dəˈleɪ]", "задержка, промедление, замедление, задерживать, откладывать, медлить"],
    ["fewer", "[ˈfjuːər]", "меньше, меньшее количество"],
    ["grammatically", "[ɡrəˈmætɪkli]", "грамматически, с точки зрения грамматики"],
    ["hire", "[ˈhaɪər]", "прокат, наем, плата за наем, нанимать, брать напрокат, сдавать внаем"],
    ["incorrectly", "[ˌɪnkəˈrektli]", "неверно"],
    ["jewellery", "[ˈdʒuːəlri]", "ювелирные изделия, драгоценности, ювелирный"],
    ["photograph", "[ˈfoʊtəɡrɑːf]", "фотография, фотографический снимок, сфотографировать, фотографировать"],
    ["plant", "[ˈplænt]", "завод, растение, установка, фабрика, сажать, насаждать, садить, подсадить"],
    ["properly", "[ˈprɑːpərli]", "правильно, должным образом, собственно, как следует, хорошенько, прилично"],
    ["punish", "[ˈpʌnɪʃ]", "наказать, наказывать, карать, расправляться, налагать взыскание"],
    ["robber", "[ˈrɑːbər]", "разбойник, грабитель"],
    ["vase", "[ˈveɪs]", "ваза"],
    ["whereas", "[hwerˈæz]", "тогда как, в то время как, принимая во внимание, поскольку"],
    ["bite", "[ˈbaɪt]", "укус, прикус, клев, кусок, сцепление, укусить, кусаться, кусать, клевать, жалить"],
    ["closely", "[ˈkloʊsli]", "тесно, близко, внимательно, вплотную, убористо"],
    ["connected", "[kəˈnektəd]", "связанный, связный, соединенный, имеющий родственные связи"],
    ["ignore", "[ɪgˈnɔːr]", "игнорировать, пренебрегать, пропускать, отклонять"],
    ["praise", "[ˈpreɪz]", "хвала, похвала, восхваление, хвалить, восхвалять, превозносить"],
    ["treat", "[ˈtriːt]", "удовольствие, угощение, наслаждение, лечить, относиться, обращаться"],
    ["unfairly", "[ənˈferli]", "несправедливо"],
    ["approval", "[əˈpruːvəl]", "утверждение, одобрение, согласие, рассмотрение"],
    ["bye", "[baɪ]", "прощание, что-либо второстепенное, прощальный привет"],
    ["goodbye", "[ˌɡʊdˈbaɪ]", "прощание"],
    ["hello", "[həˈloʊ]", "здороваться, звать, окликать, приветствие, приветственный возглас"],
    ["hi", "[ˈhaɪ]", "Гавайи"],
    ["blog", "[ˈblɑːɡ]", "блог, веблог, сетевой дневник"],
    ["burn", "[ˈbɝːn]", "ожог, клеймо, выжигание растительности, сжигать, гореть, жечь, жечься, обжечь"],
    ["commit", "[kəˈmɪt]", "совершать, фиксировать, поручать, предавать, вверять, учинять, помещать"],
    ["dessert", "[ˌdɪˈzɝːt]", "десерт, сладкое, десертный"],
    ["fire", "[ˈfaɪər]", "огонь, пожар, камин, пламя, обстрел, стрельба, стрелять, выстрелить, увольнять"],
    ["host", "[hoʊst]", "хозяин, множество, ведущий, воинство, принимать гостей, вести программу"],
    ["menu", "[ˈmenjuː]", "меню"],
    ["no", "[ˈnoʊ]", "нет, не, никакой, нисколько не, отказ, отрицание"],
    ["salad", "[ˈsæləd]", "салат, зелень"],
    ["salty", "[ˈsɒlti]", "соленый, пикантный, непристойный"],
    ["yes", "[ˈjes]", "да, да, согласие, поддакивать"],
    ["beforehand", "[bɪˈfɔːrhænd]", "заранее, заблаговременно, наперед, вперед, заблаговременный"],
    ["faint", "[ˈfeɪnt]", "слабый, тусклый, бледный, обморок, потеря сознания, падать в обморок, слабеть"],
    ["motorcycle", "[ˈmoʊtəsaɪkəl]", "мотоцикл, водить мотоцикл"],
    ["quit", "[ˈkwɪt]", "бросать, покидать, увольнение, уход с работы, отделавшийся, свободный"],
    ["register", "[ˈredʒəstər]", "регистр, реестр, запись, список, журнал, регистрировать, регистрироваться"],
    ["scholarship", "[ˈskɑːlərˌʃɪp]", "стипендия, ученость, эрудиция"],
    ["umbrella", "[ʌmˈbrelə]", "зонтик, зонт, прикрытие, зонтичный, служить прикрытием"],
    ["waiter", "[ˈweɪtər]", "официант, поднос, подавальщик, кухонный лифт, вращающийся столик для закусок"],
    ["waitress", "[ˈweɪtrəs]", "официантка, подавальщица"],
    ["wedding", "[ˈwedɪŋ]", "свадебный, венчальный, подвенечный, свадьба, венчание, бракосочетание"],
    ["give up", "[ˈɡɪv ʌp]", "сдаваться, отказываться, бросать, уступить, оставлять, покидать"],
    ["basis", "[ˈbeɪˌsɪs]", "основа, основание, база, базис"],
    ["climb", "[ˈklaɪm]", "подъем, восхождение, набор высоты, вьюнок, подниматься, взобраться, забраться"],
    ["decrease", "[dɪˈkriːs]", "уменьшение, понижение, спад, убывание, уменьшить, уменьшать, уменьшаться"],
    ["delete", "[dəˈliːt]", "удалять, стирать, исключать, вычеркивать, вымарывать"],
    ["kick", "[ˈkɪk]", "удар, пинок, толчок, удовольствие, пинать, выгонять, брыкать, брыкаться"],
    ["ladder", "[ˈlædər]", "лестница, трап, стремянка, спускаться"],
    ["unnecessary", "[ʌnˈnesəˌseri]", "ненужный, лишний, излишний"],
    ["attentively", "[əˈtentɪvli]", "внимательно"],
    ["earthquake", "[ˈɝːθˌkwek]", "землетрясение, потрясение, катастрофа"],
    ["growth", "[ɡroʊθ]", "рост, развитие, прирост, увеличение, возрастание, выращивание, опухоль, поросль"],
    ["hurricane", "[ˈhɜːrəˌken]", "ураганный, штормовой, ураган, буря, тропический циклон, взрыв"],
    ["invent", "[ˌɪnˈvent]", "изобретать, придумывать, выдумывать, сочинять, вымышлять, фабриковать"],
    ["protection", "[prəˈtekʃən]", "защита, охрана, покровительство, ограждение, протекционизм, прикрытие, блат"],
    ["push", "[ˈpʊʃ]", "толчок, нажим, давление, усилие, удар, натиск, толкать, продвигаться, продвигать"],
    ["stimulate", "[ˈstɪmjəˌlet]", "стимулировать, поощрять, побуждать, возбуждать"],
    ["subtract", "[səbˈtrækt]", "вычитать, отнимать"],
    ["catalogue", "[ˈkætəˌlɒɡ]", "каталог, справочник, список, проспект, каталогизировать, перечислять"],
    ["circumstance", "[ˈsɝːkəmˌstæns]", "обстоятельство, случай, деталь, церемония, подробность"],
    ["hug", "[ˈhəɡ]", "объятие, хватка, крепкое объятие, захват, обнимать, держаться, крепко обнимать"],
    ["possibility", "[ˌpɑːsəˈbɪləti]", "возможность, вероятность, перспектива"],
    ["stairs", "[ˈsterz]", "лестница, ступеньки, трап, сходни"],
    ["suggest", "[səgˈdʒest]", "предлагать, означать, говорить о, подсказывать, внушать, советовать, намекать"],
    ["tightly", "[ˈtaɪtli]", "плотно, тесно, крепко, туго, наглухо, натуго"],
    ["shut up", "[ˈʃət ʌp]", "закрывать, запирать, заключать, заставить замолчать, заколачивать, забивать"],
    ["allow", "[əˈlaʊ]", "позволять, допускать, разрешать, предоставлять, давать, учитывать"],
    ["data", "[ˈdeɪtə]", "данные, информация, сведения, факты"],
    ["organize", "[ˈɔːrɡəˌnaɪz]", "организовать, организоваться, устраивать, делать органическим"],
    ["redo", "[riˈduː]", "переделывать, делать вновь"],
    ["shut", "[ˈʃət]", "закрытый, запертый, закрывать, закрываться, запирать"],
    ["trust", "[ˈtrəst]", "доверять, доверяться, доверие, траст, трест, вера, доверенный"],
    ["kid", "[ˈkɪd]", "обманывать, надувать, ребенок, малыш, парень, младший, молодой, лайковый"],
    ["turn", "[ˈtɝːn]", "повернуть, поворачиваться, поворачивать, очередь, поворот, оборот, перемена, черед"],
    ["set up", "[ˈset ʌp]", "устанавливать, учреждать, открывать, выдвигать, основывать, воздвигать, набирать"],
    ["admire", "[ædˈmaɪr]", "восхищаться, любоваться, восторгаться, выражать восторг"],
    ["bonus", "[ˈboʊnəs]", "премия, тантьема"],
    ["doubt", "[ˈdaʊt]", "сомнение, сомневаться, усомниться, колебаться"],
    ["friendship", "[ˈfrendˌʃɪp]", "дружба, дружелюбие"],
    ["immorally", "[ɪˈmɒrəli]", "аморально"],
    ["quarrel", "[ˈkwɔːrəl]", "ссора, спор, раздоры, стрела самострела, ссориться, поссорить, спорить"],
    ["set", "[ˈset]", "набор, комплект, сет, установившийся, устанавливать, заходить"],
    ["sweep", "[ˈswiːp]", "подметать, мести, сметать, нестись, чистить, размах, охват, подметание, движение, мусор"],
    ["useless", "[ˈjuːsləs]", "бесполезный, никуда не годный, плохо себя чувствующий, в плохом настроении"],
    ["value", "[ˈvæljuː]", "ценить, оценивать, дорожить, значение, стоимость, ценность, величина"],
    ["visitor", "[ˈvɪzətər]", "посетитель, гость, инспектор, экскурсант, ревизор, перелетная птица"],
    ["well-known", "[ˈwel noʊn]", "известный, популярный, пресловутый, хорошо знакомый, общеизвестный"],
    ["accurately", "[ˈækjərətli]", "точно, правильно, тщательно, безошибочно"],
    ["briefcase", "[ˈbriːfˌkes]", "портфель, кожаный чемоданчик"],
    ["calculate", "[ˈkælkjəˌlet]", "вычислять, рассчитывать, подсчитывать, высчитывать, калькулировать"],
    ["carelessly", "[ˈkerləsli]", "беспечно, безалаберно"],
    ["catastrophe", "[kəˈtæstrəfi]", "катастрофа, несчастье, гибель, развязка"],
    ["confuse", "[kənˈfjuːz]", "смущать, смешивать, сбивать с толку, спутывать, создавать путаницу"],
    ["despite", "[ˌdɪˈspaɪt]", "несмотря, вопреки, несмотря на, злоба, презрение"],
    ["disaster", "[ˌdɪˈzæstər]", "бедствие, несчастье"],
    ["even", "[ˈiːvən]", "даже, ровно, точно, выровнять, четный, ровный, вечер, склон дня"],
    ["notion", "[ˈnoʊʃən]", "понятие, представление, идея, мнение, взгляд, точка зрения, намерение, галантерея"],
    ["safely", "[ˈseɪfli]", "безопасно, благополучно, в безопасности, в сохранности"],
    ["in spite of", "[ɪn spaɪt əv]", "несмотря на"],
    ["survive", "[sərˈvaɪv]", "пережить, выживать, уцелеть, выдерживать, перенести, оставаться в живых"],
    ["sweet", "[ˈswiːt]", "сладкий, милый, нежный, душистый, приятный, сладкое, милая, милый, конфета, сладость"],
    ["victory", "[ˈvɪktəri]", "победа"],
    ["audience", "[ˈɑːdiəns]", "аудитория, зрители, публика, слушатели, аудиенция, телезрители, радиослушатели"],
    ["avoid", "[əˌvɔɪd]", "избегать, избежать, уклоняться, избежание"],
    ["considerably", "[kənˈsɪdərəbli]", "изрядно"],
    ["crash", "[ˈkræʃ]", "авария, крах, аварийный, терпеть крах, с грохотом"],
    ["hospitality", "[ˌhɑːspəˈtæləti]", "гостеприимство, гостеприимность, радушие"],
    ["hunger", "[ˈhʌŋɡər]", "голод, жажда, голодание, голодуха, голодать, жаждать, быть голодным"],
    ["notebook", "[ˈnoʊtbʊk]", "блокнот, тетрадь, записная книжка"],
    ["significantly", "[sɪgˈnɪfɪkəntli]", "существенно, знаменательно"],
    ["visa", "[ˈviːzə]", "виза, прописка, визировать"],
    ["accusation", "[ˌækjəˈzeɪʃən]", "обвинение, обвинительный акт"],
    ["afford", "[əˈfɔːrd]", "предоставлять, давать, доставлять, приносить, иметь возможность"],
    ["angrily", "[ˈæŋɡrəli]", "сердито, гневно"],
    ["appear", "[əˈpɪr]", "появляться, казаться, предстать, проявляться, выступать, показываться, являться"],
    ["beard", "[ˈbɪrd]", "борода, ость, растительность на лице, смело выступать против"],
    ["both", "[boʊθ]", "оба, оба, обе, и тот и другой, также, одинаково, тоже"],
    ["cite", "[ˈsaɪt]", "цитировать, ссылаться, сослаться, вызывать в суд"],
    ["concern", "[kənˈsɝːn]", "концерн, беспокойство, интерес, забота, касаться, относиться, заботиться"],
    ["deny", "[dəˈnaɪ]", "отрицать, отказывать, отказываться, отвергать, отрекаться, отпираться"],
    ["gas", "[ˈɡæs]", "газ, газы, бензин, горючее, газовый, наполнять газом"],
    ["guilt", "[ˈɡɪlt]", "вина, виновность, чувство вины, грех"],
    ["hunt", "[ˈhənt]", "охота, поиски, охотиться, преследовать, травить, гнать"],
    ["influence", "[ˈɪnfluːəns]", "влияние, воздействие, действие, влиять, оказывать влияние"],
    ["moustache", "[ˈməˌstæʃ]", "усы, ус"],
    ["oil", "[ˌɔɪl]", "масло, нефть, нефтяной, масляный, смазать, смазывать"],
    ["shave", "[ˈʃeɪv]", "бритье, стружка, щепа, мистификация, обман, брить, бриться, побриться, обрить, строгать"],
    ["spell", "[ˈspel]", "заклинание, период, чары, время, очарование, означать, очаровывать"],
    ["strongly", "[ˈstrɒŋli]", "сильно, крепко"],
    ["suburbs", "[ˈsʌbɜːbz]", "пригород"],
    ["supply", "[səˈplaɪ]", "поставка, питание, поставлять, снабжающий, гибко"],
    ["trouble", "[ˈtrʌbəl]", "беспокойство, беспокоить, беспокоиться, аварийный"],
    ["universe", "[ˈjuːnɪˌvərs]", "вселенная, мир, космос, человечество, население Земли"],
    ["warm", "[ˈwɔːrm]", "теплый, горячий, жаркий, нагреваться, нагревать, согревание"],
    ["wire", "[ˈwaɪər]", "провод, проволока, проволочный, телеграфировать"],
    ["below", "[bəˈloʊ]", "ниже, внизу, в дальнейшем, ниже, под"],
    ["breathe in", "[ˈbriːð ɪn]", "при вдохе, брать дыхание, вдыхать"],
    ["breathe out", "[ˈbriːð ˈaʊt]", "выдохнуть"],
    ["button", "[ˈbʌtən]", "кнопка, пуговица, бутон, кнопочный, застегивать на пуговицы"],
    ["delighted", "[dəˈlaɪtəd]", "восхищенный, радостный, очарованный"],
    ["dig", "[ˈdɪɡ]", "копать, рыть, выкапывать, раскапывать, тычок, толчок, издевка, насмешка"],
    ["equipment", "[ɪˈkwɪpmənt]", "оборудование, снаряжение, оснащение, аппаратура, экипировка, вооружение, арматура"],
    ["fox", "[ˈfɑːks]", "лиса, лисица, хитрец, лисий мех, кошечка, хитрить, покрываться бурыми пятнами"],
    ["hole", "[hoʊl]", "отверстие, дыра, дырка, яма, скважина, лунка, просверлить, пробить, продырявливать"],
    ["moon", "[ˈmuːn]", "луна, месяц, лунный месяц, лунный свет, бродить как во сне"],
    ["peer", "[ˈpɪr]", "равный, пэр, ровня, лорд, всматриваться"],
    ["scan", "[ˈskæn]", "сканирование, поиск, просмотр, развертка, сканировать, скандировать"],
    ["exact", "[ɪgˈzækt]", "точный, строгий, аккуратный, взыскать, взыскивать, вымогать"],
    ["farther", "[ˈfɑːrðər]", "дальше, далее, далеко, далекий, дальний, содействовать"],
    ["furniture", "[ˈfɝːnɪtʃər]", "мебель, обстановка, оборудование, инвентарь, гарнитура, оснастка"],
    ["further", "[ˈfɝːðər]", "дальнейший, дальний, далее, дальше, кроме того, способствовать"],
    ["informative", "[ˌɪnˈfɔːrmətɪv]", "информационный, информативный, поучительный, содержащий информацию"],
    ["least", "[ˈliːst]", "наименее, как минимум, наименьший, минимальный, минимальное количество"],
    ["ours", "[ˈaʊərz]", "наш, свой"],
    ["paradise", "[ˈperəˌdaɪs]", "рай, благодать, раек, галерка"],
    ["theirs", "[ˈðerz]", "их, ихний"],
    ["eldest", "[ˈeldəst]", "старый, старейший, самый старший, прежний, давний, старческий, старообразный, бывший"],
    ["mine", "[ˈmaɪn]", "мой, моя, мое, мои, шахта, рудник, мина, добывать, минный"],
    ["previous", "[ˈpriːviəs]", "предыдущий, предшествующий, опрометчивый, преждевременный, поспешный"],
    ["scenario", "[səˈnerioʊ]", "сценарий, киносценарий"],
    ["season", "[ˈsiːzən]", "сезонный, сезон, время года, время, приправлять, сушить"],
    ["yours", "[ˈjʊrz]", "ваш, ваша, ваше, ваши, твой, твоя, твое, твои"],
    ["audio", "[ˈɑːdiˌoʊ]", "звуковой, звукозапись"],
    ["brave", "[ˈbreɪv]", "храбрый, смелый, отважный, храбро встречать, индейский воин, молодец"],
    ["cruel", "[ˈkruːəl]", "жестокий, безжалостный, бессердечный, ужасный, мучительный"],
    ["fearless", "[ˈfɪrləs]", "бесстрашный, неустрашимый, безбоязненный, мужественный"],
    ["fool", "[ˈfuːl]", "дурак, дура, глупец, шут, дурачиться, дурачить, глупый, безрассудный"],
    ["liar", "[ˈlaɪər]", "лгун, врун"],
    ["theory", "[ˈθɪri]", "теория, необоснованное предположение"],
    ["academic", "[ˌækəˈdemɪk]", "академический, учебный, академичный, ученый, преподаватель, профессор"],
    ["direct", "[dəˈrekt]", "прямой, непосредственный, прямо, непосредственно, направлять, руководить"],
    ["dry", "[ˈdraɪ]", "сухой, засушливый, сухо, насухо, досуха, сушь, засуха, сушить, сушиться"],
    ["fortress", "[ˈfɔːrtrəs]", "крепость"],
    ["gold", "[ɡoʊld]", "золотой, золотистого цвета, золото, злато, золотистый цвет"],
    ["leather", "[ˈleðər]", "кожа, ремень, краги, кожаный, колотить, крыть кожей"],
    ["medal", "[ˈmedəl]", "медаль, орден"],
    ["phenomenon", "[fəˈnɑːməˌnɑːn]", "явление, феномен, необыкновенное явление, объект чувственного восприятия"],
    ["procedure", "[prəˈsiːdʒər]", "процедура, операция, технологический процесс, методика проведения"],
    ["religious", "[rəˈlɪdʒəs]", "религиозный, верующий, культовый, монах"],
    ["runner", "[ˈrʌnər]", "бегун, дорожка, бегунок, полоз, ротор, рабочее колесо, гонец, посыльный, курьер"],
    ["silver", "[ˈsɪlvər]", "серебро, деньги, серебрить, серебриться, серебряный, серебристый"],
    ["standard", "[ˈstændərd]", "стандартный, типовой, нормальный, стандарт, эталон, норма, норматив, знамя"],
    ["bitter", "[ˈbɪtər]", "горький, ожесточенный, горько, жестоко, резко, очень, горечь, горькое пиво"],
    ["candy", "[ˈkændi]", "конфета, конфеты, леденец, сласти, засахаривать, засахариваться"],
    ["creative", "[kriˈeɪtɪv]", "творческий, созидательный"],
    ["dead", "[ˈded]", "мертвый, умерший, дохлый, умершие, покойники, совершенно, точно"],
    ["disappointment", "[ˌdɪsəˌpɔɪntmənt]", "разочарование, досада, неприятность, обманутая надежда, причина разочарования"],
    ["distance", "[ˈdɪstəns]", "расстояние, дистанция, дальность, даль, отдалять, обойти, владеть перспективой"],
    ["educational", "[ˌedʒəˈkeɪʃənəl]", "образовательный, учебный, воспитательный, педагогический"],
    ["forbid", "[fərˈbɪd]", "запрещать, не позволять, возбранять"],
    ["located", "[loʊˈkeɪtɪd]", "расположенный, размещенный, помещенный, обнаруженный"],
    ["rare", "[ˈrer]", "редкий, редкостный, исключительно, гренки с сыром"],
    ["side", "[ˈsaɪd]", "сторона, бок, борт, край, боковой, побочный, примкнуть к"],
    ["situated", "[ˈsɪtʃuːˌetəd]", "расположенный, размещенный"],
    ["technical", "[ˈteknɪkəl]", "технический, специальный, промышленный, технические подробности"],
    ["terribly", "[ˈterəbli]", "ужасно, адски"],
    ["dear", "[ˈdɪr]", "дорогой, милый, родной, милый, милая, возлюбленный, дорого"],
    ["global", "[ˈɡloʊbəl]", "глобальный, мировой, всемирный, общий, всеобщий"],
    ["opposite", "[ˈɑːpəzət]", "напротив, напротив, против, на, противоположный, противоположность"],
    ["plastic", "[ˈplæstɪk]", "пластиковый, пластмассовый, пластик, пластмасса, пластичность"],
    ["surgery", "[ˈsɝːdʒəri]", "хирургия, кабинет врача с аптекой, приемная врача с аптекой"],
    ["ugly", "[ˈʌɡli]", "уродливый, безобразный, неприятный, противный, скверный, опасный, склочный"],
    ["various", "[ˈveriəs]", "различный, разный, многие, разнообразный, разносторонний"],
    ["meaning", "[ˈmiːnɪŋ]", "значение, смысл, важность, значащий, значительный"],
    ["broad", "[ˈbrɒd]", "широкий, обширный, общий, широко, свободно, открыто, баба, девка, широкая часть"],
    ["dirty", "[ˈdɝːti]", "грязный, нечистый, подлый, пачкать, загрязнять, лахудра"],
    ["grey", "[ˈɡreɪ]", "Грэй"],
    ["hero", "[ˈhɪroʊ]", "герой, богатырь, полубог, главное действующее лицо"],
    ["hers", "[hərz]", "ее, свой, принадлежащий ей"],
    ["illegal", "[ˌɪˈliːɡəl]", "незаконный, нелегальный, беззаконный"],
    ["national", "[ˈnæʃənəl]", "национальный, государственный, народный, гражданин, соотечественник, согражданин"],
    ["ordinary", "[ˈɔːrdəˌneri]", "обычный, обыкновенный, простой, дежурное блюдо, столовая, таверна"],
    ["planet", "[ˈplænət]", "планета"],
    ["political", "[pəˈlɪtəkəl]", "политический, государственный, политзаключенный"],
    ["safe", "[ˈseɪf]", "безопасный, в безопасности, надежный, сейф, несгораемый шкаф, холодильник"],
    ["security", "[sɪˈkjʊrəti]", "безопасность, обеспечение, защита, охрана, надежность, залог, гарантия, уверенность"],
    ["shoulder", "[ˈʃoʊldə]", "плечо, буртик, уступ, выступ, лопатка, поясок, брать на себя, взвалить на плечи"],
    ["trainers", "[ˈtreɪnəz]", "кроссовки"],
    ["total", "[ˈtoʊtəl]", "полный, суммарный, сумма, итог, целое, составлять, насчитывать"],
    ["trend", "[ˈtrend]", "тенденция, направление, устремленность, иметь тенденцию, отклоняться"],
    ["patient", "[ˈpeɪʃənt]", "пациент, больной, терпеливый, упорный, настойчивый"],
    ["aggressive", "[əˈɡresɪv]", "агрессивный, энергичный, настойчивый, вызывающий, нападающий"],
    ["certain", "[ˈsɝːtən]", "определенный, некоторый, некий, уверенный, факт, нечто несомненное"],
    ["character", "[ˈkerɪktər]", "характер, символ, персонаж, знак, герой, характеризовать, запечатлевать"],
    ["climate", "[ˈklaɪmət]", "климат, атмосфера, состояние общественного мнения, настроение"],
    ["commercial", "[kəˈmɝːʃəl]", "коммерческий, торговый, доходный, реклама, коммивояжер"],
    ["recent", "[ˈriːsənt]", "последний, свежий, недавний, новый, современный"],
    ["specific", "[spəˈsɪfɪk]", "конкретный, определенный, специфический, специфическое средство"],
    ["valuable", "[ˈvæljʊəbəl]", "ценный, полезный, дорогой, драгоценности, ценные вещи"],
    ["slow", "[sˈloʊ]", "медленный, медленно, замедлять, копуша"],
    ["active", "[ˈæktɪv]", "активный, действующий, действительный, действительный залог"],
    ["announcement", "[əˈnaʊnsmənt]", "объявление, анонс, сообщение, уведомление, извещение"],
    ["approach", "[əˈproʊtʃ]", "подход, приближение, доступ, подходить, приближаться, обращаться"],
    ["complex", "[ˈkɑːmpleks]", "комплекс, совокупность, сложный, комплексный, осложнять"],
    ["cool", "[ˈkuːl]", "прохладный, клевый, прохладно, свежо, охлаждать, прохлада"],
    ["false", "[ˈfɔːls]", "ложный, фальшивый, неправильный, лживый, ошибочный, искусственный, подставной"],
    ["feature", "[ˈfiːtʃər]", "особенность, черта, характеристика, показывать, изображать, набрасывать"],
    ["frequent", "[ˈfriːkwənt]", "частый, постоянный, часто встречающийся, часто посещать"],
    ["frequently", "[ˈfriːkwəntli]", "часто, зачастую"],
    ["guilty", "[ˈɡɪlti]", "виновный, виноватый, повинный, преступный"],
    ["honest", "[ˈɑːnəst]", "честный, искренний, правдивый, подлинный, настоящий, нравственный, целомудренный"],
    ["intelligent", "[ˌɪnˈtelədʒənt]", "умный, разумный, смышленый, понимающий, понятливый"],
    ["judge", "[ˈdʒədʒ]", "судья, эксперт, ценитель, арбитр, знаток, судить, осуждать, оценивать, рассудить"],
    ["research", "[riˈsɝːtʃ]", "исследование, исследовательский, исследовать"],
    ["scientific", "[ˌsaɪənˈtɪfɪk]", "научный, ученый, техничный, высокого класса"],
    ["speed", "[ˈspiːd]", "скорость, быстрота, число оборотов, спид, ускорять, торопить, спешить"],
    ["stupid", "[ˈstuːpəd]", "глупый, тупой, дурацкий, бестолковый, дурак"],
    ["anyone", "[ˈeniwʌn]", "любой, кто-нибудь, кто-либо, никто, всякий"],
    ["clever", "[ˈklevər]", "умный, ловкий, искусный, способный, талантливый, даровитый, юркий, добродушный"],
    ["ear", "[ˈɪr]", "ухо, слух, ушко, колос, початок, дужка, колоситься, давать початки"],
    ["exactly", "[ɪgˈzæktli]", "точно, именно, ровно, как раз, точь-в-точь, совершенно верно"],
    ["ought to", "[ˈɔːt tuː]", "следует / должен"],
    ["toilet", "[ˌtɔɪlət]", "туалет, ванная, уборная, туалетный, совершать туалет"],
    ["aunt", "[ˈænt]", "тетя, тетка"],
    ["baggage", "[ˈbæɡədʒ]", "багаж, обоз, озорница, плутовка, девчонка, багажный"],
    ["balcony", "[ˈbælkəni]", "балкон, балкон первого яруса, балконный"],
    ["bear", "[ˈber]", "медведь, медведка, козел, нести, иметь, носить, переносить, терпеть"],
    ["chicken", "[ˈtʃɪkən]", "курица, цыпленок, курятина, петух, трус, новоиспеченный"],
    ["crocodile", "[ˈkrɑːkəˌdaɪl]", "крокодил, гулянье парами"],
    ["dining room", "[ˈdaɪnɪŋ ruːm]", "столовая"],
    ["dirt", "[ˈdɝːt]", "грязь, земля, грунт, нечистоты, сор, почва, гадость, подлость, оскорбление, наносы"],
    ["fridge", "[ˈfrɪdʒ]", "холодильник, рефрижератор, конденсатор, тереть, вытирать"],
    ["garage", "[ɡəˈrɑːʒ]", "гараж, гараж-магазин, ставить в гараж, держать в гараже"],
    ["giraffe", "[dʒəˈræf]", "жираф, жирафа"],
    ["grandma", "[ˈɡræma]", "бабушка"],
    ["lift", "[ˈlɪft]", "лифт, подъемник, подъем, подниматься, поднимать, подъемный"],
    ["luggage", "[ˈlʌɡədʒ]", "багаж, поклажа, багажный"],
    ["monkey", "[ˈmʌŋki]", "обезьяна, проказник, кривляка, соваться, дурачиться, передразнивать"],
    ["pig", "[ˈpɪɡ]", "свинья, поросенок, хрюшка, свинина, пороситься, есть с жадностью"],
    ["power", "[ˈpaʊər]", "мощность, власть, силовой, много"],
    ["printer", "[ˈprɪntər]", "принтер, печатник, типограф, печатное устройство, набойщик"],
    ["tiger", "[ˈtaɪɡər]", "тигр, задира, хулиган, опасный противник"],
    ["calculator", "[ˈkælkjəˌletər]", "калькулятор, вычислитель, счетчик, арифмометр, вычислительный прибор"],
    ["carpet", "[ˈkɑːrpət]", "ковер, покрытие, покрывало, защитный слой, устилать, устилать коврами"],
    ["intention", "[ˌɪnˈtentʃən]", "намерение, цель, стремление, замысел, идея, понятие, намерение жениться, виды"],
    ["literature", "[ˈlɪtərətʃər]", "литература, словесность"],
    ["rollerblades", "[ˈrəʊləbleɪdz]", "ролики"],
    ["rug", "[ˈrəɡ]", "ковер, коврик, плед, меховая полость, тащить с усилием"],
    ["similar", "[ˈsɪmələr]", "похожий, подобный, сходный, схожий, копия, двойник, дубликат"],
    ["skateboard", "[ˈskeɪtbɔːd]", "скейтборд, доска на роликах для катания на асфальте, роликовая доска"],
    ["mistaken", "[ˌmɪˈsteɪkən]", "ошибочный, ошибающийся, заблуждающийся, неуместный"],
    ["professor", "[prəˈfesər]", "профессор, преподаватель, исповедующий"],
    ["regret", "[rɪˈɡret]", "сожаление, извинения, раскаяние, горе, сожалеть, раскаиваться, горевать"],
    ["site", "[ˈsaɪt]", "место, участок, местоположение, располагать, выбирать место"],
    ["architecture", "[ˈɑːrkəˌtektʃər]", "архитектура, зодчество, структура, строение, архитектурный стиль"],
    ["cigarette", "[ˌsɪɡəˈret]", "сигарета, папироса"],
    ["fork", "[ˈfɔːrk]", "вилка, вилы, развилка, ответвление, разветвляться, расходиться"],
    ["graphic", "[ˈɡræfɪk]", "графический, наглядный, изобразительный, красочный, живописный, живой"],
    ["ice", "[ˈaɪs]", "лед, мороженое, ледяной, замораживать"],
    ["lemon", "[ˈlemən]", "лимон, лимонный цвет, лимонного цвета"],
    ["salt", "[ˈsɒlt]", "соль, поваренная соль, солить, засаливать, соленый, морской"],
    ["shall", "[ʃæl]", "м лице выражает волю говорящего, его приказ, обещание"],
    ["sugar", "[ˈʃʊɡər]", "сахар, сахароза, милый, сахарить, подслащивать, сахарный"],
    ["temperature", "[ˈtemprətʃər]", "температура, повышенная температура, температурный"],
    ["unbelievable", "[ˌʌnbəˈliːvəbəl]", "невероятно, невероятный"],
    ["zero", "[ˈzɪroʊ]", "нуль, ноль, нулевой, нулевой, ноль"],
    ["average", "[ˈævərɪdʒ]", "средний, обычный, среднее число, составлять"],
    ["either", "[ˈiːðər]", "любой, каждый, другой, или, также, оба, любой из двух"],
    ["neither", "[ˈniːðər]", "ни один, никто, никакой, также не, тоже не"],
    ["none", "[ˈnən]", "ни один, никто, ничто, никакой, совсем не, вовсе не, нисколько"],
    ["nor", "[ˈnɔːr]", "ни, и ... не, тоже ... не, также ... не"],
    ["simply", "[ˈsɪmpli]", "просто, только, попросту, легко, совершенно"],
    ["somebody", "[ˈsʌmˌbɑːdi]", "кто-то, кто-нибудь, кто-либо, кое-кто, важная персона"],
    ["just", "[dʒəst]", "просто, только что, как раз, именно, точно, точный, справедливый, верный, заслуженный"],
    ["accustomed", "[əˈkəstəmd]", "привычный, привыкший, приученный, обычный"],
    ["anxious", "[ˈæŋkʃəs]", "тревожный, беспокоящийся, озабоченный, беспокойный, встревоженный, стремящийся"],
    ["beauty", "[ˈbjuːti]", "красота, красавица, прелесть, краса"],
    ["capable", "[ˈkeɪpəbəl]", "способный, допускающий, могущий, умелый, одаренный, поддающийся, умный"],
    ["familiar", "[fəˈmɪljər]", "привычный, хорошо знакомый, близкий, близкий друг"],
    ["incapable", "[ˌɪnˈkeɪpəbəl]", "неспособный, недееспособный"],
    ["jealous", "[ˈdʒeləs]", "ревнивый, ревнующий, завидующий, завистливый, ревностный, ревниво оберегающий"],
    ["pressure", "[ˈpreʃər]", "давление, напряжение, напор, нажим, воздействие, нажатие, сжатие"],
    ["related", "[rəˈleɪtəd]", "связанный, родственный"],
    ["responsible", "[riˈspɑːnsəbəl]", "ответственный, несущий ответственность, надежный, сознательный, важный"],
    ["sensitive", "[ˈsensətɪv]", "чувствительный, деликатный, чуткий, медиум, телепат"],
    ["wealth", "[ˈwelθ]", "богатство, благосостояние, изобилие, материальные ценности, сокровища"],
    ["amazed", "[əˈmeɪzd]", "пораженный, изумленный"],
    ["ashamed", "[əˈʃeɪmd]", "совестно, пристыженный"],
    ["aware", "[əˈwer]", "знающий, осведомленный, сознающий"],
    ["beginner", "[bɪˈɡɪnər]", "начинающий, новичок, тот, кто начинает"],
    ["crowded", "[ˈkraʊdəd]", "переполненный, людный, многолюдный, полный, битком набитый, наполненный, прижатый"],
    ["dissatisfied", "[ˌdɪˈsætəsˌfaɪd]", "недовольный, неудовлетворенный, раздосадованный"],
    ["elevator", "[ˈeləˌvetər]", "лифт, элеватор, подъемник, руль высоты, грузоподъемник, руль глубины"],
    ["engine", "[ˈendʒən]", "двигатель, мотор, машина, паровоз, средство, локомотив, инструмент, орудие"],
    ["enthusiastic", "[enˌθuːziˈæstɪk]", "восторженный, полный энтузиазма, увлеченный, полный энергии"],
    ["management", "[ˈmænədʒmənt]", "управление, менеджмент, администрация, дирекция, правление, умение справляться"],
    ["scared", "[ˈskerd]", "испуганный"],
    ["selfish", "[ˈselfɪʃ]", "эгоистичный, эгоистический"],
    ["suitable", "[ˈsuːtəbəl]", "подходящий, соответствующий, годный"],
    ["unknown", "[ənˈnoʊn]", "неизвестный, безвестный, неизвестное, незнакомец, тайно, без ведома"],
    ["acquainted", "[əˈkweɪntəd]", "знакомый, знающий"],
    ["allergic", "[əˈlɝːdʒɪk]", "аллергический, питающий отвращение, не переносящий, не выносящий"],
    ["annoyed", "[əˌnɔɪd]", "раздраженный, раздосадованный"],
    ["bored", "[ˈbɔːrd]", "скучающий"],
    ["cotton", "[ˈkɑːtən]", "хлопок, хлопчатник, вата, хлопковый, согласоваться, полюбить"],
    ["dust", "[ˈdəst]", "пыль, прах, пыльца, сор, презренный металл, пылить, вытирать пыль, запылить"],
    ["envious", "[ˈenviəs]", "завистливый"],
    ["furious", "[ˈfjʊriəs]", "яростный, разъяренный, бешеный, неистовый, свирепый, взбешенный, злющий"],
    ["grateful", "[ˈɡreɪtfəl]", "благодарный, признательный, благодарственный, приятный"],
    ["hopeless", "[ˈhoʊplɪs]", "безнадежный, безвыходный, неисправимый, отчаявшийся"],
    ["irritated", "[ˈɪrəˌtetəd]", "раздраженный"],
    ["metal", "[ˈmetəl]", "металл, щебень, гарт, балласт, металлический, обшивать металлом"],
    ["nut", "[ˈnət]", "гайка, орех, муфта, псих, гаечный, шевелить мозгами"],
    ["rudeness", "[ˈruːdnəs]", "грубость"],
    ["skating", "[ˈskeɪtɪŋ]", "катание на коньках, конькобежный"],
    ["steel", "[ˈstiːl]", "сталь, огниво, меч, твердость, стальной, жестокий, закалять"],
    ["wool", "[ˈwʊl]", "шерсть, шерстяные изделия, волосы, руно, шерстяной"],
    ["blame", "[ˈbleɪm]", "вина, ответственность, обвинение, обвинять, пенять, порицать"],
    ["devote", "[dɪˈvoʊt]", "уделять, посвящать, посветить, предаваться"],
    ["tournament", "[ˈtʊrnəmənt]", "турнир, состязание рыцарей, средневековый турнир"],
    ["alternative", "[ɒlˈtɝːnətɪv]", "альтернативный, переменный, альтернатива, выбор"],
    ["conclusion", "[kənˈkluːʒən]", "заключение, вывод, завершение, окончание, результат, умозаключение, исход"],
    ["courage", "[ˈkɜːrədʒ]", "мужество, смелость, храбрость, отвага, бодрость"],
    ["demand", "[ˌdɪˈmænd]", "спрос, требование, потребность, запрос, требовать, нуждаться, спрашивать"],
    ["insomnia", "[ˌɪnˈsɑːmniə]", "бессонница"],
    ["professionalism", "[prəˈfeʃənəˌlɪzəm]", "профессионализм, профессионализация"],
    ["quote", "[kwoʊt]", "цитата, кавычки, назначенная цена, цитировать, ссылаться, котировать"],
    ["refer", "[rəˈfɝː]", "относиться, относить, ссылаться, обращаться, сослаться, направлять, говорить"],
    ["respond", "[rəˈspɑːnd]", "отвечать, реагировать, удовлетворять, отклик"],
    ["absurd", "[əbˈsɝːd]", "абсурдный, нелепый, смешной, абсурд"],
    ["aqua park", "[ˈækwə pɑːk]", "аквапарк"],
    ["career", "[kəˈrɪr]", "карьера, карьер, профессия, нестись, быстро двигаться"],
    ["centre", "[ˈsentər]", "центр, середина, средоточие, ось, стержень, центрировать, концентрировать"],
    ["construction", "[kənˈstrəkʃən]", "строительство, конструкция, построение, сооружение, здание, строение, стройка"],
    ["curiosity", "[ˌkjʊriˈɑːsəti]", "любопытство, любознательность, редкость, антикварный"],
    ["eastern", "[ˈiːstərn]", "восточный"],
    ["effort", "[ˈefərt]", "усилие, попытка, старание, напряжение, достижение, натуга"],
    ["extent", "[ɪkˈstent]", "степень, мера, экстент, размер, протяжение, пространство"],
    ["instance", "[ˈɪnstəns]", "инстанция, пример, образец, ссылаться"],
    ["northern", "[ˈnɔːrðərn]", "северный, дующий с севера, северянин, житель Севера"],
    ["railroad", "[ˈreɪlroʊd]", "железнодорожный, железная дорога, строить железную дорогу"],
    ["rationally", "[ˈræʃənəli]", "рационально, разумно"],
    ["southern", "[ˈsʌðərn]", "южный, южанин, житель Юга"],
    ["vain", "[ˈveɪn]", "напрасный, тщетный, тщеславный, пустой, тщета"],
    ["western", "[ˈwestərn]", "западный, ковбойский, вестерн, ковбойский фильм"],
    ["addition", "[əˈdɪʃən]", "добавление, дополнение, сложение, прибавление, добавка, увеличение, примесь"],
    ["charge", "[ˈtʃɑːrdʒ]", "заряд, обязанности, обвинение, заряжать, обвинять, поручать, атаковать"],
    ["contrary", "[ˈkɑːntreri]", "вопреки, против, вразрез, противоположность, противоположный"],
    ["disposal", "[dɪˈspoʊzəl]", "распоряжение, удаление, размещение, избавление, продажа, расположение, устранение"],
    ["focus", "[ˈfoʊkəs]", "фокус, центр, очаг, средоточие, сфокусировать, фокусироваться"],
    ["granted", "[ˈɡræntəd]", "предоставленный, разрешено, позволено, ладно, при условии"],
    ["mood", "[ˈmuːd]", "настроение, лад, наклонение, расположение духа, тональность"],
    ["nature", "[ˈneɪtʃər]", "природа, характер, сущность, натура, естество, тип, нрав, род, организм, сорт"],
    ["outskirts", "[ˈaʊtˌskərts]", "окраина, предместья, опушка"],
    ["stress", "[ˈstres]", "стресс, напряжение, усилие, акцент, подчеркивать, ставить ударение"],
    ["term", "[ˈtɝːm]", "срок, термин, член, терм, выражение, семестр, называть, выражать"],
    ["conduct", "[kənˈdəkt]", "поведение, руководство, управление, проводить, вести, дирижировать"],
    ["constantly", "[ˈkɑːnstəntli]", "постоянно, часто, то и дело"],
    ["length", "[ˈleŋkθ]", "длина, расстояние, продолжительность, протяжение, отрезок, долгота, кусок, отрез"],
    ["totally", "[ˈtoʊtəli]", "полностью, абсолютно"],
    ["above", "[əˈbʌv]", "выше, более, свыше, выше, над, свыше, вышеупомянутое, упомянутый выше"],
    ["billion", "[ˈbɪljən]", "миллиард, биллион, несметное количество, миллиард, биллион, несметное количество"],
    ["flat", "[ˈflæt]", "плоский, ровный, плоско, ровно, плашмя, квартира, бемоль, делать плоским"],
    ["however", "[ˌhɑːˈwevər]", "однако, как, как бы ни, каким образом, однако, тем не менее, впрочем, как бы ни"],
    ["region", "[ˈriːdʒən]", "регион, область, район, край, участок, зона, округ, округа, место, страна, сфера, полость"],
    ["surf", "[ˈsɝːf]", "прибой, бурун, буруны, заниматься серфингом"],
    ["web", "[ˈweb]", "сеть, паутина, ткань, стержень, перемычка, заманивать в сеть, заманивать в сети"],
    ["meal", "[ˈmiːl]", "еда, принятие пищи, мука, обваливать в муке, перемалывать"],
    ["race", "[ˈreɪs]", "гонки, гонка, раса, род, кольцо, бег, народ, мчаться, бежать, гнать"],
    ["seaside", "[ˈsiːˌsaɪd]", "приморский, берег моря, взморье, морское побережье"],
    ["hockey", "[ˈhɑːki]", "хоккей"],
    ["absence", "[ˈæbsəns]", "отсутствие, недостаток, неявка, отлучка"],
    ["festival", "[ˈfestəvəl]", "фестиваль, празднество, праздничный"],
    ["potential", "[pəˈtenʃəl]", "потенциал, возможность, напряжение, потенциальный, возможный"],
    ["threat", "[ˈθret]", "угроза, грозное предзнаменование"],
    ["actually", "[ˈæktʃəwəli]", "фактически, на самом деле, действительно, даже, в самом деле, в настоящее время"],
    ["anyway", "[ˈeniˌwe]", "так или иначе, в любом случае, во всяком случае, никак, как-нибудь, кое-как"],
    ["construct", "[kənˈstrəkt]", "конструкция, концепция, строить, создавать, конструировать"],
    ["everybody", "[ˈevrɪˌbɑːdi]", "все, каждый, всякий"],
    ["permission", "[pərˈmɪʃən]", "разрешение, позволение"],
    ["persuade", "[pərˈsweɪd]", "убеждать, уговаривать, склонить, отговорить, сагитировать, уверять, упросить"],
    ["policy", "[ˈpɑːləsi]", "политика, полис, курс, страховой полис, линия поведения, установка, хитрость"],
    ["probably", "[ˈprɑːbəbli]", "вероятно"],
    ["unable", "[ʌˈneɪbəl]", "неспособный"],
    ["award", "[əˈwɔːrd]", "премия, решение, присуждение, стипендия, присуждать, награждать"],
    ["everywhere", "[ˈevriˌwer]", "везде, всюду, повсюду"],
    ["experiment", "[ɪkˈsperəmənt]", "эксперимент, опыт, испытание, экспериментировать, баловаться"],
    ["monthly", "[ˈmʌnθli]", "ежемесячно, раз в месяц, ежемесячный, месячный, ежемесячник"],
    ["participant", "[parˈtɪsəpənt]", "участник, участвующий"],
    ["passenger", "[ˈpæsəndʒər]", "пассажир, седок, слабый игрок спортивной команды, неспособный член"],
    ["replace", "[ˌriˈpleɪs]", "заменять, замещать, вернуть, возмещать, восстанавливать, класть обратно на место"],
    ["rescue", "[ˈreskjuː]", "спасение, освобождение, спасание, спасать, выручать, избавлять, освобождать"],
    ["winner", "[ˈwɪnər]", "победитель, первый призер, удачная идея, перспективное предприятие"],
    ["yearly", "[ˈjɪrli]", "ежегодный, годовой, длящийся год, каждый год, раз в год"],
    ["canteen", "[kænˈtiːn]", "столовая, буфет, войсковая лавка, походный ящик"],
    ["ceiling", "[ˈsiːlɪŋ]", "потолок, перекрытие, обшивка, предельная высота, максимальная цена"],
    ["dismiss", "[ˌdɪˈsmɪs]", "увольнять, освобождать, распускать, отклонять, отпускать, прекращать, отделываться"],
    ["formally", "[ˈfɔːrməli]", "формально, официально"],
    ["injustice", "[ˌɪnˈdʒəstəs]", "несправедливость"],
    ["isolated", "[ˈaɪsəˌletəd]", "изолированный, отдельный"],
    ["novel", "[ˈnɑːvəl]", "роман, повесть, новелла, сборник новелл, новый, оригинальный, неизведанный"],
    ["politely", "[pəˈlaɪtli]", "вежливо, любезно"],
    ["portion", "[ˈpɔːrʃən]", "часть, порция, доля, удел, участь, приданое, делить на части, давать приданое"],
    ["rebuild", "[riˈbɪld]", "восстанавливать, перестраивать, отстроить заново, реконструировать"],
    ["reconstruct", "[ˌrikənˈstrəkt]", "реконструировать, восстанавливать, перестраивать, воссоздавать, пересмотреть"],
    ["survey", "[sərˈveɪ]", "обзор, обследование, обзорный, обследовать, обозревать"],
    ["worth", "[ˈwɝːθ]", "стоимость, ценность, достоинства, цена, заслуживающий, обладающий, стоящий"],
    ["base", "[ˈbeɪs]", "база, основание, основа, основывать, базировать, базовый, основной, базисный"],
    ["burglar", "[ˈbɝːɡlər]", "грабитель, взломщик, вор, ночной грабитель, вор-взломщик"],
    ["environment", "[ənˈvaɪrənmənt]", "окружающая среда, среда, окружение, окружающая обстановка, состояние, контекст"],
    ["harm", "[ˈhɑːrm]", "вред, ущерб, зло, обида, вредить, причинять вред, наносить ущерб"],
    ["headquarters", "[ˈhedˌkwɔːrtərz]", "штаб, центр, главное управление, главное командование, источник"],
    ["import", "[ˌɪmˈpɔːrt]", "импорт, ввоз, значение, импортировать, ввозить, импортный"],
    ["jail", "[ˈdʒeɪl]", "тюрьма, тюремное заключение, сажать в тюрьму, заключать в тюрьму"],
    ["pear", "[ˈper]", "груша, грушевое дерево"],
    ["unclearly", "[ʌnˈklɪəli]", "неясно"],
    ["builder", "[ˈbɪldər]", "строитель, подрядчик, плотник, каменщик"],
    ["cabinet", "[ˈkæbənət]", "кабинет, шкаф, корпус, шкафчик, правительственный, министерский"],
    ["cover", "[ˈkʌvər]", "обложка, крышка, покрывало, покров, чехол, охватывать, покрывать, закрывать"],
    ["culture", "[ˈkʌltʃər]", "разведение, возделывание, выращивать"],
    ["greet", "[ˈɡriːt]", "приветствовать, встречать, здороваться, кланяться, раскланяться, плакать"],
    ["ourselves", "[aʊərˈselvz]", "себя, себе, сами, собой, собой"],
    ["pick up", "[ˈpɪk ʌp]", "забрать, подобрать, подбирать, поднимать, подхватить, подцепить, поймать"],
    ["system", "[ˈsɪstəm]", "система, устройство, сеть, организм, метод, способ, формация, вычислительная система"],
    ["along", "[əˈlɔːŋ]", "вдоль, вместе, вперед, с собой, по длине, вдоль, по"],
    ["billionaire", "[ˌbɪljəˈner]", "миллиардер"],
    ["energetic", "[ˌenərˈdʒetɪk]", "энергичный, активный"],
    ["globalisation", "[ˌɡləʊbəlaɪˈzeɪʃən]", "глобализация"],
    ["haircut", "[ˈherˌkət]", "стрижка"],
    ["hairstyle", "[ˈherˌstaɪl]", "прическа"],
    ["ineffectively", "[ˌɪnɪˈfektɪvli]", "неэффективно, безрезультатно, бесполезно, бесплодно, напрасно, тщетно"],
    ["international", "[ˌɪntr̩ˈnæʃənəl]", "международный, интернациональный, международное состязание"],
    ["retire", "[rəˈtaɪr]", "уходить, удаляться, уходить в отставку, увольняться, увольнять, отступать"],
    ["unattractive", "[ˌʌnəˈtræktɪv]", "непривлекательный"],
    ["admiration", "[ˌædməˈreɪʃən]", "восхищение, восторг, преклонение, предмет восхищения"],
    ["due to", "[djuː tuː]", "из-за / благодаря"],
    ["edit", "[ˈedɪt]", "редактировать, отредактировать, монтаж, редакционная статья"],
    ["fasten", "[ˈfæsən]", "скрепить, закреплять, прикреплять, прикрепляться, застегиваться, застегивать"],
    ["prohibit", "[proˈhɪbət]", "запрещать, препятствовать, мешать, возбранять"],
    ["redecorate", "[riːˈdekəreɪt]", "произвести косметический ремонт помещения, новому оформить интерьер"],
    ["strictly", "[ˈstrɪktli]", "строго, точно, настрого"],
    ["cross", "[ˈkrɑːs]", "крест, кросс, крестик, перекрестный, пересекаться, косо"],
    ["freeze", "[ˈfriːz]", "замораживание, мороз, холод, замерзать, замораживать, мерзнуть"],
    ["gain", "[ˈɡeɪn]", "получать, набирать, приобретать, усиление, прирост, прибыль, выигрыш"],
    ["indoors", "[ˈɪnˌdɔːrz]", "в помещении, внутри, внутри дома"],
    ["melt", "[ˈmelt]", "расплавиться, растопить, таять, плавиться, плавка, расплавленный металл, садка"],
    ["original", "[əˈrɪdʒənəl]", "оригинальный, исходный, первоначальный, оригинал, подлинник, первоисточник, чудак"],
    ["plenty of", "[ˈplenti əv]", "достаточно / много"],
    ["press", "[ˈpres]", "пресс, пресса, печать, жим, надавливание, нажимать, давить, настаивать, жать"],
    ["radical", "[ˈrædəkəl]", "радикальный, коренной, корневой, радикал, корень, знак корня"],
    ["aggression", "[əˈɡreʃən]", "агрессия, агрессивность, нападение, вызывающее поведение"],
    ["countryside", "[ˈkʌntrisaɪd]", "сельская местность, деревня, округа, местное сельское население"],
    ["flexible", "[ˈfleksəbəl]", "гибкий, свободный, гнущийся, податливый, уступчивый"],
    ["millionaire", "[ˌmɪljəˈner]", "миллионер"],
    ["oligarch", "[ˈolɪˌɡɑːrk]", "олигарх"],
    ["paperwork", "[ˈpeɪpəˌrwərk]", "письменная работа, канцелярская работа, проверка документации"],
    ["schedule", "[ˈskedʒuːl]", "расписание, график, план, список, программа, планировать, назначать, намечать"],
    ["yacht", "[ˈjɑːt]", "яхта, яхтенный, ходить на яхте"],
    ["attempt", "[əˈtempt]", "попытка, покушение, опыт, проба, пытаться, пробовать, покушаться"],
    ["pollute", "[pəˈluːt]", "загрязнять, осквернять, загрязненный, загрязнение"],
    ["proof", "[ˈpruːf]", "доказательство, устойчивый, непроницаемый, делать непроницаемым"],
    ["author", "[ˈɒθər]", "автор, писатель, создатель, творец, сочинитель, виновник, инициатор"],
    ["depart", "[dəˈpɑːrt]", "отправляться, уходить, отступать, отклоняться, уезжать, отбывать, уклоняться"],
    ["indirect", "[ˌɪndəˈrekt]", "косвенный, непрямой, побочный, окольный, уклончивый"],
    ["magnificent", "[mægˈnɪfəsənt]", "великолепный, прекрасный, величественный, изумительный, блистательный"],
    ["supper", "[ˈsʌpər]", "ужин, время ужина, ужинать, кормить ужином"],
    ["wonder", "[ˈwʌndər]", "чудо, удивление, изумление, удивляться, изумительный"],
    ["bookshelf", "[ˈbʊkˌʃelf]", "книжная полка"],
    ["connection", "[kəˈnekʃən]", "соединение, связь, связи, присоединение, сочленение, родство, родственник"],
    ["election", "[əˈlekʃən]", "выборы, избрание, перевыборы, предопределение"],
    ["hamster", "[ˈhæmstər]", "хомяк"],
    ["pause", "[ˈpɒz]", "пауза, перерыв, остановка, замешательство, останавливаться, делать паузу"],
    ["rabbit", "[ˈræˌbɪt]", "кролик, заяц, слабый игрок, охотиться на кроликов, много говорить"],
    ["sex", "[ˈseks]", "секс, пол, половое сношение, половой, сексуальный"],
    ["whether", "[ˈweðər]", "ли, который"],
    ["fancy", "[ˈfænsi]", "фантазия, воображение, вкус, причудливый, модный, полагать"],
    ["member", "[ˈmembər]", "член, участник, представитель, деталь, звено, часть, партнер, элемент конструкции"],
    ["postpone", "[postˈpoʊn]", "откладывать, отсрочивать"],
    ["double", "[ˈdʌbəl]", "двойной, вдвое, вдвоем, двойник, удваивать"],
    ["hometown", "[ˈhomˌtɑːwn]", "родной город, город, где человек постоянно живет"],
    ["laundry", "[ˈlɒndri]", "прачечная, белье для стирки, белье из стирки"],
    ["pastime", "[ˈpæˌstaɪm]", "времяпровождение, игра, приятное времяпрепровождение, приятное развлечение"],
    ["queue", "[ˈkjuː]", "очередь, коса, хвост, косичка, стоять в очереди"],
    ["snack", "[ˈsnæk]", "закуска, легкая закуска, что-либо легко достижимое"],
    ["someday", "[ˈsʌmdeɪ]", "когда-нибудь"],
    ["tend", "[ˈtend]", "иметь тенденцию, ухаживать, заботиться, пасти, иметь склонность, присматривать"],
    ["plan", "[ˈplæn]", "план, намерение, проект, замысел, схема, планировать, строить планы"],
    ["cheque", "[ˈtʃek]", "проверка, чек, контроль, проверять, контролировать, контрольный, клетчатый"],
    ["context", "[ˈkɑːntekst]", "контекст, связь, ситуация, обстановка, фон"],
    ["direction", "[dəˈrekʃən]", "направление, руководство, постановка, управление, правление, режиссура, указание"],
    ["fan", "[ˈfæn]", "вентилятор, поклонник, веер, любитель, раздуть, раздувать, обмахивать, разжигать"],
    ["intend", "[ˌɪnˈtend]", "намереваться, предназначать, намеренно"],
    ["remain", "[rəˈmeɪn]", "оставаться, продержаться, остаток, остальное"],
    ["themselves", "[ðemˈselvz]", "себя, себе, сами"],
    ["educate", "[ˈedʒəˌket]", "воспитывать, давать образование, тренировать, консультировать"],
    ["statistics", "[stəˈtɪstɪks]", "статистика, статистические данные"],
    ["charitable", "[ˈtʃerətəbəl]", "благотворительный, милосердный, щедрый"],
    ["duty", "[ˈduːti]", "обязанность, долг, пошлина, служба, дежурный, служебный, официальный"],
    ["frank", "[ˈfræŋk]", "откровенный, открытый, искренний, явный, франк, франкировка"],
    ["mission", "[ˈmɪʃən]", "миссия, задание, цель, миссионерский, посылать с поручением"],
    ["weird", "[ˈwɪrd]", "странный, сверхъестественный, судьба, рок, предзнаменование"],
    ["iron", "[ˈaɪərn]", "железо, утюг, железный, твердый, жестокий, гладить, утюжить"],
    ["cloudy", "[ˈklaʊdi]", "облачный, мутный, непрозрачный, туманный, мрачный, хмурый, затуманенный, неясный"],
    ["economical", "[ˌekəˈnɑːmɪkəl]", "экономический, экономичный, экономный, хозяйственный, экономически выгодный"],
    ["though", "[ˈðoʊ]", "хотя, однако, несмотря на, если бы, однако, все же"],
    ["tower", "[ˈtaʊər]", "башня, вышка, крепость, опора, возвышаться, выситься, башенный"],
    ["weapon", "[ˈwepən]", "оружие, боевое средство, средства самозащиты"],
    ["used", "[ˈjuːzd]", "использованный, используемый, подержанный, отработавший, отработанный"],
    ["approximately", "[əˈprɑːksəmətli]", "приблизительно, приближенно, почти, ориентировочно"],
    ["complicated", "[ˈkɑːmpləˌketəd]", "сложный, осложненный, усложненный, запутанный"],
    ["gallery", "[ˈɡæləri]", "галерея, хоры, галерка, балкон, штольня, портик, штрек, публика на галерке"],
    ["inevitable", "[ˌɪˈnevətəbəl]", "неизбежный, неминуемый, неизменный"],
    ["player", "[ˈpleɪr̩]", "игрок, плеер, музыкант, спортсмен, актер, картежник"],
    ["sneakers", "[ˈsniːkərz]", "тапочки, тапки, теннисные туфли"],
    ["artificial", "[ˌɑːrtəˈfɪʃəl]", "искусственный, притворный, искусственное удобрение"],
    ["courageous", "[kəˈreɪdʒəs]", "смелый, отважный, храбрый, бравый"],
    ["electronic", "[əˌlekˈtrɑːnɪk]", "электронный"],
    ["exit", "[ˈeɡzət]", "выход, выезд, уход, смерть, выходить, уходить, умереть, выходной"],
    ["foolish", "[ˈfuːˌlɪʃ]", "глупый, дурацкий, безрассудный, бессмысленный, дурашливый"],
    ["gadget", "[ˈɡædʒət]", "безделушка, ерунда, новое устройство, новое приспособление, попсушка"],
    ["historical", "[ˌhɪˈstɔːrɪkəl]", "исторический, исторически установленный"],
    ["immediate", "[ˌɪˈmiːdiət]", "немедленный, непосредственный, прямой, ближайший, безотлагательный, срочный"],
    ["loyal", "[ˌlɔɪəl]", "лояльный, верный, преданный, верноподданный"],
    ["period", "[ˈpɪriəd]", "период, время, точка, промежуток времени, тайм, эпоха, урок, пауза в конце периода"],
    ["physically", "[ˈfɪzɪkəli]", "физически"],
    ["precious", "[ˈpreʃəs]", "драгоценный, дорогой, любимый, изысканный, очень, здорово"],
    ["principle", "[ˈprɪnsəpəl]", "принцип, правило, закон, элемент, причина, источник, принцип устройства"],
    ["rapid", "[ˈræpəd]", "быстрый, скоростной, скорый, крутой, стремнина, порог реки"],
    ["relation", "[riˈleɪʃən]", "отношение, связь, соотношение, зависимость, родственник, родство, родственница"],
    ["stability", "[stəˈbɪləti]", "стабильность, устойчивость, прочность, остойчивость, постоянство, твердость"],
    ["unstable", "[ʌnˈsteɪbəl]", "неустойчивый, непостоянный, нестойкий, неуравновешенный, изменчивый, нетвердый"],
    ["annual", "[ˈænjuːəl]", "ежегодный, годовой, ежегодник, кассировать"],
    ["appropriate", "[əˈproʊpriət]", "соответствующий, подходящий, присущий, ассигновать, присваивать, предназначать"],
    ["cultural", "[ˈkʌltʃərəl]", "культурный, искусственно выращиваемый, обрабатываемый"],
    ["curly", "[ˈkɝːli]", "вьющийся, кудрявый, курчавый, волнистый, изогнутый"],
    ["dreadful", "[ˈdredfəl]", "ужасный, страшный, отвратительный, роман ужасов"],
    ["environmental", "[ənˌvaɪrənˈmentəl]", "относящийся к окружающей среде, экзогенный"],
    ["especially", "[əˈspeʃəli]", "особенно, специально, главным образом, сугубо"],
    ["exceed", "[ɪkˈsiːd]", "превышать, превосходить, быть больше, превышать пределы, переходить границы"],
    ["expectation", "[ˌekspekˈteɪʃən]", "ожидание, надежда, вероятность, предвкушение, упование, чаяние, виды на наследство"],
    ["flavour", "[ˈfleɪvər]", "вкус, аромат, привкус, букет, особенность, сдабривать, приправлять, придавать вкус"],
    ["heritage", "[ˈherətədʒ]", "наследие, наследство"],
    ["impression", "[ˌɪmˈpreʃən]", "впечатление, представление, отпечаток, мнение, оттиск, печать, вдавление, издание"],
    ["impressive", "[ˌɪmˈpresɪv]", "впечатляющий, выразительный, вальяжный, производящий глубокое впечатление"],
    ["maximum", "[ˈmæksəməm]", "максимальный, высший, максимум, максимальное значение"],
    ["modest", "[ˈmɑːdəst]", "скромный, умеренный, сдержанный, застенчивый, неприхотливый, нетребовательный"],
    ["old-fashioned", "[oʊl ˈfæʃənd]", "старинный, старомодный, устарелый"],
    ["per cent", "[pə ˈsent]", "процент"],
    ["productive", "[prəˈdəktɪv]", "продуктивный, производительный, плодотворный, плодовитый, производящий"],
    ["punctual", "[ˈpʌŋktʃʊəl]", "пунктуальный, аккуратный, точный"],
    ["relatively", "[ˈrelətɪvli]", "относительно, сравнительно, соответственно, по поводу"],
    ["surface", "[ˈsɝːfəs]", "поверхность, поверхностный, внешний, всплывать"],
    ["surprisingly", "[sərˈpraɪzɪŋli]", "удивительно, неожиданно, необычайно"],
    ["tidy", "[ˈtaɪdi]", "опрятный, аккуратный, убирать, приборка, уборка"],
    ["turnover", "[ˈtɜːnoʊvə]", "оборот, опрокидывание, текучесть рабочей силы, пирог с начинкой"],
    ["unlikely", "[ʌnˈlaɪkli]", "маловероятный, невероятный, вряд ли, едва ли, навряд ли"],
    ["user", "[ˈjuːzər]", "пользователь, потребитель, наркоман, пользование правом или вещью"],
    ["fine", "[ˈfaɪn]", "штраф, пеня, оштрафовать, прекрасный, тонкий, прекрасно, хорошо"],
    ["arrogant", "[ˈerəɡənt]", "высокомерный, надменный, заносчивый, самонадеянный, кичливый"],
    ["careless", "[ˈkerləs]", "небрежный, неосторожный, беспечный, беззаботный, невнимательный, легкомысленный"],
    ["childish", "[ˈtʃaɪlˌdɪʃ]", "детский, ребяческий, ребячий, несерьезный"],
    ["extreme", "[ɪkˈstriːm]", "крайний, экстремальный, чрезвычайный, крайность, крайняя степень"],
    ["helpful", "[ˈhelpfəl]", "полезный"],
    ["made-up", "[ˌmeɪd ˈʌp]", "составной, сборный, искусственный, вымышленный, выдуманный, загримированный"],
    ["native", "[ˈneɪtɪv]", "родной, местный, нативный, природный, уроженец, уроженка, туземец, туземка"],
    ["rather", "[ˈræðər]", "скорее, довольно, вернее, лучше, несколько, предпочтительнее, слегка, охотнее"],
    ["sensible", "[ˈsensəbəl]", "разумный, здравомыслящий, здравый, заметный, благоразумный, осмысленный"],
    ["suggestion", "[səgˈdʒestʃən]", "предложение, внушение, совет, намек, указание"],
    ["unnatural", "[ʌnˈnætʃərəl]", "неестественный, противоестественный, странный, необычный, чудовищный"],
    ["bizarre", "[bəˈzɑːr]", "странный, причудливый, эксцентричный"],
    ["emotional", "[ɪˈmoʊʃənəl]", "эмоциональный, волнующий, взволнованный"],
    ["equal", "[ˈiːkwəl]", "равный, одинаковый, равняться, быть равным, ровня"],
    ["external", "[ɪkˈstɝːnəl]", "внешний, наружный, посторонний, внешняя поверхность, наружная сторона"],
    ["function", "[ˈfʌŋkʃən]", "функция, назначение, должность, прием, функционировать, работать, действовать"],
    ["gigantic", "[dʒaɪˈɡæntɪk]", "гигантский, исполинский, громадный"],
    ["inactive", "[ˌɪˈnæktɪv]", "неактивный, бездействующий, недействующий, инертный, бездеятельный"],
    ["manner", "[ˈmænər]", "способ, манера, метод, стиль, образ действий, род, нравы, обычаи, хорошие манеры, сорт"],
    ["manufacture", "[ˌmænjəˈfæktʃər]", "производство, изготовление, изделия, производить, изготовлять, выделывать"],
    ["mysterious", "[ˌmɪˈstɪriəs]", "таинственный, непостижимый"],
    ["painful", "[ˈpeɪnfəl]", "болезненный, болевой, мучительный, тягостный, тяжелый, трудный, неприятный"],
    ["sincere", "[ˌsɪnˈsɪr]", "искренний, душевный, задушевный, честный, чистосердечный, прямой, чистый"],
    ["source", "[ˈsɔːrs]", "источник, исток, исходный текст, начало, родник, первопричина, ключ, верховье"],
    ["temporarily", "[ˌtempəˈrerəli]", "временно"],
    ["unreliable", "[ˌʌnrəˈlaɪəbəl]", "ненадежный, неблагонадежный"],
    ["face", "[ˈfeɪs]", "лицо, грань, лик, циферблат, выражение лица, сталкиваться, стоять лицом"],
    ["chemical", "[ˈkeməkəl]", "химический, химикат"],
    ["determined", "[dəˈtɝːmənd]", "определенный, установленный, решительный, полный решимости, непреклонный"],
    ["dull", "[ˈdəl]", "скучный, тупой, унылый, тусклый, пасмурный, притуплять, притупляться, делать тупым"],
    ["elegant", "[ˈeləɡənt]", "элегантный, изящный, изысканный, модник, щеголь, пижон"],
    ["element", "[ˈelɪmənt]", "элемент, стихия, звено, составная часть, подразделение, секция, основы, азы, след"],
    ["impatient", "[ˌɪmˈpeɪʃənt]", "нетерпеливый, нетерпимый, раздражительный, беспокойный, нетерпящий"],
    ["rational", "[ˈræʃənəl]", "рациональный, разумный, целесообразный"],
    ["realistic", "[ˌriəˈlɪstɪk]", "реалистичный, реалистический, практический, трезвый"],
    ["ripe", "[ˈraɪp]", "спелый, зрелый, созревший, готовый, берег"],
    ["sharp", "[ˈʃɑːrp]", "резкий, острый, резко, круто, ровно, диез, жулик, знаток, плутовать"],
    ["skilful", "[ˈskɪlfəl]", "умелый, искусный, ловкий"],
    ["belief", "[bəˈliːf]", "вера, убеждение, мнение, верование, доверие"],
    ["border", "[ˈbɔːrdər]", "граница, бордюр, край, кайма, ободок, фриз, граничить, окаймлять, обшивать, походить"],
    ["buyer", "[ˈbaɪər]", "покупатель"],
    ["classic", "[ˈklæsɪk]", "классический, образцовый, классик, классика"],
    ["contradict", "[ˌkɑːntrəˈdɪkt]", "противоречить, возражать, опровергать, отрицать"],
    ["current", "[ˈkɜːrənt]", "текущий, современный, теперешний, ходячий, ток, течение, поток, струя, ход"],
    ["extraordinary", "[ɪkˈstrɔːrdəneri]", "чрезвычайный, внеочередной, экстраординарный, необычайный, необычный"],
    ["gifted", "[ˈɡɪftəd]", "одаренный, талантливый, даровитый, способный"],
    ["electrical", "[əˈlektrɪkəl]", "электрический, электробытовой, наэлектризованный, поразительный, волнующий"],
    ["heater", "[ˈhiːtər]", "нагреватель, обогреватель, подогреватель, печь, калорифер, радиатор"],
    ["highlighter", "[ˈhaɪlaɪtər]", "косметическое средство"],
    ["improvement", "[ˌɪmˈpruːvmənt]", "улучшение, усовершенствование, мелиорация, удобства, перестройка, перестановка"],
    ["moral", "[ˈmɔːrəl]", "моральный, нравственный, духовный, нравственность, мораль, нравы"],
    ["neglect", "[nəˈɡlekt]", "пренебрегать, не заботиться, упускать, пренебрежение, безнадзорность"],
    ["slight", "[ˈslaɪt]", "незначительный, легкий, пренебрежение, неуважение, пренебрегать, третировать"],
    ["spiritual", "[ˈspɪrɪtʃəwəl]", "духовный, одухотворенный, религиозный, спиричуэл, верующий, церковные дела"],
    ["thankful", "[ˈθæŋkfəl]", "благодарный"],
    ["unpopular", "[ˌənˈpɑːpjələr]", "непопулярный, не пользующийся любовью"],
    ["unwilling", "[ʌˈnwɪlɪŋ]", "несклонный, нерасположенный"],
    ["acceptable", "[ækˈseptəbəl]", "приемлемый, допустимый, подходящий, угодный, желанный, приятный"],
    ["available", "[əˈveɪləbəl]", "доступный, имеющийся, налицо, имеющийся в наличии, наличный"],
    ["bunch", "[ˈbəntʃ]", "гроздь, связка, пучок, группа, пук, сгусток, сбивать в кучу, сбиваться в кучу"],
    ["clinical", "[ˈkəlɪnəkəl]", "клинический"],
    ["enormous", "[əˈnɔːrməs]", "огромный, громадный, чудовищный, ужасный"],
    ["horrible", "[ˈhɔːrəbəl]", "ужасный, страшный, отвратительный, нечто ужасное, роман ужасов, ужастик"],
    ["legal", "[ˈliːɡəl]", "правовой, юридический, законный, легальный, узаконенный"],
    ["metre", "[ˈmiːtər]", "метр, счетчик, измеритель, измерять, дозировать, мерить"],
    ["naive", "[ˌnɑːˈjiːv]", "наивный, простодушный, безыскусный, безыскусственный"],
    ["objective", "[əbˈdʒektɪv]", "цель, задача, объект, объектив, стремление, объективный, целевой, предметный"],
    ["overtime", "[ˈoʊvr̩taɪm]", "сверхурочный, сверхурочно, передержать"],
    ["pale", "[ˈpeɪl]", "бледный, слабый, тусклый, бледнить, бледнеть, рамки, пределы, черта"],
    ["philosophical", "[ˌfɪləˈsɑːfɪkəl]", "философский"],
    ["rose", "[roʊz]", "розовый, роза, розетка, розовый цвет"],
    ["row", "[ˈroʊ]", "ряд, ссора, спор, шум, ряд домов, улица, гвалт, грести, скандалить, шуметь, отчитывать"],
    ["square", "[ˈskwer]", "квадратный, площадь, квадрат, прямо, честно"],
    ["state", "[ˈsteɪt]", "государственный, состояние, государство, констатировать"],
    ["waist", "[ˈweɪst]", "талия, шкафут, сужение, лиф, корсаж, перехват, узкая часть, детский лифчик"],
    ["calm", "[ˈkɑːm]", "спокойный, тихий, спокойствие, штиль, затишье, успокаивать, умиротворять"],
    ["cautious", "[ˈkɒʃəs]", "осторожный, осмотрительный, опасливый, предусмотрительный, бережный"],
    ["consequence", "[ˈkɑːnsəkwəns]", "следствие, последствие, значение, вывод, важность, заключение, влиятельность"],
    ["exclusive", "[ɪkˈskluːsɪv]", "эксклюзивный, исключительный, особый, исключительно, не считая"],
    ["fair", "[ˈfer]", "справедливый, честный, ярмарка, выставка, базар, честно, ясно, чисто, точно"],
    ["faithful", "[ˈfeɪθfəl]", "верный, верующий, точный, преданный, верующие, правоверные"],
    ["instrument", "[ˈɪnstrəmənt]", "инструмент, прибор, приборный, инструментовать"],
    ["jet", "[ˈdʒet]", "струя, реактивный самолет, реактивный, струйный, бить струей"],
    ["musical", "[ˈmjuːzɪkəl]", "музыкальный, мелодичный, мюзикл, музыкальный фильм"],
    ["practically", "[ˈpræktəkəli]", "практически, фактически, почти, на практике, на деле"],
    ["primary", "[ˈpraɪˌmeri]", "первичный, основной, начальный, главный, первичная обмотка, основной цвет"],
    ["property", "[ˈprɑːpərti]", "собственность, свойство, имущество, имущественный"],
    ["range", "[ˈreɪndʒ]", "диапазон, ряд, пределы, простираться, любящий порядок"],
    ["secure", "[sɪˈkjʊr]", "безопасный, обеспеченный, обезопасить, гарантировать, добиваться"],
    ["treatment", "[ˈtriːtmənt]", "лечение, обработка, обращение, терапия, уход, подход, трактовка, излечение, пропитка"],
    ["uncertain", "[ʌnˈsɝːtən]", "неопределенный, неуверенный, сомнительный, ненадежный, изменчивый, сомневающийся"],
    ["unemotional", "[ˌʌnɪˈməʊʃənəl]", "неэмоциональный, бесстрастный, невозмутимый"],
    ["uninterested", "[ʌˈnɪntrəstəd]", "незаинтересованный, безразличный, равнодушный, не интересующийся"],
    ["arrangement", "[əˈreɪndʒmənt]", "расположение, договоренность, устройство, соглашение, механизм, аранжировка"],
    ["contain", "[kənˈteɪn]", "содержать, содержаться, сдерживать, сдерживаться, содержать в себе, вмещать"],
    ["convinced", "[kənˈvɪnst]", "убежденный"],
    ["exception", "[ɪkˈsepʃən]", "исключение, особая ситуация, возражение, отвод, неодобрение"],
    ["exciting", "[ɪkˈsaɪtɪŋ]", "захватывающий, возбуждающий, волнующий"],
    ["organism", "[ˈɔːrɡəˌnɪzəm]", "организм, живое существо"],
    ["overall", "[ˌoʊvəˈrɔːl]", "общий, полный, всеобщий, полностью, повсюду, комбинезон, спецодежда"],
    ["particular", "[pərˈtɪkjələr]", "особый, особенный, частный, специфический, частность, деталь, подробность"],
    ["passive", "[ˈpæsɪv]", "пассивный, страдательный, инертный, страдательный залог, пассивная форма"],
    ["reasonable", "[ˈriːzənəbəl]", "разумный, приемлемый, обоснованный, умеренно"],
    ["romantic", "[roˈmæntɪk]", "романтический, романтичный, романтик"],
    ["silent", "[ˈsaɪlənt]", "тихий, молчаливый, немой, безмолвный, немой фильм"],
    ["unfair", "[ənˈfer]", "несправедливый, нечестный, пристрастный, неправильный"],
    ["wavy", "[ˈweɪvi]", "волнистый, волнообразный, рифленый, колеблющийся"],
    ["wild", "[ˈwaɪld]", "дикий, одичалый, бешеный, безумный, бурный, наугад, как попало"],
    ["willing", "[ˈwɪlɪŋ]", "готовый, добровольный, старательный, охотно делающий что-л."],
    ["aspect", "[ˈæspekt]", "аспект, сторона, вид, внешний вид, взгляд, видовое"],
    ["awesome", "[ˈɑːsəm]", "устрашающий, испуганный, здорово, классно"],
    ["brief", "[ˈbriːf]", "краткий, короткий, недолгий, резюме, сводка, инструкция, резюмировать"],
    ["essential", "[əˈsenʃəl]", "необходимый, существенный, основной, сущность, неотъемлемость"],
    ["illogical", "[ˌɪˈlɑːdʒɪkəl]", "нелогичный, алогичный"],
    ["irritate", "[ˈɪrəˌtet]", "раздражать, вызывать раздражение, нервировать, злить, сердить, бередить"],
    ["knowledgeable", "[ˈnɑːlədʒəbəl]", "хорошо осведомленный, умный"],
    ["logical", "[ˈlɑːdʒɪkəl]", "логический, логичный, последовательный, связный"],
    ["skin", "[ˈskɪn]", "кожа, шкура, оболочка, кожица, ободрать, покрывать кожей, накожный"],
    ["smooth", "[ˈsmuːð]", "гладкий, плавный, гладко, ровно, приглаживать"],
    ["thoughtful", "[ˈθɒtfəl]", "вдумчивый, заботливый, задумчивый, внимательный, чуткий, глубокий по мысли"],
    ["unfriendly", "[ənˈfrendli]", "недружественный, недружелюбный, неприветливый, неблагоприятный"],
    ["unsure", "[ʌnˈʃʊr]", "неуверенный, ненадежный, неопределенный, колеблющийся"],
    ["wet", "[ˈwet]", "мокрый, влажный, сырой, влажность, сырость, мочить, смачивать"],
    ["attentive", "[əˈtentɪv]", "внимательный, предупредительный, заботливый, вежливый"],
    ["broad-minded", "[brɔːrd ˈmaɪndɪd]", "либеральный, с широким кругозором, с широкими взглядами, терпимый"],
    ["competent", "[ˈkɑːmpətənt]", "компетентный, правомочный, авторитетный, знающий, полноправный, достаточный"],
    ["decisive", "[dəˈsaɪsɪv]", "решающий, решительный, окончательный, убедительный, имеющий решающее значение"],
    ["embarrassed", "[emˈberəst]", "смущенный, растерянный, стесненный, запутавшийся в долгах"],
    ["handsome", "[ˈhænsəm]", "красивый, щедрый, статный, благообразный, значительный"],
    ["incompetent", "[ˌɪnˈkɑːmpətənt]", "некомпетентный, неумелый, неспособный, несведущий, слабый, неправоспособный"],
    ["individual", "[ˌɪndəˈvɪdʒəwəl]", "индивидуальный, отдельный, личный, человек, личность, индивидуум, особа, особь"],
    ["internal", "[ˌɪnˈtɝːnəl]", "внутренний, душевный, сокровенный"],
    ["practical", "[ˈpræktəkəl]", "практический, практичный, удобный, реальный, фактический, полезный, целесообразный"],
    ["psychological", "[ˌsaɪkəˈlɑːdʒɪkəl]", "психологический"],
    ["quick", "[ˈkwɪk]", "быстрый, скорый, живой, быстро, скоро, живо, живая изгородь, чувства"],
    ["self-confident", "[ˈselfˈkɑːnfədənt]", "самоуверенный, самонадеянный"],
    ["shy", "[ˈʃaɪ]", "застенчивый, уклоняться, пугаться, бросок, попытка"],
    ["sociable", "[ˈsoʊʃəbəl]", "общительный, коммуникабельный, вечеринка, козетка, встреча"],
    ["thick", "[ˈθɪk]", "толстый, густой, плотный, густо, плотно, хрипло, часто, пекло, гуща, разгар, тупица"],
    ["wooden", "[ˈwʊdən]", "деревянный, безжизненный, топорный"],
    ["according to", "[əˈkɔːdɪŋ tuː]", "согласно"],
    ["favour", "[ˈfeɪvər]", "польза, интерес, одолжение, поддерживать, благоволить"],
    ["ironing", "[ˈaɪərnɪŋ]", "гладильный, глаженье, утюжка, белье для глаженья"],
    ["nails", "[neɪlz]", "гвозди / ногти"],
    ["necessarily", "[ˌnesəˈserəli]", "обязательно, неизбежно, непременно"],
    ["reservation", "[ˌrezərˈveɪʃən]", "бронирование, резервация, запасный"],
    ["rightly", "[ˈraɪtli]", "справедливо, правильно, должным образом"],
    ["analyst", "[ˈænələst]", "аналитик, обозреватель, психоаналитик, комментатор, психиатр"],
    ["appointment", "[əˌpɔɪntmənt]", "назначение, встреча, прием, должность, свидание, пост, определение, место"],
    ["chores", "[ˈtʃɔːrz]", "домашняя работа"],
    ["favourable", "[ˈfeɪvərəbəl]", "благоприятный, подходящий, удобный, благосклонный, симпатизирующий"],
    ["fuss", "[ˈfəs]", "суетиться, ссориться, суматоха, суета из-за пустяков"],
    ["homeless", "[ˈhoʊmləs]", "бездомный, беспризорный, бесприютный, бездомный, беспризорный"],
    ["lecturer", "[ˈlektʃərər]", "преподаватель, лектор, доцент, дьяк"],
    ["motto", "[ˈmɑːtoʊ]", "девиз, лозунг, эпиграф"],
    ["observation", "[ˌɑːbzərˈveɪʃən]", "наблюдение, замечание, соблюдение, наблюдательность, высказывание, обсервация"],
    ["prediction", "[priˈdɪkʃən]", "прогноз, предсказание, пророчество, предвычисление, предварение"],
    ["storm", "[ˈstɔːrm]", "штурм, шторм, буря, гроза, ураган, взрыв, град, штурмовать, бушевать, свирепствовать"],
    ["arrival", "[əˈraɪvəl]", "прибытие, приезд, приход, достижение, вновь прибывший, новорожденный"],
    ["bookmark", "[ˈbʊkmɑːrk]", "закладка"],
    ["distantly", "[ˈdɪstəntli]", "отдаленно, сдержанно, холодно"],
    ["enlarge", "[ɪnˈlɑːrdʒ]", "увеличивать, увеличиваться, расширять, расширяться, распространяться, укрупнять"],
    ["oversleep", "[ˌoʊvəˈsliːp]", "проспать, просыпать, заспаться"],
    ["publicly", "[ˈpʌblɪkli]", "публично, открыто, гласно, громогласно"],
    ["separate", "[ˈsepəˌret]", "отдельный, отделяться, разделяться, отдельный оттиск"],
    ["tiptoe", "[ˈtɪpˌtoʊ]", "цыпочки, на цыпочках, украдкой, ходить на цыпочках"],
    ["widen", "[ˈwaɪdən]", "расширять, расширяться, шириться, ширить"],
    ["adult", "[əˈdəlt]", "взрослый, для взрослых, взрослый человек, зрелый человек"],
    ["athletic", "[æθˈletɪk]", "спортивный, атлетический, богатырский"],
    ["bankruptcy", "[ˈbæŋkrʌpsi]", "банкротство, несостоятельность"],
    ["air conditioning", "[ˈeə kənˌdɪʃənɪŋ]", "кондиционирование"],
    ["heat", "[ˈhiːt]", "тепло, жар, жара, теплота, нагрев, зной, разгар, нагревать, нагреваться, греть, топить"],
    ["severe", "[səˈvɪr]", "тяжелый, серьезный, сильный, суровый, строгий, жестокий, резкий, трудный, выдержанный"],
    ["verge", "[ˈvɝːdʒ]", "грань, край, граница, обочина, посох, жезл, приближаться, клониться"],
    ["virtual", "[ˈvɝːtʃuːəl]", "виртуальный, действительный, фактический, мнимый, не номинальный"],
    ["deliberately", "[dəˈlɪbərətli]", "умышленно, нарочно, обдуманно, медленно, осторожно, не спеша, осмотрительно"],
    ["equally", "[ˈiːkwəli]", "одинаково, равно, в равной степени, поровну"],
    ["fashion", "[ˈfæʃən]", "мода, образ, стиль, фасон, вид, фасонировать, модельный"],
    ["hardly", "[ˈhɑːrdli]", "вряд ли, едва, едва ли, чуть, с трудом, еле, насилу, резко, сурово, несправедливо"],
    ["honestly", "[ˈɑːnəstli]", "честно, искренне, правдиво"],
    ["hopefully", "[ˈhoʊpfəli]", "с надеждой"],
    ["injured", "[ˈɪndʒərd]", "раненый, травмированный, поврежденный, пострадавший"],
    ["mainly", "[ˈmeɪnli]", "главным образом, большей частью"],
    ["occasionally", "[əˈkeɪʒənəli]", "иногда, изредка, время от времени, порой, случайно, подчас, нерегулярно"],
    ["prevent", "[prɪˈvent]", "предотвращать, препятствовать, предупреждать, не допускать, мешать, предохранять"],
    ["primarily", "[praɪˈmerəli]", "в первую очередь, прежде всего, главным образом, первоначально, сначала"],
    ["shortly", "[ˈʃɔːrtli]", "вскоре, незадолго, коротко, быстро, вкратце, отрывисто, резко, сжато"],
    ["similarly", "[ˈsɪmələrli]", "так же, подобным образом"],
    ["currently", "[ˈkɜːrəntli]", "в настоящее время, теперь, свободно, легко"],
    ["effectively", "[ɪˈfektɪvli]", "продуктивно"],
    ["employment", "[emˌplɔɪmənt]", "занятие, работа, применение, использование, наем, служба"],
    ["frankly", "[ˈfræŋkli]", "откровенно, прямо"],
    ["naturally", "[ˈnætʃərəli]", "естественно, конечно, разумеется, натурально, по природе, легко, свободно"],
    ["personally", "[ˈpɝːsənəli]", "лично, персонально, сам, собственной персоной, что касается меня, его и т.п."],
    ["possibly", "[ˈpɑːsəbli]", "возможно, по возможности, может быть"],
    ["significant", "[sɪgˈnɪfɪkənt]", "значительный, существенный, значимый, знак, символ, указание"],
    ["smoothly", "[ˈsmuːðli]", "плавно, гладко, ровно"],
    ["unavailable", "[ˌʌnəˈveɪləbəl]", "недействительный, не имеющийся в наличии"],
    ["deeply", "[ˈdiːpli]", "глубоко"],
    ["educated", "[ˈedʒəˌketəd]", "образованный, развитой, тренированный, дрессированный"],
    ["highly", "[ˈhaɪli]", "высоко, очень, весьма, сильно, чрезвычайно, в высшей степени, благоприятно"],
    ["indeed", "[ˌɪnˈdiːd]", "действительно, в самом деле, впрямь, уж"],
    ["nearly", "[ˈnɪrli]", "почти, около, приблизительно, близко"],
    ["nevertheless", "[ˌnevərðəˈles]", "тем не менее, однако, несмотря на это, тем не менее, все-таки, несмотря на"],
    ["score", "[ˈskɔːr]", "счет, партитура, количество очков, вести счет, выигрывать, засчитывать"],
    ["sensation", "[senˈseɪʃən]", "ощущение, сенсация, чувство"],
    ["slightly", "[ˈslaɪtli]", "немного, слегка, чуть, едва"],
    ["toothache", "[ˈtuːθeɪk]", "зубная боль"],
    ["undoubtedly", "[ʌnˈdaʊtədli]", "несомненно, безусловно, бесспорно"],
    ["wrongly", "[ˈrɒŋli]", "превратно"],
    ["disapprove", "[ˌdɪsəˈpruːv]", "не одобрять, неодобрительно относиться"],
    ["discomfort", "[dɪsˈkʌmfərt]", "дискомфорт, неудобство, беспокойство, беспокоить, причинять неудобство"],
    ["endless", "[ˈendləs]", "бесконечный, нескончаемый, безысходный, бесчисленный"],
    ["handwriting", "[ˈhænˌdraɪtɪŋ]", "почерк"],
    ["harmless", "[ˈhɑːrmləs]", "безвредный, безобидный, невинный, ни в чем не повинный"],
    ["illegible", "[ˌɪˈledʒəbəl]", "неразборчивый, нечеткий, неудобочитаемый"],
    ["impolite", "[ˌɪmpəˌlaɪt]", "невежливый, неучтивый"],
    ["inaccurate", "[ˌɪˈnækjərət]", "неточный, неправильный, ошибочный"],
    ["indecisive", "[ˌɪndəˈsaɪsɪv]", "нерешительный, неокончательный, нерешающий, колеблющийся, неопределенный"],
    ["indifferent", "[ˌɪnˈdɪfrənt]", "равнодушный, безразличный, индифферентный, нейтральный, безучастный"],
    ["inefficient", "[ˌɪnəˈfɪʃənt]", "неэффективный, непроизводительный, неспособный, неумелый, безрезультатный"],
    ["invaluable", "[ˌɪnˈvæljəbəl]", "бесценный, неоценимый"],
    ["irreplaceable", "[ɪrəˈpleɪsəbəl]", "незаменимый, невосстановимый, невозместимый"],
    ["irresponsible", "[ɪrəˈspɑːnsəbəl]", "безответственный, несознательный, невменяемый, неответственный"],
    ["misunderstand", "[ˌmɪsəndərˈstænd]", "неправильно понять, не понять, недоразумение"],
    ["misunderstanding", "[ˌmɪsəndərˈstændɪŋ]", "недоразумение, неправильное понимание, размолвка"],
    ["signature", "[ˈsɪɡnətʃər]", "подпись, подписание, автограф, ключ, ставить подпись"],
    ["suffering", "[ˈsʌfərɪŋ]", "страдание, страдающий"],
    ["unaware", "[ˌʌnəˈwer]", "не подозревающий, не знающий о, невосприимчивый"],
    ["unimaginable", "[ˌʌnəˈmædʒənəbəl]", "невообразимый"],
    ["unwillingly", "[ʌˈnwɪlɪŋli]", "неохотно, против желания"],
    ["pretend", "[priˈtend]", "притворяться, делать вид, претендовать, симулировать, прикидываться, ссылаться"],
    ["go on", "[ˈɡoʊ ɑːn]", "продолжать, продолжаться, длиться, происходить, твориться, случаться"],
    ["look up to", "[ˈlʊk ʌp tuː]", "уважать, восхищаться"],
    ["make up for", "[ˈmeɪk ʌp fɔː]", "компенсировать"],
    ["throw away", "[ˈθroʊ əˈweɪ]", "упускать возможность, шанс, не воспользоваться, тратить, растрачивать, сбрасывать"],
    ["take up", "[ˈteɪk ʌp]", "занимать, принимать, браться за, поднимать, отнимать, обсуждать, впитывать влагу"],
    ["look into", "[lʊk ˈɪntə]", "изучать / разбираться"],
    ["flick through", "[flɪk θruː]", "пролистать"],
    ["figure out", "[ˈfɪɡjər ˈaʊt]", "вычислять, понимать, постигать"],
    ["break up", "[ˈbreɪk ʌp]", "разбивать, разбиваться, разойтись, расходиться, разломать, расформировывать"],
    ["be back", "[bi ˈbæk]", "возвращаться"],
    ["see off", "[ˈsiː ˈɒf]", "провожать"],
    ["call off", "[ˈkɒl ˈɒf]", "отменять, прекращать, отзывать, откладывать, переносить, отвлекать"],
    ["leave out", "[ˈliːv ˈaʊt]", "пропускать, упускать, не включать, выпускать"],
    ["ask out", "[ˌɑːsk ˈaʊt]", "проситься выйти, приглашать, подавать в отставку, подавать заявление об уходе"],
    ["basics", "[ˈbeɪsɪks]", "основы"],
    ["fuel", "[ˈfjuːəl]", "топливо, горючее, разжигать, снабжать топливом"],
    ["catch up with", "[kætʃ ʌp wɪð]", "догнать / наверстать"],
    ["keep up with", "[kiːp ʌp wɪð]", "не отставать от"],
    ["be over", "[biː ˈəʊvə]", "оставаться, приходить, приезжать, закончиться, пройти"],
    ["carry on", "[ˈkæri ɑːn]", "вести, продолжать, заниматься, флиртовать, вести себя запальчиво"],
    ["come across", "[ˈkəm əˈkrɒs]", "натолкнуться, повстречаться, набрести, выложить, случайно встретиться"],
    ["run out of", "[rʌn aʊt əv]", "исчерпать запас"],
    ["look down on", "[lʊk daʊn ɒn]", "смотреть свысока"],
    ["get on", "[ˈɡet ɑːn]", "ладить, ужиться, продолжать, уживаться, преуспевать, садиться, надевать, стареть"],
    ["get along with", "[ɡet əˈlɒŋ wɪð]", "ладить с"],
    ["get on with", "[ɡet ɒn wɪð]", "ладить / продолжать"],
    ["get in", "[ˈɡet ɪn]", "входить, садиться, влезать, присоединиться, участвовать, убирать, сажать, прибывать"],
    ["get down to", "[ɡet daʊn tuː]", "приступить к"],
    ["get down", "[ˈɡet ˈdaʊn]", "спускаться, слезть, браться, засесть, слезать, сходить, снять, проглатывать"],
    ["find out", "[ˈfaɪnd ˈaʊt]", "выяснять, узнавать, понять, обнаруживать, разузнавать, доискаться, раскрывать"],
    ["do about", "[duː əˈbaʊt]", "предпринять (по поводу)"],
    ["adventure", "[ædˈventʃər]", "приключение, авантюра, риск, приключенческий, рисковать, отваживаться"],
    ["coupon", "[ˈkuːˌpɒn]", "купон, талон"],
    ["fault", "[ˈfɒlt]", "неисправность, вина, ошибка, повреждение, придираться, ошибаться"],
    ["note", "[noʊt]", "примечание, внимание, записка, замечание, отмечать, замечать, указывать"],
    ["retirement", "[riˈtaɪərmənt]", "отставка, уход на пенсию, выход в отставку, уединение, отступление, отход"],
    ["schoolwork", "[ˈskuːˌlwərk]", "урок"],
    ["uncle", "[ˈʌŋkəl]", "дядя, дядька, ростовщик, пожилой человек"],
    ["come from", "[kʌm frɒm]", "происходить из"],
    ["bring up", "[ˈbrɪŋ ʌp]", "воспитывать, поднимать, растить, подвезти, выкормить, взрастить, выкармливать"],
    ["fill in", "[ˈfɪl ɪn]", "заполнять, пополнять, замещать, вписывать, занимать, бить, колотить"],
    ["lag behind", "[læɡ bɪˈhaɪnd]", "отставать"],
    ["look after", "[lʊk ˈɑːftə]", "заботиться"],
    ["come in", "[ˈkəm ɪn]", "входить, поступать, прибывать, пригодиться, вступать, прийти к финишу"],
    ["take after", "[ˈteɪk ˈɑːftə]", "пойти в (быть похожим на родственника)"],
    ["look ahead", "[ˈlʊk əˈhed]", "смотреть вперед, думать о будущем"],
    ["look back", "[ˈlʊk ˈbæk]", "оглядываться, оглядываться на прошлое, вспоминать прошлое"],
    ["look forward to", "[lʊk ˈfɔːwəd tuː]", "ждать с нетерпением"],
    ["look in", "[ˈlʊk ɪn]", "заглянуть, навещать, смотреть телепередачу"],
    ["look out", "[ˈlʊk ˈaʊt]", "высматривать, выглядывать, быть настороже, подыскивать, иметь вид, выходить на"],
    ["watch out", "[wɒtʃ aʊt]", "остерегаться"],
    ["come back", "[ˈkəm ˈbæk]", "возвращаться, воротиться, отплатить той же монетой, вспоминаться, очнуться"],
    ["get at", "[ˈɡet æt]", "добираться до / намекать"],
    ["get back", "[ˈɡet ˈbæk]", "возвращаться, получать назад, возмещать"],
    ["look through", "[ˈlʊk θruː]", "просматривать, смотреть в, перелистать, видеть насквозь"],
    ["boxing", "[ˈbɑːksɪŋ]", "бокс, упаковка, упаковка в ящики, тара, футляр, фанера для футляров"],
    ["electricity", "[əˌlekˈtrɪsəti]", "электричество"],
    ["forced", "[ˈfɔːrst]", "принудительный, вынужденный, форсированный, принужденный, натянутый"],
    ["industry", "[ˈɪndəstri]", "промышленность, индустрия, отрасль промышленности, трудолюбие, усердие, работа"],
    ["occasion", "[əˈkeɪʒən]", "случай, повод, раз, возможность, событие, вызывать, причинять, служить поводом"],
    ["proposal", "[prəˈpoʊzəl]", "предложение, заявка, план"],
    ["underestimate", "[ˈʌndəˈrestəˌmet]", "недооценивать, недооценка"],
    ["let down", "[ˈlet ˈdaʊn]", "опускать, подводить, спускать, уронить, разочаровывать, повредить репутации"],
    ["put off", "[ˈpʊt ˈɒf]", "откладывать, отчаливать, ссадить, отделываться, мешать, вызывать отвращение"],
    ["drop out", "[ˈdrɑːp ˈaʊt]", "выпадать, выбывать, опустить, исчезать, больше не участвовать, выпускать"],
    ["think over", "[ˈθɪŋk ˈoʊvr̩]", "продумывать, обдумывать, вдумываться, обмозговывать, взвешивать, обсуждать"],
    ["talk over", "[ˈtɔːk ˈoʊvr̩]", "обсуждать, дискутировать, убеждать"],
    ["put up with", "[pʊt ʌp wɪð]", "мириться с"],
    ["get ahead", "[ˈɡet əˈhed]", "продвигаться, преуспевать"],
    ["get round", "[ˈɡet ˈraʊnd]", "обойти, обходить, перехитрить, выздороветь, обманывать, преодолевать, навещать"],
    ["get around", "[ˈɡet əˈraʊnd]", "подойти, приближаться, становиться известным, распространиться"],
    ["cut off", "[ˈkət ˈɒf]", "отрезать, обрезать, отсекать, прерывать, выключать, отстригать, обрезывать"],
    ["put up", "[ˈpʊt ʌp]", "мириться, терпеть, вкладывать, ставить, поднимать, выставлять, вывешивать"],
    ["put on", "[ˈpʊt ɑːn]", "надевать, ставить, использовать, облекаться, прибавлять, прибавляться, применять"],
    ["go by", "[ˈɡoʊ baɪ]", "проходить, руководствоваться, судить по, проходить мимо, проезжать мимо"],
    ["go in for", "[ɡəʊ ɪn fɔː]", "увлекаться"],
    ["go off", "[ˈɡoʊ ˈɒf]", "уходить, сходить, выстрелить, уезжать, сбежать, засыпать, проходить, раздаться"],
    ["go away", "[ˈɡoʊ əˈweɪ]", "уходить, отойти, разойтись"],
    ["go out", "[ˈɡoʊ ˈaʊt]", "выходить, выходить в свет, гаснуть, тухнуть, бывать в обществе, погасать, потухать"],
    ["banknote", "[ˈbæŋknoʊt]", "банкнота, купюра, кредитный билет"],
    ["brigade", "[brəˈɡeɪd]", "бригада, команда, отряд, бригадный, формировать бригаду"],
    ["crowd", "[ˈkraʊd]", "толпа, народ, множество, масса, давка, толпиться, вытеснять, тесниться, теснить"],
    ["facility", "[fəˈsɪləti]", "объект, установка, возможность, предприятие, сооружение, завод, легкость"],
    ["failure", "[ˈfeɪljər]", "отказ, недостаточность, неудача, провал, сбой, неспособность, отсутствие"],
    ["firefighter", "[ˈfaɪrˌfaɪtər]", "пожарный, пожарник, пожарник-доброволец"],
    ["flu", "[ˈfluː]", "грипп"],
    ["inattentive", "[ˌɪnəˈtentɪv]", "невнимательный, неучтивый"],
    ["inexperience", "[ˌɪnɪkˈspɪriəns]", "неопытность"],
    ["investigation", "[ˌɪnˌvestəˈɡeɪʃən]", "исследование, расследование, следствие, разбор"],
    ["laziness", "[ˈleɪzinəs]", "лень, леность"],
    ["operator", "[ˈɑːpəˌretər]", "оператор, рабочий, механик, радист, хирург, дежурный, связист, телеграфист, телефонист"],
    ["savings", "[ˈseɪvɪŋz]", "сбережения"],
    ["tiredness", "[ˈtaɪərdnəs]", "усталость, утомление"],
    ["unprofessionalism", "[ˌʌnprəˈfeʃənəlɪzəm]", "непрофессионализм"],
    ["get into", "[ɡet ˈɪntə]", "влезть / увлечься"],
    ["cheer up", "[tʃɪər ʌp]", "взбодриться / развеселить"],
    ["get by", "[ˈɡet baɪ]", "проходить, сдать, сводить концы с концами, проезжать, устраиваться"],
    ["get over", "[ˈɡet ˈoʊvr̩]", "преодолевать, пережить, перебираться, обойти кого-л., перехитрить кого-л."],
    ["get away", "[ˈɡet əˈweɪ]", "уходить, избежать, удирать, ускользнуть, выбираться, отрываться, взлететь"],
    ["cut down", "[ˈkət ˈdaʊn]", "урезать, сокращать, рубить, вырубать, свернуть, сражать, губить, ушивать одежду"],
    ["break in", "[ˈbreɪk ɪn]", "прерывать, врываться, взламывать, вламываться, разнашивать, выламывать, объезжать"],
    ["break into", "[breɪk ˈɪntə]", "ворваться"],
    ["carry out", "[ˈkæri ˈaʊt]", "проводить, выполнять, вынести, уносить, доводить до конца, завершать, выносить"],
    ["make up", "[ˈmeɪk ʌp]", "составлять, составить, компенсировать, возместить, наверстывать, помириться"],
    ["make out", "[ˈmeɪk ˈaʊt]", "разглядеть, понять, разбирать, различить, выписывать, увидеть, составлять, уяснить"],
    ["put down to", "[pʊt daʊn tuː]", "приписать (причине)"],
    ["put forward", "[ˈpʊt ˈfɔːrwərd]", "выдвигать, выставлять, предлагать, продвигать, содействовать, передвигать вперед"],
    ["put out", "[ˈpʊt ˈaʊt]", "тушить, выпускать, гасить, загасить, конфузить кого-л."],
    ["put through", "[ˈpʊt θruː]", "соединять, выполнять, заканчивать"],
    ["give away", "[ˈɡɪv əˈweɪ]", "отдавать, раздавать, выдавать, дарить, раздаривать, поддавать, жертвовать"],
    ["cut down on", "[kʌt daʊn ɒn]", "сократить (потребление)"],
    ["stand up", "[ˈstænd ʌp]", "вставать, оказываться прочным, подводить кого-л."],
    ["annoy", "[əˌnɔɪ]", "раздражать, досаждать, надоедать, докучать, наскучивать"],
    ["competitor", "[kəmˈpetətər]", "конкурент, соперник"],
    ["corporation", "[ˌkɔːrpəˈreɪʃən]", "корпорация, акционерное общество, большой живот"],
    ["objectively", "[abˈdʒektɪvli]", "объективно, беспристрастно, реально"],
    ["dress down", "[ˈdres ˈdaʊn]", "отругать, задать головомойку, задать трепку"],
    ["calm down", "[kɑːm daʊn]", "успокоиться"],
    ["cool down", "[ˌkuːl ˈdaʊn]", "успокаивать, утихомиривать, охлаждать, остужать"],
    ["check in", "[ˈtʃek ɪn]", "регистрировать, регистрироваться, проверять, записывать, записываться"],
    ["check out", "[ˈtʃek ˈaʊt]", "проверять, отстроиться, сверять, выехать из гостиницы, одергивать"],
    ["dress up", "[ˈdres ʌp]", "наряжать, наряжаться, приодеть, приодеться, принарядиться, изысканно одеваться"],
    ["put down", "[ˈpʊt ˈdaʊn]", "подавлять, опустить, записывать, класть, унижать, опускать, заносить, осадить"],
    ["back up", "[bæk ˈʌp]", "поддерживать, давать задний ход"],
    ["deal with", "[diːl wɪð]", "иметь дело с"],
    ["take over", "[ˈteɪk ˈoʊvr̩]", "вступать во владение, перенимать, принимать должность"],
    ["put aside", "[ˈpʊt əˈsaɪd]", "отложить, откладывать в сторону, стараться не замечать, копить"],
    ["put away", "[ˈpʊt əˈweɪ]", "убирать, оставлять, откладывать, прятать, отказываться, помещать, съедать, убивать"],
    ["give back", "[ˈɡɪv ˈbæk]", "отдавать, возвращать, отплатить"],
    ["move forward", "[muːv ˈfɔːwəd]", "двигаться вперёд"],
    ["call for", "[kɔːl fɔː]", "требовать / призывать"],
    ["close down", "[kloʊz ˈdaʊn]", "закрывать, прекращать работу, задраивать, подавлять, прикрываться"],
    ["come along", "[ˈkəm əˈlɔːŋ]", "идти, сопровождать, соглашаться"],
    ["break out", "[ˈbreɪk ˈaʊt]", "разразиться, вспыхивать, выламывать, бежать, грянуть, появляться, убегать"],
    ["bring down", "[ˈbrɪŋ ˈdaʊn]", "сбивать, снижать, подстрелить, свезти, ломать, дискредитировать, сваливать"],
    ["bring about", "[ˈbrɪŋ əˈbaʊt]", "вызывать, осуществлять, быть причиной"],
    ["take back", "[ˌteɪk ˈbæk]", "возвращать, брать обратно, отрекаться, отказываться"],
    ["brighten up", "[ˈbraɪtən ʌp]", "проясниться / оживиться"],
    ["come on", "[ˈkəm ɑːn]", "появляться, наступать, возникать, натыкаться, расти, наскакивать, приближаться"],
    ["do up", "[də ʌp]", "чинить, завертывать, крайне утомлять, прибирать, приводить в порядок"],
    ["go back", "[ˈɡoʊ ˈbæk]", "возвращаться, восходить, отказываться, ухудшаться, нарушать, изменять"],
    ["come through", "[ˈkəm θruː]", "проникать, перенести, проходить внутрь, проходить насквозь"],
    ["give off", "[ˈɡɪv ˈɒf]", "выделять, испускать"],
    ["eat out", "[ˈiːt ˈaʊt]", "питаться вне дома, питаться, столоваться вне дома, пробирать, выговаривать"],
    ["bring forward", "[ˈbrɪŋ ˈfɔːrwərd]", "выдвигать"],
    ["go against", "[ɡəʊ əˈɡenst]", "идти против"],
    ["move on", "[ˈmuːv ɑːn]", "идти дальше, проходить дальше, продвинуть"],
    ["take in", "[ˈteɪk ɪn]", "принимать, брать, занимать, ушивать, убирать, смотреть, осматривать, побывать"],
    ["open up", "[ˈoʊpən ʌp]", "раскрывать, раскрываться, обнаруживать, обнаруживаться, делать доступным"],
    ["stand up for", "[stænd ʌp fɔː]", "заступаться"],
    ["come to", "[ˈkəm tuː]", "доходить до, приходить в себя, стоить, очнуться, равняться, наконец-то поумнеть"],
    ["go under", "[ˈɡoʊ ˈʌndr̩]", "гибнуть, разоряться, тонуть, умирать, исчезать, закатываться, погибать"],
    ["give out", "[ˈɡɪv ˈaʊt]", "выделять, раздавать, испускать, распределять, кончаться, провозглашать, иссякать"],
    ["stay up", "[ˈsteɪ ʌp]", "не ложиться спать"],
    ["shut down", "[ˈʃət ˈdaʊn]", "закрывать, останавливать, останавливаться, прекращать работу, захлопывать"],
    ["come up against", "[kʌm ʌp əˈɡenst]", "столкнуться с"],
    ["end up", "[end ʌp]", "оказаться в итоге"],
    ["start up", "[ˈstɑːrt ʌp]", "пускать, появляться, пускать в ход, вскакивать, вскочить, возникать, бросаться"],
    ["go ahead", "[ɡəʊ əˈhed]", "действовать / продолжать"],
    ["give in", "[ˈɡɪv ɪn]", "уступать, сдаваться, признавать, подавать, вписывать, вручать, регистрировать"],
    ["see through", "[ˈsiː θruː]", "видеть насквозь, доглядеть, помогать кому-л. в чем-л."],
    ["go after", "[ɡəʊ ˈɑːftə]", "преследовать"],
    ["bring back", "[ˈbrɪŋ ˈbæk]", "возвращать, напоминать, приносить обратно, воскрешать в памяти"],
    ["come out", "[ˈkəm ˈaʊt]", "выходить, выступить, вылезать, появляться, выскочить, проявляться, выезжать"],
    ["end in", "[end ɪn]", "закончиться (чем-то)"],
    ["blow up", "[ˈbloʊ ʌp]", "взорвать, взрывать, взрываться, раздувать, взлететь на воздух, пыжиться"],
    ["get through", "[ˈɡet θruː]", "проходить через, пролезать, справляться, выживать, выдержать экзамен, кончать"],
    ["brighten", "[ˈbraɪtən]", "просиять, проясняться, улучшать, улучшаться, полировать, очищать, придавать блеск"],
    ["committee", "[kəˈmɪti]", "комитет, комиссия, опекун"],
    ["height", "[ˈhaɪt]", "высота, рост, верх, вышина, возвышенность, высь, холм, высшая степень"],
    ["leaflet", "[ˈliːflət]", "листовка, листок, листочек, листик, прокламация, тонкая брошюра, молодой лист"],
    ["profound", "[proˈfaʊnd]", "глубокий, основательный, полный, глубина, бездна, пучина, море, океан"],
    ["pullover", "[ˈpʊloʊvər]", "пуловер, свитер"],
    ["shoelaces", "[ˈʃuːleɪsɪz]", "шнурки"],
    ["tough", "[ˈtəf]", "жесткий, трудный, жестко, грубо, хулиган, бандит, выдерживать"],
    ["absorbed", "[əbˈzɔːrbd]", "поглощенный, абсорбированный, захваченный"],
    ["abstain", "[əbˈsteɪn]", "воздерживаться, не принимать участия"],
    ["astonish", "[əˈstɑːnɪʃ]", "удивлять, изумлять"],
    ["boast", "[boʊst]", "похвастать, хвастать, хвастаться, хвастовство, предмет гордости"],
    ["compatible", "[kəmˈpætəbəl]", "совместимый, сочетаемый"],
    ["content", "[ˈkɑːntent]", "содержание, содержимое, довольствоваться, довольный, согласный"],
    ["engrossed", "[ɪnˈɡroʊst]", "поглощенный, увлеченный, беспокойный, абсорбированный"],
    ["flour", "[ˈflaʊər]", "мука, порошок, пудра, мучной, молоть, размалывать"],
    ["inclined", "[ˌɪnˈklaɪnd]", "наклонный, склонный, расположенный, предрасположенный"],
    ["perseverance", "[ˌpərsəˈvɪrəns]", "настойчивость, упорство, стойкость, усидчивость"],
    ["punctuality", "[ˌpʌŋktʃuːˈæləti]", "пунктуальность, точность, аккуратность"],
    ["renew", "[rəˈnuː]", "возобновлять, возобновить, обновлять, продлить срок действия, восстанавливать"],
    ["shipping", "[ˈʃɪpɪŋ]", "перевозка груза, судоходство, перевозка, погрузка, торговые суда, торговый флот"],
    ["subscription", "[səbˈskrɪpʃən]", "подписка, абонемент, подписание, подпись, подписной"],
    ["tied", "[ˈtaɪd]", "связанный, завязанный"],
    ["underpay", "[ˈʌndərˌpe]", "недостаточно платить"],
    ["upcoming", "[ˈʌpˌkəmɪŋ]", "развивающийся, идущий вперед, подающий надежды"],
    ["violence", "[ˈvaɪələns]", "насилие, жестокость, расправа, сила, неистовство, стремительность"],
    ["advertisement", "[ədˈvɝːtəzmənt]", "реклама, объявление, анонс, извещение, известие"],
    ["anybody", "[ˈenibədi]", "никто, кто-нибудь, любой, кто-либо, всякий, важное лицо, значительное лицо"],
    ["assistance", "[əˈsɪstəns]", "помощь, содействие, вспомоществование"],
    ["contribution", "[ˌkɑːntrəˈbjuːʃən]", "вклад, взнос, содействие, пожертвование, сотрудничество, статья, налог, контрибуция"],
    ["cooperation", "[koʊˌɒpəˈreɪʃən]", "сотрудничество, взаимодействие, кооперация, кооперирование"],
    ["cuisine", "[ˌkwɪˈziːn]", "кухня, стол, кулинарное искусство"],
    ["envy", "[ˈenvi]", "зависть, предмет зависти, завидовать"],
    ["fruitful", "[ˈfruːtfəl]", "плодотворный, плодовитый, плодородный, плодоносный, выгодный"],
    ["jealousy", "[ˈdʒeləsi]", "ревность, зависть, подозрительность, ревнивость"],
    ["meaningless", "[ˈmiːnɪŋləs]", "бессмысленный"],
    ["obsessed", "[əbˈsest]", "преследовать, мучить, овладеть, завладеть умом, обуять"],
    ["occupied", "[ˈɑːkjəˌpaɪd]", "оккупированный, занятый"],
    ["prosperity", "[praˈsperəti]", "процветание, благосостояние, достаток, зажиточность, преуспевание, успех"],
    ["reform", "[rəˈfɔːrm]", "реформа, преобразование, исправление, реформировать, исправлять, исправляться"],
    ["relationship", "[riˈleɪʃənˌʃɪp]", "отношение, связь, взаимоотношение, родство, сношение"],
    ["resistance", "[rəˈzɪstəns]", "сопротивление, сопротивляемость, противодействие, резистор"],
    ["substantial", "[səbˈstæntʃəl]", "существенный, значительный, важный, реальный, прочный, сытный, вещественный, крепкий"],
    ["sympathy", "[ˈsɪmpəθi]", "сочувствие, симпатия, сострадание, общность, взаимное понимание"],
    ["undervalue", "[ˈʌndərˌvæljuː]", "недооценивать"],
    ["unjustly", "[ʌnˈdʒʌstli]", "несправедливо"],
    ["wave", "[ˈweɪv]", "волна, сигнал, вал, колебание, волновой, махать, махнуть"],
    ["wit", "[ˈwɪt]", "остроумие, ум, остряк, разум, знать, ведать"],
    ["affair", "[əˈfer]", "дело, роман, связь, любовная история, занятия, дуэль"],
    ["antibiotic", "[ˌæntibiˈɑːtɪk]", "антибиотик, антибиотический"],
    ["chaos", "[ˈkeɪas]", "хаос, беспорядок, полный беспорядок"],
    ["compensate", "[ˈkɑːmpənˌset]", "компенсировать, возместить, возмещать, вознаграждать, уравновешивать"],
    ["earnings", "[ˈɝːnɪŋz]", "доход, прибыль, заработок, выручка, заработанные деньги"],
    ["housing", "[ˈhaʊzɪŋ]", "жилье, корпус, жилищное строительство, жилищный"],
    ["interfere", "[ˌɪntərˈfɪr]", "вмешиваться, мешать, мешаться, интерферировать, сталкиваться"],
    ["luxury", "[ˈlʌɡʒəri]", "роскошь, люкс, предмет роскоши, наслаждение, излишество, большое удовольствие"],
    ["miraculous", "[məˈrækjələs]", "чудотворный, чудодейственный, удивительный, сверхъестественный"],
    ["observe", "[əbˈzɝːv]", "соблюдать, наблюдать, заметить, соблюсти, следить за, блюсти, замечать, усмотреть"],
    ["population", "[ˌpɑːpjəˈleɪʃən]", "население, народонаселение, жители, заселение"],
    ["production", "[prəˈdəkʃən]", "производство, продукция, добыча, производственный"],
    ["stare", "[ˈster]", "пялиться, глазеть, поглазеть, пристальный взгляд, изумленный взгляд"],
    ["superior", "[suːˈpɪriər]", "превосходный, высший, лучший, начальник, старший, настоятель"],
    ["temper", "[ˈtempər]", "характер, нрав, настроение, самообладание, смягчать, закалять, закаляться, умерять"],
    ["unemployed", "[ˌʌnemˌplɔɪd]", "безработный, неработающий, незанятый, безработные"],
    ["unemployment", "[ˌʌnəmˌplɔɪmənt]", "безработица"],
    ["unprecedentedly", "[ʌnˈpresɪdentɪdli]", "беспрецедентно"],
    ["apparent", "[əˈperənt]", "очевидный, видимый, кажущийся, явный, несомненный, бесспорный"],
    ["demonstrate", "[ˈdemənˌstret]", "демонстрировать, проявлять, доказывать, наглядно показывать"],
    ["documentary", "[ˌdɑːkjəˈmentəri]", "документальный, документальный фильм"],
    ["honesty", "[ˈɑːnəsti]", "честность, правдивость, лунник"],
    ["inconvenience", "[ˌɪnkənˈviːnjəns]", "неудобство, беспокойство, беспокоить, побеспокоить"],
    ["loneliness", "[ˈloʊnlinəs]", "одиночество"],
    ["movement", "[ˈmuːvmənt]", "движение, перемещение, передвижение, механизм, ход, изменение, динамика, переселение"],
    ["restrict", "[riˈstrɪkt]", "ограничивать, рестрикт"],
    ["terrified", "[ˈterəˌfaɪd]", "запуганный"],
    ["towards", "[təˈwɔːrdz]", "к, в направлении, по отношению к, по направлению к, навстречу, для, около"],
    ["viewer", "[ˈvjuːər]", "зритель, окуляр стереоскопа, осмотрщик"],
    ["benefit", "[ˈbenəˌfɪt]", "пособие, выгода, польза, прибыль, бенефис, приносить пользу, извлекать, помогать"],
    ["benefit", "[ˈbenəˌfɪt]", "пособие, выгода, польза, прибыль, бенефис, приносить пользу, извлекать, помогать"],
    ["beware", "[bəˈwer]", "остерегаться, беречься"],
    ["ceaseless", "[ˈsiːsləs]", "непрерывный, непрестанный"],
    ["confidential", "[ˌkɑːnfəˈdenʃəl]", "конфиденциальный, секретный, доверительный, доверяющий, пользующийся доверием"],
    ["creativity", "[ˌkriːeɪˈtɪvəti]", "креативность, творческие способности"],
    ["depth", "[ˈdepθ]", "глубина, высота, толщина, глубь, интенсивность, разгар, сила, густота, пучина, середина"],
    ["erudition", "[ˌerəˈdɪʃən]", "эрудиция, ученость, образованность, начитанность"],
    ["fragrance", "[ˈfreɪɡrəns]", "аромат, благоухание"],
    ["garlic", "[ˈɡɑːrlɪk]", "чеснок"],
    ["hasty", "[ˈheɪsti]", "поспешный, торопливый, скоропалительный, быстрый, опрометчивый, стремительный"],
    ["impressed", "[ɪmˈprest]", "впечатлён"],
    ["incredible", "[ˌɪnˈkredəbəl]", "невероятный, потрясающий, неимоверный, неслыханный, неправдоподобный, потрясный"],
    ["panoramic", "[ˌpænəˈræmɪk]", "панорамный"],
    ["perfume", "[pərˈfjuːm]", "духи, аромат, благоухание, запах, душить, подушить, делать благоуханным"],
    ["protect", "[prəˈtekt]", "защищать, охранять, беречь, оберегать, предохранять, ограждать"],
    ["raspberry", "[ˈræzˌberi]", "малина, пренебрежительное фырканье"],
    ["slippery", "[sˈlɪpəri]", "скользкий, увертливый, беспринципный, ненадежный"],
    ["strength", "[ˈstreŋkθ]", "сила, прочность, численность, крепость, сопротивление, численный состав"],
    ["truly", "[ˈtruːli]", "действительно, поистине, истинно, искренне, верно, правдиво, точно, в самом деле"],
    ["bin", "[ˈbɪn]", "мусорное ведро, бункер, ящик, ларь, закром, хранить в закромах, хранить"],
    ["bravery", "[ˈbreɪvəri]", "храбрость, мужество"],
    ["determination", "[dəˌtərməˈneɪʃən]", "определение, решимость, решение, решительность, установление, постановление"],
    ["detrimental", "[ˌdetrɪˈmentəl]", "вредный, убыточный, приносящий убыток, незавидная партия"],
    ["devoted", "[dɪˈvoʊtɪd]", "преданный, посвященный, увлекающийся, нежный"],
    ["genuinely", "[ˈdʒenjəwənli]", "искренне, неподдельно"],
    ["habit", "[ˈhæbət]", "привычка, обыкновение, обычай, габитус, облачать, одевать"],
    ["noticeable", "[ˈnoʊtɪsəbəl]", "заметный, приметный, достойный внимания"],
    ["rubbish", "[ˈrʌbɪʃ]", "мусор, хлам, чушь, вздор, ерунда, белиберда, превращать в обломки, ломать"],
    ["somewhat", "[ˈsʌˈmwət]", "отчасти, до некоторой степени, что-то, кое-что, некоторая часть"],
    ["well-being", "[wel ˈbiːɪŋ]", "здоровье, благосостояние, благополучие, процветание"],
    ["action", "[ˈækʃən]", "действие, деятельность, акция, боевой"],
    ["adjacent", "[əˈdʒeɪsənt]", "соседний, смежный, примыкающий"],
    ["argument", "[ˈɑːrɡjəmənt]", "аргумент, довод, спор, параметр, аргументация, доказательство, дискуссия"],
    ["assumption", "[əˈsəmpʃən]", "предположение, допущение, вступление, присвоение, успение, принятие на себя"],
    ["based", "[beɪst]", "основанный"],
    ["considerable", "[kənˈsɪdərəbəl]", "значительный, большой, немалый, важный, много, множество"],
    ["deprive", "[dəˈpraɪv]", "лишать, отнимать, обделять, отрешать от должности, отбирать бенефиций"],
    ["hail", "[ˈheɪl]", "град, оклик, приветствие, приветствовать, окликать"],
    ["liberty", "[ˈlɪbər̩ti]", "свобода, вольность, вольности, воля, бесцеремонность, привилегии"],
    ["persist", "[pərˈsɪst]", "сохраняться, упорствовать, оставаться, продолжать существовать, настоять"],
    ["prayer", "[ˈprer]", "молитва, молебен, мольба, просьба, молящийся, богомолье, проситель"],
    ["privilege", "[ˈprɪvlədʒ]", "привилегия, привилегии, честь, давать привилегию, освобождать"],
    ["rumour", "[ˌruːmər]", "слух, молва, толки, распространять слухи"],
    ["solely", "[ˈsoʊli]", "исключительно, только, единственно"],
    ["unproven", "[ʌnˈpruːvən]", "недоказанный"],
    ["worthy", "[ˈwɝːði]", "достойный, заслуживающий, достойный человек, герой, особа"],
    ["abundance", "[əˈbʌndəns]", "изобилие, множество, избыток, богатство, распространенность, благодать"],
    ["associated", "[əˈsoʊʃieɪtɪd]", "связанный, соединенный, объединенный, взаимодействующий, действующий совместно"],
    ["brand", "[ˈbrænd]", "тавро, сорт, клеймо, качество, головня, клеймить, позорить, выжигать клеймо"],
    ["contribute", "[kənˈtrɪbjuːt]", "способствовать, содействовать, сотрудничать, жертвовать, делать вклад, отдавать"],
    ["covered", "[ˈkʌvərd]", "крытый, покрытый, закрытый, прикрытый, охваченный, укрытый, защищенный, в шляпе"],
    ["equipped", "[ɪˈkwɪpt]", "оборудованный, оснащенный"],
    ["fund", "[ˈfənd]", "фонд, капитал, запас, резерв, финансировать, консолидировать"],
    ["initial", "[ˌɪˈnɪʃəl]", "начальный, первоначальный, инициал, начальная буква, парафировать"],
    ["parking", "[ˈpɑːrkɪŋ]", "стоянка, автостоянка, автомобильная стоянка"],
    ["photography", "[fəˈtɑːɡrəfi]", "фотография, фотосъемка, фотографирование"],
    ["prognosis", "[prɒgˈnoʊsɪs]", "прогноз"],
    ["projector", "[prəˈdʒektər]", "проектор, прожектор, проектировщик, проекционный фонарь, прожектер, гранатомет"],
    ["reluctance", "[rəˈləktəns]", "нежелание, неохота, магнитное сопротивление, отвращение, нерасположение"],
    ["tight", "[ˈtaɪt]", "плотный, тугой, трудный, плотно, туго, крепко, тесно, трудное положение"],
    ["unforeseen", "[ˌʌnfɔːrˈsiːn]", "непредвиденный, не предусмотренный"],
    ["utterly", "[ˈʌtərli]", "крайне, абсолютно, чрезвычайно, дотла, вконец"],
    ["safety", "[ˈseɪfti]", "безопасность, сохранность, надежность, предохранительный"],
    ["adaptation", "[ˌædəpˈteɪʃən]", "адаптация, приспособление, применение, переделка, обработка, привыкание"],
    ["amazement", "[əˈmeɪzmənt]", "изумление, удивление"],
    ["analysis", "[əˈnæləsəs]", "анализ, исследование, разбор, психоанализ, разложение, проба"],
    ["attach", "[əˈtætʃ]", "придавать, прикреплять, присоединять, привязывать, прикладывать, связывать"],
    ["bet", "[ˈbet]", "ставка, пари, предмет, человек, держать пари, биться об заклад"],
    ["bureaucracy", "[bjʊˈrɑːkrəsi]", "бюрократия, бюрократизм"],
    ["cease", "[ˈsiːs]", "прекращение, прекращать, прекращаться, переставать"],
    ["compassion", "[kəmˈpæʃən]", "сострадание, сочувствие, жалость, сочувствовать"],
    ["concerned", "[kənˈsɝːnd]", "заинтересованный, обеспокоенный, озабоченный, связанный, имеющий отношение"],
    ["corruption", "[kəˈrəpʃən]", "коррупция, развращение, искажение, разложение, порча, испорченность, продажность"],
    ["disobedience", "[ˌdɪsəˈbiːdiəns]", "неповиновение, непослушание"],
    ["distinguish", "[ˌdɪˈstɪŋˌɡwɪʃ]", "различать, отличать, выделить, значительный"],
    ["eventually", "[ɪˈventʃəwəli]", "в конечном счете, в конце концов, со временем"],
    ["inequality", "[ˌɪnɪˈkwɑːləti]", "неравенство, разница, различие, неодинаковость, изменчивость, непостоянство"],
    ["marvel", "[ˈmɑːrvəl]", "чудо, диво, замечательная вещь, удивляться, восхищаться, восхищать"],
    ["object", "[ˈɑːbdʒekt]", "объект, предмет, цель, вещь, возражать, возразить, объектный, целевой"],
    ["objection", "[əbˈdʒekʃən]", "возражение, протест, недостаток, неодобрение, неприязнь, нелюбовь, дефект"],
    ["rapidity", "[rəˈpɪdəti]", "быстрота, скорость"],
    ["teaching", "[ˈtiːtʃɪŋ]", "обучение, учение, доктрина, выучка"],
    ["unsurprisingly", "[ˌʌnsəˈpraɪzɪŋli]", "неудивительно"],
    ["wisdom", "[ˈwɪzdəm]", "мудрость, премудрость, здравый смысл"],
    ["characteristic", "[ˌkerəktəˈrɪstɪk]", "характеристика, особенность, характерный, типичный"],
    ["dismissal", "[ˌdɪˈsmɪsəl]", "увольнение, отставка, освобождение, отстранение, роспуск, отклонение"],
    ["distinguishing", "[ˌdɪˈstɪŋɡwɪʃɪŋ]", "отличительный, характерный"],
    ["doubtful", "[ˈdaʊtfəl]", "сомнительный, сомневающийся, недостоверный, подозрительный, неопределенный"],
    ["prone", "[ˈproʊn]", "склонный, распростертый, лежащий ничком, ничком"],
    ["scandal", "[ˈskændəl]", "скандал, злословие, позор, позорный, сплетничать"],
    ["bribery", "[ˈbraɪbəri]", "взяточничество"],
    ["discourage", "[dɪsˈkɝːɪdʒ]", "отговаривать, обескураживать, расхолаживать, отбивать охоту, отсоветовать"],
    ["dressed", "[ˈdrest]", "одетый"],
    ["excitement", "[ɪkˈsaɪtmənt]", "волнение, возбуждение, азарт, экзальтация"],
    ["filled", "[ˈfɪld]", "заполненный, наполненный, фасованный"],
    ["intent", "[ˌɪnˈtent]", "намерение, цель, пристальный, внимательный"],
    ["involved", "[ˌɪnˈvɑːlvd]", "вовлеченный, сложный, замешанный, запутанный"],
    ["joy", "[ˌdʒɔɪ]", "радость, удовольствие, веселье, утеха, веселить, веселиться, радовать"],
    ["lady", "[ˈleɪdi]", "леди, дама, девушка, госпожа, барыня, жена, мать, невеста, дама сердца, хозяйка дома"],
    ["limit", "[ˈlɪmət]", "предел, лимит, граница, предельный размер, ограничивать, лимитировать"],
    ["partial", "[ˈpɑːrʃəl]", "частичный, частный, парциальный, неполный, частичный тон"],
    ["puzzled", "[ˈpʌzəld]", "недоуменный"],
    ["sphere", "[sˈfɪr]", "сфера, шар, круг, сфера деятельности, замыкать в круг, придавать форму шара"],
    ["tears", "[tɪəz]", "слёзы"],
    ["theft", "[ˈθeft]", "кража, воровство, покража, украденные вещи"],
    ["anger", "[ˈæŋɡər]", "гнев, злость, ярость, раздражение, злить, раздражать, сердить, вызывать гнев"],
    ["betray", "[bəˈtreɪ]", "предавать, изменять, выдавать, обманывать, подводить, соблазнять, не оправдывать"],
    ["cast", "[ˈkæst]", "литье, бросок, бросание, отливка, бросать, лить, отбрасывать, отливать"],
    ["consequent", "[ˈkɑːnsəkwənt]", "следствие, результат, последствие, последовательный"],
    ["court", "[ˈkɔːrt]", "суд, судья, корт, двор, площадка, ухаживать, накликать, добиваться"],
    ["flooded", "[ˈflʌdɪd]", "затопленный"],
    ["glance", "[ˈɡlæns]", "взгляд, беглый взгляд, быстрый взгляд, взглянуть, глянуть, мельком взглянуть"],
    ["incompetence", "[ˌɪnˈkɑːmpətəns]", "некомпетентность, недостаточность, неспособность, неправоспособность"],
    ["net", "[ˈnet]", "чистый, нетто, конечный, сеть, сети, сетка, сальдо, забивать, покрывать сетью"],
    ["request", "[rɪˈkwest]", "просьба, запрос, требование, заявка, спрос, запрашивать, затребовать, предлагать"],
    ["tremble", "[ˈtrembəl]", "дрожать, трепетать, трястись, опасаться, дрожь, дрожание"],
    ["uncontrollably", "[ˌʌnkənˈtrəʊləbli]", "неконтролируемо"],
    ["antique", "[ænˈtiːk]", "античный, антикварный, старинный, древний, антикварная вещь, антиква"],
    ["awareness", "[əˈwernəs]", "осведомленность, осознание"],
    ["budget", "[ˈbʌdʒət]", "бюджет, запас, финансовая смета, составлять бюджет, ассигновать"],
    ["campaign", "[kæmˈpeɪn]", "кампания, поход, кампания печи, агитировать, проводить кампанию"],
    ["cruelty", "[ˈkruːlti]", "жестокость, безжалостность, бессердечие, жестокосердие"],
    ["dedicate", "[ˈdedəˌket]", "посвящать, посветить, предназначать, надписывать, открывать"],
    ["detailed", "[dəˈteɪld]", "подробный, детальный, детализированный, обстоятельный, выделенный, назначенный"],
    ["furnished", "[ˈfɝːˌnɪʃt]", "меблированный"],
    ["imprisonment", "[ˌɪmˈprɪzənmənt]", "лишение свободы, тюремное заключение, заключение"],
    ["junk", "[ˈdʒəŋk]", "барахло, хлам, рухлядь, джонка, отбросы, выбрасывать как ненужное"],
    ["needy", "[ˈniːdi]", "нуждающийся, малообеспеченный, убогий, ночлежник, бродяга"],
    ["pension", "[ˈpenʃən]", "пенсия, пансионат, пансион, пособие, назначать пенсию, субсидировать"],
    ["raise", "[ˈreɪz]", "повышение, увеличение, подъем, поднятие, поднимать, подниматься, повышать"],
    ["reliant", "[rəˈlaɪənt]", "уверенный, самонадеянный, самоуверенный"],
    ["reluctant", "[rəˈləktənt]", "неохотный, вынужденный, сопротивляющийся, упорный, делающий с неохотой"],
    ["seriousness", "[ˈsɪriəsnəs]", "серьезность, важность"],
    ["serve", "[ˈsɝːv]", "подача, служить, подавать, обслуживать, работать"],
    ["thorough", "[ˈθɝːoʊ]", "тщательный, полный, подробный, глубокий, основательный, доскональный, совершенный"],
    ["volunteer", "[ˌvɑːlənˈtɪr]", "волонтер, доброволец, добровольный, предлагать, вызываться"],
    ["art", "[ˈɑːrt]", "искусство, мастерство, художественный, статья, искусственный"],
    ["defeat", "[dəˈfiːt]", "поражение, разгром, крушение, побеждать, наносить поражение"],
    ["dissatisfaction", "[ˌdɪsætəsˈfækʃən]", "недовольство, неудовлетворенность"],
    ["equivalent", "[ɪˈkwɪvələnt]", "эквивалентный, равносильный, эквивалент"],
    ["incident", "[ˈɪnsədənt]", "инцидент, происшествие, случай, эпизод, падающий, случайный, инцидентный"],
    ["opposed", "[əˈpoʊzd]", "противоположный, враждебный, противный, встречающий сопротивление"],
    ["prepared", "[priˈperd]", "подготовленный, готовый, приготовленный, очищенный"],
    ["thirst", "[ˈθɝːst]", "жажда, тяга, жаждать, хотеть пить"],
    ["upbringing", "[ˈʌpˌbrɪŋɪŋ]", "воспитание"],
    ["acquire", "[əˈkwaɪər]", "приобретать, получать, овладевать, обзаводиться, наживать, достигать, заводить"],
    ["condemn", "[kənˈdem]", "осуждать, обречь, порицать, приговаривать, уличать, браковать, конфисковать"],
    ["debate", "[dəˈbeɪt]", "дебаты, дискуссия, прения, спор, полемика, спорить, обсуждать, дискутировать"],
    ["eager", "[ˈiːɡər]", "стремящийся, нетерпеливый, энергичный, полный страстного желания, горячий"],
    ["guidance", "[ˈɡaɪdəns]", "руководство, рекомендация, управление, водительство, совет"],
    ["hint", "[ˈhɪnt]", "намек, совет, оттенок, налет, намекать"],
    ["innocence", "[ˈɪnəsəns]", "невиновность, невинность, наивность, чистота, простодушие, безвредность, простота"],
    ["necessity", "[nəˈsesəti]", "необходимость, нужда, неизбежность, надобность, настоятельная потребность"],
    ["newsletter", "[ˈnuːzˌletər]", "информационный бюллетень"],
    ["passion", "[ˈpæʃən]", "страсть, увлечение, азарт, страстность, чувствовать страсть, выражать страсть"],
    ["peace", "[ˈpiːs]", "мир, покой, спокойствие, тишина, мирный"],
    ["position", "[pəˈzɪʃən]", "позиция, положение, должность, место, помещать, ставить"],
    ["prudence", "[ˈpruːdəns]", "благоразумие, осторожность, осмотрительность, предусмотрительность"],
    ["qualified", "[ˈkwɑːləˌfaɪd]", "компетентный, высококвалифицированный, подходящий, ограниченный, пригодный"],
    ["subscribed", "[səbˈskraɪbd]", "подписан(ный)"],
    ["tone", "[ˈtoʊn]", "тон, тонус, оттенок, характер, интонация, настраивать, гармонировать, изменять"],
    ["vice versa", "[ˌvaɪs ˈvɜːsə]", "наоборот"],
    ["accompany", "[əˈkəmpəni]", "сопровождать, сопутствовать, аккомпанировать"],
    ["cartoon", "[karˈtuːn]", "карикатура, мультипликация, анимация, рисовать карикатуры"],
    ["champion", "[ˈtʃæmpiən]", "чемпион, победитель, отстаивать, защищать, первоклассный"],
    ["commission", "[kəˈmɪʃən]", "комиссия, комиссионные, поручать, уполномочивать"],
    ["disco", "[ˈdɪskoʊ]", "дискотека, посещать дискотеку"],
    ["dishwasher", "[ˈdɪˌʃwɑːʃər]", "посудомоечная машина, судомойка, посудомойка"],
    ["divorce", "[dɪˈvɔːrs]", "развод, расторжение брака, разводиться, бракоразводный"],
    ["error", "[ˈerər]", "ошибка, погрешность, заблуждение, отклонение, рассогласование, грех, блуждание"],
    ["traffic jam", "[ˈtræfɪk dʒæm]", "пробка"],
    ["photographer", "[fəˈtɑːɡrəfər]", "фотограф"],
    ["playground", "[ˈpleɪɡraʊnd]", "детская площадка, спортивная площадка, площадка для игр, спортплощадка"],
    ["precaution", "[priˈkɒʃən]", "предосторожность, предусмотрительность, предостережение"],
    ["stuck", "[ˈstək]", "застрявший, прилипший, приклеенный, увязший, приставший, прилепившийся, склеенный"],
    ["traffic", "[ˈtræfɪk]", "движение, перевозки, транспорт, торговля, торговать"],
    ["consideration", "[kənˌsɪdəˈreɪʃən]", "рассмотрение, внимание, соображение, обсуждение, возмещение, уважение, компенсация"],
    ["consult", "[kənˈsəlt]", "консультировать, консультироваться, советоваться, советовать, справляться"],
    ["eradicate", "[ɪˈrædəˌket]", "искоренять, уничтожать, вырывать с корнем"],
    ["fee", "[ˈfiː]", "плата, взнос, сбор, гонорар, вознаграждение, платить гонорар, нанимать"],
    ["hastily", "[ˈheɪstəli]", "поспешно, торопливо, наскоро, опрометчиво, необдуманно, запальчиво"],
    ["illiteracy", "[ˌɪˈlɪtərəsi]", "неграмотность, безграмотность"],
    ["minimize", "[ˈmɪnəˌmaɪz]", "минимизировать, преуменьшать, доводить до минимума"],
    ["refrigerator", "[rəˈfrɪdʒəˌretər]", "холодильник, рефрижератор, конденсатор"],
    ["spelling", "[ˈspelɪŋ]", "правописание, орфография, написание, произнесение слова по буквам"],
    ["timetable", "[ˈtaɪmˌteɪbəl]", "расписание, график, включать в расписание"],
    ["troops", "[truːps]", "войска"],
    ["washing machine", "[ˈwɒʃɪŋ məˈʃiːn]", "стиральная машина"],
    ["access", "[ˈækˌses]", "доступ, проход, подход, выборка, припадок, обращаться, иметь доступ"],
    ["beyond", "[bɪˈɑːnd]", "за, вне, после, выше, сверх, за пределами, вне, после, загробная жизнь"],
    ["generally", "[ˈdʒenr̩əli]", "вообще, обычно, как правило, в целом, в общем, в большинстве случаев"],
    ["irrelevant", "[ɪˈreləvənt]", "неуместный, не относящийся к делу"],
    ["reckon", "[ˈrekən]", "считать, думать, рассчитываться, рассчитывать, подсчитывать, полагаться, полагать"],
    ["regardless", "[rəˈɡɑːrdləs]", "не обращающий внимания, не обращая внимания ни на что"],
    ["self-education", "[ˌself ˌedʒʊˈkeɪʃən]", "самообразование, самообучение,"],
    ["coincidence", "[koʊˈɪnsɪdəns]", "совпадение, случайное стечение обстоятельств"],
    ["firstly", "[ˈfɝːstli]", "во-первых"],
    ["fluency", "[ˈfluːənsi]", "беглость, плавность"],
    ["overestimate", "[ˌoʊvəˈrestɪmeɪt]", "переоценивать, завышать смету, раздутая смета"],
    ["ridiculously", "[rɪˈdɪkjʊləsli]", "смешно, смехотворно, возмутительно, оскорбительно"],
    ["secondly", "[ˈsekəndli]", "во-вторых, кроме того, к тому же"],
    ["sincerely", "[ˌsɪnˈsɪrli]", "искренне"],
    ["crucial", "[ˈkruːʃəl]", "решающий, ключевой, критический, крестообразный"],
    ["illusion", "[ˌɪˈluːʒən]", "иллюзия, призрак, мираж, обман чувств, прозрачная кисея, тюль"],
    ["unsuccessful", "[ˌʌnsəkˈsesfəl]", "неудачный, безуспешный, неудачливый"],
    ["worthwhile", "[ˈwɝːˈθwaɪl]", "стоящий, путный, дельный"],
    ["definite", "[ˈdefənət]", "определенный, точный, ясный"],
    ["moreover", "[mɔːˈroʊvə]", "кроме того, более того, сверх того, вдобавок, больше того"],
    ["part-time", "[ˈpɑːrtˈtaɪm]", "неполный рабочий день, не полностью занятый, внештатный"],
    ["popularity", "[ˌpɑːpjəˈlerəti]", "популярность"],
    ["sensibly", "[ˈsensəbli]", "разумно, здраво"],
    ["admit", "[ədˈmɪt]", "допускать, признавать, принимать, впустить, впускать, соглашаться, позволять"],
    ["advertising", "[ˈædvərˌtaɪzɪŋ]", "реклама, рекламирование, рекламное дело, рекламный"],
    ["beneficial", "[ˌbenəˈfɪʃəl]", "выгодный, полезный, благотворный, благодатный, целительный"],
    ["criteria", "[kraɪˈtɪriə]", "критерий, мерило, признак"],
    ["definition", "[ˌdefəˈnɪʃən]", "определение, четкость, описание, резкость, точность, ясность"],
    ["desirable", "[dəˈzaɪrəbəl]", "желательный, желанный, подходящий, хороший, соблазнительный"],
    ["desire", "[dəˈzaɪər]", "желание, пожелание, страсть, вожделение, желать, хотеть, просить, требовать"],
    ["entertainment", "[ˌentərˈteɪnmənt]", "развлечение, развлечения, угощение, увеселение, увеселения, прием, гостеприимство"],
    ["invention", "[ˌɪnˈvenʃən]", "изобретение, выдумка, изобретательность, изобретательство, измышление, инвенция"],
    ["investment", "[ˌɪnˈvestmənt]", "инвестиция, инвестирование, капиталовложение, вложение, облечение, вклад, осада"],
    ["license", "[ˈlaɪsəns]", "лицензия, разрешение, патент, разрешать, выдавать лицензию"],
    ["lover", "[ˈlʌvr̩]", "любовник, любитель, возлюбленный, поклонник, друг, приверженец, доброжелатель"],
    ["monotonous", "[məˈnɑːtənəs]", "монотонный, однообразный, однозвучный, скучный"],
    ["perceive", "[pərˈsiːv]", "воспринимать, чувствовать, ощущать, понимать, осознавать, постигать, усмотреть"],
    ["radically", "[ˈrædɪkəli]", "радикально"],
    ["relate", "[rəˈleɪt]", "относиться, связывать, рассказывать, член отношения"],
    ["barrier", "[ˈbæriər]", "барьер, препятствие, шлагбаум, преграда, ограждать, заграждать"],
    ["distract", "[ˌdɪˈstrækt]", "отвлекать, развлекать, рассеивать, смущать, расстраивать, сбивать с толку"],
    ["ecologist", "[ɪˈkɑːlədʒəst]", "эколог"],
    ["entirely", "[enˈtaɪərli]", "полностью, совершенно, целиком, исключительно, всецело, сплошь, в целом, нацело"],
    ["global", "[ˈɡloʊbəl]", "глобальный, мировой, всемирный, общий, всеобщий"],
    ["nutrition", "[nuːˈtrɪʃən]", "питание, пища, корм"],
    ["overweight", "[ˌoʊvəˈweɪt]", "избыточный вес, перевес, весящий больше нормы, перегружать, обременять"],
    ["pose", "[poʊz]", "поза, позировать, излагать, формулировать"],
    ["psychology", "[saɪˈkɑːlədʒi]", "психология"],
    ["self-development", "[ˌself dɪˈveləpmənt]", "саморазвитие, спонтанное развитие"],
    ["stunning", "[ˈstʌnɪŋ]", "ошеломляющий, великолепный, сногсшибательный, оглушающий, ошеломительный"],
    ["swimmer", "[ˈswɪmər]", "пловец, поплавок"],
    ["technological", "[ˌteknəˈlɑːdʒɪkəl]", "технологический"],
    ["theoretically", "[ˌθiəˈretɪkəli]", "гипотетически"],
    ["widespread", "[ˈwaɪdˈspred]", "широко распространенный"],
    ["basically", "[ˈbeɪsɪkəli]", "в основном, по существу, в своей основе, в основе"],
    ["controversial", "[ˌkɑːntrəˈvɝːʃəl]", "спорный, дискуссионный, любящий полемику"],
    ["deceptive", "[dəˈseptɪv]", "обманчивый, вводящий в заблуждение"],
    ["disability", "[ˌdɪsəˈbɪləti]", "инвалидность, нетрудоспособность, неспособность, бессилие, неправоспособность"],
    ["drop", "[ˈdrɑːp]", "падение, снижение, капля, понижение, падать, ронять, уронить, опускаться"],
    ["excuse", "[ɪkˈskjuːs]", "оправдание, предлог, извинение, оправдывать, служить оправданием"],
    ["fascinating", "[ˈfæsəˌnetɪŋ]", "очаровательный, пленительный, обворожительный"],
    ["probability", "[ˌprɑːbəˈbɪləti]", "вероятность, правдоподобие"],
    ["tool", "[ˈtuːl]", "инструмент, орудие, резец, станок, обтесывать, обрабатывать резцом"],
    ["yeah", "[ˈjæ]", "да, да"],
    ["background", "[ˈbækˌɡrɑːwnd]", "фон, задний план, предпосылка, происхождение, образование, подоплека, подготовка"],
    ["collocation", "[ˌkɒləˈkeɪʃən]", "словосочетание, расположение, расстановка"],
    ["concerning", "[kənˈsɝːnɪŋ]", "относительно, касательно, относительно, касательно, касающийся"],
    ["contraction", "[kənˈtrækʃən]", "сокращение, сжатие, сужение, стягивание, усадка, контракция, уменьшение, контрактура"],
    ["digest", "[daɪˈdʒest]", "дайджест, сборник, резюме, справочник, переваривать, перевариваться, усваивать"],
    ["enjoyable", "[ɪnˈdʒɔɪəbəl]", "приятный, доставляющий удовольствие"],
    ["exaggeration", "[ɪgˌzædʒəˈreɪʃən]", "преувеличение, гипербола"],
    ["idiom", "[ˈɪdiəm]", "идиома, язык, идиоматическое выражение, идиоматизм, диалект, говор"],
    ["load", "[loʊd]", "нагрузка, груз, заряд, бремя, тяжесть, грузить, грузиться, нагружать, заряжать"],
    ["mass media", "[mæs ˈmiːdiə]", "средства массовой информации"],
    ["nationality", "[ˌnæʃəˈnæləti]", "национальность, гражданство, национальная принадлежность, подданство, нация"],
    ["needless", "[ˈniːdləs]", "ненужный, излишний, бесполезный"],
    ["non-native", "[nɒn ˈneɪtɪv]", "не носитель языка"],
    ["phrasal verb", "[ˈfreɪzəl vɜːb]", "фразовый глагол"],
    ["precise", "[prəˈsaɪs]", "точный, четкий, определенный, тщательный, уточнять"],
    ["precisely", "[prəˈsaɪsli]", "именно, точно, в точности, совершенно верно"],
    ["priority", "[praˈjɔːrəti]", "приоритет, очередность, порядок очередности, порядок срочности, старшинство"],
    ["shopaholic", "[ˌʃɒpəˈhɒlɪk]", "шопоголик"],
    ["undeniable", "[ˌʌndəˈnaɪəbəl]", "неоспоримый, несомненный, явный"],
    ["adapted", "[əˈdæptəd]", "адаптированный, пригодный, подогнанный"],
    ["agenda", "[əˈdʒendə]", "повестка дня"],
    ["ahead", "[əˈhed]", "вперед, впереди, напролом, предстоящий"],
    ["clarify", "[ˈklerəˌfaɪ]", "прояснить, прояснять, вносить ясность, очищать, просветлять, делать ясным"],
    ["comparison", "[kəmˈperəsən]", "сравнение"],
    ["ideally", "[aɪˈdiːli]", "идеально, превосходно, умозрительно, в воображении"],
    ["justly", "[ˈdʒʌstli]", "справедливо, законно"],
    ["miracle", "[ˈmɪrəkəl]", "чудо, удивительная вещь, миракль, выдающееся событие"],
    ["problematic", "[ˌprɑːbləˈmætɪk]", "проблематичный, сомнительный, гадательный"],
    ["programming", "[ˈproʊɡræmɪŋ]", "программирование, программирующий"],
    ["punishment", "[ˈpʌnɪʃmənt]", "наказание, кара, взыскание, суровое обращение, грубое обращение"],
    ["simplified", "[ˈsɪmpləˌfaɪd]", "упрощенный"],
    ["tendency", "[ˈtendənsi]", "тенденция, склонность, стремление, тенденциозный"],
    ["understandable", "[ˌʌndərˈstændəbəl]", "понятный"],
    ["weakness", "[ˈwiːknəs]", "слабость, недостаток, слабое место, бессилие, вялость, пристрастие, склонность"],
    ["adjective", "[ˈædʒɪktɪv]", "прилагательное, имя прилагательное, несамостоятельный, зависимый"],
    ["adverb", "[ˈædvərb]", "наречие"],
    ["bluntly", "[ˈblʌntli]", "прямо, резко, напрямик"],
    ["cloud", "[ˈklaʊd]", "облако, туча, пятно, множество, покров, тьма, омрачать, омрачаться, мутить"],
    ["commonly", "[ˈkɑːmənli]", "обычно, обыкновенно, дешево, плохо"],
    ["competitive", "[kəmˈpetətɪv]", "конкурентный, конкурентоспособный, конкурсный, конкурирующий, соперничающий"],
    ["consequently", "[ˈkɑːnsəkwəntli]", "следовательно, поэтому, в результате"],
    ["contrast", "[ˈkɑːntræst]", "контраст, контрастность, контрастировать, противопоставлять"],
    ["disagreement", "[ˌdɪsəˈɡriːmənt]", "разногласие, расхождение во мнениях, ссора, разлад"],
    ["employer", "[emˌplɔɪər]", "работодатель, наниматель, предприниматель"],
    ["evidence", "[ˈevədəns]", "доказательство, свидетельство, данные, свидетельствовать, доказывать"],
    ["furthermore", "[ˈfɝːðərˌmɔːr]", "более того, кроме того, к тому же, больше того"],
    ["influential", "[ˌɪnfluːˈentʃəl]", "влиятельный, важный"],
    ["kindergarten", "[ˈkɪndərˌɡɑːrtən]", "детский сад, детсад, детсадовский"],
    ["limited", "[ˈlɪmətəd]", "ограниченный, ограничивающийся, отграниченный"],
    ["linking words", "[ˈlɪŋkɪŋ wɜːdz]", "слова-связки"],
    ["lion", "[ˈlaɪən]", "лев, знаменитость"],
    ["mildly", "[ˈmaɪldli]", "мягко"],
    ["minus", "[ˈmaɪnəs]", "минус, недостаток, минус, без, отрицательный, лишенный"],
    ["mouth", "[ˈmaʊθ]", "рот, уста, отверстие, горлышко, вход, зев, изрекать, брать в рот"],
    ["noun", "[ˈnaʊn]", "существительное, имя существительное"],
    ["in a nutshell", "[ɪn ə ˈnʌtʃel]", "в двух словах"],
    ["plus", "[ˈpləs]", "плюс, преимущество, плюс, положительный, добавочный"],
    ["space", "[ˈspeɪs]", "пространство, место, космический, оставлять промежутки"],
    ["stuff", "[ˈstəf]", "материал, вещи, вещество, хлам, дрянь, набивать, засовывать, фаршировать"],
    ["tip", "[ˈtɪp]", "чаевые, наконечник, кончик, совет, конец, наклонять, наклоняться, опрокидывать"],
    ["unlucky", "[ʌnˈləki]", "неудачный, несчастливый"],
    ["unnoticed", "[ˌʌnˈnoʊtɪst]", "незамеченный"],
    ["wisely", "[ˈwaɪzli]", "мудро"],
    ["debt", "[ˈdet]", "долг"],
    ["embarrassing", "[emˈberəsɪŋ]", "смущающий, затруднительный"],
    ["enrol", "[ɪnˈroʊl]", "регистрировать, записываться, вербовать, поступать на военную службу"],
    ["mortgage", "[ˈmɔːrɡɪdʒ]", "ипотека, заклад, закладная, заем, закладывать, ручаться"],
    ["optimist", "[ˈɑːptəˌmɪst]", "оптимист"],
    ["prove", "[ˈpruːv]", "доказывать, подтверждать, зарекомендовать, утверждать, пробовать, оказываться"],
    ["revise", "[rɪˈvaɪz]", "пересматривать, изменять, исправлять, проверка, сверка, вторая корректура"],
    ["visible", "[ˈvɪzəbəl]", "видимый, зримый, очевидный, явный, обозримый"],
    ["bracelet", "[ˈbreɪslət]", "браслет, наручники"],
    ["certificate", "[sərˈtɪfɪkət]", "сертификат, свидетельство, сертифицировать, удостоверять"],
    ["correction", "[kəˈrekʃən]", "коррекция, исправление, поправка, правка, наказание, выговор"],
    ["disgusting", "[ˌdɪˈsɡəstɪŋ]", "отвратительный, противный, отвратный, вызывающий отвращение"],
    ["full-time", "[fʊl ˈtaɪm]", "полная рабочая неделя, рабочий"],
    ["helicopter", "[ˈhelɪˌkɑːptər]", "вертолет, геликоптер, перевозить на вертолете"],
    ["mattress", "[ˈmætrəs]", "матрац, тюфяк, фашинный тюфяк"],
    ["personnel", "[ˌpərsəˈnel]", "персонал, кадры, личный состав"],
    ["proper", "[ˈprɑːpər]", "надлежащий, правильный, собственный, должный, подходящий, настоящий, истинный"],
    ["review", "[ˌriˈvjuː]", "обзор, рассмотрение, пересмотр, просмотр, пересмотреть, рассматривать"],
    ["souvenir", "[ˌsuːvəˈnɪr]", "сувенир, подарок на память"],
    ["sympathetic", "[ˌsɪmpəˈθetɪk]", "симпатический, симпатичный, симпатический нерв"],
    ["tremendous", "[trəˈmendəs]", "огромный, потрясающий, громадный, страшный, ужасный, потрясный"],
    ["waterfall", "[ˈwɒtərˌfɒl]", "водопад"],
    ["accessory", "[ækˈsesəri]", "аксессуар, принадлежность, вспомогательный, добавочный"],
    ["amount", "[əˈmaʊnt]", "сумма, количество, общая сумма, итог, составлять, достигать, составлять сумму"],
    ["appliance", "[əˈplaɪəns]", "прибор, устройство, приспособление, применение, электрический бытовой прибор"],
    ["blogging", "[ˈblɒɡɪŋ]", "ведение блога"],
    ["bound to", "[baʊnd tuː]", "обязательно (сделает)"],
    ["compensation", "[ˌkɑːmpənˈseɪʃən]", "компенсация, возмещение, вознаграждение, уравнивание, денежное вознаграждение"],
    ["fairly", "[ˈferli]", "довольно, достаточно, справедливо, честно, совершенно, беспристрастно, явно"],
    ["harsh", "[ˈhɑːrʃ]", "суровый, жесткий, резкий, грубый, строгий, терпкий, неприятный, шероховатый"],
    ["knit", "[ˈnɪt]", "вязать, сращивать, сращиваться, скреплять, трикотаж, вязка, трикотажное изделие"],
    ["likely", "[ˈlaɪkli]", "вероятный, возможный, подходящий, вероятно"],
    ["qualification", "[ˌkwɑːləfəˈkeɪʃən]", "квалификация, ценз, оговорка, определение, уточнение, ограничение, характеристика"],
    ["revolutionary", "[ˌrevəˈluːʃəˌneri]", "революционный, вращающийся, революционер"],
    ["simultaneously", "[ˌsaɪməlˈteɪniəsli]", "одновременно, в то же время"],
    ["wealthy", "[ˈwelθi]", "богатый, состоятельный, обильный, богачи"],
    ["afterwards", "[ˈæftərwərdz]", "впоследствии, потом, позже"],
    ["investigate", "[ˌɪnˈvestəˌɡet]", "исследовать, расследовать, изучать, разузнавать, разведывать, наводить справки"],
    ["responsibility", "[riˌspɑːnsəˈbɪləti]", "ответственность, обязанность, обязанности, обязательство, вменяемость"],
    ["subscriber", "[səbˈskraɪbər]", "абонент, подписчик, жертвователь"],
    ["tense", "[ˈtens]", "напряженный, натянутый, время, натягивать, натягиваться"],
    ["title", "[ˈtaɪtəl]", "название, титул, звание, титульный, заглавный, называть"],
    ["unpredictable", "[ˌʌnprəˈdɪktəbəl]", "непредсказуемый"],
    ["stay", "[ˈsteɪ]", "пребывание, опора, остановка, оставаться, остановиться"],
    ["historic", "[ˌhɪˈstɔːrɪk]", "исторический, исторически установленный"],
    ["conscious", "[ˈkɑːnʃəs]", "сознательный, сознающий, ощущающий, сознание"],
    ["delivery", "[dəˈlɪvəri]", "доставка, поставка, передача, роды, подача, нагнетательный, питающий"],
    ["efficiency", "[əˈfɪʃənsi]", "эффективность, производительность, оперативность, продуктивность"],
    ["electric", "[əˈlektrɪk]", "электрический, электробытовой, электрик"],
    ["exhibition", "[ˌeksəˈbɪʃən]", "выставка, показ, демонстрация, проявление, выставочный"],
    ["marvellous", "[ˈmɑːrvləs]", "чудесный, изумительный, удивительный, чудесное, удивительное, непостижимое"],
    ["sheet", "[ˈʃiːt]", "лист, листок, слой, простыня, листовой, покрывать"],
    ["transfer", "[ˈtrænsfɝː]", "передача, перевод, трансфер, передавать, переводить, переводный"],
    ["moving", "[ˈmuːvɪŋ]", "движущийся, движущий, подвижной, двигающийся, трогательный, волнующий"],
    ["applaud", "[əˈplɒd]", "аплодировать, рукоплескать, одобрять"],
    ["atrocious", "[əˈtroʊʃəs]", "зверский, жестокий, ужасный, отвратительный, свирепый"],
    ["band", "[ˈbænd]", "полоса, диапазон, оркестр, ленточный, соединять, связывать"],
    ["basket", "[ˈbæskət]", "корзина, корзинка, наружные места, бросать в корзину"],
    ["bless", "[ˈbles]", "благословить, благословлять, освящать, осчастливливать, славословить, проклинать"],
    ["ceremony", "[ˈserəˌmoʊni]", "церемония, церемонность, обряд, формальность"],
    ["Christian", "[ˈkrɪstʃən]", "христианский, христианин, Кристиан, Христиан"],
    ["Christianity", "[ˌkrɪstʃiˈænəti]", "христианство"],
    ["cock", "[ˈkɑːk]", "петух, кран, хуй, курок, пенис, самец, елдак, хер, взводить курок, поднимать, взметнуть"],
    ["convert", "[ˈkɑːnvərt]", "конвертировать, новообращенный, новообращенный"],
    ["cow", "[ˈkaʊ]", "корова, самка кита, самка носорога, запугивать, усмирять, терроризировать"],
    ["dare", "[ˈder]", "вызов, подзадоривание, дерзать, сметь, посметь, рисковать"],
    ["dinosaur", "[ˈdaɪnəˌsɔːr]", "динозавр"],
    ["duck", "[ˈdək]", "утка, парусиновые брюки, утиное мясо, нырять, окунаться, окунать, увертываться"],
    ["farmer", "[ˈfɑːrmər]", "фермер, арендатор, откупщик"],
    ["fishing", "[ˈfɪʃɪŋ]", "рыбалка, рыбная ловля, тоня, рыбные места, рыболовный, рыбацкий, рыболовецкий"],
    ["goose", "[ˈɡuːs]", "гусь, гусыня, гусятина, простушка, дурак, освистывать"],
    ["goat", "[ɡoʊt]", "коза, козел, осел, козел отпущения, дурень, остолоп, мелкий скот"],
    ["guide", "[ˈɡaɪd]", "руководство, путеводитель, гид, проводник, направлять, вести, руководить"],
    ["hen", "[ˈhen]", "курица, курочка, куропатка, женщина, тетерка, домашняя птица"],
    ["humiliate", "[hjuːˈmɪliˌet]", "унижать"],
    ["idolize", "[ˈaɪdəˌlaɪz]", "боготворить, поклоняться, поклоняться идолам, делать кумиром"],
    ["initially", "[ˌɪˈnɪʃəli]", "первоначально, сначала, вначале, с самого начала, в начальной стадии"],
    ["insult", "[ˌɪnˈsəlt]", "оскорбление, обида, выпад, инсульт, оскорблять, обижать, оплевать"],
    ["item", "[ˈaɪtəm]", "пункт, вопрос, статья, номер, равным образом, также, тоже, записывать по пунктам"],
    ["kilometre", "[kɪˈlɒmɪtər]", "километр"],
    ["madness", "[ˈmædnəs]", "безумие, сумасшествие, безумство, бешенство"],
    ["miraculously", "[məˈrækjələsli]", "чудесным образом"],
    ["optimize", "[ˈɑːptəˌmaɪz]", "оптимизировать, быть оптимистом"],
    ["percentage", "[pərˈsentɪdʒ]", "процент, доля, часть, процентный"],
    ["reflect", "[rəˈflekt]", "отражать, отражаться, размышлять, отображать, раздумывать, изображать"],
    ["Rugby", "[ˈrʌɡbi]", "регби"],
    ["sheep", "[ˈʃiːp]", "овца, баран, овчина, паства, овечья шкура, робкий человек, застенчивый человек"],
    ["truck", "[ˈtrək]", "грузовик, грузовой автомобиль, тележка, платить натурой, обменивать"],
    ["whatever", "[hwʌtˈevr̩]", "любой, какой бы ни, что бы ни, какой-либо, все равно"],
    ["adopt", "[əˈdɑːpt]", "принимать, перенимать, усыновлять, усвоить, заимствовать, усваивать, удочерять"],
    ["arrange", "[əˈreɪndʒ]", "договориться, устраивать, аранжировать, располагать, договариваться, выстраивать"],
    ["besides", "[bəˈsaɪdz]", "кроме, помимо, кроме того, притом, кроме, помимо"],
    ["bookcase", "[ˈbʊkˌkes]", "книжный шкаф, книжная полка, этажерка"],
    ["brain", "[ˈbreɪn]", "мозг, мозги, головной мозг, ум, голова, размозжить голову"],
    ["civil", "[ˈsɪvəl]", "гражданский, государственный, штатский, вежливый, воспитанный"],
    ["confess", "[kənˈfes]", "исповедовать, исповедоваться, признавать, признаваться, покаяться, сознаваться"],
    ["convey", "[kənˈveɪ]", "передавать, транспортировать, выражать, сообщать, перевозить, переправлять"],
    ["explore", "[ɪkˈsplɔːr]", "исследовать, изучать, обследовать, разведывать, выяснять, зондировать"],
    ["finance", "[fəˈnæns]", "финансы, финансовое дело, доходы, финансировать, управлять финансами"],
    ["goal", "[ɡoʊl]", "цель, ворота, гол, задача, финиш, мета, место назначения"],
    ["granddaughter", "[ˈɡrænˌdɒtər]", "внучка"],
    ["grandson", "[ˈɡrændˌsən]", "внук"],
    ["inclusive", "[ˌɪnˈkluːsɪv]", "включительно, содержащий, включающий"],
    ["legitimate", "[ləˈdʒɪtəmət]", "законный, легальный, законнорожденный, узаконивать, признавать законным"],
    ["male", "[ˈmeɪl]", "мужской, охватываемый, мужчина, самец"],
    ["minimum", "[ˈmɪnəməm]", "минимальный, минимум, минимальное количество"],
    ["mommy", "[ˈmɑːmi]", "мама, мамочка"],
    ["paste", "[ˈpeɪst]", "паста, пастила, клей, паштет, масса, клейстер, вставлять, наклеивать, оклеивать"],
    ["pink", "[ˈpɪŋk]", "розовый, либеральничающий, пинка, розовый цвет, розоветь, прокалывать"],
    ["prescribe", "[prəˈskraɪb]", "предписывать, прописывать, прописывать лекарство"],
    ["ruble", "[ˈruːbəl]", "рубль, рублевый"],
    ["sand", "[ˈsænd]", "песок, гравий, песчаный, песочный, посыпать песком"],
    ["sore", "[ˈsɔːr]", "больной, рана, болячка, язва, тяжко, жестоко, причинять боль"],
    ["spread", "[ˈspred]", "распространение, размах, распространять, распространенный"],
    ["structure", "[ˈstrʌktʃər]", "структура, конструкция, строение, структурировать"],
    ["target", "[ˈtɑːrɡət]", "цель, мишень, задание, сигнал, плановый, нацеливать, планировать"],
    ["throat", "[θroʊt]", "горло, глотка, гортань, горловина, зев, напевать хриплым голосом, бормотать"],
    ["tear", "[ˈtɪr]", "слеза, разрыв, надрыв, прореха, раздирание, рваться, рвать, слезиться, разрываться"],
    ["wanna", "[ˈwɒnə]", "хотеть (разг.)"],
    ["beetroot", "[ˈbiːtruːt]", "свекла, бурак, свекловица"],
    ["bend", "[ˈbend]", "изгиб, излучина, колено, сгиб, отвод, узел, сгибаться, сгибать, изгибаться, изгибать"],
    ["carrot", "[ˈkærət]", "морковь, приманка, стимул, рыжеволосый человек"],
    ["connect", "[kəˈnekt]", "соединять, соединяться, связать, связывать, связываться, сочетать, сочетаться"],
    ["crew", "[ˈkruː]", "экипаж, команда, бригада, компания, артель, кричать кукареку"],
    ["declare", "[dɪˈkler]", "объявлять, заявлять, провозглашать, признавать, описывать, высказываться, выражать"],
    ["decorate", "[ˈdekəˌret]", "украшать, декорировать, отделывать, награждать, расцвечивать, убирать"],
    ["dialogue", "[ˈdaɪəˌlɒɡ]", "диалог, разговор, обмен мнениями, диалоговый"],
    ["encourage", "[enˈkɜːrɪdʒ]", "поощрять, поддерживать, ободрять, воодушевлять, подстрекать, потворствовать"],
    ["eraser", "[ɪˈreɪsər]", "ластик, резинка, резинка для стирания"],
    ["fraud", "[ˈfrɒd]", "мошенничество, обман, мошенник, подделка, афера, жульничество, обманщик"],
    ["involve", "[ˌɪnˈvɑːlv]", "включать, вовлекать, включать в себя, предполагать, подразумевать, затрагивать"],
    ["location", "[loʊˈkeɪʃən]", "местонахождение, положение, нахождение, поселение, обнаружение, дислокация"],
    ["mix", "[ˈmɪks]", "смешивать, смешиваться, перемешать, смесь, смешивание, смешение, путаница"],
    ["moderately", "[ˈmɑːdərətli]", "умеренно"],
    ["moderation", "[ˌmɑːdəˈreɪʃən]", "умеренность, замедление, сдержанность, сдерживание, выдержка, регулирование"],
    ["onion", "[ˈʌnjən]", "лук, луковица, репчатый лук, голова, приправлять луком"],
    ["overtake", "[ˌoʊvəˈteɪk]", "обгон, догонять, нагнать, настигать, поравняться"],
    ["panic", "[ˈpænɪk]", "паника, могар, забава, шутка, панический, пугать, наводить панику"],
    ["permit", "[pərˈmɪt]", "разрешение, пропуск, путевка, позволять, разрешать, допускать"],
    ["prosecute", "[ˈprɑːsəˌkjuːt]", "преследовать в судебном порядке, преследовать по суду, вести, обвинять"],
    ["rob", "[ˈrɑːb]", "грабить, обкрадывать, разбойничать, грабитель"],
    ["rub", "[ˈrəb]", "тереть, тереться, втирать, натирать, трение, растирание, натирание, стирание"],
    ["select", "[səˈlekt]", "выбирать, отбирать, подбирать, избранный, отборный, разборчивый"],
    ["shoot", "[ˈʃuːt]", "стрелять, снять, застрелить, расстрелять, побег, охота, стрельба, бросок, росток, рывок"],
    ["Bible", "[ˈbaɪbəl]", "библия, экземпляр библии"],
    ["compete", "[kəmˈpiːt]", "конкурировать, соревноваться, состязаться"],
    ["cucumber", "[ˈkjuːkʌmbər]", "огурец"],
    ["diligence", "[ˈdɪlədʒəns]", "усердие, старание, прилежание, старательность, дилижанс"],
    ["dozen", "[ˈdʌzən]", "дюжина, масса"],
    ["erase", "[ɪˈreɪs]", "стирать, подчищать, вычеркивать, соскабливать, убивать, выскабливать"],
    ["expand", "[ɪkˈspænd]", "расширяться, расширять, развивать, развиваться, распространять, распространяться"],
    ["extend", "[ɪkˈstend]", "продлить, распространять, расширять, простирать, простираться, продлевать"],
    ["fulfil", "[fʊlˈfɪl]", "выполнять, исполнять, осуществлять, удовлетворять, завершать"],
    ["modesty", "[ˈmɑːdəsti]", "скромность, умеренность, сдержанность, застенчивость, благопристойность"],
    ["neck", "[ˈnek]", "шея, шейка, горлышко, горло, горловина, обниматься"],
    ["obtain", "[əbˈteɪn]", "получать, приобретать, добиваться, достигать, добывать, применяться, существовать"],
    ["peninsula", "[pəˈnɪnsələ]", "полуостров"],
    ["postcard", "[ˈpoʊstkɑːd]", "открытка, почтовая карточка"],
    ["profoundly", "[proˈfaʊndli]", "глубоко"],
    ["rapidly", "[ˈræpədli]", "быстро"],
    ["readiness", "[ˈredinəs]", "готовность, подготовленность, находчивость, живость, быстрота, охота"],
    ["relevant", "[ˈreləvənt]", "уместный, релевантный"],
    ["remove", "[riˈmuːv]", "удаление, шаг, ступень, перевод, колено, удалять, снимать, убирать, устранять"],
    ["resell", "[riˈsel]", "перепродавать"],
    ["righteous", "[ˈraɪtʃəs]", "праведный, справедливый, добродетельный"],
    ["scarf", "[ˈskɑːrf]", "шарф, кашне, галстук, скос, делать выемки, сращивать"],
    ["surround", "[səˈraʊnd]", "окружать, окружить, обступать, осаждать, обстраивать"],
    ["tomato", "[təˈmeɪˌtoʊ]", "помидор, томат, персик"],
    ["united", "[juˈnaɪtəd]", "объединенный, соединенный, дружный, совместный"],
    ["wheel", "[ˈhwiːəl]", "колесо, колесико, круг, велосипед, штурвал, катить, поворачивать, поворачиваться"],
    ["worship", "[ˈwɝːʃəp]", "поклоняться, почитать, боготворить, поклонение, богослужение, культ"],
    ["idealize", "[aɪˈdiːˌlaɪz]", "идеализировать"],
    ["bench", "[ˈbentʃ]", "скамейка, скамья, верстак, станок, судьи, суд, демонстрировать на выставке"],
    ["emphasize", "[ˈemfəˌsaɪz]", "подчеркивать, акцентировать, выделять, придавать особое значение, напирать"],
    ["fry", "[ˈfraɪ]", "мальки, жаркое, мелюзга, мелкая рыбешка, жарить, жариться, обжаривать"],
    ["including", "[ˌɪnˈkluːdɪŋ]", "включая, в том числе, вместе, включающий, заключающий"],
    ["inefficiency", "[ˌɪnəˈfɪʃənsi]", "неэффективность, неспособность, неумелость, недейственность, безрезультатность"],
    ["liquid", "[ˈlɪkwəd]", "жидкость, плавный звук, жидкий, ликвидный, водяной, водянистый"],
    ["merry", "[ˈmeri]", "веселый, радостный, развеселый, навеселе, черешня"],
    ["nation", "[ˈneɪʃən]", "нация, народ, страна, государство, народность, землячество"],
    ["platform", "[ˈplætˌfɔːrm]", "платформа, площадка, помост, перрон, планировать, помещать на платформу"],
    ["seventh", "[ˈsevənθ]", "седьмой, седьмой, септима, седьмая часть"],
    ["significance", "[sɪgˈnɪfɪkəns]", "значение, значимость, важность, смысл, значительность, многозначительность"],
    ["soap", "[soʊp]", "мыло, мыльная опера, лесть, деньги, мылить, намыливать, мыть мылом"],
    ["unkind", "[ʌnˈkaɪnd]", "недобрый, злой, суровый"],
    ["unluckily", "[ʌnˈlʌkɪli]", "к несчастью"],
    ["virtually", "[ˈvɝːtʃuːəli]", "фактически, в сущности, поистине"],
    ["wage", "[ˈweɪdʒ]", "вести, бороться, заработная плата"],
    ["fire", "[ˈfaɪər]", "огонь, пожар, камин, пламя, обстрел, стрельба, стрелять, выстрелить, увольнять"],
    ["agent", "[ˈeɪdʒənt]", "агент, средство, вещество, представитель, действующий"],
    ["conclude", "[kənˈkluːd]", "заключать, выводить, заканчивать, заканчиваться, делать вывод, решать"],
    ["hill", "[ˈhɪl]", "холм, гора, возвышенность, возвышение, окучивать, насыпать кучу"],
    ["madam", "[ˈmædəm]", "мадам, госпожа, сударыня, барыня, бандерша, женщина, любящая повелевать"],
    ["media", "[ˈmiːdiə]", "средства массовой информации, звонкий согласный"],
    ["muscles", "[ˈmʌsəlz]", "мышцы"],
    ["nineteenth", "[ˈnaɪnˈtiːnθ]", "девятнадцатый, девятнадцатый, девятнадцатое число"],
    ["notify", "[ˈnoʊtɪfaɪ]", "уведомлять, извещать, предупреждать, регистрировать, объявлять"],
    ["occur", "[əˈkɝː]", "происходить, встречаться, иметь место, случаться, являться, попадаться, бывать"],
    ["patriot", "[ˈpeɪtriət]", "патриот"],
    ["presidential", "[ˌprezəˈdentʃəl]", "президентский"],
    ["pull", "[ˈpʊl]", "тянуть, вытащить, потянуть, дергать, напряжение, тяга, усилие, натяжение"],
    ["regard", "[rəˈɡɑːrd]", "считать, считаться, рассматривать, отношение, внимание, взгляд, уважение"],
    ["reporter", "[rəˈpɔːrtər]", "репортер, корреспондент, докладчик"],
    ["resist", "[rəˈzɪst]", "сопротивление, сопротивляться, противостоять"],
    ["status", "[ˈstætəs]", "статус, состояние, положение дел, общественное положение"],
    ["submit", "[səbˈmɪt]", "подавать, подчиняться, подчинять, покоряться, покорять, утверждать, предлагать"],
    ["telephone", "[ˈteləˌfoʊn]", "телефон, телефонный, телефонировать"],
    ["temptation", "[temˈteɪʃən]", "искушение, соблазн"],
    ["terminate", "[ˈtɝːməˌnet]", "завершать, завершаться, положить конец, ограничивать, кончать, кончаться"],
    ["terrific", "[təˈrɪfɪk]", "потрясающий, ужасающий, огромный, необычайный"],
    ["water", "[ˈwɒtər]", "вода, воды, море, водоем, поливать, напоить, поить, водный, водяной"],
    ["activity", "[ækˈtɪvəti]", "активность, деятельность, радиоактивность, энергия, транзакция"],
    ["authority", "[əˈθɔːrəti]", "власть, власти, полномочие, авторитет, основание, влияние, начальство"],
    ["broadcast", "[ˈbrɒdˌkæst]", "передача, радиовещание, вещать, распространять, радиовещательный"],
    ["chewing gum", "[ˈtʃuːɪŋ ɡʌm]", "жвачка"],
    ["curious", "[ˈkjʊriəs]", "любопытный, любознательный, странный, курьезный, пытливый"],
    ["diary", "[ˈdaɪəri]", "дневник, записная книжка, записная книжка-календарь"],
    ["exclude", "[ɪkˈskluːd]", "исключать, не допускать, устранять, не впускать, изымать, снимать, выгонять"],
    ["grammatical", "[ɡrəˈmætəkəl]", "грамматический, грамматичный, грамматически правильный"],
    ["honey", "[ˈhʌni]", "мед, милая, милый, голубушка, милая, милый, подлизываться, льстить"],
    ["identify", "[aɪˈdentəˌfaɪ]", "идентифицировать, отождествлять, отождествляться, распознавать, опознавать"],
    ["imitate", "[ˈɪməˌtet]", "подражать, имитировать, копировать, подделываться, подделывать, передразнивать"],
    ["inappropriate", "[ˌɪnəˈproʊprɪət]", "неуместный, несоответствующий, неподходящий"],
    ["mislead", "[ˌmɪsˈliːd]", "вводить в заблуждение, сбивать с толку, сбивать с пути"],
    ["modify", "[ˈmɑːdəˌfaɪ]", "модифицировать, видоизменять, определять, смягчать, умерять"],
    ["motherland", "[ˈmʌðr̩lænd]", "родина, отчизна"],
    ["Muslim", "[ˈməzləm]", "мусульманский, мусульманин, мусульманка"],
    ["oven", "[ˈʌvən]", "духовка, печь, духовой шкаф"],
    ["pan", "[ˈpæn]", "кастрюля, панорамирование, поддон, панорамировать, задать жару, промывать"],
    ["per", "[ˈpɝː]", "в, за, на, по, с, согласно, через, посредством"],
    ["pour", "[ˈpɔːr]", "полить, лить, литься, наливать, вливаться, ливень, литник"],
    ["prize", "[ˈpraɪz]", "премия, приз, рычаг, выигрыш, призовой, премированный"],
    ["propose", "[prəˈpoʊz]", "предлагать, выдвинуть, выдвигать, представлять, предполагать"],
    ["reader", "[ˈriːdr̩]", "читатель, считыватель, чтец, программа чтения, хрестоматия, корректор"],
    ["splitting", "[ˈsplɪtɪŋ]", "расщепление, раскалывание, расслаивание, раскольнический, оглушительный"],
    ["swear", "[ˈswer]", "поклясться, ругать, ругаться, клясться, присяга, клятва, ругательство"],
    ["wherever", "[weˈrevər]", "где, куда, где бы ни, куда бы ни"],
    ["yawn", "[ˈjɒn]", "зевать, позевывать, зиять, разверзаться, зевота, зевание, зазор, скучища"],
    ["aeroplane", "[ˈeərəpleɪn]", "самолет, аэроплан"],
    ["approvingly", "[əˈpruːvɪŋli]", "одобрительно"],
    ["backpack", "[ˈbækˌpæk]", "рюкзак, ходить в поход"],
    ["candle", "[ˈkændəl]", "свеча, свечка, газовая горелка, проверять свежесть яиц"],
    ["cheerfully", "[ˈtʃɪrfəli]", "весело, радужно, беспечально"],
    ["cooperate", "[koʊˈɒpəreɪt]", "сотрудничать, взаимодействовать, содействовать, кооперировать, кооперироваться"],
    ["correspondence", "[ˌkɔːrəˈspɑːndəns]", "переписка, соответствие, корреспонденция, письма, соотношение, аналогия"],
    ["fourth", "[ˈfɔːrθ]", "четвертый, четверть, четвертое число, четвертый"],
    ["hesitate", "[ˈhezəˌtet]", "стесняться, колебаться, запинаться, призадумываться, не решаться, запнуться"],
    ["imply", "[ˌɪmˈplaɪ]", "означать, подразумевать, предполагать, значить, заключать в себе"],
    ["justify", "[ˈdʒʌstəˌfaɪ]", "оправдывать, объяснять, подтверждать, выравнивать, выключить строку"],
    ["king", "[ˈkɪŋ]", "царь, король, монарх, властитель, дамка, править, повелевать, управлять"],
    ["kneel", "[ˈniːl]", "становиться на колени, стоять на коленях, преклонять колени, преклонять"],
    ["maintain", "[menˈteɪn]", "поддерживать, сохранять, содержать, обслуживать, удерживать, утверждать"],
    ["nod", "[ˈnɑːd]", "кивнуть, клевать носом, кивать головой, кивок, клевание носом, дремота"],
    ["rush", "[ˈrəʃ]", "торопить, торопиться, спешка, натиск, ажиотаж, срочный, спешный"],
    ["sacrifice", "[ˈsækrəˌfaɪs]", "жертва, жертвоприношение, заклание, жертвовать, приносить жертву"],
    ["sadden", "[ˈsædən]", "печалить, печалиться, опечалить, опечалиться"],
    ["scale", "[ˈskeɪl]", "масштаб, шкала, гамма, размер, уровень, взбираться, весить, взвешивать"],
    ["scare", "[ˈsker]", "пугать, отпугивать, попугать, вспугивать, паника, внезапный испуг"],
    ["tenth", "[ˈtenθ]", "десятый, десятый, десятая часть"],
    ["unnaturally", "[ʌnˈnætʃərəli]", "неестественно, противоестественно"],
    ["violate", "[ˈvaɪəlet]", "нарушать, попирать, насиловать, преступать, изнасиловать, осквернять, вторгаться"],
    ["means", "[ˈmiːnz]", "средство, способ, состояние, богатство"],
    ["rule", "[ˈruːl]", "правило, правление, власть, норма, править, управлять, господствовать"],
    ["clothing", "[ˈkloʊðɪŋ]", "одежда, платье, обмундирование, обшивка, паруса"],
    ["bone", "[ˈboʊn]", "кость, костяк, коклюшки, костный, костяной, снимать мясо с костей"],
    ["coast", "[koʊst]", "побережье, берег, береговой, плавать вдоль побережья"],
    ["congress", "[ˈkɑːŋɡrəs]", "конгресс, съезд"],
    ["convince", "[kənˈvɪns]", "убеждать, уверять, доводить до сознания"],
    ["cough", "[ˈkɑːf]", "кашель, кашлять, покашлять, прокашлять"],
    ["development", "[dɪˈveləpmənt]", "развитие, разработка, создание, строительство, рост, совершенствование, событие"],
    ["exclaim", "[ɪkˈskleɪm]", "воскликнуть, восклицать, ахать, вскричать"],
    ["federal", "[ˈfedərəl]", "федеральный, союзный, правительственный, федералист"],
    ["formula", "[ˈfɔːrmjələ]", "формула, формулировка, рецепт, детское питание, лозунг, доктрина"],
    ["gonna", "[ˈɡɒnə]", "собираться"],
    ["indicate", "[ˈɪndəˌket]", "указывать, показывать, означать, служить признаком, предписывать, требовать"],
    ["lean", "[ˈliːn]", "наклон, постное мясо, опираться, наклоняться, постный, худой, тощий"],
    ["omelette", "[ˈɒmlɪt]", "омлет, яичница"],
    ["promotion", "[prəˈmoʊʃən]", "продвижение, поощрение, содействие, стимулирование, продвижение по службе"],
    ["recognize", "[ˈrekəgˌnaɪz]", "признавать, распознавать, узнавать, осознавать, опознавать, выражать признание"],
    ["refugee", "[ˈrefjuːdʒi]", "беженец, эмигрант, беглец"],
    ["semester", "[səˈmestər]", "семестр"],
    ["signal", "[ˈsɪɡnəl]", "сигнал, знак, связь, сигнализировать, сигнальный, выдающийся"],
    ["transform", "[ˈtrænsfɔːrm]", "трансформировать, превращать, превращение"],
    ["vomit", "[ˈvɑːmət]", "рвота, блевотина, рвотное, рвотный, извергать"],
    ["wood", "[ˈwʊd]", "дерево, древесина, лес, дрова, деревянный, лесной, сажать лес"],
    ["talk show", "[ˈtɔːk ʃəʊ]", "ток-шоу"],
    ["agility", "[əˈdʒɪləti]", "ловкость, проворство, подвижность, быстрота, прыть, живость"],
    ["association", "[əˌsoʊʃiˈeɪʃən]", "ассоциация, объединение, связь, общество, общение, союз, соединение, близость, дружба"],
    ["berry", "[ˈberi]", "ягода, икринка, зерно, ягодный, собирать ягоды"],
    ["bomb", "[ˈbɑːm]", "бомба, мина, баллон, ручная граната, бомбить, сбрасывать бомбы"],
    ["camp", "[ˈkæmp]", "лагерь, стан, табор, привал, лагерный, располагаться лагерем"],
    ["category", "[ˈkætəˌɡɔːri]", "категория, разряд, класс"],
    ["clown", "[ˈklaʊn]", "клоун, шут, паяц, неотесанный парень, дурачиться, изображать из себя клоуна"],
    ["collection", "[kəˈlekʃən]", "коллекция, сбор, собрание, инкассо, коллекционный"],
    ["combine", "[ˈkɑːmbaɪn]", "комбинат, комбайн, объединение, синдикат, сочетать, сочетаться, совмещать"],
    ["communication", "[kəˌmjuːnəˈkeɪʃən]", "связь, коммуникация, общение, сообщение, передача, соединение, информация"],
    ["confiscate", "[ˈkɑːnfəˌsket]", "конфискованный, конфисковать, реквизировать"],
    ["defend", "[dəˈfend]", "защищать, защищаться, отстаивать, оборонять, обороняться, оберегать, поддерживать"],
    ["endurance", "[ˈendərəns]", "выносливость, прочность, стойкость, долговечность, продолжительность"],
    ["establish", "[əˈstæblɪʃ]", "устанавливать, создавать, учреждать, заложить, основывать, устраивать, заключать"],
    ["expire", "[ɪkˈspaɪr]", "истекать, терять силу, выдыхать, умирать, угасать, кончаться"],
    ["explode", "[ɪkˈsploʊd]", "взорваться, взрывать, взрываться, разбивать, разражаться, подрывать"],
    ["female", "[ˈfiːˌmel]", "женщина, самка, женская особь, матка, женский, женского пола, охватывающий"],
    ["flash", "[ˈflæʃ]", "вспышка, мгновение, блеск, проблеск, блеснуть, вспыхивать, сверкать, сверкнуть"],
    ["force", "[ˈfɔːrs]", "сила, действие, войска, насилие, заставлять, форсировать, принуждать"],
    ["gather", "[ˈɡæðər]", "собираться, собирать, почерпнуть, сбор, урожай, то, что собрано"],
    ["geography", "[dʒiˈɑːɡrəfi]", "география"],
    ["humiliating", "[hjuːˈmɪliˌetɪŋ]", "унизительный, оскорбительный"],
    ["insert", "[ˌɪnˈsɝːt]", "вставка, вкладыш, вклейка, втулка, вставлять, включать, помещать, вписывать"],
    ["locate", "[loʊˈkeɪt]", "находиться, размещать, обнаруживать, определять местонахождение, устанавливать"],
    ["mushroom", "[ˈmʌʃruːm]", "гриб, выскочка, грибной, грибовидный, собирать грибы"],
    ["nick", "[ˈnɪk]", "зарубка, зазубрина, щербина, стащить, украсть, подрезать, сокращенный"],
    ["obey", "[oˈbeɪ]", "подчиняться, повиноваться, слушаться, выполнять приказание"],
    ["okay", "[ˌoʊˈkeɪ]", "хорошо, одобрение, приемлемый, одобрять"],
    ["practise", "[ˈpræktɪs]", "практика, опыт, тренировка, практиковать, практический, учебный"],
    ["promote", "[prəˈmoʊt]", "способствовать, поощрять, продвигать, стимулировать, поддерживать, рекламировать"],
    ["railway station", "[ˈreɪlweɪ ˈsteɪʃən]", "вокзал"],
    ["resign", "[rəˈzaɪn]", "отставка, уходить в отставку, отказываться"],
    ["shake", "[ˈʃeɪk]", "дрожание, сотрясение, встряска, дрожь, трястись, трясти, поколебать, дрожать"],
    ["specifically", "[spəˈsɪfɪkəli]", "конкретно, особенно, а именно"],
    ["speechless", "[ˈspiːtʃləs]", "безмолвный, немой, онемевший, невыразимый, онемелый"],
    ["stretching", "[ˈstretʃɪŋ]", "растягивание, натяжной"],
    ["technique", "[tekˈniːk]", "техника, метод, способ, технические приемы"],
    ["terrorism", "[ˈterəˌrɪzəm]", "терроризм"],
    ["trial", "[ˈtraɪəl]", "суд, испытание, пробный, испытательный"],
    ["vast", "[ˈvæst]", "огромный, обширный, громадный, безбрежный, простор"],
    ["zebra", "[ˈziːbrə]", "зебра"],
    ["end", "[ˈend]", "конец, окончание, край, цель, часть, заканчивать, заканчиваться, кончать"],
    ["pick up", "[ˈpɪk ʌp]", "забрать, подобрать, подбирать, поднимать, подхватить, подцепить, поймать"],
    ["face up to", "[ˈfeɪs ʌp tuː]", "смело встретить (проблему)"],
    ["come up with", "[kʌm ʌp wɪð]", "придумать"],
    ["liven up", "[ˈlaɪvən ʌp]", "оживить(ся)"],
    ["pick up on", "[ˈpɪk ʌp ɒn]", "заметить, уловить"],
    ["pull in", "[ˈpʊl ɪn]", "зарабатывать, отъехать, втягивать, осаживать, загребать, арестовать, прибывать"],
    ["own up", "[ˈoʊn ʌp]", "откровенно признаваться, безропотно подчиняться, покорно соглашаться"],
    ["get back to", "[ˈɡet bæk tuː]", "вернуться к (кому-то / чему-то)"],
    ["dawn on", "[dɔːn ɒn]", "осенить (о мысли)"],
    ["cling to", "[klɪŋ tuː]", "цепляться за"],
    ["take to", "[teɪk tuː]", "привязаться / начать любить"],
    ["bank on", "[bæŋk ɒn]", "рассчитывать на"],
    ["lay off", "[ˈleɪ ˈɒf]", "увольнять, снять с работы, переставать, прекращать, отдыхать, снимать одежду"],
    ["feel up to", "[ˈfiːl ʌp tuː]", "быть в состоянии"],
    ["mull over", "[mʌl ˈəʊvə]", "обдумывать"],
    ["churn out", "[ˌtʃɜːn ˈaʊt]", "особенно машинным путем, штамповать, печь как блины"],
    ["accomplishment", "[əˈkɑːmplɪʃmənt]", "достижение, выполнение, завершение, образованность, воспитание"],
    ["anxiety", "[æŋˈzaɪəti]", "тревога, беспокойство, тревожность, страх, забота, боязнь, опасение"],
    ["deceit", "[dəˈsiːt]", "обман, лживость, хитрость, уловка, трюк"],
    ["drastic", "[ˈdræstɪk]", "радикальный, решительный, коренной, сильнодействующее лекарство"],
    ["gradually", "[ˈɡrædʒuːəli]", "постепенно, понемногу, мало-помалу, исподволь"],
    ["identical", "[aɪˈdentɪkəl]", "идентичный, одинаковый, тождественный, тот же самый"],
    ["instantly", "[ˈɪnstəntli]", "немедленно, тотчас"],
    ["nervousness", "[ˈnɝːvəsnəs]", "нервозность, нервность"],
    ["prolific", "[proˈlɪfɪk]", "плодовитый, плодородный, изобилующий"],
    ["undeniably", "[ˌʌndɪˈnaɪəbli]", "неоспоримо"],
    ["look up", "[ˈlʊk ʌp]", "искать, смотреть вверх, поднимать глаза, улучшаться, навещать, повышаться"],
    ["back down", "[ˈbæk ˈdaʊn]", "отступать, отступаться, отказываться"],
    ["live up to", "[ˈlaɪv ʌp tuː]", "жить согласно, быть достойным"],
    ["account for", "[əˈkaʊnt fɔː]", "объяснять / составлять"],
    ["brush up on", "[brʌʃ ʌp ɒn]", "освежить (знания)"],
    ["spark off", "[spɑːk ɒf]", "спровоцировать"],
    ["come down to", "[kʌm daʊn tuː]", "сводиться к"],
    ["boil down to", "[bɔɪl daʊn tuː]", "сводиться к"],
    ["carried away", "[ˈkærid əˈweɪ]", "увлечённый, захваченный"],
    ["occur to", "[əˈkɜː tuː]", "приходить в голову"],
    ["fall through", "[ˈfɑːl θruː]", "проваливаться, терпеть неудачу"],
    ["slog away", "[slɒɡ əˈweɪ]", "вкалывать"],
    ["gobble up", "[ˈɡɒbəl ʌp]", "пожирать"],
    ["turn down", "[ˈtɝːn ˈdaʊn]", "отказывать, отклонять, отвергать, убавлять, отгибать, загибать, подвертывать"],
    ["iron out", "[ˈaɪərn ˈaʊt]", "сглаживать, разглаживать, улаживать, разутюживать"],
    ["astonishment", "[əˈstɑːnɪʃmənt]", "удивление, изумление"],
    ["balanced", "[ˈbælənst]", "сбалансированный, уравновешенный, гармоничный, пропорциональный, выровненный"],
    ["biscuit", "[ˈbɪskət]", "печенье, галета, сухое печенье, фарфор, светло-коричневый"],
    ["disappearance", "[ˌdɪsəˈpɪrəns]", "исчезновение, пропажа"],
    ["fatty", "[ˈfæti]", "жирный, жировой, откормленный, толстушка, толстяк"],
    ["heated", "[ˈhiːtəd]", "горячий, подогретый, разгоряченный, пылкий, возбужденный"],
    ["inability", "[ˌɪnəˈbɪləti]", "неспособность, невозможность"],
    ["missing", "[ˈmɪsɪŋ]", "отсутствующий, недостающий, без вести пропавшие"],
    ["overheat", "[ˌoʊvəˈhiːt]", "перегрев, перегревать, перегреваться"],
    ["ringer", "[ˈrɪŋər]", "звонарь, точная копия, первоклассная вещь, мастер своего дела, тот, кто звонит"],
    ["spendings", "[ˈspendɪŋz]", "траты, расходы"],
    ["strike", "[ˈstraɪk]", "забастовка, удар, стачка, биться, бить, поражать, забастовочный, стачечный"],
    ["unlocked", "[ʌnˈlɒkt]", "разблокирован"],
    ["unsolved", "[ʌnˈsɑːlvd]", "нерешенный"],
    ["via", "[ˈvaɪə]", "через, через"],
    ["pour down", "[pɔː daʊn]", "лить (о дожде)"],
    ["plug into", "[plʌɡ ˈɪntə]", "подключать к"],
    ["plug in", "[ˈpləɡ ɪn]", "подключать, включать, вставлять штепсель"],
    ["write off", "[ˈraɪt ˈɒf]", "списывать со счета, сбрасывать со счетов, аннулировать, вычеркивать"],
    ["put across", "[ˈpʊt əˈkrɒs]", "провернуть, перевозить, переправлять, обманывать, надувать"],
    ["get across", "[ˈɡet əˈkrɒs]", "перебираться, переправляться, переходить, четко изложить"],
    ["bump into", "[bʌmp ˈɪntə]", "случайно встретить"],
    ["run across", "[rʌn əˈkrɒs]", "наткнуться"],
    ["clamp down", "[ˈklæmp ˈdaʊn]", "подавлять, прекращать, становиться требовательнее"],
    ["crack down", "[ˈkræk ˈdaʊn]", "сломить"],
    ["run through", "[ˈrən θruː]", "просматривать, промотать, прокалывать, зачеркивать, истощаться"],
    ["hold up", "[hoʊld ʌp]", "задерживать, поддерживать, показывать, выставлять, подпирать, останавливать"],
    ["work out", "[ˈwɝːk ˈaʊt]", "разработать, разрабатывать, вырабатывать, отрабатывать, решать, срабатывать"],
    ["dwell on", "[dwel ɒn]", "зацикливаться на"],
    ["doze off", "[ˌdəʊz ˈɒf]", "спать на ходу, задремать"],
    ["nod off", "[nɒd ɒf]", "задремать"],
    ["run up against", "[ˈrʌn ʌp əˌɡenst]", "столкнуться с (трудностью)"],
    ["battery", "[ˈbætəri]", "батарея, аккумулятор, батарейка, побои, батарейный"],
    ["vacuum cleaner", "[ˈvækjuːm kliːnə]", "пылесос"],
    ["evasion", "[ɪˈveɪʒən]", "уклонение, бегство, увертка, отговорка"],
    ["fierce", "[ˈfɪrs]", "жестокий, свирепый, сильный, лютый, неистовый, горячий, неприятный, болезненный"],
    ["fog", "[ˈfɑːɡ]", "туман, густой туман, вуаль, туманный, затуманивать"],
    ["former", "[ˈfɔːrmər]", "бывший, прежний, первый, предшествующий, шаблон, составитель, копир"],
    ["hairdryer", "[ˈheədraɪə]", "фен"],
    ["immigration", "[ˌɪməˈɡreɪʃən]", "иммиграция"],
    ["obstacle", "[ˈɑːbstəkəl]", "препятствие, помеха, препона"],
    ["protestor", "[prəˈtestə]", "протестующий"],
    ["riot", "[ˈraɪət]", "бунт, беспорядки, мятеж, буйство, восстание, бунтовать, буйствовать, бесчинствовать"],
    ["soak", "[soʊk]", "впитывать, впитываться, вымочить, замачивание, впитывание, пьяница"],
    ["socket", "[ˈsɑːkət]", "гнездо, розетка, муфта, патрон, патрубок, раструб, втулка, углубление, впадина"],
    ["tackle", "[ˈtækəl]", "снасти, принадлежности, тали, инструмент, схватить, биться, энергично браться"],
    ["toaster", "[ˈtoʊstə]", "тостер, тамада, провозглашающий тост"],
    ["fall apart", "[fɔːl əˈpɑːt]", "разваливаться"],
    ["hold back", "[hoʊld ˈbæk]", "сдерживать, сдерживаться, удерживать, воздерживаться, утаивать, мешкать"],
    ["hack into", "[hæk ˈɪntə]", "взломать"],
    ["speed up", "[ˈspiːd ʌp]", "ускорять, убыстрять, увеличивать"],
    ["stick to", "[stɪk tuː]", "придерживаться"],
    ["hold on", "[hoʊld ɑːn]", "держаться, упорствовать, вцепляться, крепить, продолжать делать"],
    ["polish up", "[ˈpɒlɪʃ ʌp]", "отшлифовать"],
    ["go over", "[ˈɡoʊ ˈoʊvr̩]", "переходить, повторять, преодолевать, превосходить, перечитывать, переменить веру"],
    ["settle down", "[ˈsetəl ˈdaʊn]", "успокоиться, поселиться, поселить, успокаиваться, успокаивать, устраиваться"],
    ["anticipation", "[ænˌtɪsəˈpeɪʃən]", "ожидание, предвкушение, предвидение, предвосхищение, предчувствие"],
    ["central", "[ˈsentrəl]", "центральный, главный, центровой, центральная телефонная станция"],
    ["database", "[ˈdeɪtəbeɪs]", "база данных, банк данных"],
    ["gang", "[ˈɡæŋ]", "банда, шайка, группа, артель, набор, компания, идти, организовать бригаду"],
    ["junction", "[ˈdʒʌŋkʃən]", "переход, соединение, стык, узел, перекресток, узловая станция, слияние"],
    ["preference", "[ˈprefərəns]", "предпочтение, преферанс, преференция, привилегированный"],
    ["registration", "[ˌredʒəˈstreɪʃən]", "регистрация, учет, запись, прописка, пристрелка"],
    ["sunshine", "[ˈsʌnˌʃaɪn]", "солнечный свет, радость, счастье, хорошая погода, веселье, источник радости"],
    ["unlimited", "[ʌnˈlɪmətəd]", "неограниченный, безграничный, беспредельный"],
    ["unprofitable", "[ʌnˈprɑːfətəbəl]", "невыгодный, нерентабельный, бесполезный, бесприбыльный, бездоходный"],
    ["willpower", "[ˈwɪlˌpɑːwər]", "сила воли"],
    ["look in", "[ˈlʊk ɪn]", "заглянуть, навещать, смотреть телепередачу"],
    ["deprive of", "[dɪˈpraɪv əv]", "лишить"],
    ["sell out", "[ˈsel ˈaʊt]", "продавать, продаваться, распродавать, предавать, стать предателем"],
    ["buy up", "[ˈbaɪ ʌp]", "скупать, раскупать, расхватывать, откупать"],
    ["wear out", "[ˈwer ˈaʊt]", "изнашивать, изнашиваться, износить, состарить, затрахать, истощаться, истощать"],
    ["use up", "[ˈjuːz ʌp]", "использовать, израсходовать, истратить, исписывать, истощать, вымыливать"],
    ["show off", "[ˈʃoʊ ˈɒf]", "выпендриваться, красоваться, рисоваться, выставляться, пускать пыль в глаза"],
    ["take on", "[ˈteɪk ɑːn]", "приобретать, мериться силами, брать на работу, иметь успех"],
    ["rip off", "[ˈrɪp ˈɒf]", "сдирать, грабить, выдирать, отпарывать, спарывать, вымогать"],
    ["measure up", "[ˈmeʒər ʌp]", "соответствовать, достигать, отвечать"],
    ["note down", "[nəʊt daʊn]", "записать"],
    ["keep off", "[ˈkiːp ˈɒf]", "держаться в отдалении, держать в отдалении, не подпускать"],
    ["die down", "[ˈdaɪ ˈdaʊn]", "утихать, отгреметь, замирать, угасать, увядать, исчезать, улечься"],
    ["applause", "[əˈplɒz]", "аплодисменты, рукоплескания, одобрение, восхищение"],
    ["breakthrough", "[ˈbreɪkˌθruː]", "прорыв, достижение, открытие, крупное достижение, пробой, просека"],
    ["daylight", "[ˈdeɪˌlaɪt]", "дневной свет, естественное освещение, рассвет, гляделки, глаза, гласность"],
    ["eagerly", "[ˈiːɡərli]", "горячо"],
    ["efficiently", "[əˈfɪʃəntli]", "продуктивно, дельно"],
    ["gossip", "[ˈɡɑːsəp]", "сплетня, сплетни, слухи, болтовня, сплетничать, болтать, беседовать"],
    ["grass", "[ˈɡræs]", "трава, травка, газон, злак, луг, засевать травой, пастись, подножный"],
    ["imprison", "[ˌɪmˈprɪzən]", "сажать в тюрьму, заключать в тюрьму, лишать свободы"],
    ["inspiring", "[ˌɪnˈspaɪrɪŋ]", "вдохновляющий, вдохновительный"],
    ["mask", "[ˈmæsk]", "маска, противогаз, личина, слепок, маскировать, скрывать, надевать маску"],
    ["persistent", "[pərˈsɪstənt]", "стойкий, постоянный, настойчивый, устойчивый, упорный, неотступный, неотвязный"],
    ["premiere", "[preˈmɪr]", "премьера, прима, первый, ведущий, давать премьеру"],
    ["pursuit", "[pərˈsuːt]", "преследование, погоня, стремление, поиски, занятие, гонка преследования"],
    ["reckless", "[ˈrekləs]", "безрассудный, безумный, бесшабашный, опрометчивый, отчаянный, безответственный"],
    ["rip-off", "[ˈrɪp ɒf]", "грабеж, мошенничество, плагиат"],
    ["sadly", "[ˈsædli]", "грустно, уныло"],
    ["trainee", "[ˈtreɪˈniː]", "стажер, практикант, проходящий обучение, проходящий подготовку"],
    ["carry out", "[ˈkæri ˈaʊt]", "проводить, выполнять, вынести, уносить, доводить до конца, завершать, выносить"],
    ["drop off", "[ˈdrɑːp ˈɒf]", "заснуть, довезти до, высаживать, завезти, подбросить, умереть, расходиться, умирать"],
    ["copy down", "[kɔːpi daʊn]", "переписать"],
    ["cut up", "[ˈkət ʌp]", "разрезать, раскритиковать, взрезать, подрывать, разрубать на куски, уничтожать"],
    ["clear up", "[ˈklɪr ʌp]", "проясняться, убирать, выяснять, прибирать, разгуливаться, просветлеть, распутывать"],
    ["drop in", "[ˈdrɑːp ɪn]", "зайти, заглянуть, забежать, зайти в гости, забрести, похаживать"],
    ["call in", "[ˈkɒl ɪn]", "вызывать, приглашать, изымать из обращения, наведаться, потребовать назад"],
    ["call on", "[ˈkɒl ɑːn]", "призывать, взывать, навещать, апеллировать, наведаться, звонить по телефону"],
    ["grow out of", "[ɡrəʊ aʊt əv]", "перерасти"],
    ["hand out", "[ˈhænd ˈaʊt]", "раздавать, выдавать, тратить деньги, помочь выйти, помочь сойти"],
    ["cut in", "[ˈkət ɪn]", "врубать, врезать, включать, вклиниваться между машинами"],
    ["bring off", "[ˈbrɪŋ ˈɒf]", "спасать, успешно завершать, успешно выполнять"],
    ["black out", "[ˈblæk ˈaʊt]", "вычеркивать, затемнять, вымарывать, не пропускать, выключать свет, маскировать"],
    ["pass out", "[ˈpæs ˈaʊt]", "раздавать, умереть, распространять, терять сознание, успешно пройти, сбывать"],
    ["anytime", "[ˈeniˌtaɪm]", "в любое время, всегда"],
    ["blood", "[ˈbləd]", "кровь, кровопролитие, кровяной, кровный, пускать кровь"],
    ["breakdown", "[ˈbreɪkˌdaʊn]", "пробой, распад, поломка, разрушение, распределение, авария, развал, анализ, упадок"],
    ["brink", "[ˈbrɪŋk]", "край, берег"],
    ["clutter", "[ˈklʌtər]", "беспорядок, шум, хаос, суматоха, гам, создавать суматоху, мешать"],
    ["helper", "[ˈhelpər]", "помощник, подручный, подсобный рабочий, вспомогательный паровоз"],
    ["pot", "[ˈpɑːt]", "горшок, банка, кастрюля, котел, чайник, сажать ребенка на горшок"],
    ["tragic", "[ˈtrædʒɪk]", "трагический, печальный, катастрофический, ужасный, прискорбный, трагедийный"],
    ["whenever", "[hwenˈevr̩]", "когда бы ни, всякий раз когда, когда же"],
    ["wonderfully", "[ˈwʌndəfəli]", "замечательно"],
    ["worksheet", "[ˈwɝːkʃit]", "анкета"],
    ["clean up", "[ˈkliːn ʌp]", "убираться, убирать, прибирать, приводить в порядок, заканчивать"],
    ["be into", "[biː ˈɪntə]", "увлекаться"],
    ["fall behind", "[ˈfɑːl bəˈhaɪnd]", "отставать, оставаться позади, опаздывать с уплатой"],
    ["catch up", "[ˈkætʃ ʌp]", "нагнать, подхватить, поднять, прерывать, догнать в чем-л."],
    ["hand in", "[ˈhænd ɪn]", "вручать, подавать, сажать"],
    ["read up on", "[riːd ʌp ɒn]", "начитаться / изучить"],
    ["draw up", "[ˈdrɒ ʌp]", "составить, составлять, выстраивать, выстраиваться, поднимать, выписывать"],
    ["knock off", "[ˈnɑːk ˈɒf]", "скостить, сбивать, сшибать, состряпать, стряхнуть, прикончить, шабашить, трахать"],
    ["slack off", "[sˈlæk ˈɒf]", "травить"],
    ["weigh up", "[ˈweɪ ʌp]", "оценивать, взвесить и решить, поднимать, составлять себе мнение"],
    ["come up", "[ˈkəm ʌp]", "подойти, подниматься, возникать, достигать, доходить, приезжать, всходить"],
    ["call back", "[ˈkɒl ˈbæk]", "звать обратно, брать назад"],
    ["annoyance", "[əˌnɔɪəns]", "раздражение, досада, неприятность, надоедание, приставание"],
    ["berserk", "[bərˈsɝːk]", "берсерк, неистовый человек, воин, неистовый, бешеный"],
    ["pros and cons", "[ˌprəʊz ənd ˈkɒnz]", "плюсы и минусы"],
    ["deadline", "[ˈdedˌlaɪn]", "предельный срок, предел, ремонт, сдать в ремонт"],
    ["deceive", "[dəˈsiːv]", "обманывать, вводить в заблуждение, изменять, сбивать с толку"],
    ["inseparable", "[ˌɪnˈsepərəbəl]", "неотделимый, неразделимый, неразлучный, неразлучные друзья"],
    ["option", "[ˈɑːpʃən]", "вариант, опция, выбор, опцион, средство, оптация, право выбора, свобода выбора"],
    ["persuasive", "[pərˈsweɪsɪv]", "убедительный, побуждение, мотив"],
    ["resignation", "[ˌrezəgˈneɪʃən]", "отставка, заявление об отставке, смирение, покорность, отказ от должности"],
    ["straight", "[ˈstreɪt]", "прямой, гетеросексуальный, прямо, сразу, немедленно, прямая, стрит, прямая линия"],
    ["update", "[ˌʌpˈdeɪt]", "обновлять, модернизировать, модернизация, свежие новости"],
    ["yoga", "[ˈjoʊɡə]", "йога"],
    ["sort out", "[ˈsɔːrt ˈaʊt]", "разбирать, разбираться, перебирать, рассортировать, улаживать, отсортировывать"],
    ["bail out", "[ˈbeɪl ˈaʊt]", "выручить из беды, прыгать с парашютом, брать на поруки, выпутываться"],
    ["do up", "[də ʌp]", "чинить, завертывать, крайне утомлять, прибирать, приводить в порядок"],
    ["pop into", "[pɒp ˈɪntə]", "заскочить"],
    ["splash out", "[ˌsplæʃ ˈaʊt]", "кутить, транжирить"],
    ["fall for", "[fɔːl fɔː]", "влюбиться / попасться"],
    ["take out", "[ˈteɪk ˈaʊt]", "вывезти, вынимать, брать, вынести, выводить, выбирать, выводить на прогулку"],
    ["mop up", "[ˈmɑːp ʌp]", "вытирать, поглощать, очищать, приканчивать, разделываться, осушать, убивать"],
    ["cloth", "[ˈklɒθ]", "ткань, полотно, сукно, суконный, свертываться, сгущаться"],
    ["garbage", "[ˈɡɑːrbɪdʒ]", "мусор, отбросы, чушь, барахло, чтиво, кухонные отбросы, макулатура, гниющий мусор"],
    ["lad", "[ˈlæd]", "парень, мальчик, юноша, паренек, лихой парень"],
    ["originally", "[əˈrɪdʒənəli]", "первоначально, оригинально, по происхождению"],
    ["persistence", "[pərˈsɪstəns]", "настойчивость, упорство, стойкость, постоянство, живучесть, продолжительность"],
    ["rural", "[ˈrʊrəl]", "сельский, деревенский, сельскохозяйственный"],
    ["seatbelt", "[ˈsiːtbelt]", "ремень безопасности"],
    ["spilt", "[ˈspɪlt]", "пролитый"],
    ["trash", "[ˈtræʃ]", "мусор, хлам, дрянь, халтура, отбросы, вздор, очищать от мусора, портить"],
    ["trick", "[ˈtrɪk]", "трюк, хитрость, уловка, фокус, обманывать, сложный, обманчивый"],
    ["unsatisfactory", "[ˌʌnsətəsˈfæktəri]", "неудовлетворительный"],
    ["look down", "[ˈlʊk ˈdaʊn]", "смотреть свысока, смотреть сверху вниз, презирать, падать"],
    ["write down", "[ˈraɪt ˈdaʊn]", "записывать, уценить, отзываться неодобрительно в печати"],
    ["aversion", "[əˈvɝːʒən]", "отвращение, антипатия, гадливость, неохота, предмет отвращения"],
    ["blank", "[ˈblæŋk]", "пустой, чистый, чистейший, заготовка, пробел, бланк, прикрывать, закрывать"],
    ["cooperative", "[koʊˈɒpərətɪv]", "совместный, кооперативный, объединенный, кооператив, кооперативное общество"],
    ["dawn", "[ˈdɒn]", "рассвет, заря, начало, утренняя заря, рассветать, светать, начинаться"],
    ["jog", "[ˈdʒɑːɡ]", "бегать трусцой, подталкивать, толкать, трусца, толчок, подталкивание"],
    ["let in", "[ˈlet ɪn]", "впустить, впускать, напускать, давать доступ, не выдержать, обманом впутывать"],
    ["add up to", "[æd ʌp tuː]", "составлять (сумму)"],
    ["hold out", "[hoʊld ˈaʊt]", "дотягивать, протягивать, выдерживать, держаться до конца, выстаивать"],
    ["knock down", "[ˈnɑːk ˈdaʊn]", "ломать, сносить, сбивать с ног, разрушать, опрокидывать, разбивать, понижать цены"],
    ["turn out", "[ˈtɝːn ˈaʊt]", "оказываться, обернуться, выпускать, выворачивать, наработать, тушить, выгонять"],
    ["turn in", "[ˈtɝːn ɪn]", "сдавать, зайти, возвращать, зайти мимоходом, отдавать, поворачивать вовнутрь"],
    ["turn on", "[ˈtɝːn ɑːn]", "включать, зависеть, открывать, волновать, одурманивать, сексуально возбуждать"],
    ["run into", "[rʌn ˈɪntə]", "наткнуться на"],
    ["run off", "[ˈrən ˈɒf]", "убегать, сбегать, смываться, забежать, удирать, отцеживать, спускать, строчить стихи"],
    ["turn up", "[ˈtɝːn ʌp]", "оказаться, появляться, поднимать, подниматься, загибаться, загибать, открыть"],
    ["ad", "[ˈæd]", "объявление, реклама, анонс, объявление, реклама"],
    ["bankrupt", "[ˈbæŋkrʌpt]", "банкрот, несостоятельный, лишенный, доводить до банкротства"],
    ["commute", "[kəˈmjuːt]", "смягчать, заменять, переключать, смягчать наказание"],
    ["delight", "[dəˈlaɪt]", "восторг, восхищение, наслаждение, наслаждаться, восхищать, восхищаться"],
    ["ensure", "[enˈʃʊr]", "обеспечивать, гарантировать, застраховать, ручаться, подстраховаться"],
    ["freezing", "[ˈfriːzɪŋ]", "замораживание, замерзание, заморозка, ледяной, замораживающий, леденящий"],
    ["opponent", "[əˈpoʊnənt]", "противник, оппонент, враг, противоположный, враждебный"],
    ["unbearable", "[ʌnˈberəbəl]", "невыносимый, невыносимо, невмоготу, невтерпеж"],
    ["safe", "[ˈseɪf]", "безопасный, в безопасности, надежный, сейф, несгораемый шкаф, холодильник"],
    ["pay back", "[ˈpeɪ ˈbæk]", "выплачивать, возвращать, отплачивать"],
    ["look out", "[ˈlʊk ˈaʊt]", "высматривать, выглядывать, быть настороже, подыскивать, иметь вид, выходить на"],
    ["broke in", "[ˈbrəʊk ɪn]", "вломился / вторгся"],
    ["fall out", "[ˈfɑːl ˈaʊt]", "выпадать, вываливаться, ссориться, рассориться, случаться, выходить из строя"],
    ["come across as", "[kʌm əˈkrɒs æz]", "производить впечатление"],
    ["piece together", "[ˈpiːs təˈɡeðər]", "соединять, собирать по кусочкам"],
    ["look out for", "[lʊk aʊt fɔː]", "высматривать"],
    ["mix up", "[ˈmɪks ʌp]", "смешивать, перепутать, спутать, заболтать, напутать, хорошо перемешивать"],
    ["pass away", "[ˈpæs əˈweɪ]", "исчезать, скончаться, умереть, проходить, преставиться, прекращаться"],
    ["detective", "[dəˈtektɪv]", "детектив, сыщик, агент сыскной полиции, детективный, сыскной"],
    ["graduation", "[ˌɡrædʒuːˈeɪʃən]", "градация, градуировка, окончание учебного заведения"],
    ["handy", "[ˈhændi]", "удобный, портативный, ловкий, легко управляемый, искусный, имеющийся под рукой"],
    ["intervene", "[ˌɪntərˈviːn]", "вмешиваться, помешать, вступаться, происходить, вступать в дело, лежать между"],
    ["loan", "[ˈloʊn]", "заем, ссуда, заимствование, ссудный, давать взаймы, ссужать"],
    ["mystery", "[ˈmɪstəri]", "тайна, таинство, мистерия, таинственный"],
    ["pickpocket", "[ˈpɪkˌpɑːkət]", "карманник, вор-карманник"],
    ["shyness", "[ˈʃaɪnəs]", "застенчивость"],
    ["show up", "[ˈʃoʊ ʌp]", "появляться, являться, разоблачать, выделяться, изобличать, унижать, вырисовываться"],
    ["set off", "[ˈset ˈɒf]", "отправлять, отправляться, выделять, выделяться, оттенять, запускать"],
    ["get together", "[ˈɡet təˈɡeðər]", "собираться, собирать, встречаться, встречать, объединяться"],
    ["tell off", "[ˈtel ˈɒf]", "отрядить, отбирать, отсчитывать, выругать, производить строевой расчет, отделать"],
    ["try on", "[ˈtraɪ ɑːn]", "примерять, примеряться, пробовать"],
    ["go through", "[ˈɡoʊ θruː]", "испытывать, обшаривать, подвергаться, обыскивать, доводить до конца, проноситься"],
    ["get out", "[ˈɡet ˈaʊt]", "выходить, убираться, выбираться, вылезать, уходить, проваливать, отвертеться"],
    ["speak up", "[ˈspiːk ʌp]", "высказываться, говорить громко, говорить громко и отчетливо"],
    ["barely", "[ˈberli]", "едва, только, лишь, еле-еле, просто, прямо, открыто"],
    ["bully", "[ˈbʊli]", "хулиган, задира, забияка, запугивать, задирать, великолепный"],
    ["lunchtime", "[ˈlʌntʃtaɪm]", "время (второго) завтрака, обеденный перерыв, ленч"],
    ["microphone", "[ˈmaɪkrəfoʊn]", "микрофон"],
    ["predict", "[prəˈdɪkt]", "предсказывать"],
    ["warning", "[ˈwɔːrnɪŋ]", "предупреждение, предостережение, знак, предупреждающий, предупредительный"],
    ["go on", "[ˈɡoʊ ɑːn]", "продолжать, продолжаться, длиться, происходить, твориться, случаться"],
    ["burst into", "[bɜːst ˈɪntə]", "ворваться / разразиться"],
    ["get in", "[ˈɡet ɪn]", "входить, садиться, влезать, присоединиться, участвовать, убирать, сажать, прибывать"],
    ["burn down", "[ˈbɝːn ˈdaʊn]", "сгорать, сжигать дотла, догорать, выжигать, выгореть, погореть"],
    ["turn into", "[tɜːn ˈɪntə]", "превратиться в"],
    ["burst out", "[ˈbɝːst ˈaʊt]", "вспыхивать, разражаться, выметываться"],
    ["stand out", "[ˈstænd ˈaʊt]", "выделяться, выдаваться, выступать, выстоять, маячить, держаться, не сдаваться"],
    ["freak out", "[ˌfriːk ˈaʊt]", "беситься, сходить с ума, приводить в возбуждение, волновать"],
    ["hang up", "[ˈhæŋ ʌp]", "вешать трубку, навешивать, подвешивать, медлить, давать отбой, откладывать"],
    ["hang on", "[ˈhæŋ ɑːn]", "уцепиться, прицепиться, крепко держаться, зависеть от, повиснуть, виснуть"],
    ["fairy tale", "[ˈfeəri teɪl]", "сказка"],
    ["adorable", "[əˈdɔːrəbəl]", "прелестный, восхитительный, обожаемый"],
    ["businesswoman", "[ˈbɪznəswʊmən]", "бизнес-леди"],
    ["cute", "[ˈkjuːt]", "милый, привлекательный, умный, миловидный, сообразительный, находчивый, остроумный"],
    ["invader", "[ˌɪnˈveɪdər]", "захватчик, оккупант, посягатель"],
    ["judgment", "[ˈdʒʌdʒmənt]", "суждение, мнение, приговор, усмотрение, решение суда, взгляд, наказание"],
    ["laughter", "[ˈlæftər]", "смех, хохот"],
    ["prisoner", "[ˈprɪzənər]", "заключенный, пленный, узник, арестант, арестованный, подсудимый, военнопленный"],
    ["tiny", "[ˈtaɪni]", "крошечный, крохотный, малюсенький, маленький ребенок"],
    ["worldwide", "[ˈwɝːlˈdwaɪd]", "всемирный, мировой, общемировой, всемирно известный"],
    ["knock out", "[ˈnɑːk ˈaʊt]", "нокаутировать, выбивать, выколачивать, свалить с ног, состряпать, изнурять"],
    ["break off", "[ˈbreɪk ˈɒf]", "обламывать, обламываться, отломить, отломиться, обрывать, отламывать, отламываться"],
    ["run over", "[ˈrən ˈoʊvr̩]", "пробегать, переехать, задавить, просматривать, съездить, сходить, повторять"],
    ["do without", "[duː wɪˈðaʊt]", "обходиться без"],
    ["sum up", "[ˈsəm ʌp]", "подытожить, подводить итог, суммировать, резюмировать, складывать"],
    ["throw up", "[ˈθroʊ ʌp]", "подбрасывать, подкидывать, рвать, бросать, извергать, упрекать, вскидывать"],
    ["move in", "[ˈmuːv ɪn]", "въезжать, вдвигать, вводить"],
    ["call round", "[ˌkɔːl ˈraʊnd]", "заходить, навещать, посещать"],
    ["jot down", "[dʒɒt daʊn]", "набросать (запись)"],
    ["warm up", "[ˈwɔːrm ʌp]", "разогревать, разогреваться, прогревать, прогреваться, подогревать, подогреваться"],
    ["hang around", "[ˈhæŋ əˈraʊnd]", "бродить вокруг, надвигаться, шляться, слоняться, околачиваться, быть поблизости"],
    ["act up", "[ˈækt ʌp]", "барахлить, паясничать, скандалить, сдержать обещание, плохо себя вести"],
    ["go together", "[ˈɡoʊ təˈɡeðər]", "сочетаться, гармонировать"],
    ["do over", "[də ˈoʊvr̩]", "переделывать, делать вновь, обмазывать, оклеивать обоями, покрывать, убивать"],
    ["see to", "[siː tuː]", "позаботиться о"],
    ["blouse", "[ˈblaʊs]", "блузка, блуза, кофточка, кофта, гимнастерка, рабочая блуза"],
    ["breath", "[ˈbreθ]", "дыхание, вздох, дуновение, жизнь, веяние"],
    ["diplomatic", "[ˌdɪpləˈmætɪk]", "дипломатический, дипломатичный, тактичный, буквальный, неискренний, текстуальный"],
    ["distant", "[ˈdɪstənt]", "далекий, отдаленный, дальний, сдержанный, давний, холодный, сухой, слабый, прошлый"],
    ["engagement", "[enˈɡeɪdʒmənt]", "зацепление, обязательство, помолвка, бой, обручальный"],
    ["formality", "[fɔːrˈmæləti]", "формальность, педантичность"],
    ["lorry", "[ˈlɔːri]", "грузовик, грузовой автомобиль, автомобиль, платформа, вагонетка"],
    ["poison", "[ˈpɔɪzən]", "яд, отрава, отравлять, портить, развращать"],
    ["upstairs", "[əpˈsterz]", "наверху, наверх, верхний этаж, находящийся наверху"],
    ["youth", "[ˈjuːθ]", "молодежь, молодость, юность, юноша, молодежный"],
    ["will", "[wɪl]", "воля, желание, завещание, волеизъявление, хотеть, желать, завещать, проявлять волю"],
    ["go ahead with", "[ɡəʊ əˈhed wɪð]", "приступить к"],
    ["answer back", "[ˈænsər ˈbæk]", "дерзить, возражать, грубить"],
    ["leap at", "[liːp æt]", "ухватиться за (шанс)"],
    ["leave aside", "[liːv əˈsaɪd]", "оставить в стороне"],
    ["move out", "[ˈmuːv ˈaʊt]", "съезжать, выдвинуть, выдвинуться, выезжать, выселить, выселять, выдвигать, выводить"],
    ["bring in", "[ˈbrɪŋ ɪn]", "приносить, вводить, вносить, ввозить, выносить, импортировать, пригонять в"],
    ["meet up", "[miːt ʌp]", "встретиться"],
    ["drop by", "[drɒp baɪ]", "зайти на минутку"],
    ["run down", "[ˈrən ˈdaʊn]", "сбежать, наезжать, сбегать, изнуряться, изнурять, истощаться, истощать"],
    ["hang about", "[ˈhæŋ əˈbaʊt]", "слоняться, болтаться без дела, околачиваться, шляться, валандаться, быть близким"],
    ["head for", "[hed fɔː]", "направляться к"],
    ["come round", "[ˈkəm ˈraʊnd]", "приходить в себя, заходить, заглянуть, обойти, обойти кругом, заезжать"],
    ["advisor", "[ædˈvaɪzər]", "советник, консультант"],
    ["aside", "[əˈsaɪd]", "в стороне, в сторону, отдельно, прочь, в резерве"],
    ["cyclist", "[ˈsaɪkələst]", "велосипедист"],
    ["disadvantaged", "[ˌdɪsədˈvæntɪdʒd]", "обделенный, обиженный, лишенный благоприятных условий"],
    ["highway", "[ˈhaɪˌwe]", "шоссе, автомагистраль, магистраль, дорожный"],
    ["pride", "[ˈpraɪd]", "гордость, гордыня, чувство гордости, гордиться"],
    ["unsurprising", "[ˌʌnsəˈpraɪzɪŋ]", "неудивительный"],
    ["stand up for", "[stænd ʌp fɔː]", "заступаться"],
    ["turn back", "[ˈtɝːn ˈbæk]", "повернуть назад, обернуться, повернуть обратно, оглянуться, прогнать, прогонять"],
    ["set out", "[ˈset ˈaʊt]", "излагать, отправляться, выставлять, выехать, двинуться, намереваться"],
    ["mess around", "[ˌmes əˈraʊnd]", ", бездельничать"],
    ["wind up", "[wɪnd ʌp]", "заводиться, заводить, ликвидировать, ликвидироваться, завершать, наматывать"],
    ["stay in", "[ˈsteɪ ɪn]", "оставаться дома, не выходить"],
    ["rub out", "[ˌrʌb ˈaʊt]", "убивать, ликвидировать, стирать, вычищать, счищаться"],
    ["stay out", "[ˈsteɪ ˈaʊt]", "отсутствовать, не возвращаться домой, пересидеть"],
    ["think ahead", "[θɪŋk əˈhed]", "думать наперёд"],
    ["get off", "[ˈɡet ˈɒf]", "слезть, отделаться, слезать, сходить, отделываться, вылезать, откалывать, снимать"],
    ["sign out", "[ˈsaɪn ˈaʊt]", "регистрировать, отъезд, регистрироваться, при отъезде, выписаться"],
    ["move on to", "[muːv ɒn tuː]", "перейти к"],
    ["read out", "[riːd ˈaʊt]", "читать вслух, исключать из организации, перечесть"],
    ["scribble down", "[ˈskrɪbəl daʊn]", "нацарапать"],
    ["sign up", "[ˈsaɪn ʌp]", "нанимать на работу, наниматься на работу, давать знак начала передачи"],
    ["bottle up", "[ˈbɑːtəl ʌp]", "сдерживать, скрывать"],
    ["hatred", "[ˈheɪtrəd]", "ненависть, отвращение"],
    ["hostility", "[haˈstɪləti]", "враждебность, неприятие, военные действия, враждебный акт"],
    ["journey", "[ˈdʒɝːni]", "путешествие, поездка, рейс, вояж, прогулка, путешествовать, совершать путешествие"],
    ["resentment", "[rəˈzentmənt]", "негодование, возмущение, чувство обиды"],
    ["stain", "[ˈsteɪn]", "пятно, краска, позор, красящее вещество, окрашивать, окрашиваться, пачкать"],
    ["sunrise", "[ˈsʌnˌraɪz]", "восход, восход солнца, утренняя заря"],
    ["teen", "[ˈtiːn]", "подросток, горе, страдание, гнев, злоба, подростковый, юношеский"],
    ["traveller", "[ˈtrævələr]", "путешественник, бегунок, коммивояжер"],
    ["be up to", "[biː ʌp tuː]", "затевать / быть годным"],
    ["sign in", "[ˈsaɪn ɪn]", "прибытие, например, регистрироваться по прибытии, вступать в члены"],
    ["let out", "[ˈlet ˈaʊt]", "выпускать, освобождать, проговориться, сдавать внаем, выбалтывать"],
    ["pay off", "[ˈpeɪ ˈɒf]", "окупаться, расплачиваться, рассчитываться с, покрывать, отплатить"],
    ["plan ahead", "[plæn əˈhed]", "планировать заранее"],
    ["fill out", "[ˈfɪl ˈaʊt]", "заполнять, наполняться, наполнять, пополнять, расширяться, расширять, замещать"],
    ["heat up", "[ˌhiːt ˈʌp]", "нагревать, подогревать, повышать, увеличивать"],
    ["shop around", "[ˈʃɑːp əˈraʊnd]", "искать работу"],
    ["try out", "[ˌtraɪ ˈaʊt]", "испытывать, опробовать, пробовать, проверять, пытаться сделать"],
    ["taken aback", "[ˈteɪkən əˈbæk]", "застигнут врасплох"],
    ["dwell upon", "[dwel əˈpɒn]", "задерживаться на"],
    ["mess about", "[ˌmes əˈbaʊt]", "лодырничать, слоняться без дела, бездельничать"],
    ["keep on", "[ˈkiːp ɑːn]", "продолжать, оставлять, не снимать, бранить, сохранять в прежнем положении"],
    ["go with", "[ɡəʊ wɪð]", "сочетаться / выбрать"],
    ["laze around", "[leɪz əˈraʊnd]", "лениться"],
    ["move up to", "[muːv ʌp tuː]", "перейти на (уровень)"],
    ["switch on", "[ˈswɪtʃ ɑːn]", "включать, соединять абонента, включать ток"],
    ["pick at", "[ˈpɪk æt]", "теребить, трогать, клевать, придираться, ворчать, пилить, вертеть в руках"],
    ["comfortably", "[ˈkʌmfərtəbli]", "удобно, обеспеченно"],
    ["declaration", "[ˌdekləˈreɪʃən]", "декларация, заявление, объявление, описание, таможенная декларация"],
    ["directness", "[dəˈrektnəs]", "непосредственность, прямота"],
    ["instruct", "[ˌɪnˈstrəkt]", "инструктировать, учить, обучать, информировать, распоряжаться, давать материал"],
    ["long-term", "[ˈlɔːŋˈtɝːm]", "долгосрочный, долговременный, длительный"],
    ["microwave", "[ˈmaɪkrəweɪv]", "микроволновая печь, микроволновый"],
    ["pill", "[ˈpɪl]", "таблетка, пилюля, шарик, грабить, давать пилюли, забаллотировать"],
    ["planning", "[ˈplænɪŋ]", "планирование, проектирование, землеустройство"],
    ["reception", "[rəˈsepʃən]", "прием, получение, восприятие, принятие, встреча, вечеринка, вечер"],
    ["upon", "[əˈpɑːn]", "на, по, после, в, о, за, об, относительно, во время, из, у, в течение, касательно"],
    ["vary", "[ˈveri]", "изменять, изменяться, варьировать, менять, меняться, разнообразить, разниться"],
    ["fool about", "[ˈfuːl əˈbaʊt]", "болтаться без дела, дурачиться, валять дурака, зря болтаться"],
    ["zoom in", "[zuːm ɪn]", "приблизить"],
    ["zoom out", "[zuːm aʊt]", "отдалить"],
    ["hear from", "[hɪə frɒm]", "получать весточку от"],
    ["grow on", "[ɡrəʊ ɒn]", "начинать нравиться"],
    ["ring back", "[rɪŋ bæk]", "перезвонить"],
    ["ask around", "[ˌɑːsk əˈraʊnd]", "расспрашивать тут и там"],
    ["keep away from", "[kiːp əˈweɪ frɒm]", "держаться подальше"],
    ["phone back", "[fəʊn bæk]", "перезвонить"],
    ["eat in", "[ˈiːt ɪn]", "питаться дома, столоваться по месту работы, въедаться"],
    ["tear up", "[teər ʌp]", "вырвать, изорвать, перервать"],
    ["boot up", "[buːt ʌp]", "загрузить (компьютер)"],
    ["laze about", "[leɪz əˈbaʊt]", "лениться"],
    ["lie down", "[ˈlaɪ ˈdaʊn]", "лечь, ложиться, прилечь, полежать, улечься, полеживать, перелечь, полечь"],
    ["tidy up", "[ˈtaɪdi ʌp]", "прибрать"],
    ["call by", "[kɔːl baɪ]", "зайти ненадолго"],
    ["ardent", "[ˈɑːrdənt]", "горячий, ярый, пылкий, страстный, ревностный, горящий, пылающий"],
    ["despair", "[ˌdɪˈsper]", "отчаяние, безысходность, отчаиваться, терять надежду"],
    ["edge", "[ˈedʒ]", "край, кромка, ребро, окраина, преимущество, пробираться, окаймлять, обрамлять"],
    ["examination", "[ɪgˌzæməˈneɪʃən]", "экспертиза, экзамен, исследование, осмотр, проверка, освидетельствование, допрос"],
    ["fabulously", "[ˈfæbjʊləsli]", "потрясающе, невероятно, вымышленно, неправдоподобно"],
    ["meaningful", "[ˈmiːnɪŋfəl]", "многозначительный, выразительный"],
    ["observer", "[əbˈzɝːvər]", "наблюдатель, обозреватель, соблюдающий законы, блюститель нравов"],
    ["onlooker", "[ˈɒnˌlʊkər]", "зритель, наблюдатель, свидетель"],
    ["phenomenally", "[fɪˈnɒmɪnəli]", "феноменально"],
    ["notice", "[ˈnoʊtɪs]", "уведомление, извещение, внимание, замечать, отмечать, обращать внимание"],
    ["look over", "[ˈlʊk ˈoʊvr̩]", "просматривать, осматривать, перебирать, проглядеть, не заметить, простить"],
    ["beat up", "[ˈbiːt ʌp]", "взбить, избивать, взбивать, вербовать, зверски избивать, отколотить"],
    ["cross out", "[ˈkrɑːs ˈaʊt]", "перечеркнуть, вычеркивать"],
    ["keep up", "[ˈkiːp ʌp]", "поддерживать, продолжать, поспевать, соблюдать, быть в курсе, придерживаться"],
    ["change into", "[tʃeɪndʒ ˈɪntə]", "переодеться / превратиться"],
    ["go back on", "[ɡəʊ bæk ɒn]", "нарушить (обещание)"],
    ["see after", "[siː ˈɑːftə]", "присматривать за"],
    ["destination", "[ˌdestəˈneɪʃən]", "назначение, пункт назначения, место назначения, адресат, цель, предназначение"],
    ["deteriorate", "[dəˈtɪriəˌret]", "ухудшать, ухудшаться, портить, портиться, разрушаться, вырождаться, дегенерировать"],
    ["inexplicable", "[ˌɪnəkˈsplɪsəbəl]", "необъяснимый, непонятный"],
    ["reject", "[rɪˈdʒekt]", "отвергать, отклонять, брак, бракованное изделие, признанный негодным"],
    ["restriction", "[riˈstrɪkʃən]", "ограничение"],
    ["for the sake", "[fə ðə seɪk]", "ради"],
    ["spot", "[ˈspɑːt]", "место, пятно, пятнышко, капля, определить, увидеть, местный, наличный"],
    ["fall down", "[ˈfɑːl ˈdaʊn]", "срываться, спасть, терпеть неудачу"],
    ["bear with", "[beə wɪð]", "потерпеть"],
    ["help out", "[ˈhelp ˈaʊt]", "выручить, помочь выйти, выводить из затруднения, помочь в затруднении, класть"],
    ["wash up", "[ˈwɑːʃ ʌp]", "мыть посуду, умываться"],
    ["slip up", "[sˈlɪp ʌp]", "ошибиться, споткнуться, совершать ошибку"],
    ["branch out", "[ˈbræntʃ ˈaʊt]", "отходить, ответвляться"],
    ["send out", "[ˈsend ˈaʊt]", "отправлять, рассылать, высылать, испускать, излучать, выставлять, выпускать ростки"],
    ["balloon", "[bəˈluːn]", "воздушный шар, баллон, аэростат, кружок, подниматься на воздушном шаре"],
    ["electronics", "[əˌlekˈtrɑːnɪks]", "электроника, электронная аппаратура"],
    ["fake", "[ˈfeɪk]", "фальшивка, подделка, липа, подлог, фальсифицировать, подделывать"],
    ["grab", "[ˈɡræb]", "захват, грейфер, самосхват, ковш, черпак, захватывать, схватывать, прихватывать"],
    ["pavement", "[ˈpeɪvmənt]", "тротуар, мостовая, дорожное покрытие, панель, почва, пол, выложенный мозаикой"],
    ["prepayment", "[priˈpeɪmənt]", "предоплата, аванс"],
    ["rotten", "[ˈrɑːtən]", "гнилой, тухлый, прогнивший, испорченный, отвратительный, прелый, негодный"],
    ["scent", "[ˈsent]", "запах, нюх, след, чутье, духи, амбре, пахнуть, душить, чуять, обонять"],
    ["straightaway", "[ˈstreɪtəˌwe]", "прямой, быстрый, честный, откровенный, простой, движущийся прямо вперед"],
    ["stumble", "[ˈstʌmbəl]", "спотыкаться, споткнуться, оступаться, задержка, спотыкание, запинка, ложный шаг"],
    ["unfamiliar", "[ˌʌnfəˈmɪljər]", "незнакомый, непривычный, чужой, малознакомый, неведомый, чуждый, не знающий о"],
    ["within", "[wɪðˈɪn]", "в, в течение, в пределах, в пределах, внутри, внутренняя сторона"],
    ["acknowledge", "[ækˈnɑːlɪdʒ]", "признавать, подтверждать, сознавать, допускать, награждать, быть признательным"],
    ["distinctly", "[ˌdɪˈstɪŋktli]", "отчетливо, четко, заметно, ясно, внятно, определенно, членораздельно"],
    ["immense", "[ˌɪˈmens]", "огромный, громадный, необъятный, безмерный, великолепный, бескрайний"],
    ["impact", "[ˌɪmˈpækt]", "воздействие, влияние, удар, столкновение, ударять, ударяться, сталкиваться"],
    ["pronounce", "[prəˈnaʊns]", "произносить, выговорить, высказываться, произношение"],
    ["puzzle", "[ˈpʌzəl]", "головоломка, загадка, недоумение, озадачивать, усложнять, запутывать"],
    ["repeatedly", "[rəˈpiːtədli]", "неоднократно, повторно, несколько раз"],
    ["requirement", "[rɪˈkwaɪrmənt]", "требование, потребность, необходимое условие, нужда"],
    ["schoolchildren", "[ˈskuːltʃɪldrən]", "школьники"],
    ["strategy", "[ˈstrætədʒi]", "стратегия, оперативное искусство"],
    ["untouched", "[ʌnˈtətʃt]", "нетронутый"],
    ["examine", "[ɪgˈzæmən]", "исследовать, рассматривать, проверять, обследовать, осматривать, допрашивать"],
    ["victim", "[ˈvɪktəm]", "жертва, потерпевший"],
    ["review", "[ˌriˈvjuː]", "обзор, рассмотрение, пересмотр, просмотр, пересмотреть, рассматривать"],
    ["depress", "[dəˈpres]", "подавлять, угнетать, нажимать, ослаблять, опускать, огорчать, понижать цену"],
    ["prolong", "[prəˈlɒŋ]", "продлить, продлевать, пролонгировать, продолжать, отсрочивать, продолжать дальше"],
    ["skilfully", "[ˈskɪlfəli]", "умело, искусно, ловко"],
    ["ungrateful", "[ʌnˈɡreɪtfəl]", "неблагодарный, неприятный"],
    ["amateur", "[ˈæməˌtər]", "любительский, самодеятельный, любитель, дилетант, спортсмен-любитель"],
    ["breathtaking", "[ˈbreθˌtekɪŋ]", "захватывающий, потрясающий, поразительный"],
    ["impatiently", "[ɪmˈpeɪʃəntli]", "нетерпеливо"],
    ["low-paid", "[ˈləʊˈpeɪd]", "низкооплачиваемый"],
    ["overseas", "[ˌoʊvəˈsiːz]", "за границу, за границей, за морем, заморский, заграничный, заокеанский"],
    ["picturesque", "[ˌpɪktʃəˈresk]", "живописный, колоритный, образный, яркий, картинный"],
    ["scarcely", "[ˈskersli]", "едва, едва ли, вряд ли, еле, с трудом, еле-еле, не совсем, только что, как только"],
    ["spill", "[ˈspɪl]", "проливать, проливаться, пролить, пролитие, поток"],
    ["well-behaved", "[ˌwel bɪˈheɪvd]", "благонравный, выдрессированный"],
    ["prime minister", "[praɪm ˈmɪnɪstə]", "премьер-министр"],
    ["indecisively", "[ˌɪndɪˈsaɪsɪvli]", "нерешительно"],
    ["offensively", "[əˈfensɪvli]", "оскорбительно"],
    ["pointless", "[ˌpɔɪntləs]", "бессмысленный, бесцельный, тупой, незаостренный, с неоткрытым счетом"],
    ["reinvent", "[riˌɪnˈvent]", "заново изобретать, открывать, обновлять, переделывать"],
    ["scream", "[ˈskriːm]", "орать, вопить, вскрикнуть, зареветь, визг, вопль, умора, клекот"],
    ["stage", "[ˈsteɪdʒ]", "этап, стадия, сцена, ступень, сценический, ступенчатый, ставить, инсценировать"],
    ["systematically", "[ˌsɪstəˈmætɪkəli]", "систематически"],
    ["worthless", "[ˈwɝːθləs]", "бесполезный, никчемный, дрянной, никудышный, бесценный, матерный, херовый"],
    ["accomplished", "[əˈkɑːmplɪʃt]", "опытный, завершенный, выполненный, совершенный, законченный, искусный, изысканный"],
    ["banquet", "[ˈbæŋkwət]", "банкет, пир, званый обед, пировать, угощать, давать банкет"],
    ["blissfully", "[ˈblɪsfəli]", "блаженно"],
    ["composer", "[kəmˈpoʊzə]", "композитор, автор"],
    ["dreadfully", "[ˈdredfəli]", "ужасно, страшно, чрезвычайно"],
    ["enterprise", "[ˈentərˌpraɪz]", "предприятие, предпринимательство, предприимчивость, инициатива"],
    ["hugely", "[ˈhjuːdʒli]", "очень, весьма"],
    ["illustrate", "[ˈɪləˌstret]", "иллюстрировать, пояснять, служить примером"],
    ["miserable", "[ˈmɪzərəbəl]", "несчастный, жалкий, убогий, плохой, печальный, скудный"],
    ["negotiations", "[nɪˌɡəʊʃiˈeɪʃənz]", "переговоры"],
    ["niche", "[ˈnɪtʃ]", "ниша, убежище, надлежащее место, удобно устроиться, помещать в нишу"],
    ["schooldays", "[ˈskuːldeɪz]", "школьные годы"],
    ["vaguely", "[ˈveɪɡli]", "неясно"],
    ["appalled", "[əˈpɔːld]", "в ужасе"],
    ["confession", "[kənˈfeʃən]", "исповедь, признание, вероисповедание"],
    ["devastated", "[ˈdevəˌstetəd]", "опустошенный, разоренный"],
    ["eternally", "[ˌɪˈtɝːnəli]", "вечно, неизменно"],
    ["exceptionally", "[ɪkˈsepʃənəli]", "исключительно"],
    ["painfully", "[ˈpeɪnfəli]", "больно, болезненно"],
    ["patently", "[ˈpætəntli]", "явно, очевидно, открыто"],
    ["probable", "[ˈprɑːbəbəl]", "вероятный, возможный, предполагаемый, вероятный выбор, возможный кандидат"],
    ["readily", "[ˈredəli]", "легко, с готовностью, охотно, быстро, без труда"],
    ["widely", "[ˈwaɪdli]", "широко, далеко"],
    ["astoundingly", "[əˈstaʊndɪŋli]", "поразительно"],
    ["chronic", "[ˈkrɑːnɪk]", "хронический, постоянный, застарелый, хроник"],
    ["contaminated", "[kənˈtæməˌnetəd]", "загрязненный, зараженный"],
    ["critical", "[ˈkrɪtɪkəl]", "критический, решающий, критичный, переломный, опасный, предельный"],
    ["desperately", "[ˈdespərətli]", "в отчаянии"],
    ["excessively", "[ɪkˈsesɪvli]", "чрезмерно, излишне, безмерно"],
    ["extraordinarily", "[ɪkˌstrɔːrdəˈnerəli]", "необычайным образом, совершенно необычно"],
    ["fantastically", "[fænˈtæstɪkli]", "причудливо, фантастически, прихоти, произвольно, необычно, странно"],
    ["guitarist", "[ˌɡɪˈtɑːrəst]", "гитарист"],
    ["incomprehensible", "[ˌɪŋˌkɑːmprəˈhensəbəl]", "непонятный, непостижимый"],
    ["landscape", "[ˈlændˌskep]", "пейзаж, ландшафт, ландшафтный, пейзажный, благоустраивать участок"],
    ["naughty", "[ˈnɒti]", "непослушный, озорной, капризный, шаловливый, гадкий, испорченный, дурной"],
    ["plot", "[ˈplɑːt]", "сюжет, график, заговор, участок земли, план, наносить, вычерчивать, чертить"],
    ["priced", "[ˈpraɪst]", "оцененный"],
    ["remarkably", "[rəˈmɑːrkəbli]", "удивительно, замечательно, необыкновенно, в высшей степени"],
    ["snake", "[ˈsneɪk]", "змея, змей, предатель, вероломный человек, змеиться, виться"],
    ["strikingly", "[ˈstraɪkɪŋli]", "поразительно, удивительно, замечательно"],
    ["terrifically", "[təˈrɪfɪkli]", "очень сильно, страшно, очень хорошо, замечательно"],
    ["uncommonly", "[ˌənˈkɑːmənli]", "необыкновенно, удивительно, замечательно"],
    ["unqualified", "[ʌnˈkwɑːləˌfaɪd]", "неквалифицированный, безоговорочный, неограниченный, негодный, неподходящий"],
    ["unusually", "[ʌnˈjuːʒuəli]", "необычно"],
    ["accepted", "[ækˈseptəd]", "принятый, принявший, признанный, общепринятый, распространенный, одобренный"],
    ["commercially", "[kəˈmɝːʃəli]", "в торговом отношении, с коммерческой точки зрения, в больших количествах, в…"],
    ["comparatively", "[kəmˈperətɪvli]", "сравнительно, относительно"],
    ["contagious", "[kənˈteɪdʒəs]", "заразный, инфекционный, заразительный, контагиозный"],
    ["debated", "[dɪˈbeɪtɪd]", "спорный"],
    ["densely", "[ˈdensli]", "плотно, густо, компактно, скученно"],
    ["enormously", "[əˈnɔːrməsli]", "чрезвычайно, крайне"],
    ["fiercely", "[ˈfɪrsli]", "свирепо"],
    ["genetically", "[dʒəˈnetɪkəli]", "генетически, по происхождению, с генетической точки зрения, в отношении генезиса"],
    ["hotly", "[ˈhɑːtli]", "горячо, жарко, пылко, страстно, остро, с перцем, с жаром, взволнованно"],
    ["immensely", "[ˌɪˈmensli]", "очень, чрезвычайно, безмерно"],
    ["memorable", "[ˈmemərəbəl]", "памятный, незабываемый, достопамятный, незабвенный, легко запоминающийся"],
    ["merely", "[ˈmɪrli]", "просто, только, единственно"],
    ["modified", "[ˈmɑːdəˌfaɪd]", "модифицированный, видоизмененный"],
    ["outlook", "[ˈaʊˌtlʊk]", "перспектива, кругозор, вид, точка зрения, виды на будущее, наблюдение"],
    ["populated", "[ˈpɑːpjəˌletəd]", "населенный"],
    ["relaxed", "[rəˈlækst]", "расслабленный"],
    ["respected", "[rəˈspektəd]", "уважаемый, почитаемый"],
    ["severely", "[səˈvɪrli]", "сильно, жестоко, сурово, строго-настрого"],
    ["straightforward", "[ˈstreɪtˈfɔːrwərd]", "простой, прямой, откровенный, честный, движущийся прямо вперед"],
    ["superficial", "[ˌsuːpərˈfɪʃəl]", "поверхностный, внешний, неглубокий, наносный, аллювиальный, суперфициальный"],
    ["throughout", "[θruːˈaʊt]", "по всему, через, в продолжение, повсюду, через, на всем протяжении"],
    ["unconvincing", "[ˌʌnkənˈvɪnsɪŋ]", "неубедительный"],
    ["viable", "[ˈvaɪəbəl]", "жизнеспособный, жизнедеятельный, всхожий"],
    ["astounding", "[əˈstaʊndɪŋ]", "поразительный, удивительный"],
    ["comedy", "[ˈkɑːmədi]", "комедия, комичный случай, забавное событие"],
    ["dazzling", "[ˈdæzəlɪŋ]", "ослепительный"],
    ["filthy", "[ˈfɪlθi]", "грязный, отвратительный, мерзкий, замызганный, непристойный, отвратный, гадкий"],
    ["gorgeous", "[ˈɡɔːrdʒəs]", "великолепный, прекрасный, витиеватый, ярко окрашенный, ярко расцвеченный"],
    ["hilarious", "[həˈleriəs]", "веселый, шумный"],
    ["impeccable", "[ˌɪmˈpekəbəl]", "безупречный, непогрешимый"],
    ["starving", "[ˈstɑːrvɪŋ]", "голодающий"],
    ["terrifying", "[ˈterəˌfaɪɪŋ]", "ужасающий"],
    ["thrilled", "[θrɪld]", "в восторге"],
    ["tranquillity", "[trænˈkwɪlɪti]", "спокойствие"],
    ["trivial", "[ˈtrɪviəl]", "тривиальный, банальный, незначительный, пустой, обыденный, мелкий, ненаучный"],
    ["virtue", "[ˈvɝːtʃuː]", "добродетель, сила, достоинство, целомудрие, действие, хорошее качество, свойство"],
    ["apart", "[əˈpɑːrt]", "отдельно, обособленно, на части, врозь, в стороне, в сторону, порознь, на куски, особо"],
    ["appeal", "[əˈpiːl]", "апелляция, обращение, обжалование, призыв, обращаться, апеллировать, взывать"],
    ["confusion", "[kənˈfjuːʒən]", "путаница, замешательство, беспорядок, смятение, беспорядки, смущение, неразбериха"],
    ["gardener", "[ˈɡɑːrdənər]", "садовник, садовод, огородник"],
    ["haste", "[ˈheɪst]", "спешка, поспешность, торопливость, спешить, торопить, торопиться"],
    ["knitting", "[ˈnɪtɪŋ]", "вязание, вязанье, трикотаж, вязка, вязальный"],
    ["proverb", "[ˈprɑːvərb]", "пословица, поговорка, олицетворение"],
    ["relieve", "[rəˈliːv]", "облегчать, освобождать, уменьшать, разрядить, ослаблять, деблокировать, сменять"],
    ["sewing", "[ˈsoʊɪŋ]", "швейный, шитье, пошив, пошивка"],
    ["tricky", "[ˈtrɪki]", "сложный, хитрый, каверзный, мудреный, запутанный, ловкий, искусный, находчивый"],
    ["unanimously", "[juːˈnænəməsli]", "единогласно"],
    ["volunteering", "[ˌvɒlənˈtɪərɪŋ]", "волонтёрство"],
    ["affectionate", "[əˈfekʃənət]", "любящий, нежный, привязчивый"],
    ["ambiguous", "[æmˈbɪɡjuːəs]", "двусмысленный, неопределенный, сомнительный, неясный"],
    ["amusing", "[əˈmjuːzɪŋ]", "забавный, смешной, занятный, занимательный"],
    ["civilisation", "[ˌsɪvəlaɪˈzeɪʃən]", "цивилизация"],
    ["instability", "[ˌɪnstəˈbɪləti]", "неустойчивость, непостоянство"],
    ["interpretation", "[ˌɪnˌtərprəˈteɪʃən]", "интерпретация, толкование, перевод, объяснение, дешифрирование"],
    ["mild", "[ˈmaɪld]", "мягкий, легкий, умеренный, слабый, кроткий, тихий, неострый, незлобивый"],
    ["misconception", "[ˌmɪskənˈsepʃən]", "неправильное представление, недоразумение"],
    ["modestly", "[ˈmɒdɪstli]", "скромно"],
    ["overwhelmed", "[ˌəʊvəˈwelmd]", "перегружен / ошеломлён"],
    ["piracy", "[ˈpaɪrəsi]", "пиратство, нарушение авторского права, перехват одной реки другой"],
    ["sheer", "[ˈʃɪr]", "чистый, явный, отвесно, абсолютно"],
    ["spicy", "[ˈspaɪsi]", "пряный, острый, пикантный, ароматный, живой, энергичный, неприличный"],
    ["toast", "[toʊst]", "тост, гренки, гренок, жареный хлеб, поджаривать, подрумянивать на огне"],
    ["trustworthy", "[ˈtrʌˌstwərði]", "надежный, заслуживающий доверия, благонадежный"],
    ["balance", "[ˈbæləns]", "баланс, равновесие, остаток, сбалансировать, балансовый"],
    ["believer", "[bəˈliːvər]", "верующий, сторонник, защитник"],
    ["endangered", "[enˈdeɪndʒərd]", "подвергать опасности"],
    ["eyesight", "[ˈaɪˌsaɪt]", "зрение, поле зрения, кругозор"],
    ["friendliness", "[ˈfrendlinəs]", "дружелюбие, дружественное отношение, дружеское отношение"],
    ["humble", "[ˈhʌmbəl]", "скромный, смиренный, покорный, простой, смирять, смиряться, унижать"],
    ["inextricably", "[ˌɪˈnekstrɪkəbli]", "неразрывно, запутанно, сложно"],
    ["itself", "[ətˈself]", "сам, сама, само, себя, себе"],
    ["mass", "[ˈmæs]", "масса, месса, множество, массовый, повальный, массировать"],
    ["mental", "[ˈmentəl]", "психический, умственный, мысленный, псих, ненормальный"],
    ["nonsense", "[ˈnɑːnsens]", "ерунда, глупости, вздор, чушь, чепуха, бессмыслица, абсурд, ахинея, дребедень, пустяк"],
    ["openness", "[ˈoʊpən nəs]", "откровенность, прямота, очевидность"],
    ["overlook", "[ˌoʊvəˈlʊk]", "игнорировать, упускать из виду, обзор, упущение, осмотр, пропуск"],
    ["overrated", "[ˌoʊvəˈreɪtɪd]", "переоцененный"],
    ["paragraph", "[ˈperəˌɡræf]", "пункт, параграф, абзац, газетная заметка, разделять на абзацы"],
    ["perspective", "[pərˈspektɪv]", "перспектива, вид, виды на будущее, перспективный"],
    ["prank", "[ˈpræŋk]", "шалость, шутка, выходка, проделка, проказа, наряжать, наряжаться, украшать"],
    ["roughly", "[ˈrʌfli]", "грубо, приблизительно, бурно, резко, неровно, небрежно, начерно, невежливо"],
    ["sentimental", "[ˌsentəˈmentəl]", "сентиментальный, чувствительный, затрагивающий чувства"],
    ["species", "[ˈspiːʃiz]", "вид, разновидность, порода, род, класс"],
    ["spit", "[ˈspɪt]", "плевать, плеваться, плюнуть, сплюнуть, вертел, плевок, слюна, намывная коса"],
    ["superb", "[sʊˈpɝːb]", "превосходный, великолепный, прекрасный, роскошный, величественный, благородный"],
    ["track", "[ˈtræk]", "трек, дорожка, след, путь, трасса, курс, колея, следить, прослеживать, выслеживать"],
    ["vitally", "[ˈvaɪtəli]", "жизненно"],
    ["weekly", "[ˈwiːkli]", "еженедельный, недельный, еженедельно, раз в неделю, еженедельник"],
    ["relative", "[ˈrelətɪv]", "относительный, сравнительный, взаимный, родственник, родственница"],
    ["appealing", "[əˈpiːlɪŋ]", "привлекательный, умоляющий, трогательный, подкупающий"],
    ["assessment", "[əˈsesmənt]", "оценка, суждение, обложение, обложение налогом, сумма обложения, размер налога"],
    ["assume", "[əˈsuːm]", "предполагать, принимать, брать на себя, допускать, принимать на себя"],
    ["awkward", "[ˈɑːkwərd]", "неловкий, неуклюжий, неудобный, угловатый, жопа"],
    ["compulsory", "[kəmˈpəlsəri]", "обязательный, принудительный, вынужденный"],
    ["discrimination", "[ˌdɪˌskrɪməˈneɪʃən]", "дискриминация, распознавание, проницательность, разборчивость"],
    ["disregard", "[ˌdɪsrɪˈɡɑːrd]", "пренебрежение, игнорирование, игнорировать, пренебрегать"],
    ["drastically", "[ˈdræstɪkəli]", "радикально, кардинально, коренным образом"],
    ["real estate", "[ˈrɪəl ɪˈsteɪt]", "недвижимость"],
    ["exploitation", "[ˌekˈsplɔɪˈteɪʃən]", "эксплуатация, разработка месторождения"],
    ["gender", "[ˈdʒendər]", "пол, род, порождать"],
    ["genuineness", "[ˈdʒenjəwənəs]", "подлинность, искренность"],
    ["insulting", "[ˌɪnˈsəltɪŋ]", "оскорбительный, обидный"],
    ["labour", "[ˈleɪbr̩]", "труд, рабочие, работа, роды, трудиться, работать, трудовой, лейбористский"],
    ["overpriced", "[ˌoʊvəˈpraɪstɪd]", "завышенный"],
    ["pleasantly", "[ˈplezəntli]", "приятно, любезно, весело, славно"],
    ["presume", "[prəˈzuːm]", "предполагать, полагать, допускать, осмеливаться, считать доказанным"],
    ["responsibly", "[rɪˈspɒnsəbli]", "ответственно"],
    ["rhetorical", "[rəˈtɔːrɪkəl]", "риторический, высокопарный"],
    ["ruthless", "[ˈruːθləs]", "беспощадный, безжалостный, жестокий"],
    ["subjective", "[səbˈdʒektɪv]", "субъективный, свойственный подлежащему"],
    ["surpass", "[sərˈpæs]", "превосходить, превышать, переплюнуть, перегонять"],
    ["unavoidable", "[ˌʌnəˌvɔɪdəbəl]", "неизбежный, неминуемый"],
    ["underline", "[ˈʌndərˌlaɪn]", "подчеркивание, объяснительная надпись, подчеркивать"],
    ["relieved", "[rɪˈliːvd]", "испытавший облегчение"],
    ["anyhow", "[ˈeniˌhɑːw]", "так или иначе, во всяком случае, как-нибудь, никак, кое-как"],
    ["colossal", "[kəˈlɑːsəl]", "колоссальный, грандиозный, громадный, великолепный, замечательный"],
    ["commitment", "[kəˈmɪtmənt]", "обязательство, совершение, передача, вручение, заключение под стражу, арест"],
    ["dedication", "[ˌdedəˈkeɪʃən]", "посвящение, преданность, самоотверженность, освящение"],
    ["edition", "[ɪˈdɪʃən]", "издание, выпуск, тираж, вариант, копия"],
    ["emphasis", "[ˈemfəˌsɪs]", "акцент, особое внимание, ударение, подчеркивание, особое значение"],
    ["frustrating", "[ˈfrəˌstretɪŋ]", "расстраивающий, разочаровывающий"],
    ["gentleman", "[ˈdʒentəlmən]", "джентльмен, господин, барин, дворянин, мужская уборная"],
    ["outdated", "[ˈaʊtˌdetəd]", "устаревший, устарелый"],
    ["painstaking", "[ˈpeɪnˌstekɪŋ]", "кропотливый, тщательный, старательный, усердие, старание"],
    ["relief", "[rəˈliːf]", "облегчение, помощь, рельеф, освобождение, рельефный, вспомогательный, запасной"],
    ["scheme", "[ˈskiːm]", "схема, система, план, программа, проект, интриговать, замышлять, планировать"],
    ["seemingly", "[ˈsiːmɪŋli]", "на вид, судя по виду, по-видимому"],
    ["ventilation", "[ˌventəˈleɪʃən]", "вентиляция, проветривание, выяснение, обсуждение"],
    ["wholly", "[ˈhoʊl li]", "полностью, целиком, всецело"],
    ["drawback", "[ˈdrɒˌbæk]", "недостаток, препятствие, помеха, возвраты пошлин, отрицательная сторона, уступка"],
    ["inaccuracy", "[ˌɪˈnækjərəsi]", "неточность, ошибка"],
    ["interestingly", "[ˈɪntəˌrestɪŋli]", "интересно"],
    ["irresistible", "[ˌɪrəˈzɪstəbəl]", "непреодолимый, неотразимый, неопровержимый"],
    ["meantime", "[ˈmiːnˌtaɪm]", "тем временем, между тем"],
    ["meanwhile", "[ˈmiːˌnwaɪl]", "тем временем, между тем"],
    ["minor", "[ˈmaɪnər]", "минор, несовершеннолетний подросток, незначительный, несовершеннолетний"],
    ["mispronounce", "[ˌmɪsprəˈnaʊns]", "неправильно произносить"],
    ["neighbourhood", "[ˈneɪbəhʊd]", "окрестности, район, соседство, соседи, округа, близость, соседские отношения"],
    ["seminar", "[ˈseməˌnɑːr]", "семинар, научная конференция, интенсивный курс обучения"],
    ["therefore", "[ˈðerˌfɔːr]", "поэтому, следовательно, потому"],
    ["thinking", "[ˈθɪŋkɪŋ]", "мышление, размышление, размышления, мыслящий, разумный"],
    ["unwise", "[ʌˈnwaɪz]", "неразумный, неблагоразумный, глупый"],
    ["unwisely", "[ʌnˈwaɪzli]", "неблагоразумно"],
    ["well-informed", "[ˌwel ɪnˈfɔːrmd]", "хорошо осведомленный"],
    ["abundantly", "[əˈbʌndəntli]", "обильно, изобильно"],
    ["applicant", "[ˈæplɪkənt]", "кандидат, претендент, проситель"],
    ["competence", "[ˈkɑːmpətəns]", "компетенция, компетентность, правомочность, способность, умение, достаток"],
    ["compromise", "[ˈkɑːmprəˌmaɪz]", "компромисс, компромиссный, идти на компромисс"],
    ["crystal", "[ˈkrɪstəl]", "кристалл, хрусталь, хрустальная посуда, кристаллический, кристальный"],
    ["dominate", "[ˈdɑːməˌnet]", "доминировать, преобладать, господствовать, властвовать, главенствовать"],
    ["foresee", "[fɔːrˈsiː]", "предвидеть, провидеть, знать заранее"],
    ["legend", "[ˈledʒənd]", "легенда, надпись, поверье"],
    ["misinterpret", "[ˌmɪˌsɪnˈtɝːpret]", "неверно истолковывать, перевирать, перетолковывать, неверно понимать"],
    ["perk", "[ˈpɝːk]", "воспрянуть духом, привилегия, приработок, высокомерный"],
    ["riser", "[ˈraɪzər]", "стояк, прибыль, выпор, подступень лестницы, подъем ступени лестницы"],
    ["summarise", "[ˈsʌməraɪz]", "резюмировать"],
    ["thirdly", "[ˈθɝːdli]", "в-третьих"],
    ["thus", "[ˈðəs]", "таким образом, так, поэтому, таким образом, поэтому, стало"],
    ["uncover", "[ʌnˈkəvər]", "открывать, раскрывать, обнаруживать, снимать, обнажать голову"],
    ["unlike", "[ənˈlaɪk]", "в отличие от, непохожий на, разноименный"],
    ["unrealistic", "[ənˌriˈlɪstɪk]", "нереальный"],
    ["unveil", "[ənˈveɪl]", "открывать, раскрывать, снимать покрывало, предстать в истинном свете"],
    ["workaholic", "[ˌwɜːkəˈhɒlɪk]", "трудоголик, человек, на работе, на работе"],
    ["adequate", "[ˈædəkwət]", "адекватный, достаточный, быть достаточным"],
    ["allocation", "[ˌæləˈkeɪʃən]", "распределение, размещение, ассигнование, назначение, отчисление, локализация"],
    ["aptitude", "[ˈæptəˌtuːd]", "способность, способности, склонность, пригодность, уместность"],
    ["deforestation", "[dəˌfɔːrəˈsteɪʃən]", "вырубка леса"],
    ["disproportionate", "[ˌdɪsprəˈpɔːrʃənət]", "непропорциональный, несоразмерный"],
    ["drunk", "[ˈdrəŋk]", "пьяный, выпивший, опьяненный, захмелевший, пьяный, запой, попойка"],
    ["flashcard", "[ˈflæʃkɑːd]", "карточка для запоминания"],
    ["groundless", "[ˈɡraʊndləs]", "беспочвенный, безосновательный, беспричинный, неосновательный"],
    ["horrified", "[ˈhɔːrəˌfaɪd]", "в ужасе"],
    ["humanity", "[hjuːˈmænəti]", "человечество, человечность, гуманность, люди, человеколюбие"],
    ["infinite", "[ˈɪnfənət]", "бесконечный, безграничный, бесконечность, множество"],
    ["lucrative", "[ˈluːkrətɪv]", "прибыльный, выгодный, доходный"],
    ["major", "[ˈmeɪdʒər]", "крупный, главный, больший, большой, майор, большинство, совершеннолетний"],
    ["miscalculate", "[ˌmɪˈskælkjəˌlet]", "прогадать, просчитаться, просчитываться, ошибаться в расчете, обсчитываться"],
    ["misinform", "[ˌmɪˌsɪnˈfɔːrm]", "дезинформировать, вводить в заблуждение, дезориентировать"],
    ["nonetheless", "[ˌnʌnðəˈles]", "тем не менее, все же"],
    ["non-traditional", "[nɒn trəˈdɪʃənəl]", "нетрадиционный"],
    ["obligation", "[ˌɑːbləˈɡeɪʃən]", "обязательство, обязанность, долг, обязательность, принудительная сила"],
    ["prevail", "[prɪˈveɪl]", "преобладать, превалировать, возобладать, господствовать, одержать победу"],
    ["reassure", "[ˌriəˈʃʊr]", "успокаивать, убеждать, заверять, уверять, утешать, восстановить доверие"],
    ["shadow", "[ˈʃæˌdoʊ]", "тень, призрак, мрак, полумрак, намек, покров, затенять, омрачать, осенять, предвещать"],
    ["simplicity", "[ˌsɪmˈplɪsəti]", "простота, простодушие, наивность, несложность, непритязательность, скромность"],
    ["spontaneously", "[spanˈteɪniəsli]", "спонтанно"],
    ["sufficient", "[səˈfɪʃənt]", "достаточный, достаточное количество"],
    ["suspicion", "[səˈspɪʃən]", "подозрение, подозрительность, привкус, чуточка, оттенок"],
    ["tension", "[ˈtenʃən]", "напряжение, натяжение, растяжение, давление, упругость, растягивающее усилие"],
    ["undecided", "[ˌʌndəˈsaɪdəd]", "нерешительный, нерешенный, не принявший решения, не решившийся"],
    ["unsystematic", "[ˌʌnsɪstəˈmætɪk]", "бессистемный, несистематичный"],
    ["utmost", "[ˈʌtmoʊst]", "предельный, крайний, наивысший, все возможное, самое большое"],
    ["achievable", "[əˈtʃiːvəbəl]", "достижимый, достигаемый"],
    ["affect", "[əˈfekt]", "влиять, затрагивать, затронуть, аффект"],
    ["capriciously", "[kəˈprɪʃəsli]", "капризно"],
    ["consistency", "[kənˈsɪstənsi]", "последовательность, согласованность, консистенция, постоянство, логичность"],
    ["consistent", "[kənˈsɪstənt]", "последовательный, согласующийся, совместимый, стойкий, плотный, твердый"],
    ["convict", "[ˈkɑːnvɪkt]", "осужденный, осужденный, каторжник, осуждать"],
    ["creatively", "[kriˈeɪtɪvli]", "креативно"],
    ["determine", "[dəˈtɝːmən]", "определять, устанавливать, решать, решаться, измерять, детерминировать"],
    ["distribution", "[ˌdɪstrəˈbjuːʃən]", "распределение, распространение, дистрибуция, раздача, рассылка, разверстка"],
    ["doubtless", "[ˈdaʊtləs]", "несомненно, вероятно, несомненный"],
    ["elimination", "[əˌlɪməˈneɪʃən]", "устранение, исключение, элиминация, отобранный путем отсева"],
    ["endure", "[enˈdjʊr]", "терпеть, вынести, выдюжить, вытерпеть, выдерживать, продолжаться, длиться"],
    ["fearlessly", "[ˈfɪələsli]", "смело, бесстрашно, неустрашимо, дерзновенно"],
    ["flexibility", "[ˌfleksəˈbɪləti]", "гибкость, уступчивость"],
    ["foremost", "[ˈfɔːmoʊst]", "прежде всего, в первую очередь, передовой, выдающийся, передний"],
    ["fruitfully", "[ˈfruːtfəli]", "плодотворно"],
    ["guru", "[ˈɡuːˌruː]", "гуру, учитель, духовный учитель, духовный наставник, духовный руководитель"],
    ["highlight", "[ˈhaɪˌlaɪt]", "основной момент, световой эффект, выдвигать на первый план"],
    ["incomparable", "[ˌɪnˈkɑːmpərəbəl]", "несравненный, несравнимый, бесподобный"],
    ["incurable", "[ˌɪnˈkjʊrəbəl]", "неизлечимый, неисцелимый, неискоренимый, безнадежный больной"],
    ["indifferently", "[ɪnˈdɪfrəntli]", "равнодушно, безразлично, посредственно, скверно"],
    ["inexpensively", "[ˌɪnɪkˈspensɪvli]", "недорого"],
    ["innovative", "[ˈɪnəˌvetɪv]", "инновационный, новаторский, передовой"],
    ["intolerable", "[ˌɪnˈtɑːlərəbəl]", "невыносимый, недопустимый, нестерпимый, непереносимый, адский"],
    ["irregularly", "[ɪˈreɡjələrli]", "нерегулярно, неравномерно, беспорядочно"],
    ["meteorological", "[ˌmitiˌɔːrəˈlɑːdʒəkəl]", "метеорологический, атмосферический"],
    ["misspell", "[ˌmɪsˈspel]", "делать орфографические ошибки"],
    ["overnight", "[ˌoʊvəˈnaɪt]", "быстро, всю ночь, ночной, оставаться на ночь"],
    ["pollution", "[pəˈluːʃən]", "загрязнение, осквернение, поллюция"],
    ["realist", "[ˈriːˌlɪst]", "реалист, реалистический, реалистичный, трезвый"],
    ["realistically", "[ˌrɪəˈlɪstɪkli]", "реалистично"],
    ["restore", "[rəˈstɔːr]", "восстанавливать, восстанавливаться, реставрировать, возобновить, возвращать"],
    ["selfishness", "[ˈselfɪʃnəs]", "эгоизм, эгоистичность"],
    ["shoplifting", "[ˈʃɒplɪftɪŋ]", "шоплифтинг, магазинная кража, магазинная кража, кража в магазине"],
    ["starvation", "[starˈveɪʃən]", "голод, голодание, голодная смерть, истощение, голодовка"],
    ["trait", "[ˈtreɪt]", "особенность, штрих, характерная черта, стать"],
    ["unafraid", "[ˌʌnəˈfreɪd]", "бесстрашный, смелый, незапуганный"],
    ["unearth", "[ʌˈnɝːθ]", "раскапывать, извлекать, вырывать из земли, выгонять из норы, вырывать"],
    ["unequal", "[əˈniːkwəl]", "неравный, неравноценный, неровный, неадекватный, плохо подобранный"],
    ["variety", "[vəˈraɪəti]", "разнообразие, разнообразность, множество, ряд, сорт, разновидность, варьете, вид"],
    ["stress", "[ˈstres]", "стресс, напряжение, усилие, акцент, подчеркивать, ставить ударение"],
    ["respect", "[rəˈspekt]", "уважать, соблюдать, почитать, чтить, уважение, отношение, почтение, внимание"],
    ["accessible", "[ækˈsesəbəl]", "доступный, достижимый, податливый, поддающийся"],
    ["adverse", "[ædˈvɝːs]", "неблагоприятный, вредный, противоположный, враждебный, лежащий напротив"],
    ["alteration", "[ˌɒltəˈreɪʃən]", "изменение, перестройка, переделка, перемена, деформация, логическое сложение"],
    ["ambition", "[æmˈbɪʃən]", "амбиция, честолюбие, стремление, цель, домогаться чего-л."],
    ["barbarian", "[barˈberiən]", "варвар, варварский"],
    ["conversely", "[ˈkɑːnvərsli]", "наоборот, обратно, вспять"],
    ["e-book", "[ˈiː bʊk]", "электронная книга"],
    ["entertaining", "[ˌentərˈteɪnɪŋ]", "развлекательный, занимательный, забавный"],
    ["evil", "[ˈiːvəl]", "зло, бедствие, грех, вред, несчастье, порок, злой, дурной, зловещий, порочный, вредный"],
    ["harvest", "[ˈhɑːrvəst]", "урожай, сбор, жатва, плоды, собирать урожай, жать, связанный с урожаем"],
    ["intermediary", "[ˌɪntərˈmiːdieri]", "посредник, посреднический, промежуточный"],
    ["irresponsibly", "[ˌɪrɪˈspɒnsəbli]", "безответственно, несерьезно, легкомысленно"],
    ["longevity", "[lɔːnˈdʒevəti]", "долголетие, долговечность, долгожительство"],
    ["nerve", "[ˈnɝːv]", "нерв, нервы, нахальство, нервный, придавать силу"],
    ["radiation", "[ˌrediˈeɪʃən]", "излучение, радиация, облучение, радиационный, лучевой"],
    ["regarding", "[rəˈɡɑːrdɪŋ]", "относительно, о, касательно"],
    ["scornful", "[ˈskɔːrnfəl]", "презрительный, насмешливый"],
    ["self-evident", "[ˌselˈfevədənt]", "самоочевидный, очевидный, не требующий доказательств"],
    ["shatter", "[ˈʃætər]", "разрушать, поколебать, разбивать, обломок, осколок"],
    ["spectacular", "[spekˈtækjələr]", "захватывающий, эффектный, импозантный, эффектное зрелище"],
    ["suspiciously", "[səˈspɪʃəsli]", "с подозрением"],
    ["tedious", "[ˈtiːdiəs]", "утомительный, скучный, нудный, муторный"],
    ["toddler", "[ˈtɑːdlər]", "ползунок, ребенок, начинающий ходить"],
    ["tolerate", "[ˈtɑːləˌret]", "терпеть, допускать, быть толерантным, дозволять, выносить"],
    ["tribute", "[ˈtrɪbjuːt]", "дань, должное, награда, подношение, коллективный дар"],
    ["youngster", "[ˈjʌŋstər]", "мальчик, юноша, юнец, детеныш"],
    ["will", "[wɪl]", "воля, желание, завещание, волеизъявление, хотеть, желать, завещать, проявлять волю"],
    ["agitation", "[ˌædʒəˈteɪʃən]", "агитация, перемешивание, возбуждение, волнение, ажитация, беспокойство"],
    ["angle", "[ˈæŋɡəl]", "угол, уголок, угольник, угловой, удить, удить рыбу, искажать"],
    ["broadcasting", "[ˈbrɒdˌkæstɪŋ]", "вещание, трансляция, радиовещание, вещательный, широковещательный"],
    ["challenge", "[ˈtʃæləndʒ]", "вызов, проблема, сомнение, отвод, оспаривать, бросать вызов, вызывать"],
    ["improbable", "[ˌɪmˈprɑːbəbəl]", "невероятный, неправдоподобный"],
    ["phenomenal", "[fəˈnɑːmənəl]", "феноменальный, необыкновенный"],
    ["propaganda", "[ˌprɑːpəˈɡændə]", "пропаганда"],
    ["terminology", "[ˌtərməˈnɑːlədʒi]", "терминология"],
    ["unanimous", "[juːˈnænəməs]", "единогласный, единодушный"],
    ["unauthorized", "[ʌˈnɒθəˌraɪzd]", "неразрешенный, самовольный, неправомочный"],
    ["unresolved", "[ənrəˈzɑːlvd]", "неразрешенный, нерешительный, не принявший решения, не решившийся"],
    ["unsubscribe", "[ˌʌnsəbˈskraɪb]", "отписаться"],
    ["whichever", "[wɪtʃˈevr̩]", "какой, какой бы ни, какой угодно, какой бы ни"],
    ["live", "[ˈlaɪv]", "живой, под напряжением, житься, жить, дожить, в прямом эфире, прямо"],
    ["bear", "[ˈber]", "медведь, медведка, козел, нести, иметь, носить, переносить, терпеть"],
    ["amazingly", "[əˈmeɪzɪŋli]", "удивительно"],
    ["apparently", "[əˈperəntli]", "видимо, очевидно, явно, вероятно, по-видимому"],
    ["claim", "[ˈkleɪm]", "претензия, требование, иск, утверждение, утверждать, претендовать, требовать"],
    ["crown", "[ˈkraʊn]", "коронный, корона, крона, венец, коронка, увенчать, короновать"],
    ["curriculum", "[kəˈrɪkjələm]", "учебный план, курс обучения"],
    ["decade", "[deˈkeɪd]", "десятилетие, десяток, группа из десяти"],
    ["entrance", "[ˈentrəns]", "вход, въезд, выход, доступ, входной, вступительный, заворожить, восторгать"],
    ["evidently", "[ˈevədəntli]", "очевидно, видимо, явно, видно, несомненно, внешне, по внешнему виду, по-видимому"],
    ["lifelong", "[ˈlaɪˈflɒŋ]", "пожизненный, жизненный, продолжающийся всю жизнь"],
    ["likelihood", "[ˈlaɪklihʊd]", "вероятность, многообещающая будущность"],
    ["omit", "[oˈmɪt]", "пропускать, не включать, пренебрегать, упускать"],
    ["perform", "[pərˈfɔːrm]", "выполнять, исполнять, совершать, выступать, играть, проделать, представлять"],
    ["pilgrim", "[ˈpɪlɡrəm]", "паломник, странник, пилигрим, богомолец"],
    ["poisonous", "[ˌpɔɪzənəs]", "ядовитый, отвратительный, противный"],
    ["predictably", "[prəˈdɪktəbli]", "как и ожидалось, предсказуемо, с уверенностью, очевидно"],
    ["presumably", "[prəˈzuːməbli]", "предположительно, по-видимому"],
    ["prosper", "[ˈprɑːspər]", "процветать, преуспевать, благоденствовать, благоприятствовать"],
    ["reveal", "[rɪˈviːl]", "показывать, раскрывать, открывать, откровение, разоблачение, четверть"],
    ["theoretical", "[ˌθiəˈretɪkəl]", "теоретический, умозрительный, спекулятивный"],
    ["tourism", "[ˈtʊˌrɪzəm]", "туризм"],
    ["trained", "[ˈtreɪnd]", "обученный, тренированный, дрессированный, вышколенный, выученный"],
    ["Upper-Intermediate", "[ˈʌpər ˌɪntəˈmiːdiət]", "выше среднего (уровень)"],
    ["admission", "[ædˈmɪʃən]", "вход, признание, впуск, допущение, принятие, вступительный, пропускной, впускной"],
    ["cheerful", "[ˈtʃɪrfəl]", "веселый, бодрый, светлый, яркий, повеселеть"],
    ["complicate", "[ˈkɑːmpləˌket]", "усложнять, осложнять"],
    ["dispute", "[ˌdɪˈspjuːt]", "спор, разногласия, диспут, полемика, спорить, оспаривать, дискутировать"],
    ["foundation", "[faʊnˈdeɪʃən]", "основа, основы, фундамент, основание, обоснованность, учреждение, организация"],
    ["importantly", "[ˌɪmˈpɔːrtəntli]", "важно"],
    ["increasingly", "[ˌɪnˈkriːsɪŋli]", "все больше и больше"],
    ["junior", "[ˈdʒuːnjər]", "младший, юношеский, подчиненный, младший, юниор, подчиненный, сынок"],
    ["lab", "[ˈlæb]", "лаборатория"],
    ["largely", "[ˈlɑːrdʒli]", "в значительной степени, щедро, обильно, в широком масштабе, на широкую ногу"],
    ["minority", "[maɪˈnɔːrəti]", "меньшинство, несовершеннолетие, меньшая часть, меньшее число"],
    ["notwithstanding", "[ˌnɑːˌtwɪθˈstændɪŋ]", "несмотря на, вопреки, хотя, тем не менее, однако, тем не менее, однако"],
    ["odd", "[ˈɑːd]", "нечетный, странный, лишний, необычный, нечетное число, решающая взятка, удар"],
    ["paradoxical", "[ˌperəˈdɑːksɪkəl]", "парадоксальный, необычный"],
    ["paradoxically", "[ˌpærəˈdɒksɪkli]", "парадоксально"],
    ["peak", "[ˈpiːk]", "пик, максимум, вершина, пиковый, максимальный, чахнуть, слабеть"],
    ["permanent", "[ˈpɝːmənənt]", "постоянный, перманентный, бессменный, неизменный, долговременный, необратимый"],
    ["regrettably", "[rɪˈɡretəbli]", "прискорбно"],
    ["remarkable", "[rəˈmɑːrkəbəl]", "замечательный, выдающийся, удивительный"],
    ["revolutionise", "[ˌrevəˈluːʃənaɪz]", "революционизировать"],
    ["similarity", "[ˌsɪməˈlerəti]", "сходство, подобие"],
    ["ultimately", "[ˈʌltəmətli]", "в конечном счете, в конце концов"],
    ["undermine", "[ˈʌndərˌmaɪn]", "подрывать, разрушать, подкапывать, подмывать, минировать, делать подкоп"],
    ["undetermined", "[ˌʌndəˈtɝːmənd]", "неопределенный, нерешенный, нерешительный"],
    ["additionally", "[əˈdɪʃənəli]", "дополнительно, кроме того, сверх того"],
    ["admittedly", "[ædˈmɪtədli]", "по общему признанию, предположительно, по общему согласию"],
    ["broadly", "[ˈbrɒdli]", "широко"],
    ["conscience", "[ˈkɑːnʃəns]", "совесть, сознание"],
    ["consuming", "[kənˈsuːmɪŋ]", "потребление, потребляющий"],
    ["conviction", "[kənˈvɪkʃən]", "убеждение, убежденность, осуждение, уверенность, признание виновным"],
    ["curiously", "[ˈkjʊriəsli]", "странно, необычайно"],
    ["fabulous", "[ˈfæbjələs]", "невероятный, потрясающий, легендарный, баснословный, мифический"],
    ["fluent", "[ˈfluːənt]", "беглый, текучий, плавный, жидкий, гладкий, переменная величина"],
    ["founder", "[ˈfaʊndər]", "основатель, учредитель, основоположник, оседать, охрометь, разбивать ногу, падать"],
    ["funnily", "[ˈfʌnɪli]", "забавно, смешно, странно"],
    ["intentionally", "[ˌɪnˈtenʃənəli]", "умышленно"],
    ["irrespective", "[ɪrəˈspektɪv]", "независимый, безотносительный"],
    ["oddly", "[ˈɑːdli]", "странно"],
    ["token", "[ˈtoʊkən]", "знак, жетон, признак, примета, талон, символический, имеющий видимость"],
    ["undergo", "[ˌʌndərˈɡoʊ]", "подвергаться, испытывать, переносить, вытерпеть"],
    ["unintentionally", "[ˌʌˌnɪnˈtenʃənəli]", "нечаянно"],
    ["unite", "[juˈnaɪt]", "объединять, объединяться, соединить, сплотиться, соединять, соединяться, приобщать"],
    ["unstoppable", "[ʌnˈstɒpəbəl]", "необоримый, неодолимый, не могущий быть забитым, засоренным, неостанавливаемый"],
    ["applied", "[əˈplaɪd]", "прикладной, приложенный"],
    ["availability", "[əˌveləˈbɪləti]", "наличие, пригодность, полезность, годность"],
    ["brilliantly", "[ˈbrɪljəntli]", "блестяще, блистательно"],
    ["commerce", "[ˈkɑːmərs]", "торговля, коммерция, общение, оптовая торговля"],
    ["completion", "[kəmˈpliːʃən]", "завершение, окончание, заключение, отбывание наказания"],
    ["countless", "[ˈkaʊntləs]", "бесчисленный, несчетный, неисчислимый, бессчетный"],
    ["density", "[ˈdensəti]", "плотность, густота, удельный вес, скученность, компактность, тупость, глупость"],
    ["diversity", "[dɪˈvɝːsəti]", "разнообразие, многообразие, различие, разнородность, разновидность, несходство"],
    ["dramatically", "[drəˈmætɪkəli]", "драматично, ярко, театрально"],
    ["encounter", "[enˈkaʊntər]", "сталкиваться, наталкиваться, столкновение, схватка, стычка"],
    ["equality", "[ɪˈkwɑːləti]", "равенство, равноправие"],
    ["essence", "[ˈesəns]", "сущность, существо, эссенция, экстракт, аромат, существование, бензин, духи"],
    ["excursion", "[ɪkˈskɝːʒən]", "экскурсия, экскурс, поездка, прогулка, возвратно-поступательное движение"],
    ["explicit", "[ɪkˈsplɪsət]", "явный, точный, откровенный, подробный, определенный, явно заданный"],
    ["extinction", "[ɪkˈstɪŋkʃən]", "вымирание, угасание, гашение, прекращение, погашение, тушение, потухание"],
    ["forum", "[ˈfɔːrəm]", "форум, суд, собрание, свободная дискуссия, зал для совещаний"],
    ["healthily", "[ˈhelθɪli]", "полезно для здоровья"],
    ["misfortune", "[ˌmɪsˈfɔːrtʃən]", "несчастье, беда, неудача, злоключение"],
    ["pedagogy", "[ˈpedəˌɡodʒi]", "педагогика"],
    ["rehearsal", "[rəˈhɝːsəl]", "репетиция, повторение, перечисление, пересказ"],
    ["repetition", "[ˌrepəˈtɪʃən]", "повторение, повтор, репетиция, копия, заучивание наизусть, повторение наизусть"],
    ["sponsor", "[ˈspɑːnsər]", "спонсор, организатор, поручитель, финансировать, поддерживать"],
    ["sponsorship", "[ˈspɑːnsərˌʃɪp]", "спонсорство"],
    ["steadily", "[ˈstedɪli]", "устойчиво"],
    ["substantially", "[səbˈstænʃəli]", "по существу, в основном, в значительной степени, основательно, прочно"],
    ["subtle", "[ˈsʌtəl]", "тонкий, утонченный, нежный, неуловимый, хитрый, коварный, искусный, едва различимый"],
    ["triumph", "[ˈtraɪəmf]", "триумф, торжество, победа, восторжествовать, побеждать, ликовать"],
    ["understandably", "[ˌʌndərˈstændəbli]", "понятно, ясно"],
    ["unfavourable", "[ʌnˈfeɪvərəbəl]", "неблагоприятный, неприятный, неблагосклонный"],
    ["whatsoever", "[ˌhwʌtsoʊˈevər̩]", "что бы ни, любой, какой бы ни"],
    ["absurdly", "[əbˈsɝːdli]", "нелепо, абсурдно, глупо"],
    ["academy", "[əˈkædəmi]", "академия, школа, высшее учебное заведение, специальное учебное заведение"],
    ["altogether", "[ˌɒltəˈɡeðr̩]", "целое, в целом, совсем, всего, совершенно, вполне"],
    ["clarity", "[ˈklerəti]", "ясность, четкость, прозрачность, чистота, доходчивость"],
    ["clumsiness", "[ˈklʌmzinəs]", "неуклюжесть, неловкость, топорность"],
    ["coherence", "[ˌkoʊˈhɪərəns]", "согласованность, слаженность, связность, связь, сцепление"],
    ["cohesion", "[koʊˈhiːʒən]", "сплоченность, сцепление, связь, связность, сила сцепления"],
    ["colloquial", "[kəˈloʊkwɪəl]", "разговорный, нелитературный, разговорный язык"],
    ["colourless", "[ˈkʌlələs]", "бесцветный, бледный"],
    ["disappointing", "[ˌdɪsəˌpɔɪntɪŋ]", "разочаровывающий, неутешительный, вызывающий разочарование"],
    ["fanaticism", "[fəˈnætəˌsɪzəm]", "фанатизм, изуверство, фанатичность"],
    ["honorary", "[ˈɑːnəˌreri]", "почетный, неоплачиваемый"],
    ["inclination", "[ˌɪnkləˈneɪʃən]", "наклон, наклонность, наклонение, склонность, влечение, уклон, отклонение, склонение"],
    ["incomplete", "[ˌɪnkəmˈpliːt]", "неполный, незавершенный, незаконченный, несовершенный, дефектный"],
    ["innumerable", "[ˌɪˈnuːmərəbəl]", "бесчисленный, неисчислимый, несметный, бессчетный"],
    ["irreparable", "[ɪˈrepərəbəl]", "непоправимый"],
    ["outweigh", "[ˈaʊˌtwe]", "перевешивать, быть тяжелее, превосходить в весе, быть более влиятельным"],
    ["overshadow", "[ˈovərˈʃædoʊ]", "затмевать, омрачать, затенять, затемнять, бросать тень"],
    ["overstep", "[ˌoʊvəˈstep]", "переступить, перешагнуть, переходить границы"],
    ["phrasal", "[ˈfreɪzəl]", "фразовый"],
    ["preferable", "[ˈprefərəbəl]", "предпочтительный"],
    ["prepay", "[priˈpeɪ]", "предоплата, платить вперед, франкировать"],
    ["protective", "[prəˈtektɪv]", "защитный, охранный, покровительственный, оградительный, защитительный"],
    ["recession", "[rəˈseʃən]", "рецессия, спад, падение, снижение, удаление, углубление, отступание, понижение, уход"],
    ["renewable", "[riˈnuːəbəl]", "возобновляемый"],
    ["researcher", "[ˈriːsərtʃər]", "исследователь"],
    ["revival", "[rɪˈvaɪvəl]", "возрождение, оживление, восстановление, возобновление, возрождение веры"],
    ["revive", "[rɪˈvaɪv]", "возрождать, оживлять, оживать, восстанавливать, воскрешать, возобновлять"],
    ["scrupulously", "[ˈskruːpjələsli]", "скрупулезно, тщательно, добросовестно"],
    ["stagnation", "[stægˈneɪʃən]", "стагнация, застой, косность, загнивание, застой крови"],
    ["storyteller", "[ˈstɔːriˌtelər]", "рассказчик, сказочник, сказитель, выдумщик, сочинитель, автор рассказов, лгун"],
    ["straighten", "[ˈstreɪtən]", "выпрямлять, выпрямляться, приводить в порядок, выправляться, выправлять"],
    ["supplementary", "[ˌsʌpləˈmentəri]", "дополнительный, добавочный"],
    ["trade", "[ˈtreɪd]", "торговля, сделка, торговать, обменивать, торговый, товарный"],
    ["tranquil", "[ˈtræŋkwəl]", "спокойный, тихий, невозмутимый, уравновешенный"],
    ["unfit", "[ənˈfɪt]", "негодный, неподходящий, нездоровый, делать непригодным"],
    ["unhealthy", "[ʌnˈhelθi]", "нездоровый, вредный, больной, болезненный, опасный"],
    ["unprotected", "[ˌʌnprəˈtektəd]", "незащищенный, беззащитный, открытый"],
    ["unsafe", "[ʌnˈseɪf]", "опасный, ненадежный"],
    ["accordance", "[əˈkɔːrdəns]", "соответствие, согласие, предоставление, гармония"],
    ["astonishingly", "[əˈstɒnɪʃɪŋli]", "удивительно"],
    ["boastful", "[ˈboʊstfəl]", "хвастливый"],
    ["bureaucratic", "[ˌbjʊrəˈkrætɪk]", "бюрократический, чиновнический"],
    ["co-founder", "[kəʊ ˈfaʊndə]", "сооснователь"],
    ["comprehension", "[ˌkɑːmpriˈhenʃən]", "понимание, осмысление, постижение, охват, разумение, понятливость, включение"],
    ["comprehensive", "[ˌkɑːmpriˈhensɪv]", "всесторонний, исчерпывающий, обширный, общеобразовательная школа"],
    ["diligently", "[ˈdɪlədʒəntli]", "старательно, усердно, прилежно"],
    ["disheartening", "[ˌdɪsˈhɑːrtənɪŋ]", "удручающий"],
    ["disrespectful", "[ˌdɪsrəˈspektfəl]", "непочтительный, невежливый"],
    ["disrespectfully", "[ˌdɪsrɪˈspektfəli]", "непочтительно, невежливо, дерзко"],
    ["eradication", "[ɪˌrædəˈkeɪʃən]", "искоренение, уничтожение"],
    ["excessive", "[ɪkˈsesɪv]", "чрезмерный, излишний, безмерный, эксцессив, чрезмерная степень"],
    ["fashionably", "[ˈfæʃənəbli]", "модно, фешенебельно"],
    ["greatness", "[ˈɡreɪtnəs]", "величие, величина, сила"],
    ["indescribable", "[ˌɪndəˈskraɪbəbəl]", "неописуемый"],
    ["insensitive", "[ˌɪnˈsensətɪv]", "нечувствительный, невосприимчивый, равнодушный, лишенный чувствительности"],
    ["insignificance", "[ˌɪnˌsɪgˈnjɪfɪkəns]", "незначительность, ничтожность, маловажность, бессодержательность"],
    ["installation", "[ˌɪnstəˈleɪʃən]", "установка, оборудование, сборка, сооружение, настройка, проводка, вселение"],
    ["irreversible", "[ˌɪrɪˈvɝːsəbəl]", "необратимый, нереверсивный, неотменяемый, непреложный, нерушимый"],
    ["irritably", "[ˈɪrɪtəbli]", "раздражённо"],
    ["irritation", "[ˌɪrəˈteɪʃən]", "раздражение, возбуждение, гнев"],
    ["linguist", "[ˈlɪŋɡwəst]", "лингвист, языковед"],
    ["mismanagement", "[ˌmɪsˈmænɪdʒmənt]", "бесхозяйственность, плохое управление"],
    ["murmur", "[ˈmɝːmər]", "роптать, бормотать, шептать, журчать, ропот, журчание, шепот, ворчание, шорох"],
    ["presence", "[ˈprezəns]", "присутствие, наличие, явка, общество, внешний вид, осанка, соседство"],
    ["reconsider", "[ˌrikənˈsɪdər]", "пересматривать, реконструировать"],
    ["respectful", "[rəˈspektfəl]", "почтительный, уважающий, вежливый"],
    ["romantically", "[rəʊˈmæntɪkli]", "романтически, романтично"],
    ["shareholder", "[ˈʃeəhoʊldə]", "акционер, держатель акций, пайщик"],
    ["sorrow", "[ˈsɑːroʊ]", "горе, скорбь, печаль, грусть, сожаление, печалиться, горевать, скорбеть"],
    ["stabilize", "[ˈsteɪbəˌlaɪz]", "стабилизировать, стабилизироваться, делать устойчивым"],
    ["suspect", "[səˈspekt]", "подозреваемый, подозревать, полагать, подозреваемый человек"],
    ["uncertainty", "[ʌnˈsɝːtənti]", "неопределенность, неуверенность, сомнения, нерешительность, изменчивость"],
    ["uninstall", "[ˌʌnɪnˈstɔːl]", "удалить (программу)"],
    ["unmanageable", "[ʌnˈmænɪdʒəbəl]", "трудный, непокорный"],
    ["unsuitable", "[ʌnˈsuːtəbəl]", "неподходящий, неподобающий"],
    ["visibility", "[ˌvɪzəˈbɪləti]", "видимость, обзор"],
    ["well-balanced", "[wel ˈbælənst]", "уравновешенный, выдержанный, рассудительный, гармонический, симметричный"],
    ["well-organized", "[ˌwel ˈɔːɡənaɪzd]", "хорошо организованный, благоустроенный"],
    ["well-qualified", "[wel ˈkwɒlɪfaɪd]", "высококвалифицированный"],
    ["anti-virus", "[ˌænti ˈvaɪrəs]", "антивирус"],
    ["beggar", "[ˈbeɡər]", "нищий, попрошайка, бедняк, парень, малый, разорять, превосходить"],
    ["clarification", "[ˌklerəfɪˈkeɪʃən]", "пояснение, прояснение, осветление, очищение, очистка"],
    ["continuation", "[kənˌtɪnjuːˈeɪʃən]", "продолжение, возобновление"],
    ["encouragement", "[enˈkɜːrɪdʒmənt]", "поощрение, ободрение"],
    ["faithfulness", "[ˈfeɪθflnəs]", "верность, лояльность"],
    ["fantasy", "[ˈfæntəsi]", "фантазия, фантастика, воображение, импровизировать, представлять себе"],
    ["fist", "[ˈfɪst]", "кулак, рука, почерк, ударить кулаком, зажимать в руке"],
    ["immoral", "[ˌɪˈmɔːrəl]", "аморальный, безнравственный, распутный, распущенный"],
    ["imperfect", "[ˌɪmˈpɝːfɪkt]", "несовершенный, неполный, дефектный, имперфект"],
    ["improper", "[ˌɪmˈprɑːpər]", "неправильный, неподходящий, неуместный, неприличный, непристойный, ложный"],
    ["imprudent", "[ˌɪmˈpruːdənt]", "неосторожный, неблагоразумный, неосмотрительный, опрометчивый"],
    ["irrational", "[ɪˈræʃənəl]", "иррациональный, нерациональный, иррациональное число"],
    ["irregularity", "[ɪˌreɡjəˈlerəti]", "неравномерность, неправильность, неровность, беспорядочность, распущенность"],
    ["modernize", "[ˈmɑːdərˌnaɪz]", "модернизировать, осовременить"],
    ["novelty", "[ˈnɑːvəlti]", "новизна, новинка, новинки, новшество, новость, нововведение, обнова"],
    ["participation", "[parˌtɪsəˈpeɪʃən]", "участие, соучастие"],
    ["publication", "[ˌpʌblɪˈkeɪʃən]", "публикация, издание, опубликование, оглашение, гласность"],
    ["retake", "[ˈriːˈteɪk]", "пересдавать, снова взять, пересъемка, переснятый кадр"],
    ["self-esteem", "[self ɪˈstiːm]", "самоуважение, чувство собственного достоинства, уважение к себе, самолюбие"],
    ["simplify", "[ˈsɪmpləˌfaɪ]", "упростить, упрощать"],
    ["software", "[ˈsɒfˌtwer]", "программное обеспечение, программные системы, программы для компьютера"],
    ["staggered", "[ˈstæɡəd]", "ошеломлён"],
    ["stubbornness", "[ˈstʌbərnnəs]", "упрямство, упорство"],
    ["unambitious", "[ˌʌnæmˈbɪʃəs]", "нечестолюбивый, непритязательный, нечистолюбивый"],
    ["well-developed", "[ˌwel dɪˈveləpt]", "развитой, хорошо развитый, высокого технического уровня"],
    ["width", "[ˈwɪdθ]", "ширина, расстояние, широта, полоса, мощность, полотнище, пролет"],
    ["birth", "[ˈbɝːθ]", "рождение, роды, начало, происхождение, нарождение, источник"],
    ["heaven", "[ˈhevən]", "небо, небеса, бог, провидение"],
    ["iceberg", "[ˈaɪsbərɡ]", "айсберг, ледяная гора, бесчувственный человек, холодный человек"],
    ["lesser", "[ˈlesər]", "меньший, малый, небольшой, незначительный, ничтожный, ограниченный, мелочный"],
    ["single-room", "[ˈsɪŋɡəl ruːm]", "одноместная комната"],
    ["spirit", "[ˈspɪrət]", "дух, спирт, душа, настроение, спиртовой, спиритический, воодушевлять"],
    ["tape", "[ˈteɪp]", "лента, тесьма, рулетка, волокита, записывать на пленку"],
    ["beans", "[biːnz]", "бобы / фасоль"],
    ["blessing", "[ˈblesɪŋ]", "благословение, благо, счастье, блаженство, благодеяние, молитва"],
    ["bull", "[ˈbʊl]", "бык, самец, бугай, буйвол, бычий"],
    ["clumsy", "[ˈklʌmzi]", "неуклюжий, неловкий, неповоротливый, топорный, грубый, мешковатый, аляповатый"],
    ["disguise", "[ˌdɪˈsɡaɪz]", "маскировка, сокрытие, переодевание, маска, скрывать, маскировать, переодевать"],
    ["dump", "[ˈdəmp]", "свалка, отвал, сбрасывать, сваливать, коренастый, самосвальный"],
    ["fiddle", "[ˈfɪdəl]", "скрипка, надувательство, играть, играть на скрипке, обманывать"],
    ["grip", "[ˈɡrɪp]", "захват, ручка, рукоятка, власть, контроль, захватывать, схватить, сжимать, зажимать"],
    ["master", "[ˈmæstr̩]", "мастер, хозяин, магистр, главный, основной, ведущий, управлять, владеть"],
    ["rag", "[ˈræɡ]", "тряпка, ветошь, лоскут, тряпичный, тряпочный, дразнить, разыгрывать"],
    ["riches", "[ˈrɪtʃəz]", "богатство, богатства, сокровища, обилие, наваристый"],
    ["spur", "[ˈspɝː]", "шпора, стимул, побуждение, побуждать, подстрекать, пришпоривать"],
    ["swing", "[ˈswɪŋ]", "качели, свинг, поворот, качание, колебание, качаться, качать, размахивать"],
    ["tighten", "[ˈtaɪtən]", "затянуть, сжимать, сжиматься, натягивать, натягиваться, стягиваться, уплотнять"],
    ["credit", "[ˈkredət]", "кредитный, кредит, честь, зачет, доверие, кредитовать, верить"],
    ["glasses", "[ˈɡlæsəz]", "очки"],
    ["spoon", "[ˈspuːn]", "ложка, блесна, желонка, недотепа, черпать ложкой, любезничать"],
    ["act", "[ˈækt]", "закон, акт, действие, поступок, дело, деяние, действовать, поступать, работать, играть"],
    ["salmon", "[ˈsæmən]", "лосось, семга, лососина, лососевый, цвета сомон"],
    ["cooker", "[ˈkʊkər]", "плита, печь, кастрюля, сорт фруктов, годный для варки, тот, кто подделывает"],
    ["suitcase", "[ˈsuːtˌkes]", "чемодан, небольшой плоский чемодан"],
    ["subway", "[ˈsʌˌbwe]", "метро, метрополитен, подземка, подземный переход, тоннель"],
    ["elections", "[ɪˈlekʃənz]", "выборы"],
    ["rubber", "[ˈrʌbər]", "резиновый, прорезиненный, резина, каучук, резинка, любопытствовать"],
    ["alike", "[əˈlaɪk]", "подобный, одинаковый, похожий, подобно, одинаково, точно так же"],
    ["administration", "[ædˌmɪnəˈstreɪʃən]", "администрация, управление, администрирование, правительство, назначение"],
    ["petrol", "[ˈpeˌtrol]", "бензин, нефть, горючее, бензиновый, чистить бензином"],
    ["frame", "[ˈfreɪm]", "рама, кадр, рамка, каркас, рамочный, создавать, обрамлять"],
    ["guard", "[ˈɡɑːrd]", "гвардия, охрана, стража, охранять, караулить, сторожевой, караульный"],
    ["opera", "[ˈɑːprə]", "опера, оперное искусство"],
    ["sauce", "[ˈsɒs]", "соус, подливка, приправа, фруктовое пюре, приправлять соусом, дерзить"],
    ["stomach", "[ˈstʌmək]", "желудок, живот, аппетит, мужество, стерпеть, сносить"],
    ["section", "[ˈsekʃən]", "раздел, секция, часть, сечение, разрез, подразделять, делить на части"],
    ["dolphin", "[ˈdɑːlfɪn]", "дельфин, швартовый пал, деревянный кранец, свайный куст, дельфин-белобочка"],
    ["blanket", "[ˈblæŋkət]", "одеяло, покров, общий, полный, охватывать, заглушать"],
    ["combination", "[ˌkɑːmbəˈneɪʃən]", "комбинация, сочетание, соединение, объединение, совмещение, союз"],
    ["swimsuit", "[ˈswɪmsuːt]", "купальник"],
    ["deaf", "[ˈdef]", "глухой, глуховатый, тугой на ухо, отказывающийся слушать"],
    ["chef", "[ˈʃef]", "главный повар, шеф-повар"],
    ["heroic", "[hɪˈroʊɪk]", "героический, геройский, богатырский, напыщенный язык, высокопарный язык"],
    ["cupboard", "[ˈkʌbərd]", "шкаф, буфет, чулан, стенной шкаф"],
    ["pilot", "[ˈpaɪlət]", "пилот, летчик, лоцман, контрольный, пробный, пилотировать, вести"],
    ["quiz", "[ˈkwɪz]", "викторина, опрос, экзамен, насмешник, шутка, насмехаться, подшучивать"],
    ["digital", "[ˈdɪdʒətəl]", "цифровой, пальцевидный, пальцеобразный, клавиша, палец, перст"],
    ["board", "[ˈbɔːrd]", "совет, борт, доска, правление, питание, садиться, столоваться, лавировать"],
    ["sunglasses", "[ˈsʌnˌɡlæsəz]", "темные очки"],
    ["costume", "[kaˈstuːm]", "костюм, платье, одежда, костюмированный, одевать, снабжать одеждой"],
    ["queen", "[ˈkwiːn]", "королева, ферзь, царица, дама, богиня, педик, царить, быть королевой, править"],
    ["represent", "[ˌreprəˈzent]", "представлять, изображать, отражать, представление"],
    ["mouse", "[ˈmaʊs]", "мышь, подбитый глаз, выслеживать, выискивать, ловить мышей"],
    ["gun", "[ˈɡən]", "пистолет, пушка, оружие, пушечный, оружейный, стрелять, охотиться"],
    ["chemist", "[ˈkeˌmɪst]", "химик, аптекарь"],
    ["leader", "[ˈliːdər]", "лидер, руководитель, вождь, глава, ведущий, предводитель, вожак, командир, полководец"],
    ["borrow", "[ˈbɑːˌroʊ]", "заимствовать, занимать, брать на время, залог, отрицательный перенос"],
    ["shampoo", "[ʃæmˈpuː]", "шампунь, мытье головы, мыть голову, массировать"],
    ["bee", "[ˈbiː]", "пчела, пчелка, встреча, помощь соседям"],
    ["pepper", "[ˈpepər]", "перец, живость, едкость, перцовый, поперчить, перчить, усыпать"],
    ["keyboard", "[ˈkiːˌbɔːrd]", "клавиатура, клавишная панель, набирать на клавиатуре"],
    ["bother", "[ˈbɑːðr̩]", "беспокоить, беспокоиться, надоедать, беспокойство, хлопоты"],
    ["scanner", "[ˈskænər]", "сканер, сканирующее устройство, развертывающее устройство"],
    ["aloud", "[əˈlaʊd]", "вслух, громко, ощутимо, громким голосом, заметно, сильно"],
    ["raincoat", "[ˈreɪnkoʊt]", "дождевик, непромокаемый плащ, непромокаемое пальто"],
    ["anywhere", "[ˈeniˌwer]", "где угодно, везде, где-нибудь, никуда, где-либо, куда угодно, куда-нибудь, куда-либо"],
    ["calendar", "[ˈkæləndər]", "календарь, летоисчисление, список, святцы, регистрировать, инвентаризировать"],
    ["yoghurt", "[ˈjoʊɡr̩t]", "йогурт"],
    ["wing", "[ˈwɪŋ]", "крыло, фланг, флигель, крайний нападающий, лететь, ранить, ускорять"],
    ["scene", "[ˈsiːn]", "сцена, происшествие, эпизод, место происшествия, картина, место действия, пейзаж"],
    ["watermelon", "[ˈwɒtərˌmelən]", "арбуз"],
    ["concept", "[ˈkɑːnsept]", "концепция, понятие, идея, общее представление"],
    ["butcher", "[ˈbʊtʃər]", "мясник, палач, убийца, разносчик в поезде, безжалостно убивать, портить, бить"],
    ["purple", "[ˈpɝːpəl]", "фиолетовый, пурпурный, пурпур, фиолетовый цвет, багроветь"],
    ["pedestrian", "[pəˈdestriən]", "пешеходный, пеший, скучный, прозаический, пешеход"],
    ["police station", "[pəˈliːs ˈsteɪʃən]", "полицейский участок"],
    ["branch", "[ˈbræntʃ]", "отделение, филиал, ветвь, боковой, вспомогательный, разветвляться"],
    ["second-hand", "[ˈsekənd hænd]", "подержанный, букинистический"],
    ["manufacturer", "[ˌmænjəˈfæktʃərər]", "производитель, изготовитель, фабрикант, промышленник, предприниматель, заводчик"],
    ["shore", "[ˈʃɔːr]", "побережье, подпорка, подкос, подпирать, береговой"],
    ["occupy", "[ˈɑːkjəˌpaɪ]", "занимать, оккупировать, захватывать, завладеть, арендовать, завладевать"],
    ["essentially", "[əˈsenʃəli]", "по существу, существенным образом"],
    ["hunter", "[ˈhʌntər]", "охотник, ловец, искатель, охотничья собака, гунтер, карманные часы с крышкой"],
    ["consultant", "[kənˈsəltənt]", "консультант"],
    ["rhythm", "[ˈrɪðəm]", "ритм, ритмичность, размер, цикличность, периодичность"],
    ["soul", "[soʊl]", "душа, дух, сердце, человек, сущность, воплощение, энергия, энтузиазм, образец"],
    ["nose", "[noʊz]", "нос, носик, аромат, обоняние, носовой, головной, совать нос, нюхать, чуять"],
    ["split", "[ˈsplɪt]", "раскол, расщепление, расколотый, расщепленный, расщеплять, расщепляться"],
    ["poetry", "[ˈpoʊɪtri]", "поэзия, стихи, поэтичность"],
    ["captain", "[ˈkæptən]", "капитан, капитан команды, полководец, руководить, быть капитаном корабля"],
    ["politician", "[ˌpɑːləˈtɪʃən]", "политик, политикан, государственный деятель"],
    ["potato", "[pəˈteɪˌtoʊ]", "картофель, картошка, картофелина, дырка, картофельный"],
    ["leadership", "[ˈliːdərˌʃɪp]", "руководство, лидерство, руководящая роль, превосходство, предводительство"],
    ["pregnancy", "[ˈpreɡnənsi]", "беременность, богатство, содержательность, чреватость"],
    ["satellite", "[ˈsætəˌlaɪt]", "спутник, сателлит, искусственный спутник, приспешник, приверженец"],
    ["expansion", "[ɪkˈspænʃən]", "расширение, экспансия, рост, распространение, раскрытие, растяжение, подъем"],
    ["recall", "[ˈriːˌkɒl]", "отзыв, память, отозвание, воспоминание, вспоминать, напоминать, отзывать"],
    ["proposed", "[prəˈpoʊzd]", "предложенный"],
    ["bowl", "[boʊl]", "чаша, миска, шар, чашка, ваза, таз, кубок, катить, катиться, играть в шары"],
    ["substance", "[ˈsʌbstəns]", "вещество, субстанция, содержание, сущность, суть, материя, имущество, состояние"],
    ["consume", "[kənˈsuːm]", "потреблять, поглощать, расходовать, съедать, пожирать, уничтожать, истреблять"],
    ["prompt", "[ˈprɑːmpt]", "быстрый, подсказывать, быстро, точно, ровно, подсказка"],
    ["attorney", "[əˈtɝːni]", "адвокат, прокурор, поверенный, юрист, поверенный"],
    ["coalition", "[ˌkoʊəˈlɪʃən]", "коалиция, объединение, союз, коалиционный"],
    ["fighter", "[ˈfaɪtər]", "истребитель, боец, борец, вояка, самолет-истребитель"],
    ["union", "[ˈjuːnjən]", "союз, объединение, соединение, единение, союзный"],
    ["provision", "[prəˈvɪʒən]", "положение, обеспечение, условие, снабжать продовольствием"],
    ["revenue", "[ˈrevəˌnuː]", "доход, годовой доход"],
    ["bean", "[ˈbiːn]", "боб, фасоль, башка, голова, фасолевый, дать по башке"],
    ["opening", "[ˈoʊpənɪŋ]", "открытие, отверстие, вскрытие, начало, открывающий, первый, начальный"],
    ["settle", "[ˈsetəl]", "скамья, скамья-ларь, поселить, решать, поселиться, селиться"],
    ["fundamental", "[ˌfʌndəˈmentəl]", "основной, фундаментальный, основы, принцип, основной тон"],
    ["grave", "[ˈɡreɪv]", "могила, смерть, серьезный, тяжелый, важный, вырезывать, гравировать"],
    ["abuse", "[əˈbjuːs]", "злоупотребление, оскорбление, брань, злоупотреблять, браниться, оскорблять"],
    ["editor", "[ˈedətər]", "редактор, монтажер, автор передовиц"],
    ["violent", "[ˈvaɪələnt]", "насильственный, сильный, яростный, неистовый, вспыльчивый, интенсивный, страстный"],
    ["boundary", "[ˈbaʊndəri]", "граница, черта, межа, пограничный"],
    ["grain", "[ˈɡreɪn]", "зерно, зернистость, зернышко, гран, волокно, раздроблять, красить под дерево"],
    ["pile", "[ˈpaɪl]", "куча, свая, груда, стопка, ворс, штабель, ворох, нагружать, заваливать, навалить"],
    ["coal", "[koʊl]", "уголь, каменный уголь, каменноугольный, грузить углем"],
    ["pip", "[ˈpɪp]", "типун, зернышко, очко, косточка, подстрелить, провалить, пищать, чирикать"],
    ["consciousness", "[ˈkɑːnʃəsnəs]", "сознание, самосознание, сознательность, понимание, память"],
    ["invasion", "[ˌɪnˈveɪʒən]", "вторжение, нашествие, инвазия, посягательство, набег"],
    ["negotiate", "[nɪˈɡoʊʃɪeɪt]", "вести переговоры, договариваться, преодолевать, обсуждать условия, реализовать"],
    ["transformation", "[ˌtrænsfərˈmeɪʃən]", "преобразование, трансформация, превращение, преображение, метаморфоза"],
    ["elect", "[əˈlekt]", "избранный, лучший, избранные, избранник, избирать, выбирать"],
    ["index", "[ˈɪndeks]", "индекс, показатель, указатель, снабжать указателем"],
    ["tail", "[ˈteɪl]", "хвост, конец, хвостовой, задний, выслеживать"],
    ["telescope", "[ˈtelɪskoʊp]", "телескоп, оптическая подзорная труба, складывать, складываться, сжимать"],
    ["impose", "[ɪmˈpoʊz]", "навязать, навязаться, навязывать, налагать, облагать, облагаться, верстать"],
    ["evaluation", "[ɪˌvæljuːˈeɪʃən]", "оценка, определение качества, определение количества"],
    ["script", "[ˈskrɪpt]", "сценарий, подлинник, почерк, писать сценарий"],
    ["leaf", "[ˈliːf]", "лист, створка, листва, листовой, лиственный, перелистывать, листать"],
    ["label", "[ˈleɪbəl]", "этикетка, метка, ярлык, маркировка, бирка, помечать, метить, наклеивать ярлыки"],
    ["agricultural", "[ˌæɡrəˈkəltʃərəl]", "сельскохозяйственный, земледельческий"],
    ["loose", "[ˈluːs]", "свободный, сыпучий, свободно, освобождать, выход, проявление"],
    ["origin", "[ˈɔːrɪdʒɪn]", "происхождение, начало, источник, возникновение, первоисточник, первопричина"],
    ["formation", "[fɔːrˈmeɪʃən]", "формирование, образование, формация, создание, строй, построение, составление"],
    ["joint", "[ˌdʒɔɪnt]", "совместный, объединенный, соединение, сустав, шарнир, соединять, сочленять"],
    ["compose", "[kəmˈpoʊz]", "составлять, составить, сочинять, создавать, сочинять музыку, состоять, набирать"],
    ["frequency", "[ˈfriːkwənsi]", "частота, частотность, частое повторение, частотный"],
    ["representative", "[ˌreprəˈzentətɪv]", "представитель, уполномоченный, делегат, представительный, представляющий"],
    ["selection", "[səˈlekʃən]", "выбор, отбор, подбор, селекция, подборка, набор, выборка"],
    ["corn", "[ˈkɔːrn]", "кукуруза, зерно, мозоль, кукурузный, зерновой, зернить, гранулировать"],
    ["chart", "[ˈtʃɑːrt]", "диаграмма, график, схема, карта, таблица, составлять карту, наносить на карту"],
    ["motor", "[ˈmoʊtə]", "двигатель, мотор, моторный, автомобильный, ехать на автомобиле"],
    ["activist", "[ˈæktɪvəst]", "активист, сторонник активных мер"],
    ["downtown", "[ˈdaʊntaʊn]", "в деловой части города, деловая часть города"],
    ["raw", "[ˈrɑː]", "сырой, необработанный, сырье, больное место, сдирать кожу"],
    ["adviser", "[ædˈvaɪzər]", "советник, консультант, советчик"],
    ["manufacturing", "[ˌmænjəˈfæktʃərɪŋ]", "производство, производственный, промышленный"],
    ["enable", "[eˈneɪbəl]", "разрешать, давать возможность, облегчать, делать возможным"],
    ["protein", "[ˈproʊtiːn]", "белок, протеин"],
    ["embrace", "[emˈbreɪs]", "объятие, объятия, охватывать, обнимать, обниматься"],
    ["decline", "[dɪˈklaɪn]", "снижение, спад, падение, упадок, уменьшение, уменьшаться, отказывать, отказываться"],
    ["headline", "[ˈhedˌlaɪn]", "заголовок, новости одной строкой, озаглавливать"],
    ["destruction", "[dəˈstrəkʃən]", "уничтожение, разрушение, деструкция, разорение, причина гибели"],
    ["reference", "[ˈrefərəns]", "ссылка, справка, упоминание, эталон, снабжать текст ссылками, справляться"],
    ["deer", "[ˈdɪr]", "олень, лань, красный зверь, красно-коричневый цвет"],
    ["scholar", "[ˈskɑːlər]", "ученый, знаток, стипендиат, ученик, грамотей, учащийся, филолог-классик"],
    ["tragedy", "[ˈtrædʒədi]", "трагедия, трагизм, трагичность, трагическое событие, трагическая ситуация"],
    ["commander", "[kəˈmændər]", "командир, командующий, начальник, командующий"],
    ["minister", "[ˈmɪnəstər]", "министр, священник, слуга, посланник, служить, прислуживать, помогать"],
    ["symbol", "[ˈsɪmbəl]", "символ, знак, обозначение, эмблема, идентификатор"],
    ["coach", "[koʊtʃ]", "тренер, автобус, карета, вагон, экипаж, тренировать, натаскивать, репетировать"],
    ["holy", "[ˈhoʊli]", "святой, священный, праведный, безгрешный, святыня, святилище"],
    ["journal", "[ˈdʒɝːnəl]", "журнал, дневник, газета, цапфа, ведомости, шейка вала"],
    ["gently", "[ˈdʒentli]", "осторожно, мягко, нежно, тихо, спокойно, кротко, умеренно"],
    ["contemporary", "[kənˈtempəˌreri]", "современный, одновременный, одной эпохи, современник, ровесник, сверстник"],
    ["administrator", "[ədˈmɪnəˌstretər]", "администратор, управляющий, должностное лицо, опекун, экзекутор"],
    ["root", "[ˈruːt]", "коренной, основной, корень, корнеплод, корениться, укореняться"],
    ["column", "[ˈkɑːləm]", "колонка, колонна, столбец, столб, графа, столбик, стойка, строй кильватера"],
    ["naked", "[ˈneɪkəd]", "голый, обнаженный, открытый, нагой, незащищенный, явный, голословный, лишенный"],
    ["strategic", "[strəˈtiːdʒɪk]", "стратегический, оперативный"],
    ["regime", "[rəˈʒiːm]", "режим, строй, диета"],
    ["celebration", "[ˌseləˈbreɪʃən]", "празднование, торжества, чествование, церковная служба"],
    ["mixture", "[ˈmɪkstʃər]", "смесь, смешивание, микстура, месиво, смешивать, мешать"],
    ["carrier", "[ˈkæriər]", "перевозчик, носитель, несущий, курьер, держатель, багажник, авианосец, транспортер"],
    ["county", "[ˈkaʊnti]", "графство, округ, жители графства, окружной, относящийся к графству"],
    ["entry", "[ˈentri]", "вход, вступление, запись, въезд, вхождение, входной, въездной"],
    ["fate", "[ˈfeɪt]", "судьба, участь, рок, удел, жребий, гибель, предопределять"],
    ["grocery", "[ˈɡroʊsəri]", "продуктовый, бакалейный, бакалея, бакалейная лавка"],
    ["institution", "[ˌɪnstəˈtuːʃən]", "учреждение, институт, организация, заведение, учебное заведение, ведомство"],
    ["stock", "[ˈstɑːk]", "запас, акции, шток, фонд, фондовый, шаблонный, снабжать, заготовить"],
    ["crane", "[ˈkreɪn]", "кран, журавль, сифон, кран грузоподъемный, вытягивать шею, поднимать краном"],
    ["prime", "[ˈpraɪm]", "простое число, начало, грунтовать, заправлять, основной, главный"],
    ["blade", "[ˈbleɪd]", "лезвие, лопасть, диск, клинок, полотно, перо, клинковый"],
    ["patch", "[ˈpætʃ]", "пластырь, заплата, пятно, клочок, накладка, латать, ставить заплаты"],
    ["negotiation", "[nɪˌɡoʊʃɪˈeɪʃən]", "переговоры, согласование, преодоление, обсуждение условий, переуступка"],
    ["threaten", "[ˈθretən]", "угрожать, грозить, пригрозить, замахиваться, припугивать, предвещать"],
    ["priest", "[ˈpriːst]", "священник, жрец, посвящать в священники"],
    ["nowhere", "[ˈnoʊweə]", "нигде, никуда, некуда, негде"],
    ["intervention", "[ˌɪntərˈventʃən]", "вмешательство, интервенция, посредничество"],
    ["chairman", "[ˈtʃermən]", "председатель, председательствовать"],
    ["oppose", "[əˈpoʊz]", "выступать против, противиться, сопротивляться, противопоставлять"],
    ["explosion", "[ɪkˈsploʊʒən]", "взрыв, вспышка, быстрый рост"],
    ["reflection", "[rəˈflekʃən]", "отражение, размышление, рефлексия, отблеск, отсвет, раздумье, обдумывание, образ, тень"],
    ["industrial", "[ˌɪnˈdəstriəl]", "промышленный, производственный, промышленник, промышленный рабочий"],
    ["crop", "[ˈkrɑːp]", "обрезать, давать урожай, подстригать, культура, урожай, посев, зоб, масса, жатва"],
    ["rating", "[ˈreɪtɪŋ]", "рейтинг, оценка, номинал, класс, номинальная мощность, нормирование, разряд, ранг"],
    ["partnership", "[ˈpɑːrtnərˌʃɪp]", "сотрудничество, товарищество, компания, участие, компаньоны"],
    ["trace", "[ˈtreɪs]", "след, черта, отпечаток, постромка, следить, прослеживаться, прослеживать"],
    ["recruit", "[rəˈkruːt]", "вербовать, завербовать, комплектовать, рекрут, новобранец, новичок, призывник"],
    ["aid", "[ˈeɪd]", "помощь, пособия, поддержка, содействие, помогать, способствовать"],
    ["coverage", "[ˈkʌvərədʒ]", "охват, зона действия, прикрытие, освещение в печати, зона наблюдения"],
    ["personality", "[ˌpərsəˈnæləti]", "личность, индивидуальность, деятель, персона, особенности характера"],
    ["slice", "[sˈlaɪs]", "ломтик, часть, ломоть, доля, тонкий слой, нарезать, делить на части"],
    ["urban", "[ˈɝːbən]", "городской"],
    ["define", "[dəˈfaɪn]", "определять, устанавливать, обозначать, очерчивать, устанавливать значение"],
    ["desperate", "[ˈdesprət]", "отчаянный, безнадежный, доведенный до отчаяния, безрассудный, ужасный"],
    ["generate", "[ˈdʒenəreɪt]", "порождать, генерировать, производить, вызывать, образовывать"],
    ["sample", "[ˈsæmpəl]", "образец, выборка, проба, пример, образчик, пробовать, брать пробу"],
    ["funding", "[ˈfʌndɪŋ]", "субсидирование"],
    ["witness", "[ˈwɪtnəs]", "свидетель, свидетельство, очевидец, свидетельствовать, видеть"],
    ["wound", "[ˈwaʊnd]", "рана, ранение, обида, ущерб, оскорбление, ранить, поранить, причинять боль"],
    ["recipe", "[ˈresəpi]", "рецепт, способ, средство, предписание"],
    ["owner", "[ˈoʊnər]", "владелец, собственник, хозяин, владетель, командир корабля"],
    ["rough", "[ˈrəf]", "грубый, шероховатый, грубо, неровность, грубиян, терпеть лишения"],
    ["switch", "[ˈswɪtʃ]", "переключатель, переключение, переключать, переключаться, менять"],
    ["intense", "[ˌɪnˈtens]", "интенсивный, напряженный, сильный, впечатлительный, ревностный"],
    ["utility", "[juːˈtɪləti]", "утилита, полезность, утилитарность, полезный, вспомогательный, утилитарный"],
    ["struggle", "[ˈstrʌɡəl]", "борьба, усилие, напряжение, бороться, биться, пробиваться, отбиваться"],
    ["evolution", "[ˌevəˈluːʃən]", "эволюция, развитие, перестроение, развертывание, постепенное изменение, маневр"],
    ["horror", "[ˈhɔːrər]", "ужас, отвращение, что-либо нелепое, что-либо смешное, мрачное настроение"],
    ["flame", "[ˈfleɪm]", "пламя, пыл, страсть, предмет любви, пылать, гореть, пламенеть, вспылить"],
    ["penalty", "[ˈpenəlti]", "штраф, пенальти, наказание, взыскание, расплата"],
    ["senior", "[ˈsiːnjər]", "старший, выпускной, последний, пожилой человек, старшеклассник"],
    ["monitor", "[ˈmɑːnətər]", "монитор, дисплей, наставник, варан, контролировать, проверять, наставлять"],
    ["drama", "[ˈdrɑːmə]", "драма, драматургия"],
    ["faculty", "[ˈfækəlti]", "факультет, способность, дар, право, дарование, власть, область науки"],
    ["investigator", "[ˌɪnˈvestəˌɡetər]", "следователь, исследователь, испытатель"],
    ["carbon", "[ˈkɑːrbən]", "углерод, уголь, графит, карбонат, угольный"],
    ["everyday", "[ˈevrɪdeɪ]", "повседневный, ежедневный, каждодневный, бытовой, обычный, житейский, обиходный"],
    ["wrap", "[ˈræp]", "обертка, шаль, плед, платок, одеяло, упаковывать, обертывать, заворачивать"],
    ["expose", "[ɪkˈspoʊz]", "подвергать, разоблачать, выставлять, публичное разоблачение"],
    ["muscle", "[ˈmʌsəl]", "мышца, мускул, сила"],
    ["interpret", "[ˌɪnˈtɝːprət]", "интерпретировать, толковать, переводить, объяснять, растолковать"],
    ["conservative", "[kənˈsɝːvətɪv]", "консервативный, осторожный, умеренный, консерватор, реакционер"],
    ["legislation", "[ˌledʒəsˈleɪʃən]", "законодательство, законы, законодательный"],
    ["profile", "[ˈproʊfaɪl]", "профиль, контур, очертание, профилировать, изображать в профиле"],
    ["constitute", "[ˈkɑːnstəˌtuːt]", "составлять, образовывать, учреждать, основывать, назначать, издавать"],
    ["variation", "[ˌveriˈeɪʃən]", "изменение, вариация, вариант, отклонение, вариационный"],
    ["inquiry", "[ˌɪnˈkwaɪri]", "запрос, расследование, дознание, спрос, вопрос, исследование, справка, следствие"],
    ["remote", "[rɪˈmoʊt]", "удаленный, дистанционный, отдаленный, запасная лошадь"],
    ["nomination", "[ˌnɑːməˈneɪʃən]", "назначение, выставление, наименование, именование, выставление кандидата"],
    ["ally", "[ˈælaɪ]", "союзник, помощник, мраморный шарик, соединять, вступать в союз"],
    ["taxpayer", "[ˈtækˌspeər]", "налогоплательщик"],
    ["detect", "[dəˈtekt]", "обнаруживать, выявлять, детектировать, находить, замечать, открывать, расследовать"],
    ["disorder", "[ˌdɪˈsɔːrdər]", "расстройство, беспорядок, беспорядки, приводить в беспорядок, расстраивать"],
    ["prospect", "[ˈprɑːspekt]", "перспектива, разведка, исследовать, разведывать, ожидаемый"],
    ["bullet", "[ˈbʊlət]", "пуля, ядро, грузило, горох"],
    ["arise", "[əˈraɪz]", "возникать, появляться, происходить, являться, вставать, проистекать, подниматься"],
    ["silence", "[ˈsaɪləns]", "молчание, тишина, безмолвие, забвение, заставить замолчать, заглушать"],
    ["nuclear", "[ˈnuːkliər]", "ядерный, атомный, нуклеарный, содержащий ядро"],
    ["counter", "[ˈkaʊntər]", "счетчик, прилавок, встречный, противостоять, обратно, напротив"],
    ["diverse", "[daɪˈvɝːs]", "разнообразный, разный, иной, несходный, отличный от"],
    ["tap", "[ˈtæp]", "кран, отвод, ответвление, метчик, постукивать, стучать, перехватывать"],
    ["saving", "[ˈseɪvɪŋ]", "экономия, спасение, экономящий, спасительный, кроме, исключая"],
    ["mode", "[moʊd]", "режим, способ, режим работы, мода, вид, метод, форма, лад, уклад, образ действия"],
    ["distinction", "[ˌdɪˈstɪŋkʃən]", "различие, отличие, разграничение, различение, разница, честь, распознавание"],
    ["shell", "[ˈʃel]", "оболочка, корпус, раковина, скорлупа, шелушить, шелушиться, лущить, лущиться"],
    ["flow", "[ˈfloʊ]", "поток, течь, течение, струя, прилив, наплыв, течь, протекать, струиться, литься"],
    ["retain", "[rəˈteɪn]", "сохранять, удерживать, хранить, поддерживать, помнить, приглашать"],
    ["resemble", "[rəˈzembəl]", "походить на, иметь сходство, быть похожим"],
    ["expense", "[ɪkˈspens]", "расходы, расход, издержки, цена, трата, расходование"],
    ["violation", "[vaɪəˈleɪʃən]", "нарушение, насилие, изнасилование"],
    ["chase", "[ˈtʃeɪs]", "погоня, преследование, охота, рама, паз, преследовать, охотиться, гнаться"],
    ["session", "[ˈseʃən]", "сессия, заседание, сеанс, совещание, учебный год, учебный семестр"],
    ["chest", "[ˈtʃest]", "грудь, грудная клетка, сундук, ящик, ларь, грудной"],
    ["voter", "[ˈvoʊtə]", "избиратель, участник голосования"],
    ["core", "[ˈkɔːr]", "ядро, сердечник, сердцевина, стержень, суть, вырезать сердцевину"],
    ["cable", "[ˈkeɪbəl]", "кабельный, канатный, кабель, трос, канат, телеграфировать"],
    ["string", "[ˈstrɪŋ]", "строка, струна, ряд, шнурок, струнный, нанизывать, натягивать"],
    ["creation", "[kriˈeɪʃən]", "создание, творение, творчество, сотворение, созидание, произведение, мироздание"],
    ["regional", "[ˈriːdʒənəl]", "региональный, областной, районный, краевой, местный"],
    ["jungle", "[ˈdʒʌŋɡəl]", "джунгли, дебри, густые заросли, притон, связанный с джунглями"],
    ["chin", "[ˈtʃɪn]", "подбородок, болтать, разговаривать"],
    ["cleaner", "[ˈkliːnər]", "уборщик, чистильщик, средство для чистки, мойщик окон"],
    ["expected", "[ɪkˈspektəd]", "ожидаемый, предполагаемый"],
    ["brochure", "[ˈbroʊʃə]", "брошюра"],
    ["log", "[lɔːɡ]", "журнал, бревно, лаг, регистрировать, бревенчатый"],
    ["grilled", "[ˈɡrɪld]", "жареный, запеченный"],
    ["messy", "[ˈmesi]", "грязный, беспорядочный, неряшливый"],
    ["booking", "[ˈbʊkɪŋ]", "заказ, продажа билетов"],
    ["spice", "[ˈspaɪs]", "специя, специи, пряность, пикантность, приправлять, придавать пикантность"],
    ["mate", "[ˈmeɪt]", "мате, мат, помощник, приятель, товарищ, спариваться, спаривать, сопрягать"],
    ["hammer", "[ˈhæmər]", "молот, молоток, молоточек, курок, ударник, ковать, бить, стучать, ударять, вбивать"],
    ["frog", "[ˈfrɑːɡ]", "лягушка, стрелка, крестовина, лягушатник, крючок, воздушная стрелка, аксельбант"],
    ["customs", "[ˈkʌstəmz]", "таможенный, таможенные пошлины"],
    ["leisure", "[ˈleʒər]", "досуг, свободное время, свободный"],
    ["harmony", "[ˈhɑːrməni]", "гармония, согласие, лад, стройность, согласованность, созвучие, благозвучие"],
    ["flood", "[ˈfləd]", "наводнение, поток, потоп, половодье, затоплять, заливать, наводнять"],
    ["pants", "[ˈpænts]", "брюки, штаны, кальсоны, трусы"],
    ["kangaroo", "[ˌkæŋɡəˈruː]", "кенгуру"],
    ["recycling", "[riˈsaɪkəlɪŋ]", "рециклинг, повторное использование, переработка отходов, утилизация отходов"],
    ["plain", "[ˈpleɪn]", "равнина, простой, ясный, ясно, откровенно, жаловаться"],
    ["jug", "[ˈdʒəɡ]", "кувшин, тюрьма, щелканье, тушить, сажать в тюрьму"],
    ["handkerchief", "[ˈhæŋkərtʃəf]", "носовой платок, шейный платок, шейная косынка"],
    ["shark", "[ˈʃɑːrk]", "акула, шулер, вымогатель, мошенник, мошенничать, пожирать, жульничать"],
    ["considering", "[kənˈsɪdərɪŋ]", "учитывая, принимая во внимание, в общем"],
    ["comma", "[ˈkɑːmə]", "запятая"],
    ["kit", "[ˈkɪt]", "комплект, набор, ящик, снаряжение, экипировать"],
    ["softly", "[ˈsɒftli]", "мягко, тихо"],
    ["kite", "[ˈkaɪt]", "коршун, воздушный змей, бумажный змей, летать в воздухе, парить в воздухе"],
    ["bay", "[ˈbeɪ]", "бухта, залив, отсек, пролет, гнедой, преследовать, лаять"],
    ["forbidden", "[fərˈbɪdən]", "запрещенный, запретный, заповедный, недопустимый"],
    ["ruler", "[ˈruːlər]", "правитель, линейка, властитель, ежовый"],
    ["coconut", "[ˈkoʊkənʌt]", "кокосовый, кокос, башка, плод кокосовой пальмы"],
    ["herb", "[ˈɝːb]", "трава, растение"],
    ["defence", "[dəˈfens]", "оборона, защита, оправдание, укрепления, оборонительные сооружения"],
    ["grape", "[ˈɡreɪp]", "виноград, виноградина, гроздь винограда, вино, крупная картечь, мокрец, подсед, флер"],
    ["skip", "[ˈskɪp]", "пропуск, скип, прыжок, скачок, бадья, пропускать, скакать, перескакивать"],
    ["shallow", "[ˈʃæloʊ]", "мелкий, мелководный, мелководье, отмель, мель, мелеть, уменьшать глубину"],
    ["gamble", "[ˈɡæmbəl]", "азартная игра, авантюра, рисковать, играть в азартные игры"],
    ["stove", "[stoʊv]", "плита, печь, печка, печной, сушить"],
    ["unconscious", "[ˌənˈkɑːnʃəs]", "бессознательный, не сознающий, подсознание"],
    ["rival", "[ˈraɪvəl]", "соперник, конкурент, конкурировать, конкурирующий"],
    ["dishonestly", "[dɪsˈɒnɪstli]", "нечестно, непорядочно, мошеннически, бессовестно, позорно"],
    ["bush", "[ˈbʊʃ]", "куст, втулка, кустарник, буш, вкладыш, гильза, обсаживать кустарником"],
    ["illegally", "[ɪˈliːɡəli]", "незаконно"],
    ["offence", "[əˈfens]", "нападение, преступление, правонарушение, нарушение, оскорбление, проступок, обида"],
    ["controlled", "[kənˈtroʊld]", "контролируемый, управляемый"],
    ["lightly", "[ˈlaɪtli]", "слегка, легко, несерьезно, беспечно, чуть, обращаться пренебрежительно"],
    ["jointly", "[ˌdʒɔɪntli]", "совместно, сообща"],
    ["crisp", "[ˈkrɪsp]", "четкий, хрустящий, свежий, хрустящий картофель, хрустеть, завивать"],
    ["sweat", "[ˈswet]", "пот, тяжелый труд, потение, испарина, потеть, вспотеть, запотевать, волноваться"],
    ["removal", "[rəˈmuːvəl]", "удаление, устранение, отстранение, перемещение, смещение, снос, переезд, выемка"],
    ["chop", "[ˈtʃɑːp]", "отбивная, легкое волнение, нарезать, рубить, нарубить, привередливый"],
    ["traditionally", "[trəˈdɪʃənəli]", "по традиции"],
    ["offend", "[əˈfend]", "оскорблять, обижать, задевать, нарушать, погрешить, нарушать закон"],
    ["nest", "[ˈnest]", "гнездо, гнездышко, притон, гнездиться, вставлять, вмонтировать"],
    ["whistle", "[ˈwɪsəl]", "свисток, свист, горло, гортань, глотка, свистеть, насвистывать, посвистывать"],
    ["willingly", "[ˈwɪlɪŋli]", "охотно, с готовностью"],
    ["proudly", "[ˈpraʊdli]", "гордо, с гордостью, величественно"],
    ["bubble", "[ˈbʌbəl]", "пузырь, пузырек, мыльный пузырь, пузыриться, бить ключом, кипеть, дурачить"],
    ["skilled", "[ˈskɪld]", "квалифицированный, опытный, умелый, искусный"],
    ["sack", "[ˈsæk]", "мешок, куль, разграбление, сак, постель, увольнять, грабить"],
    ["disapproval", "[ˌdɪsəˈpruːvəl]", "неодобрение, осуждение"],
    ["feather", "[ˈfeðər]", "перо, оперение, плюмаж, гребень, шпонка, дичь, украшать перьями, украшаться перьями"],
    ["inevitably", "[ˌɪˈnevətəbli]", "неизбежно, неминуемо"],
    ["washing", "[ˈwɑːʃɪŋ]", "мытье, стирка, промывка, мойка, промывание, моющий, стирающийся"],
    ["amaze", "[əˈmeɪz]", "удивлять, поражать, изумлять, изумление, удивление"],
    ["lump", "[ˈləmp]", "кусок, глыба, ком, опухоль, кусковой, взятый в целом, сваливать"],
    ["saucepan", "[ˈsɒˌspæn]", "кастрюля"],
    ["questionnaire", "[ˌkwestʃəˈner]", "вопросник, анкета"],
    ["lighter", "[ˈlaɪtər]", "зажигалка, лихтер, легче, светлее, более светлый"],
    ["hardware", "[ˈhɑːrˌdwer]", "аппаратные средства, аппаратура, скобяной"],
    ["babysitter", "[ˈbeɪbiˌsɪtər]", "приходящая няня"],
    ["tablecloth", "[ˈteɪbəlˌklɒθ]", "скатерть"],
    ["sticky", "[ˈstɪki]", "липкий, клейкий, липучий, несговорчивый, трудный, затруднительный"],
    ["lightning", "[ˈlaɪtənɪŋ]", "молния, молниеносный, экстренный"],
    ["circumstances", "[ˈsɝːkəmˌstænsəz]", "обстоятельства, условия, материальное положение"],
    ["innovation", "[ˌɪnəˈveɪʃən]", "инновация, новшество, нововведение, новаторство, начинание"],
    ["evident", "[ˈevədənt]", "очевидный, явный, бросающийся в глаза, кажущийся"],
    ["cargo", "[ˈkɑːrˌɡoʊ]", "груз, груз корабля, грузовой"],
    ["hardship", "[ˈhɑːrdˌʃɪp]", "трудность, лишение, лишения, нужда, тяжелое испытание, неудобство"],
    ["vinegar", "[ˈvɪnəɡər]", "уксус, нелюбезный ответ, уксусный, неприятный, кислый"],
    ["esteem", "[əˈstiːm]", "уважение, почтение, оценка, почитать, уважать, рассматривать, считать"],
    ["shrimp", "[ˈʃrɪmp]", "креветка, козявка, ничтожный человечек, ловить креветок"],
    ["gloomy", "[ˈɡluːmi]", "мрачный, угрюмый, хмурый, сумрачный, темный, унылый, печальный"],
    ["dense", "[ˈdens]", "плотный, густой, дремучий, компактный, тупой, частый, непрозрачный, глупый"],
    ["reproduce", "[ˌriprəˈduːs]", "воспроизводить, размножаться, производить, восстанавливать, порождать"]
];